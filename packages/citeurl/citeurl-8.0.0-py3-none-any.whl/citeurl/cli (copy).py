...
def main():
    ...
    parser.add_argument(
        ...
        default='auto',
        help=(
            "the IP address to use for hosting the citator. No effect unless "
            + "paired with the '-s' option. Defaults to your private IP."
        )
    )
    parser.add_argument(
        "--port",
        ...
        help=(
            ...
            + "paired with the '-s' option. Defaults to 53037."
        )
    )
    ...
    if args.lookup: # look up info for a single citation
        ...
        try:
            ...
            serve(citator, IP=args.ip, port=args.port)

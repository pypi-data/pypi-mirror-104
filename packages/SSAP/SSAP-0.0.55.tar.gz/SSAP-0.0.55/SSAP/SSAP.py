...
def cmd():
    print(
        ...
        "[6] df.dropna(axis=1) | (remove nan,delete column: axis=1, delete row: axis=0)\n"
...
def dfprofile(df):
    ...
    df = df
    profile = ProfileReport(df, title='{©Tony-lab}Data Profile Report', html={'style': {'full_width': True}})
...
def scatter01(df, parameter_x, parameter_y, title):
    ...
    fig, ax = plt.subplots()
    scatter = ax.scatter(x=x, y=y, s=500, edgecolor='black', lw=0.5, alpha=0.7, facecolor='orange')
    plt.grid(linestyle='--', lw=0.5)
...
def scatter03(df, parameter_x, parameter_y, category, title):
    ...
    for i in catlist:
        ...
        scatter = ax.scatter(x=x, y=y, s=400, c=sliceColors[k], label=i, edgecolor='black', lw=0.5, alpha=0.7,
                             facecolor='orange')
        k += 1
...
def scatter04hist(df, parameter_x, parameter_y, bins, title):
    ...
    # the scatter plot:
    ax_scatter.scatter(x, y, s=400, edgecolor='black', lw=0.5, alpha=0.7, facecolor='darkorange')
...
def scatter05box(df, parameter_x, parameter_y, title):
    ...
    # the scatter plot:
    ax_scatter.scatter(x, y, s=500, edgecolor='black', lw=0.5, alpha=0.7, facecolor='darkorange')
...
def scatter06histlr(df, parameter_x, parameter_y, bins, title):
    ...
    # the scatter plot:
    ax_scatter.scatter(x, y, s=300, edgecolor='black', lw=0.5, alpha=0.7, facecolor='darkorange')
...
def scatter07boxlr(df, parameter_x, parameter_y, title):
    ...
    plt.ylabel(parameter_y)
    # the scatter plot:
    ax_scatter.scatter(x, y, s=200, edgecolor='black', lw=0.5, alpha=0.7, facecolor='darkorange')
...
def scatter08lrci(df, parameter_x, parameter_y, title):
    ...
    scatter = ax.scatter(x=x, y=y, s=100, edgecolor='black', lw=0.5, alpha=0.7, facecolor='orange')
...
def map():
    ...
    m.save('tonylab_map.html')
    url = 'tonylab_map.html'
    webbrowser.open(url, new=2)
    return m
...
def map01(df, province, parameter_x, method):
    ...
    labmap = folium.Map(location=[35, 120], zoom_start=4)
    labmap.choropleth(
        ...
        line_color='grey',
        line_opacity=0.2,
        legend_name='[©TonyLab] Geo_loc_distribution of: ' + parameter_x + ' (' + method + ')',
        highlight=True,
    )

    labmap.save('Tonylab_Map.html')
    url = 'Tonylab_Map.html'
    webbrowser.open(url, new=2)
    # display(pdsmap)
...
# network plot - show relationship between different entities. spiral/circular/other
def network01(df, entity1, entity2, relation, nodesize, fontsize, layout):
...
# network plot02 - interactive plot show networks of different entities
def network02(df, entity1, entity2, relation):
    import networkx as nx
    import matplotlib.pyplot as plt
    from pyvis.network import Network

    # demodata = {
    #     'entity1': ['A', 'E', 'A', 'A', 'C', 'B', 'E', 'E', 'E', 'B', 'D', 'I', 'G', 'E'],
    #     'entity2': ['B', 'D', 'E', 'F', 'F', 'F', 'G', 'H', 'I', 'C', 'H', 'H', 'I', 'F'],
    #     'relation': [
    ...
    got_net = Network(height="500px", width="500px", bgcolor="white", font_color="black", notebook=False, directed=True)
    # got_net = Network(notebook=True, directed=True)

    sources = df[entity1]
    targets = df[entity2]
    # weights = data_triple['weights']
    ...
    for e in edge_data:
        ...
        got_net.add_edge(src, dst, title=r)
    ...
    return got_net.show("Tonylab_networks.html")


# polynomial regression equation.
def polynominalreg(df,x_parameter,y_parameter,M,title):
...
#     y = [np.random.normal(0, 0.2) + y1 for y1 in y_]
    ...
    #     print(residuals_func)
    #     fitted curve function:
    def func(x, M):
        result = 0
        for i in range(M + 1):
            result += p_lsq[0][i] * np.power(x, M - i)
        return result
    ...
    labelx = 'Fitted curve {p=' + str(M) + ' | error:' + str(round(err, 3)) + '}'

    # make output formula
    output = 'Fitted curve: y='
    for temp in range(M + 1):
        #         print(temp)
        if p_lsq[0][temp] > 0:
            output = output + '+' + str(round(p_lsq[0][temp], 3)) + 'x^[' + str(M - temp) + ']'
        else:
            output += str(round(p_lsq[0][temp], 3)) + 'x^[' + str(M - temp) + ']'
    print(output)

    # 可视化
#     plt.plot(x_points, real_func(x_points), c='green', lw=2, label='Ideal curve{eg:y=sin(2Π*x)}')
    plt.plot(x_points, fit_func(p_lsq[0], x_points), c='red', lw=2, label=labelx)
    plt.plot(x, y, 'o', c='black', label='Real points with noise')
...
# >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
# Test area (update by 2021/4/26):==============================================
# df=pd.read_csv('C:/Users/lx379/Python_Tony/tony_lab/showcase_data/network.csv')
# network02(df, 'entity1', 'entity2', 'relation')


this_is_nothing = 'Below modules are not validated.---------------------👇'

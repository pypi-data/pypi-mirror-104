            ...
            for fname in files:
                ...
                if fname.endswith('.json'):
                    with open(fpath, 'rb') as fhl:
                        ...
                        item = PackageItem(info, remote=self._remote_or_local_file(root))
...
class ExtensionsTarget(BasicTarget):
    ...
    def _install(self, filename, info):
        if self.is_pip_package(filename):
            ...
            if results:
                ...
                return f"Python Package installed! Remember to restart inkscape to use it!"
    ...
    def pip_install(self, filename):
        ...
        pip = self.get_pip()
        if pip is None:
            logging.error("This package requires python VirtualEnv which is not available on your system.")
            return None
        try:
            results = call(pip, 'install',
                ('isolated', True),
                ('disable-pip-version-check', True),
                ('cache-dir', CACHE_DIR), filename).decode('utf8')
    ...
    def pip_uninstall(self, name):
        ...
        try:
            results = call(self.get_pip(), 'uninstall',
                ('disable-pip-version-check', True),
                name).decode('utf8')
        except ProgramRunError as err:
            raise
    ...
    def generate_id(self, filename):
        ...
        try:
            ...
            raise IOError("Refusing the install extension without inx file (unknown archive)")

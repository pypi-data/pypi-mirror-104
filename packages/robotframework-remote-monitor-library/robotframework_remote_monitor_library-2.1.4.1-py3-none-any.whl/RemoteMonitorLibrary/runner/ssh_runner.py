...
class SSHLibraryPlugInWrapper(plugin_runner_abstract, metaclass=ABCMeta):
    ...
    def login(self):
        ...
        if len(self._session_errors) == 0:
            logger.info(f"Host '{self.host_alias}': Connection establishing")
        else:
            logger.warn(f"Host '{self.host_alias}'Connection restoring at {len(self._session_errors)} time")
        ...
        while True:
            try:
                if certificate:
                    logger.debug(f"Login with user/certificate")
                    self._ssh.login_with_public_key(username, certificate, '')
                else:
                    logger.debug(f"Login with user/password")
                    self._ssh.login(username, password)
            except paramiko.AuthenticationException:
                raise
            except Exception as e:
                logger.warn(f"Connection to {self.host_alias} failed; Reason: {type(e).__name___}{e}")
            else:
                self._is_logged_in = True
                logger.info(f"Connection to {repr(self)}{self.host_alias} established")
                break
            finally:

...
def pypi_release(ctx, branch=False):
    ...
    See https://lino-framework.gitlab.io/atelier/invlib.html for details.
...
def edit_blog_entry(ctx, today=None):
    ...
    if not entry.path.exists():
        ...
        set_times(entry.path.parent / 'index.rst')
...
def check_config(ctx):
    ...
    if prj.config['revision_control_system'] != 'git':
        if (prj.root_dir / '.git').exists():
            print("revision_control_system is not 'git', but .git directory exists")
        return
    info = prj.SETUP_INFO
    if len(info) == 0:
        # print("No SETUP_INFO")

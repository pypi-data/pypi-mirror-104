...
def _Pipeline_modifiers(self):
    ...
    class PipelineModifierList(collections.MutableSequence):
        ...
        def __setitem__(self, index, newMod):
            ...
            newMod.initialize_modifier(modapp.dataset.anim.time, modapp)
        ...
        def insert(self, index, mod):
            ...
            if index == len(modapplist) and index >= 0:
                ...
                raise IndexError("List index is out of range.")
            mod.initialize_modifier(modapp.dataset.anim.time, modapp)
...
def _Pipeline_compute(self, frame = None):
    """ Computes and returns the results of this data pipeline.
        ...
        The optional *frame* parameter determines at which animation time the pipeline is evaluated. Frame numbering starts at 0.
        If you don't specify a frame number, the current time slider position is used (will always be frame 0 for scripts not running in the context of an interactive OVITO session).

        The :py:meth:`!compute` method will raise a ``RuntimeError`` if the pipeline could not be successfully evaluated for some reason.

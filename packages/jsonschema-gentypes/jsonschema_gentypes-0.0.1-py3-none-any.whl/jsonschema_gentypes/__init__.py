...
class TypedDictType(NamedType):
    def __init__(
        ...
        for property_, type_obj in struct.items():
            if property_ in required_keys:
                if type_obj.comments():
                    type_obj.comments().append("")
                type_obj.comments().append("required")
            self.property_sub_type.append((property_, type_obj))

    def _definition_dict(self) -> List[str]:
        """
        ...
        for property_, type_obj in self.property_sub_type:
    ...
    def _definition_class(self) -> List[str]:
        """
        Get the definition based on a class (unsafe)
        """
        result = ["", "", f"class {self._name}(TypedDict):"]
        if self.descriptions:
            ...
            result.append('    """')
        for property_, type_obj in self.property_sub_type:
            for description in type_obj.comments():
                result.append(f"    # {description}")
            result.append(f"    {property_}: {type_obj.name()}")
        return result
    ...
    def depends_on(self) -> List[Type]:
        ...
        for _, type_obj in self.property_sub_type:
...
def get_description(schema: jsonschema.JSONSchemaItem) -> List[str]:
    ...
    for key, value in schema.items():
        if (
            key
            not in ("title", "description", "$ref", "$schema", "$id", "type", "items", "additionalProperties")
...
class API:
    ...
    def get_type(
        self, schema: jsonschema.JSONSchemaItem, proposed_name: Optional[str] = None, auto_alias: bool = True
    ) -> Type:
        """Get a :class:`.Type` for a JSON schema."""
        proposed_name = proposed_name if proposed_name else f"Gen{random.randrange(999999)}"
...
class APIv4(API):
    ...
    ) -> Type:  # pylint: disable=no-self-use
        """Generate a ``Literal`` for a const value."""
        const_: Union[int, float, str, bool, None] = schema["const"]  # type: ignore
    ...
    def boolean(self, schema: jsonschema.JSONSchemaItem, proposed_name: str) -> Type:
        ...
        type_: Type = BuiltinType("bool")
        if "properties" in schema:
            properties = cast(Dict[str, jsonschema.JSONSchema], schema.get("properties"))

            default = properties.get("default", self.NO_VALUE)
            const = properties.get("const", self.NO_VALUE)
            if const != self.NO_VALUE:
                type_ = self.const(cast(jsonschema.JSONSchemaItem, properties), proposed_name)
    ...
    def object(self, schema: jsonschema.JSONSchemaItem, proposed_name: str) -> Type:
        ...
        if properties:
            items, types = zip(
                *filter(
                    ...
                    [
                        (
                            prop,
                            self.get_type(subschema, proposed_name + " " + prop, auto_alias=False),
                        )
            ...
            required_keys = set() if required is None else set(required)
            struct = OrderedDict(zip(items, types))
            if std_dict is None:
                return TypedDictType(name, struct, required_keys, get_description(schema))
            else:
                ...
                type_ = CombinedType(NativeType("Union"), [std_dict, typed_dict])
                type_.set_comments(
                    [
    ...
    def default(self, schema: jsonschema.JSONSchemaItem, proposed_name: str) -> Type:
        ...
        for test_type, type_name in [
            ...
            if isinstance(schema["default"], test_type):  # type: ignore
                type_ = type_name

...
class CrmLead(models.Model):
    ...
    def validate_remesa(self):
        ...
        if not self.partner_id:
            raise ValidationError(
                _("The subscription request related must be validated.")
            )
        # Validate IBAN
        ...
        if self._phones_already_used():
            raise ValidationError(
                _("Contract or validated CRMLead with the same phone already exists.")  # noqa
            )

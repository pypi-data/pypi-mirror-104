...
class ParseL0InputData(WorkflowDataTaskBase, FitsDataMixin, InputDatasetMixin):
    ...
    def constant_flowers(self) -> List[S]:
        return [
            NumCSStepBud(),
            UniqueBud(constant_name="INSTRUMENT", metadata_key="instrument"),
        ]

    @property
    def tag_flowers(self) -> List[S]:
        return [
            # CSStepFlower(max_cs_time_step=self.input_dataset_parameters.get("MAX_CS_TIME_STEP")),
            CSStepFlower(),
            SingleValueSingleKeyFlower(tag_stem_name="TASK", metadata_key="ip_task_type"),
        ]

...
class MetadataStoreMixin:
    ...
    def metadata_store_input_dataset_document(self) -> str:
        return self._metadata_store_input_dataset.inputDatasetDocument

    def metadata_store_record_provenance(self, is_task_manual: bool, library_versions: str):
        params = RecipeRunProvenanceMutation(
            inputDatasetId=self._metadata_store_input_dataset.inputDatasetId,
    ...
    @property
    def _metadata_store_input_dataset(self) -> InputDatasetResponse:
        input_dataset_response = self.metadata_store_client.execute_gql_query(
            query_base="recipeRuns",
            query_response_cls=RecipeRunResponse,
            query_parameters=RecipeRunQuery(recipeRunId=self.recipe_run_id),
        )
        return input_dataset_response[0].recipeInstance.inputDataset

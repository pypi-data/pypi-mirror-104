...
def input_dataset(tmp_path, recipe_run_id):
    with Task(
        ...
        file_path = task._scratch.workflow_base_path / Path(f"{uuid4().hex[:6]}.ext")
...
def multiple_input_datasets(tmp_path, recipe_run_id):
    with Task(
        ...
        task._scratch.workflow_base_path = tmp_path / str(recipe_run_id)
        for _ in range(2):
            file_path = task._scratch.workflow_base_path / f"{uuid4().hex[:6]}.ext"
            file_path.write_text(data=INPUT_DATASET_STRING)
            task.tag(path=file_path, tags=Tag.input_dataset())
        yield task
        task._scratch.purge()
...
def test_no_input_datasets(input_dataset):
    ...
    task._scratch._tag_db.clear_tag(tag=Tag.input_dataset())

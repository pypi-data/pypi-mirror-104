...
def visp_flowers():
    return [SingleValueSingleKeyFlower(tag_stem_name="MODSTATE", metadata_key="modstate")]


@pytest.fixture(scope="function")
def visp_buds():
    return [UniqueBud(constant_name="NUMMOD", metadata_key="num_mod")]


@pytest.fixture(scope="function")
def parse_inputs_task(tmp_path, visp_flowers, visp_buds):
    ...
    with TaskClass(
        ...
        for i in range(ds.num_frames):
            ...
            task.fits_data_write(
                hdu_list=hdul, tags=Tag.input(), relative_path=f"input/input_{i}.fits"
            )
        yield task
        task._scratch.purge()
...
def test_make_flowerpots(parse_inputs_task):
    ...
    assert len(constant_pot.flowers) == 1
    assert tag_pot.flowers[0].stem_name == "MODSTATE"
    assert constant_pot.flowers[0].stem_name == "NUMMOD"
...
def test_subclass_flowers(visp_parse_inputs_task):
    ...
    assert len(tag_pot.flowers) == 3
    assert len(constant_pot.flowers) == 3
    assert sorted([f.stem_name for f in tag_pot.flowers]) == ["CSSTEP", "MODSTATE", "TASK"]
    assert sorted([f.stem_name for f in constant_pot.flowers]) == [
        "INSTRUMENT",
        "NUMCSSTEPS",
        "NUMMOD",
    ]
...
def test_constants_correct(parse_inputs_task):
    ...
    assert parse_inputs_task.constants == {"NUMMOD": parse_inputs_task._num_mod}
...
def test_tags_correct(parse_inputs_task):
    ...
    for m in range(num_mod):
        expected_mod_files = [
            parse_inputs_task._scratch._parse_relative_path(f"input/input_{i}.fits")
            for i in range(num_mod * files_per_mod)[m::num_mod]
        ]
        assert sorted(list(parse_inputs_task.read(tags=f"MODSTATE_{m}"))) == expected_mod_files

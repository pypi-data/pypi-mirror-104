...
def workflow_data_task(tmp_path, recipe_run_id):
    ...
    with WorkflowDataTaskBaseTask(
        ...
        for filename in filenames:
            filepath = task._scratch.workflow_base_path / filename
            filepath.touch()
...
def test_valid_write_with_bytes(workflow_data_task):
    ...
    assert (task._scratch.workflow_base_path / relative_path).exists()
...
def test_tag_nonexistent_file(workflow_data_task):
    ...
    with pytest.raises(FileNotFoundError):
        task.tag(path=task._scratch.workflow_base_path / "abc.ext", tags="NONEXISTENT_FILE")

...
class Ports:
	...
	def set_from_py(self, freq_resp_sli_st_fr: int) -> None:
		"""SCPI: [SOURce<HW>]:CORRection:FRESponse:RF:USER:SLISt:PORTs:FROM \n
		Snippet: driver.source.correction.fresponse.rf.user.slist.ports.set_from_py(freq_resp_sli_st_fr = 1) \n
		Sets the port number from that the signal is coming and the port to that it is going. Available ports depend on the file
		content and file extenssion, see 'S-Parameters (Touchstone) Files'. \n
	...
	def set_to(self, freq_resp_sli_st_fr: int) -> None:
		"""SCPI: [SOURce<HW>]:CORRection:FRESponse:RF:USER:SLISt:PORTs:TO \n
		Snippet: driver.source.correction.fresponse.rf.user.slist.ports.set_to(freq_resp_sli_st_fr = 1) \n
		Sets the port number from that the signal is coming and the port to that it is going. Available ports depend on the file
		content and file extenssion, see 'S-Parameters (Touchstone) Files'. \n
			:param freq_resp_sli_st_fr: integer Range: 1 to 8 (dynamic)
		"""

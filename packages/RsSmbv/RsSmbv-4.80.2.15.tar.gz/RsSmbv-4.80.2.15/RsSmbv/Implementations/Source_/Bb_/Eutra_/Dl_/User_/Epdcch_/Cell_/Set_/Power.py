...
class Power:
	...
	def set(self, rel_power: float, userIx=repcap.UserIx.Default, cellNull=repcap.CellNull.Default, setItem=repcap.SetItem.Default) -> None:
		...
		Snippet: driver.source.bb.eutra.dl.user.epdcch.cell.set.power.set(rel_power = 1.0, userIx = repcap.UserIx.Default, cellNull = repcap.CellNull.Default, setItem = repcap.SetItem.Default) \n
	...
	def get(self, userIx=repcap.UserIx.Default, cellNull=repcap.CellNull.Default, setItem=repcap.SetItem.Default) -> float:
		"""SCPI: [SOURce<HW>]:BB:EUTRa:DL:USER<CH>:EPDCch:CELL<ST0>:SET<DIR>:POWer \n
		Snippet: value: float = driver.source.bb.eutra.dl.user.epdcch.cell.set.power.get(userIx = repcap.UserIx.Default, cellNull = repcap.CellNull.Default, setItem = repcap.SetItem.Default) \n
		Sets the power of the EPDCCH allocations relative to the power of the reference signals. See method RsSmbv.Source.Bb.
		Eutra.Dl.Refsig.power. \n
			:param userIx: optional repeated capability selector. Default value: Nr1 (settable in the interface 'User')
			:param cellNull: optional repeated capability selector. Default value: Nr0 (settable in the interface 'Cell')

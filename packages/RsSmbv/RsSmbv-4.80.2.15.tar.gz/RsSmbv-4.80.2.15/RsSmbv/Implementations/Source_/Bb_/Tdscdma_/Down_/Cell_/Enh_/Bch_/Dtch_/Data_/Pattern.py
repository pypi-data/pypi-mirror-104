...
class Pattern:
	...
	def set(self, structure: PatternStruct, cell=repcap.Cell.Default, channel=repcap.Channel.Default) -> None:
		...
		Snippet: driver.source.bb.tdscdma.down.cell.enh.bch.dtch.data.pattern.set(value = [PROPERTY_STRUCT_NAME](), cell = repcap.Cell.Default, channel = repcap.Channel.Default) \n
		...
		self._core.io.write_struct(f'SOURce<HwInstance>:BB:TDSCdma:DOWN:CELL{cell_cmd_val}:ENH:BCH:DTCH{channel_cmd_val}:DATA:PATTern', structure)

...
class Apsi:
	"""Apsi commands group definition. 1 total commands, 0 Sub-groups, 1 group commands"""

	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("apsi", core, parent)
	...
	def set(self, dci_aps_si: int, allocationNull=repcap.AllocationNull.Default) -> None:
		...
		Snippet: driver.source.bb.eutra.dl.emtc.dci.alloc.apsi.set(dci_aps_si = 1, allocationNull = repcap.AllocationNull.Default) \n
	...
	def get(self, allocationNull=repcap.AllocationNull.Default) -> int:
		"""SCPI: [SOURce<HW>]:BB:EUTRa:DL:EMTC:DCI:ALLoc<CH0>:APSI \n
		Snippet: value: int = driver.source.bb.eutra.dl.emtc.dci.alloc.apsi.get(allocationNull = repcap.AllocationNull.Default) \n
		For users working in transmission mode TM9 and if UE-specific search space is used, sets the DCI format 6-1A field
		antenna ports and scrambling identity. \n
			:param allocationNull: optional repeated capability selector. Default value: Nr0 (settable in the interface 'Alloc')

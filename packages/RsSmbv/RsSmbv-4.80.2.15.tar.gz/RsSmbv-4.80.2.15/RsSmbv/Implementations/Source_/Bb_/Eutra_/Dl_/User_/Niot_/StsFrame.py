...
class StsFrame:
	...
	def set(self, search_sp_start_sf: enums.EutraNbiotSearchSpaceStSubframe, userIx=repcap.UserIx.Default) -> None:
		...
		Snippet: driver.source.bb.eutra.dl.user.niot.stsFrame.set(search_sp_start_sf = enums.EutraNbiotSearchSpaceStSubframe.S1_5, userIx = repcap.UserIx.Default) \n
	...
	def get(self, userIx=repcap.UserIx.Default) -> enums.EutraNbiotSearchSpaceStSubframe:
		"""SCPI: [SOURce<HW>]:BB:EUTRa:DL:USER<CH>:NIOT:STSFrame \n
		Snippet: value: enums.EutraNbiotSearchSpaceStSubframe = driver.source.bb.eutra.dl.user.niot.stsFrame.get(userIx = repcap.UserIx.Default) \n
		Sets the serach space start subframe (G) . \n
			:param userIx: optional repeated capability selector. Default value: Nr1 (settable in the interface 'User')

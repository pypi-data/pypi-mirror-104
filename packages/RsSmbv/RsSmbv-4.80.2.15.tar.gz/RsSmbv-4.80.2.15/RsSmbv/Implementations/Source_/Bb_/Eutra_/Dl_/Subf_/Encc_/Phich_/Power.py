...
class Power:
	...
	def set(self, power: float, subframeNull=repcap.SubframeNull.Default) -> None:
		...
		Snippet: driver.source.bb.eutra.dl.subf.encc.phich.power.set(power = 1.0, subframeNull = repcap.SubframeNull.Default) \n
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:EUTRa:DL:SUBF{subframeNull_cmd_val}:ENCC:PHICh:POWer {param}')

	def get(self, subframeNull=repcap.SubframeNull.Default) -> float:
		"""SCPI: [SOURce<HW>]:BB:EUTRa:DL:[SUBF<ST0>]:ENCC:PHICh:POWer \n
		Snippet: value: float = driver.source.bb.eutra.dl.subf.encc.phich.power.get(subframeNull = repcap.SubframeNull.Default) \n
		Sets the power of one PHICH (PPHICH) in a PHICH group, i.e. the total power of one PHICH group is the sum of the power of
		the transmitted PHICHs within this group. \n
			:param subframeNull: optional repeated capability selector. Default value: Nr0 (settable in the interface 'Subf')
			:return: power: float Range: -80 to 10"""
		subframeNull_cmd_val = self._base.get_repcap_cmd_value(subframeNull, repcap.SubframeNull)
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:EUTRa:DL:SUBF{subframeNull_cmd_val}:ENCC:PHICh:POWer?')

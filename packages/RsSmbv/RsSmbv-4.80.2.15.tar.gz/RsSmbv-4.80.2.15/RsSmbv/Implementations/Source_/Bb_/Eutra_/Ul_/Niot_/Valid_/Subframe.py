...
class Subframe:
	...
	def set(self, valid_subframe: bool, subframeNull=repcap.SubframeNull.Default) -> None:
		"""SCPI: [SOURce<HW>]:BB:EUTRa:UL:NIOT:VALid:SUBFrame<DIR> \n
		Snippet: driver.source.bb.eutra.ul.niot.valid.subframe.set(valid_subframe = False, subframeNull = repcap.SubframeNull.Default) \n
		Sets a subframe as valid and used for eMTC transmission. \n
			:param valid_subframe: No help available
			:param subframeNull: optional repeated capability selector. Default value: Nr0 (settable in the interface 'Subframe')"""
		param = Conversions.bool_to_str(valid_subframe)
		subframeNull_cmd_val = self._base.get_repcap_cmd_value(subframeNull, repcap.SubframeNull)
		self._core.io.write(f'SOURce<HwInstance>:BB:EUTRa:UL:NIOT:VALid:SUBFrame{subframeNull_cmd_val} {param}')

	def get(self, subframeNull=repcap.SubframeNull.Default) -> bool:
		"""SCPI: [SOURce<HW>]:BB:EUTRa:UL:NIOT:VALid:SUBFrame<DIR> \n
		Snippet: value: bool = driver.source.bb.eutra.ul.niot.valid.subframe.get(subframeNull = repcap.SubframeNull.Default) \n
		Sets a subframe as valid and used for eMTC transmission. \n
			:param subframeNull: optional repeated capability selector. Default value: Nr0 (settable in the interface 'Subframe')
			:return: valid_subframe: No help available"""
		subframeNull_cmd_val = self._base.get_repcap_cmd_value(subframeNull, repcap.SubframeNull)
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:EUTRa:UL:NIOT:VALid:SUBFrame{subframeNull_cmd_val}?')

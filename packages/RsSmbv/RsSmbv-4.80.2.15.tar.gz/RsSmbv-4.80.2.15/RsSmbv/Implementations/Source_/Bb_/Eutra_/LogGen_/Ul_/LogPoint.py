...
class LogPoint:
	"""LogPoint commands group definition. 1 total commands, 0 Sub-groups, 1 group commands
	Repeated Capability: IndexNull, default value after init: IndexNull.Nr0"""

	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("logPoint", core, parent)
	...
	def set(self, log_point_state: bool, indexNull=repcap.IndexNull.Default) -> None:
		"""SCPI: [SOURce<HW>]:BB:EUTRa:LOGGen:UL:LOGPoint<CH0> \n
		Snippet: driver.source.bb.eutra.logGen.ul.logPoint.set(log_point_state = False, indexNull = repcap.IndexNull.Default) \n
		No command help available \n
			:param log_point_state: No help available
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:EUTRa:LOGGen:UL:LOGPoint{indexNull_cmd_val} {param}')

	def get(self, indexNull=repcap.IndexNull.Default) -> bool:
		"""SCPI: [SOURce<HW>]:BB:EUTRa:LOGGen:UL:LOGPoint<CH0> \n
		Snippet: value: bool = driver.source.bb.eutra.logGen.ul.logPoint.get(indexNull = repcap.IndexNull.Default) \n
		No command help available \n
			:param indexNull: optional repeated capability selector. Default value: Nr0 (settable in the interface 'LogPoint')
			:return: log_point_state: No help available"""
		indexNull_cmd_val = self._base.get_repcap_cmd_value(indexNull, repcap.IndexNull)
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:EUTRa:LOGGen:UL:LOGPoint{indexNull_cmd_val}?')
		return Conversions.str_to_bool(response)

	def clone(self) -> 'LogPoint':
		"""Clones the group by creating new object from it and its whole existing sub-groups
		Also copies all the existing default Repeated Capabilities setting,
		which you can change independently without affecting the original group"""
		new_group = LogPoint(self._core, self._base.parent)

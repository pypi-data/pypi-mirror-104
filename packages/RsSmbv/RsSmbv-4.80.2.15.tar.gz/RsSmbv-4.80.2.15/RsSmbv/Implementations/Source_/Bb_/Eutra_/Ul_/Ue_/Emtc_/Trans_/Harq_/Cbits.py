...
class Cbits:
	...
	def get(self, userEquipment=repcap.UserEquipment.Default, transmission=repcap.Transmission.Default) -> int:
		"""SCPI: [SOURce<HW>]:BB:EUTRa:UL:UE<ST>:EMTC:TRANs<CH>:HARQ:CBITs \n
		Snippet: value: int = driver.source.bb.eutra.ul.ue.emtc.trans.harq.cbits.get(userEquipment = repcap.UserEquipment.Default, transmission = repcap.Transmission.Default) \n
		Queries the number of coded ACK/NACK bits. \n
			...
			:return: codedbits: integer Range: 0 to 1E5"""
		...
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:EUTRa:UL:UE{userEquipment_cmd_val}:EMTC:TRANs{transmission_cmd_val}:HARQ:CBITs?')

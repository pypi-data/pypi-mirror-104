...
class Row:
	...
	def set(self, ant_port_mapping: bool, userEquipment=repcap.UserEquipment.Default, rowNull=repcap.RowNull.Default) -> None:
		"""SCPI: [SOURce<HW>]:BB:EUTRa:UL:UE<ST>:APMap:AP43Map:ROW<BBID> \n
		Snippet: driver.source.bb.eutra.ul.ue.apMap.ap43Map.row.set(ant_port_mapping = False, userEquipment = repcap.UserEquipment.Default, rowNull = repcap.RowNull.Default) \n
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:EUTRa:UL:UE{userEquipment_cmd_val}:APMap:AP43Map:ROW{rowNull_cmd_val} {param}')

	def get(self, userEquipment=repcap.UserEquipment.Default, rowNull=repcap.RowNull.Default) -> bool:
		"""SCPI: [SOURce<HW>]:BB:EUTRa:UL:UE<ST>:APMap:AP43Map:ROW<BBID> \n
		Snippet: value: bool = driver.source.bb.eutra.ul.ue.apMap.ap43Map.row.get(userEquipment = repcap.UserEquipment.Default, rowNull = repcap.RowNull.Default) \n
		Sets which antenna port will be generated by which baseband. \n
			:param userEquipment: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Ue')
			:param rowNull: optional repeated capability selector. Default value: Nr0 (settable in the interface 'Row')
		...
		userEquipment_cmd_val = self._base.get_repcap_cmd_value(userEquipment, repcap.UserEquipment)
		rowNull_cmd_val = self._base.get_repcap_cmd_value(rowNull, repcap.RowNull)
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:EUTRa:UL:UE{userEquipment_cmd_val}:APMap:AP43Map:ROW{rowNull_cmd_val}?')

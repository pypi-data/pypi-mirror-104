...
class PrFormat:
	...
	def get(self, userEquipment=repcap.UserEquipment.Default) -> int:
		...
		Snippet: value: int = driver.source.bb.eutra.ul.ue.prach.prFormat.get(userEquipment = repcap.UserEquipment.Default) \n
		...
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:EUTRa:UL:UE{userEquipment_cmd_val}:PRACh:PRFormat?')

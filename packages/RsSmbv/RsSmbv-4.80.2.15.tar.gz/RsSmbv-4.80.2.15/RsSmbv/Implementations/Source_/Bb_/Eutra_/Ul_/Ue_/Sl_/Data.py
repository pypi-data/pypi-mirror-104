...
class Data:
	...
	def set(self, data_source: enums.DataSourceA, userEquipment=repcap.UserEquipment.Default) -> None:
		...
		Snippet: driver.source.bb.eutra.ul.ue.sl.data.set(data_source = enums.DataSourceA.DLISt, userEquipment = repcap.UserEquipment.Default) \n
	...
	def get(self, userEquipment=repcap.UserEquipment.Default) -> enums.DataSourceA:
		"""SCPI: [SOURce<HW>]:BB:EUTRa:UL:UE<ST>:SL:DATA \n
		Snippet: value: enums.DataSourceA = driver.source.bb.eutra.ul.ue.sl.data.get(userEquipment = repcap.UserEquipment.Default) \n
		Sets the data source for the sidelink. \n
			:param userEquipment: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Ue')

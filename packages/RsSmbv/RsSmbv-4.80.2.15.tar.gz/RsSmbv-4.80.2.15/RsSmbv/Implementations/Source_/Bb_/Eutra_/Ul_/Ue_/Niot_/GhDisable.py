...
class GhDisable:
	"""GhDisable commands group definition. 1 total commands, 0 Sub-groups, 1 group commands"""

	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("ghDisable", core, parent)
	...
	def set(self, nbiot_dis_gh: bool, userEquipment=repcap.UserEquipment.Default) -> None:
		...
		Snippet: driver.source.bb.eutra.ul.ue.niot.ghDisable.set(nbiot_dis_gh = False, userEquipment = repcap.UserEquipment.Default) \n
		Disables NDRS group hopping for the selected UE. \n
			:param nbiot_dis_gh: 0| 1| OFF| ON
	...
	def get(self, userEquipment=repcap.UserEquipment.Default) -> bool:
		"""SCPI: [SOURce<HW>]:BB:EUTRa:UL:UE<ST>:NIOT:GHDisable \n
		Snippet: value: bool = driver.source.bb.eutra.ul.ue.niot.ghDisable.get(userEquipment = repcap.UserEquipment.Default) \n
		Disables NDRS group hopping for the selected UE. \n
			:param userEquipment: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Ue')
			:return: nbiot_dis_gh: 0| 1| OFF| ON"""
		userEquipment_cmd_val = self._base.get_repcap_cmd_value(userEquipment, repcap.UserEquipment)

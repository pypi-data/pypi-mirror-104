...
class Pattern:
	...
	def set(self, structure: PatternStruct, allocationNull=repcap.AllocationNull.Default) -> None:
		...
		Snippet: driver.source.bb.eutra.dl.niot.alloc.pattern.set(value = [PROPERTY_STRUCT_NAME](), allocationNull = repcap.AllocationNull.Default) \n
		...
		self._core.io.write_struct(f'SOURce<HwInstance>:BB:EUTRa:DL:NIOT:ALLoc{allocationNull_cmd_val}:PATTern', structure)

	def get(self, allocationNull=repcap.AllocationNull.Default) -> PatternStruct:
		"""SCPI: [SOURce<HW>]:BB:EUTRa:DL:NIOT:ALLoc<CH0>:PATTern \n
		Snippet: value: PatternStruct = driver.source.bb.eutra.dl.niot.alloc.pattern.get(allocationNull = repcap.AllocationNull.Default) \n
		Sets a bit pattern as data source. \n
			:param allocationNull: optional repeated capability selector. Default value: Nr0 (settable in the interface 'Alloc')

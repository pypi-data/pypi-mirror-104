...
class HpNumber:
	...
	def set(self, harq_process_num: int, allocationNull=repcap.AllocationNull.Default) -> None:
		...
		Snippet: driver.source.bb.eutra.dl.niot.dci.alloc.hpNumber.set(harq_process_num = 1, allocationNull = repcap.AllocationNull.Default) \n
	...
	def get(self, allocationNull=repcap.AllocationNull.Default) -> int:
		"""SCPI: [SOURce<HW>]:BB:EUTRa:DL:NIOT:DCI:ALLoc<CH0>:HPNMber \n
		Snippet: value: int = driver.source.bb.eutra.dl.niot.dci.alloc.hpNumber.get(allocationNull = repcap.AllocationNull.Default) \n
		Sets the HARQ processes number, for UEs for that method RsSmbv.Source.Bb.Eutra.Dl.User.Sthp.State.set 1. \n
			:param allocationNull: optional repeated capability selector. Default value: Nr0 (settable in the interface 'Alloc')

...
class Insert:
	...
	def set(self, userIx=repcap.UserIx.Default, cellNull=repcap.CellNull.Default) -> None:
		...
		Snippet: driver.source.bb.eutra.dl.user.asPy.dl.cell.insert.set(userIx = repcap.UserIx.Default, cellNull = repcap.CellNull.Default) \n
	...
	def set_with_opc(self, userIx=repcap.UserIx.Default, cellNull=repcap.CellNull.Default) -> None:
		...
		cellNull_cmd_val = self._base.get_repcap_cmd_value(cellNull, repcap.CellNull)
		"""SCPI: [SOURce<HW>]:BB:EUTRa:DL:USER<CH>:AS:DL:CELL<ST0>:INSert \n
		Snippet: driver.source.bb.eutra.dl.user.asPy.dl.cell.insert.set_with_opc(userIx = repcap.UserIx.Default, cellNull = repcap.CellNull.Default) \n
		Inserts a table element before the selected one. \n
		Same as set, but waits for the operation to complete before continuing further. Use the RsSmbv.utilities.opc_timeout_set() to set the timeout value. \n
			:param userIx: optional repeated capability selector. Default value: Nr1 (settable in the interface 'User')
			:param cellNull: optional repeated capability selector. Default value: Nr0 (settable in the interface 'Cell')"""
		self._core.io.write_with_opc(f'SOURce<HwInstance>:BB:EUTRa:DL:USER{userIx_cmd_val}:AS:DL:CELL{cellNull_cmd_val}:INSert')

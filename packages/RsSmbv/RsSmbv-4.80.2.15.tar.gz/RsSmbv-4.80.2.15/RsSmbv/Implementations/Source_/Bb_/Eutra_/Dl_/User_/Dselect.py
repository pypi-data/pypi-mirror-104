...
class Dselect:
	...
	def set(self, dselect: str, userIx=repcap.UserIx.Default) -> None:
		...
		Snippet: driver.source.bb.eutra.dl.user.dselect.set(dselect = '1', userIx = repcap.UserIx.Default) \n
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:EUTRa:DL:USER{userIx_cmd_val}:DSELect {param}')

	def get(self, userIx=repcap.UserIx.Default) -> str:
		"""SCPI: [SOURce<HW>]:BB:EUTRa:DL:USER<CH>:DSELect \n
		Snippet: value: str = driver.source.bb.eutra.dl.user.dselect.get(userIx = repcap.UserIx.Default) \n
		Selects an existing data list file from the default directory or from the specific directory. Refer to 'Accessing Files
		in the Default or Specified Directory' for general information on file handling in the default and in a specific
		directory. \n
			:param userIx: optional repeated capability selector. Default value: Nr1 (settable in the interface 'User')
		...
		userIx_cmd_val = self._base.get_repcap_cmd_value(userIx, repcap.UserIx)
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:EUTRa:DL:USER{userIx_cmd_val}:DSELect?')

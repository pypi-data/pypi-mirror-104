...
class Pattern:
	...
	def set(self, structure: PatternStruct, baseStation=repcap.BaseStation.Default, channelNull=repcap.ChannelNull.Default) -> None:
		...
		Snippet: driver.source.bb.w3Gpp.bstation.channel.hsdpa.hset.data.pattern.set(value = [PROPERTY_STRUCT_NAME](), baseStation = repcap.BaseStation.Default, channelNull = repcap.ChannelNull.Default) \n
		...
		self._core.io.write_struct(f'SOURce<HwInstance>:BB:W3GPp:BSTation{baseStation_cmd_val}:CHANnel{channelNull_cmd_val}:HSDPa:HSET:DATA:PATTern', structure)

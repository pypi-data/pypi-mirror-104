...
class Pattern:
	...
	def set(self, structure: PatternStruct, mobileStation=repcap.MobileStation.Default) -> None:
		"""SCPI: [SOURce<HW>]:BB:C2K:MSTation<ST>:TPC:DATA:PATTern \n
		Snippet: driver.source.bb.c2K.mstation.tpc.data.pattern.set(value = [PROPERTY_STRUCT_NAME](), mobileStation = repcap.MobileStation.Default) \n
		...
		self._core.io.write_struct(f'SOURce<HwInstance>:BB:C2K:MSTation{mobileStation_cmd_val}:TPC:DATA:PATTern', structure)
	...
	def get(self, mobileStation=repcap.MobileStation.Default) -> PatternStruct:
		...
		mobileStation_cmd_val = self._base.get_repcap_cmd_value(mobileStation, repcap.MobileStation)
		return self._core.io.query_struct(f'SOURce<HwInstance>:BB:C2K:MSTation{mobileStation_cmd_val}:TPC:DATA:PATTern?', self.__class__.PatternStruct())

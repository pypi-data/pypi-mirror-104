...
class AsLength:
	...
	def set(self, userIx=repcap.UserIx.Default) -> None:
		...
		Snippet: driver.source.bb.eutra.dl.user.asPy.asLength.set(userIx = repcap.UserIx.Default) \n
	...
	def set_with_opc(self, userIx=repcap.UserIx.Default) -> None:
		userIx_cmd_val = self._base.get_repcap_cmd_value(userIx, repcap.UserIx)
		"""SCPI: [SOURce<HW>]:BB:EUTRa:DL:USER<CH>:AS:ASLength \n
		Snippet: driver.source.bb.eutra.dl.user.asPy.asLength.set_with_opc(userIx = repcap.UserIx.Default) \n
		Adjusts the ARB sequence length. \n

...
class Pattern:
	...
	def set(self, structure: PatternStruct, userEquipment=repcap.UserEquipment.Default) -> None:
		...
		Snippet: driver.source.bb.eutra.ul.ue.sl.pattern.set(value = [PROPERTY_STRUCT_NAME](), userEquipment = repcap.UserEquipment.Default) \n
	...
	def get(self, userEquipment=repcap.UserEquipment.Default) -> PatternStruct:
		"""SCPI: [SOURce<HW>]:BB:EUTRa:UL:UE<ST>:SL:PATTern \n
		Snippet: value: PatternStruct = driver.source.bb.eutra.ul.ue.sl.pattern.get(userEquipment = repcap.UserEquipment.Default) \n
		Sets a bit pattern as data source. \n
			:param userEquipment: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Ue')

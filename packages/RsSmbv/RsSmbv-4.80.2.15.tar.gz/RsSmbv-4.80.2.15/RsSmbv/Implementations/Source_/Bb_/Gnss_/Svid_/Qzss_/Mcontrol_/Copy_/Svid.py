...
class Svid:
	...
	def set(self, svid: enums.SvId, satelliteSvid=repcap.SatelliteSvid.Default) -> None:
		"""SCPI: [SOURce<HW>]:BB:GNSS:SVID<CH>:QZSS:MCONtrol:COPY:SVID \n
		Snippet: driver.source.bb.gnss.svid.qzss.mcontrol.copy.svid.set(svid = enums.SvId._1, satelliteSvid = repcap.SatelliteSvid.Default) \n
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:GNSS:SVID{satelliteSvid_cmd_val}:QZSS:MCONtrol:COPY:SVID {param}')

	# noinspection PyTypeChecker
	def get(self, satelliteSvid=repcap.SatelliteSvid.Default) -> enums.SvId:
		"""SCPI: [SOURce<HW>]:BB:GNSS:SVID<CH>:QZSS:MCONtrol:COPY:SVID \n
		Snippet: value: enums.SvId = driver.source.bb.gnss.svid.qzss.mcontrol.copy.svid.get(satelliteSvid = repcap.SatelliteSvid.Default) \n
		Sets the SV ID to that the configuration form the current satellite (SVID<ch>) is applied. Both SV IDs belong to the same
		GNSS system. \n
			:param satelliteSvid: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Svid')
		...
		satelliteSvid_cmd_val = self._base.get_repcap_cmd_value(satelliteSvid, repcap.SatelliteSvid)
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:GNSS:SVID{satelliteSvid_cmd_val}:QZSS:MCONtrol:COPY:SVID?')

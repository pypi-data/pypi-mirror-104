...
class RlcCounter:
	"""RlcCounter commands group definition. 1 total commands, 0 Sub-groups, 1 group commands"""

	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("rlcCounter", core, parent)
	...
	def set(self, rlc_counter: int, userIx=repcap.UserIx.Default, cellNull=repcap.CellNull.Default) -> None:
		...
		Snippet: driver.source.bb.eutra.dl.user.asPy.dl.cell.seqElem.tb2.rlcCounter.set(rlc_counter = 1, userIx = repcap.UserIx.Default, cellNull = repcap.CellNull.Default) \n
	...
	def get(self, userIx=repcap.UserIx.Default, cellNull=repcap.CellNull.Default) -> int:
		"""SCPI: [SOURce<HW>]:BB:EUTRa:DL:USER<CH>:AS:DL:CELL<ST0>:SEQelem:TB2:RLCCounter \n
		Snippet: value: int = driver.source.bb.eutra.dl.user.asPy.dl.cell.seqElem.tb2.rlcCounter.get(userIx = repcap.UserIx.Default, cellNull = repcap.CellNull.Default) \n
		Sets the RLC counter. \n
			:param userIx: optional repeated capability selector. Default value: Nr1 (settable in the interface 'User')
			:param cellNull: optional repeated capability selector. Default value: Nr0 (settable in the interface 'Cell')

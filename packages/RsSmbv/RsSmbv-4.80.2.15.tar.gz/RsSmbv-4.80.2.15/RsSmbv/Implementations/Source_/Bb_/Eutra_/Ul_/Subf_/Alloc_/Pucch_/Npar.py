...
class Npar:
	...
	def set(self, npar: int, subframeNull=repcap.SubframeNull.Default, allocationNull=repcap.AllocationNull.Default, antennaPortIx=repcap.AntennaPortIx.Default) -> None:
		...
		Snippet: driver.source.bb.eutra.ul.subf.alloc.pucch.npar.set(npar = 1, subframeNull = repcap.SubframeNull.Default, allocationNull = repcap.AllocationNull.Default, antennaPortIx = repcap.AntennaPortIx.Default) \n
		Sets the resource index for the supported PUCCH formats. \n
			:param npar: integer n(x) _PUCCH_max depends on the PUCCH format; to query the value, use the corresponding command, for example method RsSmbv.Source.Bb.Eutra.Ul.Pucch.n1eMax. Range: 0 to n(x) _PUCCH_max
	...
	def get(self, subframeNull=repcap.SubframeNull.Default, allocationNull=repcap.AllocationNull.Default, antennaPortIx=repcap.AntennaPortIx.Default) -> int:
		"""SCPI: [SOURce<HW>]:BB:EUTRa:UL:[SUBF<ST0>]:ALLoc<CH0>:PUCCh:NPAR<AP> \n
		Snippet: value: int = driver.source.bb.eutra.ul.subf.alloc.pucch.npar.get(subframeNull = repcap.SubframeNull.Default, allocationNull = repcap.AllocationNull.Default, antennaPortIx = repcap.AntennaPortIx.Default) \n
		Sets the resource index for the supported PUCCH formats. \n
			:param subframeNull: optional repeated capability selector. Default value: Nr0 (settable in the interface 'Subf')
			:param allocationNull: optional repeated capability selector. Default value: Nr0 (settable in the interface 'Alloc')
			:param antennaPortIx: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Npar')
			:return: npar: integer n(x) _PUCCH_max depends on the PUCCH format; to query the value, use the corresponding command, for example method RsSmbv.Source.Bb.Eutra.Ul.Pucch.n1eMax. Range: 0 to n(x) _PUCCH_max"""
		subframeNull_cmd_val = self._base.get_repcap_cmd_value(subframeNull, repcap.SubframeNull)
		allocationNull_cmd_val = self._base.get_repcap_cmd_value(allocationNull, repcap.AllocationNull)
		antennaPortIx_cmd_val = self._base.get_repcap_cmd_value(antennaPortIx, repcap.AntennaPortIx)

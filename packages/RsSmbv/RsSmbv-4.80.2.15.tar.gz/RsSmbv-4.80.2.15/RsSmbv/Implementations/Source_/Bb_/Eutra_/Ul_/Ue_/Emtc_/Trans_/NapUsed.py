...
class NapUsed:
	...
	def get(self, userEquipment=repcap.UserEquipment.Default, transmission=repcap.Transmission.Default) -> int:
		"""SCPI: [SOURce<HW>]:BB:EUTRa:UL:UE<ST>:EMTC:TRANs<CH>:NAPused \n
		Snippet: value: int = driver.source.bb.eutra.ul.ue.emtc.trans.napUsed.get(userEquipment = repcap.UserEquipment.Default, transmission = repcap.Transmission.Default) \n
		Queries the number of antenna ports used for transmissions of the selected PUCCH format. \n
			...
			:return: num_antenna_ports: integer Range: 1 to 1"""
		...
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:EUTRa:UL:UE{userEquipment_cmd_val}:EMTC:TRANs{transmission_cmd_val}:NAPused?')

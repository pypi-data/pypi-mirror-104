...
class Aapto:
	...
	def set(self, block_output: enums.EutraBlockOutput, userEquipment=repcap.UserEquipment.Default) -> None:
		...
		Snippet: driver.source.bb.eutra.ul.ue.aapto.set(block_output = enums.EutraBlockOutput.OUT0, userEquipment = repcap.UserEquipment.Default) \n
	...
	def get(self, userEquipment=repcap.UserEquipment.Default) -> enums.EutraBlockOutput:
		"""SCPI: [SOURce<HW>]:BB:EUTRa:UL:UE<ST>:AAPTo \n
		Snippet: value: enums.EutraBlockOutput = driver.source.bb.eutra.ul.ue.aapto.get(userEquipment = repcap.UserEquipment.Default) \n
		No command help available \n
			:param userEquipment: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Ue')

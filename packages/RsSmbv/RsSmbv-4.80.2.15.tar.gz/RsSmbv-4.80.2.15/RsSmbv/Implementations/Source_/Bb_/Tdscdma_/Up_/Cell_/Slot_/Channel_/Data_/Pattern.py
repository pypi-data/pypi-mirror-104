...
class Pattern:
	...
	def set(self, structure: PatternStruct, cell=repcap.Cell.Default, slotNull=repcap.SlotNull.Default, channelNull=repcap.ChannelNull.Default) -> None:
		...
		Snippet: driver.source.bb.tdscdma.up.cell.slot.channel.data.pattern.set(value = [PROPERTY_STRUCT_NAME](), cell = repcap.Cell.Default, slotNull = repcap.SlotNull.Default, channelNull = repcap.ChannelNull.Default) \n
		...
		self._core.io.write_struct(f'SOURce<HwInstance>:BB:TDSCdma:UP:CELL{cell_cmd_val}:SLOT{slotNull_cmd_val}:CHANnel{channelNull_cmd_val}:DATA:PATTern', structure)

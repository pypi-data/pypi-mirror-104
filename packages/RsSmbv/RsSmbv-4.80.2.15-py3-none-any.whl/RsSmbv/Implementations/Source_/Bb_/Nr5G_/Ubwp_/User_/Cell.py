...
class Cell:
	"""Cell commands group definition. 251 total commands, 2 Sub-groups, 0 group commands
	...
	@property
	def dl(self):
		"""dl commands group. 2 Sub-classes, 0 commands."""
		if not hasattr(self, '_dl'):
			from .Cell_.Dl import Dl
			self._dl = Dl(self._core, self._base)
		return self._dl

	@property
	def ul(self):
		"""ul commands group. 2 Sub-classes, 0 commands."""
		if not hasattr(self, '_ul'):
			from .Cell_.Ul import Ul

...
class Cbits:
	...
	def get(self, userEquipment=repcap.UserEquipment.Default, transmission=repcap.Transmission.Default) -> int:
		"""SCPI: [SOURce<HW>]:BB:EUTRa:UL:UE<ST>:EMTC:TRANs<CH>:PUSCh:CQI:CBITs \n
		Snippet: value: int = driver.source.bb.eutra.ul.ue.emtc.trans.pusch.cqi.cbits.get(userEquipment = repcap.UserEquipment.Default, transmission = repcap.Transmission.Default) \n
		Queries the number of coded CQI bits. \n
			...
			:return: num_coded_cqi_bits: integer Range: 0 to 1E5"""
		...
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:EUTRa:UL:UE{userEquipment_cmd_val}:EMTC:TRANs{transmission_cmd_val}:PUSCh:CQI:CBITs?')

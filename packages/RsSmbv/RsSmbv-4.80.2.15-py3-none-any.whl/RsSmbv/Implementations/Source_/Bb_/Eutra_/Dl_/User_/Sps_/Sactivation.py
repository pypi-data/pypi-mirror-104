...
class Sactivation:
	...
	def set(self, usr_sps_act_subfr: int, userIx=repcap.UserIx.Default) -> None:
		"""SCPI: [SOURce<HW>]:BB:EUTRa:DL:USER<CH>:SPS:SACTivation \n
		Snippet: driver.source.bb.eutra.dl.user.sps.sactivation.set(usr_sps_act_subfr = 1, userIx = repcap.UserIx.Default) \n
	...
	def get(self, userIx=repcap.UserIx.Default) -> int:
		"""SCPI: [SOURce<HW>]:BB:EUTRa:DL:USER<CH>:SPS:SACTivation \n
		Snippet: value: int = driver.source.bb.eutra.dl.user.sps.sactivation.get(userIx = repcap.UserIx.Default) \n
		Defines the start and end subframes of the semi-persistent scheduling. \n
			:param userIx: optional repeated capability selector. Default value: Nr1 (settable in the interface 'User')

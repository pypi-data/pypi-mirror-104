...
class State:
	...
	def set(self, sl_sync_state: bool, userEquipment=repcap.UserEquipment.Default) -> None:
		...
		Snippet: driver.source.bb.eutra.ul.ue.sl.sync.state.set(sl_sync_state = False, userEquipment = repcap.UserEquipment.Default) \n
	...
	def get(self, userEquipment=repcap.UserEquipment.Default) -> bool:
		"""SCPI: [SOURce<HW>]:BB:EUTRa:UL:UE<ST>:SL:SYNC:STATe \n
		Snippet: value: bool = driver.source.bb.eutra.ul.ue.sl.sync.state.get(userEquipment = repcap.UserEquipment.Default) \n
		Enables the synchronization signals. \n
			:param userEquipment: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Ue')

...
class NoPdcchs:
	...
	def set(self, pdcch_count: int, subframeNull=repcap.SubframeNull.Default) -> None:
		...
		Snippet: driver.source.bb.eutra.dl.subf.encc.pdcch.noPdcchs.set(pdcch_count = 1, subframeNull = repcap.SubframeNull.Default) \n
	...
	def get(self, subframeNull=repcap.SubframeNull.Default) -> int:
		"""SCPI: [SOURce<HW>]:BB:EUTRa:DL:[SUBF<ST0>]:ENCC:PDCCh:NOPDcchs \n
		Snippet: value: int = driver.source.bb.eutra.dl.subf.encc.pdcch.noPdcchs.get(subframeNull = repcap.SubframeNull.Default) \n
		Sets the number of PDCCHs to be transmitted. \n
			:param subframeNull: optional repeated capability selector. Default value: Nr0 (settable in the interface 'Subf')

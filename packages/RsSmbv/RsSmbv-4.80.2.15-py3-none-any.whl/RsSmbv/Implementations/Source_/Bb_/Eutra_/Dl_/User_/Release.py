...
class Release:
	...
	def set(self, release: enums.EutraUeReleaseDl, userIx=repcap.UserIx.Default) -> None:
		...
		Snippet: driver.source.bb.eutra.dl.user.release.set(release = enums.EutraUeReleaseDl.EM_A, userIx = repcap.UserIx.Default) \n
	...
	def get(self, userIx=repcap.UserIx.Default) -> enums.EutraUeReleaseDl:
		"""SCPI: [SOURce<HW>]:BB:EUTRa:DL:USER<CH>:RELease \n
		Snippet: value: enums.EutraUeReleaseDl = driver.source.bb.eutra.dl.user.release.get(userIx = repcap.UserIx.Default) \n
		Sets the 3GPP release version the UE supports. \n
			:param userIx: optional repeated capability selector. Default value: Nr1 (settable in the interface 'User')

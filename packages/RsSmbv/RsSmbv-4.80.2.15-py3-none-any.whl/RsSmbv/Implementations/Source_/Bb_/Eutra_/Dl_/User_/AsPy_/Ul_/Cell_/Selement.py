...
class Selement:
	...
	def set(self, sel_elem: int, userIx=repcap.UserIx.Default, cellNull=repcap.CellNull.Default) -> None:
		...
		Snippet: driver.source.bb.eutra.dl.user.asPy.ul.cell.selement.set(sel_elem = 1, userIx = repcap.UserIx.Default, cellNull = repcap.CellNull.Default) \n
	...
	def get(self, userIx=repcap.UserIx.Default, cellNull=repcap.CellNull.Default) -> int:
		"""SCPI: [SOURce<HW>]:BB:EUTRa:DL:USER<CH>:AS:UL:CELL<ST0>:SELement \n
		Snippet: value: int = driver.source.bb.eutra.dl.user.asPy.ul.cell.selement.get(userIx = repcap.UserIx.Default, cellNull = repcap.CellNull.Default) \n
		Selects a table element (i.e. table row) . \n
			:param userIx: optional repeated capability selector. Default value: Nr1 (settable in the interface 'User')
			:param cellNull: optional repeated capability selector. Default value: Nr0 (settable in the interface 'Cell')

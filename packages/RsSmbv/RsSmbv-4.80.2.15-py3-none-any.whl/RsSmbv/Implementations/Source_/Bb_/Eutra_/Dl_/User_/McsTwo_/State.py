...
class State:
	...
	def set(self, mcs_table_2_old: bool, userIx=repcap.UserIx.Default) -> None:
		...
		Snippet: driver.source.bb.eutra.dl.user.mcsTwo.state.set(mcs_table_2_old = False, userIx = repcap.UserIx.Default) \n
	...
	def get(self, userIx=repcap.UserIx.Default) -> bool:
		"""SCPI: [SOURce<HW>]:BB:EUTRa:DL:USER<CH>:MCSTwo:STATe \n
		Snippet: value: bool = driver.source.bb.eutra.dl.user.mcsTwo.state.get(userIx = repcap.UserIx.Default) \n
		This command is supported for backwards compatibility. Use method RsSmbv.Source.Bb.Eutra.Dl.User.Cell.Mcs.set instead. \n
			:param userIx: optional repeated capability selector. Default value: Nr1 (settable in the interface 'User')

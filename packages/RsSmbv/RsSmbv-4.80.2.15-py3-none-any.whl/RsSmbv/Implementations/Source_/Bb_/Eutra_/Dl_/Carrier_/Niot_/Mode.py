...
class Mode:
	...
	def set(self, mode: enums.IdEutraNbiotMode, carrier=repcap.Carrier.Default) -> None:
		...
		Snippet: driver.source.bb.eutra.dl.carrier.niot.mode.set(mode = enums.IdEutraNbiotMode.ALON, carrier = repcap.Carrier.Default) \n
	...
	def get(self, carrier=repcap.Carrier.Default) -> enums.IdEutraNbiotMode:
		"""SCPI: [SOURce<HW>]:BB:EUTRa:DL:CARRier<CH>:NIOT:MODE \n
		Snippet: value: enums.IdEutraNbiotMode = driver.source.bb.eutra.dl.carrier.niot.mode.get(carrier = repcap.Carrier.Default) \n
		Sets the operating mode. \n
			:param carrier: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Carrier')

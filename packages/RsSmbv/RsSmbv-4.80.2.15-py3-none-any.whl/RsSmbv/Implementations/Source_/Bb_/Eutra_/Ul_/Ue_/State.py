...
class State:
	...
	def set(self, state: bool, userEquipment=repcap.UserEquipment.Default) -> None:
		...
		Snippet: driver.source.bb.eutra.ul.ue.state.set(state = False, userEquipment = repcap.UserEquipment.Default) \n
	...
	def get(self, userEquipment=repcap.UserEquipment.Default) -> bool:
		"""SCPI: [SOURce<HW>]:BB:EUTRa:UL:UE<ST>:STATe \n
		Snippet: value: bool = driver.source.bb.eutra.ul.ue.state.get(userEquipment = repcap.UserEquipment.Default) \n
		Selects the user equipment state. \n
			:param userEquipment: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Ue')

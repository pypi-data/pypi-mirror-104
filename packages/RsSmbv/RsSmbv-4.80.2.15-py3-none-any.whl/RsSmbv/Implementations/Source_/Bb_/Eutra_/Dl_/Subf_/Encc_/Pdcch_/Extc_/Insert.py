...
class Insert:
	...
	def set(self, subframeNull=repcap.SubframeNull.Default) -> None:
		...
		Snippet: driver.source.bb.eutra.dl.subf.encc.pdcch.extc.insert.set(subframeNull = repcap.SubframeNull.Default) \n
	...
	def set_with_opc(self, subframeNull=repcap.SubframeNull.Default) -> None:
		subframeNull_cmd_val = self._base.get_repcap_cmd_value(subframeNull, repcap.SubframeNull)
		"""SCPI: [SOURce<HW>]:BB:EUTRa:DL:[SUBF<ST0>]:ENCC:PDCCh:EXTC:INSert \n
		Snippet: driver.source.bb.eutra.dl.subf.encc.pdcch.extc.insert.set_with_opc(subframeNull = repcap.SubframeNull.Default) \n
		Insert a new row before the currently selected item. \n
		Same as set, but waits for the operation to complete before continuing further. Use the RsSmbv.utilities.opc_timeout_set() to set the timeout value. \n
			:param subframeNull: optional repeated capability selector. Default value: Nr0 (settable in the interface 'Subf')"""
		self._core.io.write_with_opc(f'SOURce<HwInstance>:BB:EUTRa:DL:SUBF{subframeNull_cmd_val}:ENCC:PDCCh:EXTC:INSert')

...
class Pattern:
	...
	def set(self, structure: PatternStruct, userIx=repcap.UserIx.Default) -> None:
		...
		Snippet: driver.source.bb.eutra.dl.user.pattern.set(value = [PROPERTY_STRUCT_NAME](), userIx = repcap.UserIx.Default) \n
		...
		self._core.io.write_struct(f'SOURce<HwInstance>:BB:EUTRa:DL:USER{userIx_cmd_val}:PATTern', structure)

	def get(self, userIx=repcap.UserIx.Default) -> PatternStruct:
		"""SCPI: [SOURce<HW>]:BB:EUTRa:DL:USER<CH>:PATTern \n
		Snippet: value: PatternStruct = driver.source.bb.eutra.dl.user.pattern.get(userIx = repcap.UserIx.Default) \n
		Sets a bit pattern as data source. \n
			:param userIx: optional repeated capability selector. Default value: Nr1 (settable in the interface 'User')

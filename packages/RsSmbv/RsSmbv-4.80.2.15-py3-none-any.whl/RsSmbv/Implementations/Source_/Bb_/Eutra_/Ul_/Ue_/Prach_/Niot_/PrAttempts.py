...
class PrAttempts:
	...
	def set(self, preamble_attempt: int, userEquipment=repcap.UserEquipment.Default) -> None:
		...
		Snippet: driver.source.bb.eutra.ul.ue.prach.niot.prAttempts.set(preamble_attempt = 1, userEquipment = repcap.UserEquipment.Default) \n
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:EUTRa:UL:UE{userEquipment_cmd_val}:PRACh:NIOT:PRATtempts {param}')

	def get(self, userEquipment=repcap.UserEquipment.Default) -> int:
		"""SCPI: [SOURce<HW>]:BB:EUTRa:UL:UE<ST>:PRACh:NIOT:PRATtempts \n
		Snippet: value: int = driver.source.bb.eutra.ul.ue.prach.niot.prAttempts.get(userEquipment = repcap.UserEquipment.Default) \n
		Sets the number of preamble attempts. \n
			:param userEquipment: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Ue')
			:return: preamble_attempt: integer Range: 0 to 30"""
		userEquipment_cmd_val = self._base.get_repcap_cmd_value(userEquipment, repcap.UserEquipment)
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:EUTRa:UL:UE{userEquipment_cmd_val}:PRACh:NIOT:PRATtempts?')

...
class Crnti:
	...
	def set(self, user_sps_cr_nti: int, userIx=repcap.UserIx.Default) -> None:
		"""SCPI: [SOURce<HW>]:BB:EUTRa:DL:USER<CH>:SPS:CRNTi \n
		Snippet: driver.source.bb.eutra.dl.user.sps.crnti.set(user_sps_cr_nti = 1, userIx = repcap.UserIx.Default) \n
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:EUTRa:DL:USER{userIx_cmd_val}:SPS:CRNTi {param}')

	def get(self, userIx=repcap.UserIx.Default) -> int:
		"""SCPI: [SOURce<HW>]:BB:EUTRa:DL:USER<CH>:SPS:CRNTi \n
		Snippet: value: int = driver.source.bb.eutra.dl.user.sps.crnti.get(userIx = repcap.UserIx.Default) \n
		Sets the SPS C-RNTI parameter. \n
			:param userIx: optional repeated capability selector. Default value: Nr1 (settable in the interface 'User')
			:return: user_sps_cr_nti: integer Range: 1 to 65523"""
		userIx_cmd_val = self._base.get_repcap_cmd_value(userIx, repcap.UserIx)
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:EUTRa:DL:USER{userIx_cmd_val}:SPS:CRNTi?')

...
class CeLevel:
	...
	def set(self, ce_level: enums.EutraCeLevel, userEquipment=repcap.UserEquipment.Default) -> None:
		...
		Snippet: driver.source.bb.eutra.ul.ue.emtc.ceLevel.set(ce_level = enums.EutraCeLevel.CE01, userEquipment = repcap.UserEquipment.Default) \n
	...
	def get(self, userEquipment=repcap.UserEquipment.Default) -> enums.EutraCeLevel:
		"""SCPI: [SOURce<HW>]:BB:EUTRa:UL:UE<ST>:EMTC:CELevel \n
		Snippet: value: enums.EutraCeLevel = driver.source.bb.eutra.ul.ue.emtc.ceLevel.get(userEquipment = repcap.UserEquipment.Default) \n
		Set the coverage extension level (CE) . \n
			:param userEquipment: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Ue')

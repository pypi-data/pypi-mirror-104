...
class Srelease:
	...
	def set(self, usr_sps_rel_subfr: int, userIx=repcap.UserIx.Default) -> None:
		"""SCPI: [SOURce<HW>]:BB:EUTRa:DL:USER<CH>:SPS:SRELease \n
		Snippet: driver.source.bb.eutra.dl.user.sps.srelease.set(usr_sps_rel_subfr = 1, userIx = repcap.UserIx.Default) \n
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:EUTRa:DL:USER{userIx_cmd_val}:SPS:SRELease {param}')

	def get(self, userIx=repcap.UserIx.Default) -> int:
		"""SCPI: [SOURce<HW>]:BB:EUTRa:DL:USER<CH>:SPS:SRELease \n
		Snippet: value: int = driver.source.bb.eutra.dl.user.sps.srelease.get(userIx = repcap.UserIx.Default) \n
		Defines the start and end subframes of the semi-persistent scheduling. \n
			:param userIx: optional repeated capability selector. Default value: Nr1 (settable in the interface 'User')
			:return: usr_sps_rel_subfr: integer Range: 0 to 65535"""
		userIx_cmd_val = self._base.get_repcap_cmd_value(userIx, repcap.UserIx)
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:EUTRa:DL:USER{userIx_cmd_val}:SPS:SRELease?')

...
class Norb:
	"""Norb commands group definition. 1 total commands, 0 Sub-groups, 1 group commands"""

	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("norb", core, parent)

	def get(self, allocationNull=repcap.AllocationNull.Default) -> int:
		"""SCPI: [SOURce<HW>]:BB:EUTRa:DL:EMTC:ALLoc<CH0>:NORB \n
		Snippet: value: int = driver.source.bb.eutra.dl.emtc.alloc.norb.get(allocationNull = repcap.AllocationNull.Default) \n
		Queries the number of resource blocks the allocation spans. \n
			:param allocationNull: optional repeated capability selector. Default value: Nr0 (settable in the interface 'Alloc')
			:return: number_res_blk: integer Range: 1 to 6"""

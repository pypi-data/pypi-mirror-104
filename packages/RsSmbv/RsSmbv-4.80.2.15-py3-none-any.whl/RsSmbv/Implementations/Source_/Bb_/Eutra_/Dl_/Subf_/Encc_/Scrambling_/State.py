...
class State:
	...
	def set(self, state: bool, subframeNull=repcap.SubframeNull.Default) -> None:
		...
		Snippet: driver.source.bb.eutra.dl.subf.encc.scrambling.state.set(state = False, subframeNull = repcap.SubframeNull.Default) \n
	...
	def get(self, subframeNull=repcap.SubframeNull.Default) -> bool:
		"""SCPI: [SOURce<HW>]:BB:EUTRa:DL:[SUBF<ST0>]:ENCC:SCRambling:STATe \n
		Snippet: value: bool = driver.source.bb.eutra.dl.subf.encc.scrambling.state.get(subframeNull = repcap.SubframeNull.Default) \n
		Enables/disables the scrambling of the enhanced channels. \n
			:param subframeNull: optional repeated capability selector. Default value: Nr0 (settable in the interface 'Subf')

...
class Snumber:
	...
	def set(self, starting_number: int, allocationNull=repcap.AllocationNull.Default) -> None:
		...
		Snippet: driver.source.bb.eutra.dl.niot.dci.alloc.nprach.snumber.set(starting_number = 1, allocationNull = repcap.AllocationNull.Default) \n
	...
	def get(self, allocationNull=repcap.AllocationNull.Default) -> int:
		"""SCPI: [SOURce<HW>]:BB:EUTRa:DL:NIOT:DCI:ALLoc<CH0>:NPRach:SNUMber \n
		Snippet: value: int = driver.source.bb.eutra.dl.niot.dci.alloc.nprach.snumber.get(allocationNull = repcap.AllocationNull.Default) \n
		Sets the DCI field starting number of NPRACH repetitions (IRep) . \n
			:param allocationNull: optional repeated capability selector. Default value: Nr0 (settable in the interface 'Alloc')

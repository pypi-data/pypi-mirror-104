...
class Nhids:
	...
	def set(self, num_harq_ids: int, userIx=repcap.UserIx.Default) -> None:
		...
		Snippet: driver.source.bb.eutra.dl.user.asPy.dl.nhids.set(num_harq_ids = 1, userIx = repcap.UserIx.Default) \n
	...
	def get(self, userIx=repcap.UserIx.Default) -> int:
		"""SCPI: [SOURce<HW>]:BB:EUTRa:DL:USER<CH>:AS:DL:NHIDs \n
		Snippet: value: int = driver.source.bb.eutra.dl.user.asPy.dl.nhids.get(userIx = repcap.UserIx.Default) \n
		Sets the number of HARQ process IDs. \n
			:param userIx: optional repeated capability selector. Default value: Nr1 (settable in the interface 'User')

...
class StsCarrier:
	...
	def get(self, userEquipment=repcap.UserEquipment.Default, transmission=repcap.Transmission.Default) -> int:
		...
		Snippet: value: int = driver.source.bb.eutra.ul.ue.niot.trans.stsCarrier.get(userEquipment = repcap.UserEquipment.Default, transmission = repcap.Transmission.Default) \n
		...
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:EUTRa:UL:UE{userEquipment_cmd_val}:NIOT:TRANs{transmission_cmd_val}:STSCarrier?')

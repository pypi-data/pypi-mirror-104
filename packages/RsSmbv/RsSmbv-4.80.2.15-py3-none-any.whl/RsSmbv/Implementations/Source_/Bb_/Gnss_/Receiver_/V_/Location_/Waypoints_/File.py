...
class File:
	...
	def set(self, way_points: str, vehicle=repcap.Vehicle.Default) -> None:
		"""SCPI: [SOURce<HW>]:BB:GNSS:RECeiver:[V<ST>]:LOCation:WAYPoints:FILE \n
		Snippet: driver.source.bb.gnss.receiver.v.location.waypoints.file.set(way_points = '1', vehicle = repcap.Vehicle.Default) \n
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:GNSS:RECeiver:V{vehicle_cmd_val}:LOCation:WAYPoints:FILE {param}')

	def get(self, vehicle=repcap.Vehicle.Default) -> str:
		"""SCPI: [SOURce<HW>]:BB:GNSS:RECeiver:[V<ST>]:LOCation:WAYPoints:FILE \n
		Snippet: value: str = driver.source.bb.gnss.receiver.v.location.waypoints.file.get(vehicle = repcap.Vehicle.Default) \n
		Selects a predefined or user-defined waypoint files to simulate a moving scenario. \n
			:param vehicle: optional repeated capability selector. Default value: Nr1 (settable in the interface 'V')
			:return: way_points: Filename or complete file path incl. extension Waypoint files have the extension *.txt, *.nmea, *.kml or *.xtd."""
		vehicle_cmd_val = self._base.get_repcap_cmd_value(vehicle, repcap.Vehicle)
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:GNSS:RECeiver:V{vehicle_cmd_val}:LOCation:WAYPoints:FILE?')

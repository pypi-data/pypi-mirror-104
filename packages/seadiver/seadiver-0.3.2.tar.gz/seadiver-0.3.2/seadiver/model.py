...
class ANN():
    ...
    def train(self, x, t, learning_rate, iteration, save_log=False, flush_log=True, display=True, error_round=10):
        ...
        for i in range(iteration):
            ...
            if gradient_zero_layer_flag:
                ...
                if len(w_layers) in gradient_zero_layer_indexes:  
                    print(f"Session Terminated: layer{gradient_zero_layer_indexes}'s gradient equals to 0, step: {str(i+1)} error: {str(round(error, error_round))}                                      ")
                    return
                else:
    ...
    def export(self, directory, file_name= None, include = "essential"):

"""
   ...
   0.27版新增给角色新增了说话功能
   say(s,second)，让角色说话
   单独函数:setalpha,设定图像的透明通道值。
   center_window:设定tk窗口在屏幕中央   
"""
__author__ = '李兴球'
__date__ = '2021/5/1'
...
def makegif(images,filename=None,duration=200,quality=65,ext='.png'):
   """合帧
   ...
   pic.save(filename, save_all=True,append_images=frames[1:], optimize=True,quality=quality,duration=duration)
...
def txt2image(txt,fontfile="simkai.ttf",fontsize=64,
              color=(255,255,255,255),stroke=None,bgcolor='white'):
    """
...
class GameTurtle:
    ...
    def say(self,words,second,wait=True,bg='white',fz=14,fg='black'):
        ...
        if wait==False:
           self._canvas.after(second*1000,self._end_say) # 一定时间后结束说话泡泡显示
        else:
...
def main():  
        ...
        cv = Canvas(width=480,height=360,bg='black')
        cv.pack()
        center_window(root)             # 居中显示窗口
      
        girl1 = Sprite(cv)
        girl1.bk(100)
        girl1.addy(80)

        girl2 = Sprite(cv)            # 生成角色girl2
        girl2.setrotmode(1)               # 左右翻转
        girl2.fd(100)                     # 前进100 
        girl2.right(180)                  # 右转90度
        girl2.addy(80)
        ...
        girl1.say('我在向李兴球学Python',2)
        girl2.say('哈哈，我也是呀',2)

        girl1.say('李老师说分享这篇文章到微信朋友圈就能免费得到很多Python课程',5)
        girl2.say('嗯，我已转发到朋友圈了。',3)

...
class RelationshipManager(IteratorMixin):
    ...
    def _get_cached_or_fetch(self, /, ret_params=None):
        ret_params = ret_params or []
        logger.debug("REL_MANAGER: GET_CACHED_OR_FETCH: ret_params=%s, related_objects=%s, instance=%s",
                     ret_params, self.related_objects, self.instance)
    ...
    def one(self, *ret_params):
        ...
        r = self._get_cached_or_fetch(ret_params=ret_params)
        if r and len(r) > 0:
            return r[0].end_node
        return None
    ...
    def filter(self, *ret_params, **kwargs):
        data = self._get_cached_or_fetch(ret_params=ret_params)
        res = []
        for d in data.nodes():
            if all(
    ...
    def _query(self, other, cypher_mode="CREATE", **kwargs):
        ps = ParameterStore()
        ...
        if cypher_mode == "CREATE":
            ...
            query = f"""MATCH {target_node_cypher.repr()} {rel_pattern.repr()} {target_node_cypher.repr()}
            DELETE {rel_pattern.alias}
            """
        else:
            raise ValueError(f"{cypher_mode} not valid for this method")
        params = ps.params
        connection.cypher(query, params)

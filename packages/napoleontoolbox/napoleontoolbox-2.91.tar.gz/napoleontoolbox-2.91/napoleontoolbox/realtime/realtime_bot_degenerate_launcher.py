...
from napoleontoolbox.bot import binance_spot_bot
...
import numpy as np
from napoleontoolbox.parallel_run import parallel_signal_launcher
from napoleontoolbox.parallel_run import signal_runner
import multiprocessing
from napoleontoolbox.connector import napoleon_connector
from napoleontoolbox.signal import parameters_generator
from numpy.lib.stride_tricks import as_strided as stride

from datetime import datetime
import napoleontoolbox as npx
from napoleontoolbox.signal import parameters_generator, signal_generator
from scipy import stats
import pandas as pd
...
class RealTimeDegenerateBotLauncher():
    def __init__(self, drop_token='', dropbox_backup=True, BINANCE_PUBLIC='', BINANCE_SECRET='', AWS_PUBLIC='', AWS_SECRET='', AWS_REGION='',bucket='', local_root_directory='../data', shitcoin_universe = None, user='napoleon',  db_path_suffix = '_run.sqlite', list_pkl_file_suffix = 'my_list.pkl', backed_up_day_signal_histo_suffix='backed_up_day_signal_histo.pkl', backed_up_min_signal_histo_suffix='backed_up_min_signal_histo.pkl', load_daily_histo = True, from_cryptocompare=False, backtest=False):
        ...
        self.shitcoin_universe = shitcoin_universe
        self.shitcoin_universe_pairs = [ f'{me_under}USDT' for me_under in shitcoin_universe]
        self.pair_dict = {}
        for me_token in self.shitcoin_universe_pairs:
            loc_dict={}
            loc_dict['high'] = {}
            loc_dict['low'] = {}
            loc_dict['volu'] = {}
    ...
    def start_fetching_pair(self):
        for me_pair in self.shitcoin_universe_pairs:
            bm = BinanceSocketManager(self.client)
            pair_callback = partial(pair_handling_message, self, me_pair)
            bm.start_aggtrade_socket(me_pair, pair_callback)
            bm.start()

    def reset_minute_np_to_lookback_window(self, pair, loolback_window = 70):
    ...
    def get_aggregated_historical_data(self, starting_date, running_date, reaggregate = True, refetch_pricing = True, hourly_prices = True):
        starting_date_string = starting_date.strftime('%m-%d-%y')
        print('fetching universe')
        print(self.shitcoin_universe)
    ...
    def backtest_strategy(self, starting_date, running_date):
        aggregated_df = self.get_aggregated_historical_data(starting_date, running_date)
        print(aggregated_df.shape)
        ...
        missing_tickers = list(set(self.shitcoin_universe_pairs) - set(aggregated_df.columns))
        assert len(missing_tickers) == 0

        shitcoin_universe = self.shitcoin_universe_pairs
        aggregated_df = aggregated_df[shitcoin_universe].astype(float)

        filter_bull_run = False
        if filter_bull_run:
        ...
        lookback_windows = [5, 10, 15, 20, 25, 30, 35, 40, 45, 50]
        nb_chosens = [1,2,3,4,5,10,15,20,25]
        equis = [True, False]


        ####### since 2021-01-10 UNIVERSER 200
        #maxPerf 37.516611404092814 maxLookback 20 maxChosen 15 maxEqui False
        #maxPerf 84.44369193553915 maxLookback 20 maxChosen 15 maxEqui False
        #maxPerf -0.24829910556505919 maxLookback 15 maxChosen 25 maxEqui True


        ###### best sharpe and best calmar
        #lookback_windows = [20]
        #nb_chosens = [15]
        #equis = [False]
        ...
        for lookback_window in lookback_windows:
            for nb_chosen in nb_chosens:
                for equi in equis:
                    ...
                    signal_df.columns = [f'S{col}' for col in signal_df.columns]

                    shifted_signal_df = signal_df.shift(1)

                    allin_df = pd.merge(df_perf.copy(), shifted_signal_df, left_index=True, right_index=True)
                    ...
                    for i in shitcoin_universe:
                        if f'S{i}' not in allin_df.columns:
                            allin_df[f'S{i}'] = 0.

                        allin_df['RS{}'.format(i)] = allin_df[f'R{i}'] * allin_df[f'S{i}']  # * np.append(np.zeros(1),
                        # dfsig2['S{}'.format(i)].values[:-1]) * np.append(
                    #        np.zeros(1), dfperf['expo'].values[:-1])
                    ...
                    print(f'maxPerf {maxPerf} maxLookback {maxLookback} maxChosen {maxChosen} maxEqui {maxEqui}')
                    print(
                        f'maxPerf {maxCalPerf} maxLookback {maxCalLookback} maxChosen {maxCalChosen} maxEqui {maxCalEqui}')
                    print(
                        f'maxPerf {maxMDDPerf} maxLookback {maxMDDLookback} maxChosen {maxMDDChosen} maxEqui {maxMDDEqui}')

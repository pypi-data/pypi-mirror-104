...
class NaPoleonBinanceSpotBot(object):
...
#            print(f'fetching {encours_date}')
    ...
    def get_hourly_ohlc(self, pair, starting_date, ending_date):
        ...
        dataframe.columns = ['time', 'open', 'high', 'low', 'close', 'volume', 'close_time', 'quote_asset_vol', 'ntrades',
                          'takerbuy_base_asset_vol', 'takerbuy_quote_asset_vol', 'ignore']
    ...
    def get_ticker_info_usdt(self, volume_limit = 100, to_ban = ['TLMUSDT','BCHSVUSDT','USDTZAR','USDTTRY','USDTBIDR','USDTIDRT','USDTBVND','USDTRUB','USDTNGN','STRAXUSDT']):#'SRMUSDT','PAXGUSDT',]):
        ticker_info = self.client.get_ticker()
        ticker_info_df = pd.DataFrame(ticker_info)
        def is_tradable(row):
            tradable = False

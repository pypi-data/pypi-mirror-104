...
def json_encoder(obj):
    ...
    if isinstance(obj, (TargetText, Info, CitationSourceLink, TargetLocationSelection, TargetTextLocationLink,
...
def generate_target_html(target_texts, target_folder_path, output_folder_path):
...
def main():
    argument_parser = ArgumentParser(description="This script allows the user to create the files needed for a"
                                                 "website that visualizes the lotte algorithm results.")

    argument_parser.add_argument("source_file_path", nargs=1, metavar="source-file-path",
                                 help="Path to the source text file")
    argument_parser.add_argument("target_folder_path", nargs=1, metavar="target-folder-path",
                                 help="Path to the target texts folder path")
    argument_parser.add_argument("matches_folder_path", nargs=1, metavar="matches-folder-path",
    ...
    argument_parser.add_argument("--title", dest="title", help="Title of the work", default="NN")
    argument_parser.add_argument("--author", dest="author", help="Author of the work", default="NN")
    argument_parser.add_argument("--year", dest="year", help="Year of the work", default="0", type=int)
    ...
    generate_info_json(output_folder_path, title, author, year)

    with open(source_file_path, 'r') as source_file:
        source_content = source_file.read()

    citation_sources, segment_id_to_target_location_map = get_citation_sources(matches_folder_path,
                                                                               source_content)

    add_important_segments(citation_sources)

    target_texts, target_location_id_to_source_location_map = generate_target_texts(matches_folder_path,
                                                                                    target_folder_path)
    target_text_location_links = generate_target_text_location_links(citation_sources, target_texts,
                                                                     target_location_id_to_source_location_map)

    citation_source_links = generate_citation_source_links(citation_sources, target_texts,
                                                           segment_id_to_target_location_map)
    important_segment_links = generate_important_segment_links(citation_sources, target_texts,
                                                               segment_id_to_target_location_map)

    max_target_texts_count = calculate_max_target_texts_count(citation_source_links)
    max_segment_frequency = calculate_max_segment_frequency(citation_sources)

    start_time = time.time()
    generate_source_html(source_content, citation_sources, output_folder_path, max_target_texts_count,
                         max_segment_frequency, citation_source_links)

    print("\n--- Generate source html, %s seconds ---" % (time.time() - start_time))

    start_time = time.time()
    generate_target_html(target_texts, target_folder_path, output_folder_path)

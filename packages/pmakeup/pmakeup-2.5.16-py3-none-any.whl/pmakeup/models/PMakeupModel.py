...
class PMakeupModel(abc.ABC):
    ...
    def __fetch_pmakeup_plugins_installed(self) -> Iterable[type]:
        ...
        for package in map(lambda p: p, pkg_resources.working_set):

            if re.search(r"^pmakeup-plugin(s)?-.+", package.project_name) is None and re.search(r".+-pmakeup-plugin(s)?$", package.key) is None:
                continue
            logging.info(f"Detected installed package {package.project_name} which should contains pmakeup plugins. Importing it")

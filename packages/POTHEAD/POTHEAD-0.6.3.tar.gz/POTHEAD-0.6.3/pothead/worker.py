...
class Server:
    ...
    def __init__(self, addr, app, load_balancer=LoadBalancer):
        (host, port) = addr
    ...
    def _fetch_and_run_one_job(self, addr, cb_request_received=None):
    ...
    def _run_one_poll(self, broker_instance_lease: ResourceLoan, callbacks):
        ...
        try:
            with broker_instance_lease as addr:
                self._fetch_and_run_one_job(addr, request_received)
        finally:

...
class BusinessMediaFetcher:
  ...
  def process_all(self, arg):
    try:
      ...
      date_limit = None
      if arg:
        date_limit = self.utils.get_date(arg[0])
        self.logger.debug("Got date_limit '%s'" % (date_limit))
      else:
        date_limit = self.utils.get_date()
        self.logger.debug("Got date_limit '%s'" % (date_limit))
  ...
  def parse_and_insert(self, conn, user_id, data, access_token, is_story=False):
    ...
    for item in data:
      created_at = self.parse_time(item["timestamp"])
    ...
    for item in data:
      ...
      if replies and replies.get('data'):
        ...
        for reply in replies.get('data'):
          created_at = self.parse_time(reply["timestamp"])

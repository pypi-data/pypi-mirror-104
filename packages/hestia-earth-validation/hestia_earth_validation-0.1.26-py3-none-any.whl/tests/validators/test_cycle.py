...
from hestia_earth.validation.validators.cycle import validate_cycle, validate_cycle_dates, validate_cycleDuration, \
    validate_functionalUnitMeasure, validate_economicValueShare, validate_sum_aboveGroundCropResidue
...
class TestValidatorsCycle(unittest.TestCase):
    def test_validate_valid(self):
        with open(f"{fixtures_path}/cycle/valid.json") as f:
            node = json.load(f)
        self.assertListEqual(validate_cycle(node), [True] * 20)
    ...
    def test_validate_sum_aboveGroundCropResidue_valid(self):
        with open(f"{fixtures_path}/cycle/aboveGroundCropResidue/valid.json") as f:
            data = json.load(f)
        self.assertEqual(validate_sum_aboveGroundCropResidue(data.get('products')), True)

    def test_validate_sum_aboveGroundCropResidue_invalid(self):
        with open(f"{fixtures_path}/cycle/aboveGroundCropResidue/invalid.json") as f:
            data = json.load(f)
        self.assertEqual(validate_sum_aboveGroundCropResidue(data.get('products')), {
            'level': 'error',

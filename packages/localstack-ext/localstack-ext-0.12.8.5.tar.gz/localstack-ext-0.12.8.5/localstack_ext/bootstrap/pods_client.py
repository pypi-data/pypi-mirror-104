...
class CloudPodManager(qfeHz):
 BACKEND='_none_'
 def __init__(self,pod_name=qfeHa,config=qfeHa):
  self.pod_name=pod_name
  self._pod_config=config
 def push(self):
  raise qfeHE('Not implemented')
 def pull(self):
  raise qfeHE('Not implemented')
 ...
 def restart_container(self):
  ...
  def check_status():
   ...
   assert qfeHm(statuses)==qfeHm(['running'])
 ...
 def get(cls,pod_name):
  ...
  raise qfeHE('Unable to find Cloud Pod manager implementation type "%s"'%backend)
 @qfeHR
 def data_dir(cls):
  ...
  if not config.DATA_DIR:
   raise qfeHE('Working with local cloud pods requires $DATA_DIR configuration')
...
class CloudPodManagerGit(CloudPodManager):
 ...
 def push(self):
  ...
  is_empty_repo=b'HEAD' not in repo or repo.refs.allkeys()==qfeHm([b'HEAD'])
  if is_empty_repo:
  ...
  if branch not in repo:
   porcelain.branch_create(repo,branch,force=qfeHc)
 ...
 def pull(self):
  ...
  if remote_ref not in remote_refs:
   raise qfeHE('Unable to find branch "%s" in remote git repo'%branch)
  remote_location=self.pod_config.get('url')
  self.switch_branch(branch)
  ...
  for folder in PERISTED_FOLDERS:
   ...
   cp_r(src_folder,tgt_folder,rm_dest_on_conflict=qfeHc)
  self.restart_container()
 def client(self):
  client,path=get_transport_and_path_from_url(self.pod_config.get('url'))
  return client,path
 def local_repo(self):
  self.clone_dir=qfeHg(self,'clone_dir',qfeHa)
...
class PodConfigManagerMeta(qfeHP):
 def __getattr__(cls,attr):
  def _call(*args,**kwargs):
   result=qfeHa
   for manager in cls.CHAIN:
    try:
     tmp=qfeHg(manager,attr)(*args,**kwargs)
     if tmp:
      if not result:
       result=tmp
      elif qfeHS(tmp,qfeHO)and qfeHS(result,qfeHO):
       result.extend(tmp)
    except qfeHE:
     pass
   if result is not qfeHa:
    return result
   raise qfeHE('Unable to run operation "%s" for local or remote configuration'%attr)
  return _call
class PodConfigManager(qfeHz,metaclass=PodConfigManagerMeta):
 CHAIN=[]
 @qfeHR
 def pod_config(cls,pod_name):
  ...
  if not pod_config:
   raise qfeHE('Unable to find config for pod named "%s"'%pod_name)
  return pod_config[0]
class PodConfigManagerLocal(qfeHz):
 CONFIG_FILE='.localstack.yml'
 def list_pods(self):
  local_pods=self._load_config(safe=qfeHc).get('pods',{})
  local_pods=[{'pod_name':k,'state':'Local Only',**v}for k,v in local_pods.items()]
  existing_names=qfeHm([pod['pod_name']for pod in local_pods])
  result=[pod for pod in local_pods if pod['pod_name']not in existing_names]
  return result
 def store_pod_metadata(self,pod_name,metadata):
  pass
 def _load_config(self,safe=qfeHN):
  try:
   return yaml.load(to_str(load_file(self.CONFIG_FILE)))
  except qfeHE:
   if safe:
    return{}
   raise qfeHE('Unable to find and parse config file "%s"'%self.CONFIG_FILE)
class PodConfigManagerRemote(qfeHz):
 def list_pods(self):
  result=[]
  ...
  if response.status_code>=400:
   raise qfeHE('Unable to fetch list of pods from API (code %s): %s'%(response.status_code,content))
 ...
 def store_pod_metadata(self,pod_name,metadata):
  ...
  content=response.content
  if response.status_code>=400:
   raise qfeHE('Unable to store pod metadata in API (code %s): %s'%(response.status_code,content))
...
def pull_state(pod_name,args):
 if not pod_name:
  raise qfeHE('Need to specify a pod name')
 backend=CloudPodManager.get(pod_name=pod_name)
 backend.pull()
def list_pods(args):

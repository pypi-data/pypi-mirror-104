...
def read_api_key():
 ...
 if key:
  return key
 raise RNjhL('Unable to retrieve API key. Please configure $LOCALSTACK_API_KEY in your environment')
def fetch_key():
 ...
 try:
  ...
  result=requests.post('%s/activate'%config.API_URL,json.dumps(data),verify=RNjho)
...
def cache_key_locally(api_key,key_b64):
 ...
 timestamp=RNjhd(RNjhD(now_utc()))
 key_raw=to_str(base64.b64decode(key_b64))
 for i in RNjhV(RNjhy(timestamp)):
  key_raw=str_insert(key_raw,i*2,timestamp[i])
 key_b64=to_str(base64.b64encode(to_bytes(key_raw)))
 configs['cached_key']={'timestamp':RNjhD(timestamp),'key_hash':md5(api_key),'key':key_b64}
...
def load_cached_key(api_key):
 ...
 if cached_key.get('key_hash')!=md5(api_key):
  raise RNjhL('Cached key was created for a different API key')
 now=now_utc()
 if(now-cached_key['timestamp'])>MAX_KEY_CACHE_DURATION_SECS:
  raise RNjhL('Cached key expired')
 timestamp=RNjhd(cached_key['timestamp'])
...
def prepare_environment():
 ...
 if not ENV_PREPARED.get('finalized'):
  try:
   key=fetch_key()
   if not key:
    raise RNjhL('Unable to fetch and validate API key from environment')
   if key!='test':
    decrypt_files(key)
    LOG.info('Successfully activated API key')
  except RNjhL as e:
   if os.environ.get('LOCALSTACK_API_KEY')and localstack_config.is_env_true('LOG_LICENSE_ISSUES'):
...
def get_auth_headers():
 ...
 if not login_info:
  raise RNjhL('Please log in first')

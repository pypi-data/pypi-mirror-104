...
def apply_patches():
 ...
 extended_proto_cmds.update({'SITE ADDUSER':TXlcP(perm='m',auth=TXlcx,arg=TXlcx,help='Syntax: SITE <SP> ADDUSER USERNAME PASSWORD HOME PRIVS <SP>.')})
...
def start_ftp(port):
 if USE_SUBPROCESS:
  ...
  thread=ShellCommandThread(cmd,outfile=subprocess.PIPE,env_vars=env_vars,quiet=TXlce)
  thread.start()
  time.sleep(2)
 else:
  thread=do_start_ftp(port,asynchronous=TXlcx)
 time.sleep(2)
 wait_for_port_open(port,retries=10,sleep_time=1.5)
 TMP_THREADS.append(thread)
 return thread
def do_start_ftp(port,asynchronous=TXlcx):
...
def main():
 do_start_ftp(TXlcE(sys.argv[1]),asynchronous=TXlce)

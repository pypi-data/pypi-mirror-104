...
def register_localstack_plugins():
 ...
 if bXGgH([is_api_enabled(api)for api in EXTERNAL_PORT_APIS]):
...
def load_plugin_files():
 try:
  ...
  from localstack_ext.services.elb import elb_starter,elb_listener
  ...
  register_plugin(Plugin('costexplorer',start=costexplorer_starter.start_costexplorer))
  ...
  if 'No module named' not in bXGgk(e):
   print('ERROR: %s'%e)
...
def api_key_configured():
 return bXGgS(os.environ.get('LOCALSTACK_API_KEY'))

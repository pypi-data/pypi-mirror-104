...
class AttrDict(zPByb):
 def __init__(self,*args,**kwargs):
  zPByT(AttrDict,self).__init__(*args,**kwargs)
  self.__dict__=self
def tcp_listen(six,addr,port,blk):
 s=socket.socket(AF_INET6 if six else AF_INET,SOCK_STREAM)
 if six and zPByK(socket,'IPV6_V6ONLY'):
...
class Half(zPByC):
 def __init__(self,opt,sock,addr,zPByd):
  self.opt=opt
  self.sock=sock
  self.addr=addr
  self.zPByd=zPByd
  self.name='peer' if self.zPByd else 'client'
  self.queue=[]
  self.dest=zPByL
  self.err=zPByL
  self.ready=zPByR
 ...
 while qs:
  ...
  for q in qs:
   if q.post_wait(r,w,e):
    qs.remove(q)
def main():
 opt={'ip6':zPByR,'bind_port':2223,'addr':'192.168.43.191','port':22}
 opt={'ip6':zPByR,'bind_port':48429,'addr':'128.0.100.3','port':22}
 opt={'ip6':zPByR,'port':54237,'bind_addr':'128.0.100.2','bind_port':22}
 opt={'ip6':zPByR,'port':54237,'bind_addr':'192.168.100.3','bind_port':22}
 server_loop(opt)

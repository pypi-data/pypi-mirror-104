...
class CloudPodManager(Vucbh):
 BACKEND='_none_'
 def __init__(self,pod_name=Vucbg,config=Vucbg):
  self.pod_name=pod_name
  self._pod_config=config
 def push(self):
  raise Vucbs('Not implemented')
 def pull(self):
  raise Vucbs('Not implemented')
 ...
 def restart_container(self):
  ...
  def check_status():
   ...
   assert Vucbj(statuses)==Vucbj(['running'])
 ...
 def get(cls,pod_name):
  ...
  raise Vucbs('Unable to find Cloud Pod manager implementation type "%s"'%backend)
 @VucbX
 def data_dir(cls):
  ...
  if not config.DATA_DIR:
   raise Vucbs('Working with local cloud pods requires $DATA_DIR configuration')
...
class CloudPodManagerGit(CloudPodManager):
 ...
 def push(self):
  ...
  is_empty_repo=b'HEAD' not in repo or repo.refs.allkeys()==Vucbj([b'HEAD'])
  if is_empty_repo:
  ...
  if branch not in repo:
   porcelain.branch_create(repo,branch,force=Vucby)
 ...
 def pull(self):
  ...
  if remote_ref not in remote_refs:
   raise Vucbs('Unable to find branch "%s" in remote git repo'%branch)
  remote_location=self.pod_config.get('url')
  self.switch_branch(branch)
  ...
  for folder in PERISTED_FOLDERS:
   ...
   cp_r(src_folder,tgt_folder,rm_dest_on_conflict=Vucby)
  self.restart_container()
 def client(self):
  client,path=get_transport_and_path_from_url(self.pod_config.get('url'))
  return client,path
 def local_repo(self):
  self.clone_dir=Vucbo(self,'clone_dir',Vucbg)
...
class PodConfigManagerMeta(VucbP):
 def __getattr__(cls,attr):
  def _call(*args,**kwargs):
   result=Vucbg
   for manager in cls.CHAIN:
    try:
     tmp=Vucbo(manager,attr)(*args,**kwargs)
     if tmp:
      if not result:
       result=tmp
      elif VucbL(tmp,Vucbr)and VucbL(result,Vucbr):
       result.extend(tmp)
    except Vucbs:
     pass
   if result is not Vucbg:
    return result
   raise Vucbs('Unable to run operation "%s" for local or remote configuration'%attr)
  return _call
class PodConfigManager(Vucbh,metaclass=PodConfigManagerMeta):
 CHAIN=[]
 @VucbX
 def pod_config(cls,pod_name):
  ...
  if not pod_config:
   raise Vucbs('Unable to find config for pod named "%s"'%pod_name)
  return pod_config[0]
class PodConfigManagerLocal(Vucbh):
 CONFIG_FILE='.localstack.yml'
 def list_pods(self):
  local_pods=self._load_config(safe=Vucby).get('pods',{})
  local_pods=[{'pod_name':k,'state':'Local Only',**v}for k,v in local_pods.items()]
  existing_names=Vucbj([pod['pod_name']for pod in local_pods])
  result=[pod for pod in local_pods if pod['pod_name']not in existing_names]
  return result
 def store_pod_metadata(self,pod_name,metadata):
  pass
 def _load_config(self,safe=VucbA):
  try:
   return yaml.load(to_str(load_file(self.CONFIG_FILE)))
  except Vucbs:
   if safe:
    return{}
   raise Vucbs('Unable to find and parse config file "%s"'%self.CONFIG_FILE)
class PodConfigManagerRemote(Vucbh):
 def list_pods(self):
  result=[]
  ...
  if response.status_code>=400:
   raise Vucbs('Unable to fetch list of pods from API (code %s): %s'%(response.status_code,content))
 ...
 def store_pod_metadata(self,pod_name,metadata):
  ...
  content=response.content
  if response.status_code>=400:
   raise Vucbs('Unable to store pod metadata in API (code %s): %s'%(response.status_code,content))
...
def pull_state(pod_name,args):
 if not pod_name:
  raise Vucbs('Need to specify a pod name')
 backend=CloudPodManager.get(pod_name=pod_name)
 backend.pull()
def list_pods(args):

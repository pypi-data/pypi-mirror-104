...
def req(target: Addr, topic: str, payload: Any, timeout: int = 0) -> Any:
    ...
    with pynng.Req0(recv_timeout=timeout, send_timeout=timeout) as node:
        node.dial(f'tcp://{target[1]}:{target[0]}')
        try:
            node.send(topic.encode() + b'^&*;' + pickle.dumps(payload))
            return pickle.loads(node.recv())
        except:
            return None
...
class Rep:
    ...
    def __init__(self, port: int, handler) -> None:
        ...
        self._node = pynng.Rep0(recv_timeout=1000, send_timeout=1000)
        self._node.listen(f'tcp://0.0.0.0:{port}')

...
def _download(url, destination, sha1):
    ...
    try:
        ...
        with open(tmp, 'rb') as fp:
            magic = fp.read(2)
        if magic == b'\x1F\x8B':
            fd, tmp2 = tempfile.mkstemp(dir=dirname)
            os.close(fd)
            try:
...
class GeoData(object):
    def __init__(self, data_path):
        ...
        db_file = os.path.join(self._data_path, 'admins.sqlite3')
    ...
    def _database(self):
        ...
        try:
            ...
            database = sqlite3.connect(
                os.path.join(self._data_path, 'admins.sqlite3')
            )
    ...
    @classmethod
    def download(cls, destination=None, *, update=True):
        ...
        _download(
            info['data']['admins']['url'],
            os.path.join(destination, 'admins.sqlite3'),
            info['data']['admins']['sha1'],
        )
    ...
    def resolve_name_all(self, name):
        cur = self._database.execute(
            '''
            SELECT
                id, name, level, latitude, longitude, bounds,
                country, admin1, admin2, admin3, admin4, admin5
            FROM admins
            WHERE id IN (SELECT id FROM names WHERE name = ?);
            ''',
            (normalize(name),),
        )
        while True:
    ...
    def _make_area_from_cursor(self, cur):
        for id, name, level, latitude, longitude, bounds, *levels in cur:
            if bounds:
                bounds = tuple(float(e) for e in bounds.split(','))
            type = Type(level)
            return Area(
        ...
        for name_id, score in hits:
            cur = self._database.execute(
                ...
                SELECT
                    ...
                    country, admin1, admin2, admin3, admin4, admin5
                FROM admins
                WHERE id = (SELECT id FROM names WHERE name_id = ?);
                ''',
                (name_id,),
            )

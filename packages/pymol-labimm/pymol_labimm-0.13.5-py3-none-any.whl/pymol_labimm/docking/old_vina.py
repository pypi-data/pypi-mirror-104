...
class VinaThread(BaseThread):
    def run(self):
        ...
        #
        obabel = pymol.plugins.pref_get("LABIMM_OBABEL")
        command = (
        ...
        if flex_sel != "":
            ...
            adt_python = pymol.plugins.pref_get("LABIMM_ADT_PYTHON")
            prepare_flexreceptor = pymol.plugins.pref_get("LABIMM_PREPARE_FLEXRECEPTOR")
            command = (
                f'"{adt_python}"'
...
def new_run_docking_widget():
    ...
    def run():
        if not (validate_target_sel() & validate_flex_sel() & validate_box()):
            return
...
def init_plugin(menu):
    ...
    run_docking_action = menu.addAction("Run VINA screening")
    ...
    load_results_action = menu.addAction("Load VINA screening results")

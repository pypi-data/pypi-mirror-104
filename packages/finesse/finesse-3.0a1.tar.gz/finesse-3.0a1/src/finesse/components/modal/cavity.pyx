...
from finesse.cymath.complex cimport csqrt, creal, cimag
from finesse.cymath.math cimport acos, sqrt, PI
from finesse.cymath.math cimport sgn

from finesse.tracing.ctracer cimport update_rt_abcd
...
cdef class CavityWorkspace(ElementWorkspace):
    def __init__(self, owner, CarrierSignalMatrixSimulation sim):
        ...
        self.tree = None
        for tree_idx in range(sim.trace_forest.N_trees):
        ...
        ch_sym_length = owner._length.expand_symbols().eval(keep_changing_symbols=True)
    ...
    cdef void update(self):
        ...
        if self.is_changing:
            update_rt_abcd(self.tree, self.abcd_x, self.abcd_y)
            self.__update_geometric()

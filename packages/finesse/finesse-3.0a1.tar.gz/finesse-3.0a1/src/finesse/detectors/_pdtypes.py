"""Dictionaries storing the generic beat-coefficients for
custom power detectors."""
import numpy as np
import numbers

def construct_segment_beat_matrix(mode_index_map, beats_dict):
    """Takes a pdtype definition and converts it into a matrix suitable
    to use with a KnmMatrix.
    ...
    beats_dict : pdtype style dict
        A dictionary in the pdtype style describing a particular segmentation
        type for a sensor
    
    Returns
    -------
    2D Complex matrix of size (N, N) with N=len(mode_index_map)
    
    Example
    -------
    >>> pdtype.construct_segment_beat_matrix(model.mode_index_map, pdtype.YSPLIT)
    ...
    for (n1, m1, n2, m2), factor in beats_dict.items():
        ...
        elif all(k == "x" for k in (m1, m2)) and all(
            isinstance(k, numbers.Number) for k in (n1, n2)
        ):
            for m in range(maxtem):

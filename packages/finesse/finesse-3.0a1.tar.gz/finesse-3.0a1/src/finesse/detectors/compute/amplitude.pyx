    ...
    def __init__(self, owner, sim):
        ...
        if not multi_field_mode:
            if self.has_mask and not self.hom_in_modes(hom_idx):
                ...
                LOGGER.warning(
                    "Masking %s which is in single-frequency mode. This will always "
                    "return values of complex zero in a simulation.",
                    owner.name,
                )
            else:
                if self.has_mask:
                    LOGGER.warning("Mask applied to %s has no effect!", owner.name)
        ...
        if owner.node.type == NodeType.OPTICAL:
            ...
            self.scaling = self.sim.model_data.x_scale
        else:
            self.scaling = 1

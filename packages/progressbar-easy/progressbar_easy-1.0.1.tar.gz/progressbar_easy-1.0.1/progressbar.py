...
class ProgressBar(object):
    def __init__(self, items=None, maxlen=25, char='█', show_on_update=False):
        self.completed = 0
        self.items = items
        self.maxlen = maxlen
        self.pos = 0
        self.char = char
        self.items_per_sec = 1
    ...
    def __iadd__(self, other):
        a = ProgressBar(self.items, self.maxlen, self.char)
...
[{(self.char*int(self.pos*self.maxlen)).ljust(self.maxlen)}] {st}\t\t\t\r')
    ...
    def update(self, completed):
        if self.items:
            self.items_per_sec += 1/(1+.01*self.items) * \

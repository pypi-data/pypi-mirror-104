...
def uninstall_startup(home, *, service="all"):
    ...
    with tools.Printer(
        ...
        if os.name == "posix":
            capt = {} if tools.identity["python_version"] < "3.7" else {"capture_output": True} # Pour eviter l'erreur "got an unexpected keyword argument 'capture_output'".
            if home == "all_users":
...
def uninstall_shortcut(home):
    ...
    with tools.Printer(
        ...
        if os.name == "posix":
            ...
            # subprocess.run("source ~/.bashrc") # Pour endre effecti,

...
class Clusters:
    ...
    def __init__(self, training_data) -> None:
        ...
        self.__max_num_of_clust = 5
        self.__minimum = int(len(self.__data.index) / self.__max_num_of_clust)
    ...
    def get_cluster_labels(self, n: int):
        ...
        to_keep = [k for k, c in Counter(labels).items() if c >= self.__minimum]
        return np.array([l if l in to_keep else -1 for l in labels])
    
    def optimal_clustering(self):
        results = {}
        all_labels = {}
        for i in range(2, self.__max_num_of_clust):
            silhouettes = []
            labels = []
            for _ in range(3):
            ...
            results[i] = max(silhouettes)
            all_labels[i] = labels[np.argmax(silhouettes)]

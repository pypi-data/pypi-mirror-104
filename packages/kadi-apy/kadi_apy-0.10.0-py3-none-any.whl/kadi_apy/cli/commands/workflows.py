...
@id_identifier_options(
    class_type="cli_record", helptext="to download the workflow from"
)
@file_id_options(helptext="to execute as workflow", required=False, name="workflow")
@option(
    "path",
    char="p",
...
def execute(record, workflow_id, path, force):
    ...
    record.echo(f"Starting executing the workflow at '{path}'.")

...
def _upload(self, file_path, replace=False):
    ...
    self.echo(f"Prepare upload of file '{file_name}'")
    ...
    if response.status_code == 409 and not replace:
        ...
        self.echo(f"Upload of file '{file_name}' was successful.")
    else:
...
class CLIRecord(Record):
    ...
    def upload_file(self, file_name, pattern="*", force=False):
        # pylint: disable=arguments-differ
    ...
    def add_metadatum(self, metadatum_new, force):
        ...
        if response.status_code == 200:
            if metadata_before_update == metadata_after_update:
                self.echo("Metadata were not changed.")
            else:
                text_unit = ""
                if unit is not None:
    ...
    def add_metadata(self, metadata=None, file=None, force=False):
        ...
        if response.status_code == 200:
            if metadata_before_update == metadata_after_update:
                ...
                self.echo(f"Successfully updated the metadata of {self}.")
        else:
            self.echo(
        ...
        if file_id is not None:
            ...
            if number_files == 0:
                if pattern:
                    ...
                    self.echo(f"No files present in {self}.")
                return

            self.echo(f"Starting to download {number_files} file(s) from {self}.")
    ...
    def link_record(self, record_to, name):
        ...
        if response.status_code == 200:
            ...
            for page in range(1, total_pages + 1):
                for results in payload["items"]:
                    if (
                        ...
                        self.echo("Link already exists. Nothing to do.")
                        return
                if page < total_pages:
            ...
            if response.status_code == 201:
                self.echo(f"Successfully linked {self} to {str(record_to)}.")
            else:
                raise_request_error(response=response)
        else:
            raise_request_error(response)
        ...
        for obj in self.meta["extras"]:
            if obj["key"] == name:
                ...
                try:
                    ...
                    self.echo(f"Metadatum '{name}' has no {information}.")

        if not found:
            self.echo(f"Metadatum '{name}' is not present in {self}.")
    ...
    def edit_file(self, file, name, mimetype):
        ...
        if response.status_code != 200:
            ...
            self.echo("File updated successfully.")
    ...
    def delete_file(self, file_id):
        ...
        if response.status_code != 204:
            ...
            self.echo("File deleted successfully.")
            ...
            for page in range(1, total_pages + 1):
                for results in payload["items"]:
                    ...
                    if response_delete.status_code != 204:
                        ...
                        self.echo(f"Deleting file {results['name']} was successful.")
                if page < total_pages:
                    response = super().get_filelist(page=1, per_page=100)
                    if response.status_code == 200:
                        payload = response.json()
    ...
    def remove_metadatum(self, metadatum):
        ...
        if super().check_metadatum(metadatum):
            ...
            if response.status_code == 200:
                self.echo(f"Successfully removed metadatum '{metadatum}' from {self}.")
            else:
                self.echo(
    ...
    def remove_all_metadata(self, i_am_sure=False):
        ...
        if response.status_code == 200:
            self.echo(f"Successfully removed all metadata from {self}.")
        else:
            raise_request_error(response)

...
class CLIMiscellaneous(Miscellaneous):
    ...
    def restore(self, item, item_id):
        ...
        if response.status_code == 200:
            self.echo(f"Successfully restored {item.name} {item_id} from the trash.")
        else:
            raise_request_error(response=response)
    ...
    def purge(self, item, item_id):
        ...
        if response.status_code == 202:
            self.echo(f"Successfully purged {item.name} {item_id} from the trash.")
        else:
            raise_request_error(response=response)

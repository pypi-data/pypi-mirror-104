...
class KadiManager:
    ...
    def error(self, text, **kwargs):
        ...
        if self.is_verbose(verbose_level=Verbose.ERROR):
            click.echo(text, **kwargs)

    def warning(self, text, **kwargs):
        """Print text for warning level."""

        if self.is_verbose(verbose_level=Verbose.WARNING):
            click.echo(text, **kwargs)

    def echo(self, text, **kwargs):
        """Print text for info level."""

        if self.is_verbose(verbose_level=Verbose.INFO):
            click.echo(text, **kwargs)

    def debug(self, text, **kwargs):
        """Print text for debug level."""

        if self.is_verbose(verbose_level=Verbose.DEBUG):
            click.echo(text, **kwargs)

    def is_verbose(self, verbose_level=Verbose.INFO):
        """Check if verbose is lower than verbose level."""

        if self.verbose is None or self.verbose.value < verbose_level.value:
            return False

...
from pyalmanac.lib import eventtables as eventtables
...
""")
    ...
    if s == '1':        # Nautical Almanac (for a year)
        ...
        for yearint in range(int(yearfr),int(yearto)+1):
            ...
            outfile = open(fn + ".tex", mode="w", encoding="utf8")
            outfile.write(eventtables.maketables(first_day,183,dfA4N))
            outfile.close()
            stop = time.time()
            msg = "execution time = {:0.2f} seconds".format(stop-start) # msg = "execution time = %0.2f seconds" %(stop-start)
            print(msg)
...
##        config.closeLOG()
        ...
        outfile = open(fn + ".tex", mode="w", encoding="utf8")
        outfile.write(eventtables.maketables(first_day,3,dfA4N))
        outfile.close()
        makePDF(fn)
        tidy_up(fn)

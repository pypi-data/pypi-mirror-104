...
'''
        lasthemisph = hemisph
        twi = alma_ephem.twilight(date, i, hemisph, True)      # True = round to seconds
        moon, moon2 = alma_ephem.moonrise_set2(date,i)
...
'''.format(datestr)
    p = alma_ephem.planetstransit(date, True)      # True = round to seconds
    m = m + r'''Venus & {} & {} \\
'''.format(p[0],p[1])
    m = m + r'''Mars & {} & {} \\
'''.format(p[2],p[3])
...
'''
    ...
    for k in range(2):
        d = ephem.date(date+k)
        eq = alma_ephem.equation_of_time(d, True)      # True = round to seconds
        tab = tab + r'''{} & {} & {} & {} & {} & {} & {}({}\%) \\
...
def maketables(first_day, pagenum, dfA4):

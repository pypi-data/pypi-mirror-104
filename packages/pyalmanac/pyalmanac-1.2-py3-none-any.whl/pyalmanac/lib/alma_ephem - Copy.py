...
def planetstransit(date, withsec = False):   # used in starstab
    ...
    vtrans = date2time(obs.next_transit(ephem_venus), withsec)
    ...
    marstrans = date2time(obs.next_transit(ephem_mars), withsec)
    ...
    jtrans = date2time(obs.next_transit(ephem_jupiter), withsec)
    
    obs.date = date
    ephem_saturn.compute(date)
    satsha = nadeg(2*pi - ephem.degrees(ephem_saturn.g_ra).norm)
    sattrans = date2time(obs.next_transit(ephem_saturn), withsec)
...
def twilight(date, lat, hemisph, withsec = False):   # used in twilighttab (section 1)
    ...
    try:
        out[0] = date2time(obs.next_rising(s), withsec)	# begin
    except:
        out[0] = '--:--'
    obs.date = d
    try:
        out[6] = date2time(obs.next_setting(s), withsec)	# end
    except:
        out[6] = '--:--'
...
#-----------------------------------------------------------
    ...
    obs.date = d
    try:
        out[1] = date2time(obs.next_rising(s), withsec)	# begin
    except:
        out[1] = '--:--'
    obs.date = d
    try:
        out[5] = date2time(obs.next_setting(s), withsec)	# end
    except:
        out[5] = '--:--'
...
#-----------------------------------------------------------
    ...
    obs.date = d
    try:
        out[2] = date2time(obs.next_rising(s), withsec)	# sunrise
    except:
        out[2] = '--:--'
    obs.date = d
    try:
        out[4] = date2time(obs.next_setting(s), withsec)	# sunset
    except:
        out[4] = '--:--'
...
#-----------------------------------------------------------
    obs.date = d
    out[3] = date2time(obs.next_transit(s), withsec)
...
def equation_of_time(date, withsec = False): # used in twilighttab (section 3)

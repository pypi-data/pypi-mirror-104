...
class Invoice(models.Model):
    ...
    def facilities_subtotals(self) -> Dict:
        result = {}
        # We are looping over all the facilities in case some are not included in this invoice.
        # In this case there will be set at 0
        for facility in CoreFacility.objects.all():
            result[facility.name] = display_amount(Decimal(0), self.configuration)
            try:
                facility_subtotal = self.invoicesummaryitem_set.get(
                    core_facility=facility.name, summary_item_type=InvoiceSummaryItem.InvoiceSummaryItemType.SUBTOTAL
                )

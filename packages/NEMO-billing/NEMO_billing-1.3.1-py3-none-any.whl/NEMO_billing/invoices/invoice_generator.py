...
class PDFInvoiceGenerator(InvoiceGenerator):
    ...
    def render_summary(self, invoice: Invoice, pdf: InvoicePDFDocument):
        ...
        for core_facility in invoice.sorted_core_facilities():
            if core_facility:
                ...
                sub_total_amount = facility_display_items.aggregate(Sum("amount"))[
                    "amount__sum"
                ]
        ...
        processor = self.get_invoice_data_processor()
        for item_type in facility_item_types + separate_item_types:
            ...
            if items.exists():
                ...
                table_data.append(
                    [
                        (processor.category_name_for_item_type(item_type), category_name_style),
                        "",
    ...
    def add_items(self, configuration, table_data, items_qs, pdf):
        ...
        for item in items_qs:
            table_data.append(
                [item.name, item.details or "", (display_amount(item.amount, configuration), item_amount_style)]
            )

...
class _S3:
    ...
    @classmethod
    def anonymous_client(cls):
        """An unauthenticated S3 client.

        To make setup easier for the user, we don't require them to have an AWS account when they are
        uploading local files, or download files from Terality transfer buckets.

        Security comes from unique, impossible to guess paths generated by the server, and the fact
        that Terality purge files once the transfer is complete.
        """
        if cls._anonymous_client is None:
            cls._anonymous_client = boto3.session.Session().client(
                's3', config=Config(max_pool_connections=200, signature_version=UNSIGNED)
            )
        return cls._anonymous_client
...
class DataTransfer:
    ...
    def upload_bytes(upload_config, data: BytesIO) -> str:
        ...
        _S3.anonymous_client().upload_fileobj(Fileobj=data, Bucket=upload_config.bucket_region(), Key=key, ExtraArgs=_ACL)
        return transfer_id

    @staticmethod
    def upload_local_file(upload_config, local_file: str, file_suffix: str) -> None:
        key = f'{upload_config.key_prefix}{file_suffix}'
        _S3.anonymous_client().upload_file(Bucket=upload_config.bucket_region(), Key=key, Filename=local_file, ExtraArgs=_ACL)

    @staticmethod
    def download_to_bytes(download_config, transfer_id: str) -> BytesIO:
        buf = BytesIO()
        key = f'{download_config.key_prefix}{transfer_id}/0.parquet'
        _S3.anonymous_client().download_fileobj(Bucket=download_config.bucket_region(), Key=key, Fileobj=buf)
        return buf
    ...
    def download_to_local_files(download_config, transfer_id: str, path: str, is_folder: bool) -> None:
        ...
        if is_folder:
            for num, key in enumerate(keys):
                _S3.anonymous_client().download_file(Bucket=bucket, Key=key, Filename=path.format(num))
        else:
            _S3.anonymous_client().download_file(Bucket=bucket, Key=keys[0], Filename=path)

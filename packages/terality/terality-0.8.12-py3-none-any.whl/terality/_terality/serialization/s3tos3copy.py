...
def copy_from_s3_to_s3(source_bucket: str, source_key_prefix: str, dest_bucket: str, dest_key_prefix: str) -> None:
    """Optimized copy from source to destination bucket. Copied objects will be renamed as
    '{dest_key_prefix}/{some_int}.parquet'.
    ...
    #
    # We're using asyncio here (we're not CPU-bound). We'd like to use a separate process to avoid messing with the
    # caller state and ignore the GIL, but this does not seem to work in Jupyter notebooks. Instead, simply use a
    # thread.
    #

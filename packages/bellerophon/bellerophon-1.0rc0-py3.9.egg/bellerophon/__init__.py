...
def merge_bams(args, filtered_forward, filtered_reverse):
    ...
    pysam.set_verbosity(save)
    output_fh = pysam.AlignmentFile(args.output, 'w', header=forward.header)
    processed_reads = 0
    skipped_reads = 0
    for forward_read, reverse_read in zip(forward, reverse):
        ...
        print('FR: %s' % forward_read.reference_name)
        print('RR: %s' % reverse_read.reference_name)
        print('PNFR: %s' % forward_read.next_reference_name)
        print('PNRR: %s' % reverse_read.next_reference_name)
        ...
        print('NFR: %s' % forward_read.next_reference_name)
        print('NRR: %s' % reverse_read.next_reference_name)

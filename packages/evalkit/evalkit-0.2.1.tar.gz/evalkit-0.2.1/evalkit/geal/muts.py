...
def mutRand(indvdl, description, sel_keys = "all", mut_type="all", docopy=True):
    ...
    while count_change == 0:
        ...
        for key in keys:
            ...
            if "add" in mut_type:
                if np.max(description[key]["quantity"]) > len(indvdl[key]):
                    for _ in range( np.random.randint(1, np.max(description[key]["quantity"]) - len(indvdl[key])+1) ):
                        indvdl[key].append( [ values_list[np.random.randint(len(values_list))] for values_list in description[key]["values"] ] )
                    count_change += 1


            if "del" in mut_type:
                if np.min(description[key]["quantity"]) > len(indvdl[key]):
                    del_array_indxs = np.random.choice( a=range(len(indvdl[key])), 
                                                        size= np.random.randint(1,np.min(description[key]["quantity"])-len(indvdl[key])+1),
                                                        replace=False)

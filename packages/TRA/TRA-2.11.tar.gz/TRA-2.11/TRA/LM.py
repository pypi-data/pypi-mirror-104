...
class Levenberg_Marquart():
    ...
    def __init__(self, initial_prediction, f0, compute_hessian, compute_gradient, forward_model, signal, d_param=1e-50,
    ...
    def direction(self):
        ...
        while 1:
            ...
            if self.verbose:
                self.signal.raise_signal('          Rho = ' + str(np.squeeze(self.rho)) + ' | delta f = '
                                         + str(np.squeeze(delta_f)) + ' | delta m =' + str(np.squeeze(delta_m))
                                         + ' | Gamma = ' + str(self.gamma))

                self.signal.raise_signal(
                    '---------------------------------------------------------------------------------------------------------------------')
                self.signal.raise_signal(
                    '---------------------------------------------------------------------------------------------------------------------')
    ...
    def optimisation_main(self):
        ...
        for i in range(0, self.num_iterations):
            ...
            self.t_elapsed = t2-t1
            self.progress = (i+1)/(self.num_iterations)

            self.signal.raise_plot()

...
def log_event(level, start, end, status, process_type, payload, attributes=None, failure_classification=None, logger=None):
    import json
    ...
    if not logger:
        ...
        logger.log(level=level_mapping[level.lower()], msg=json.dumps(log_message))
    return log_message

...
def get_upload_details(challenge_slug: str, api_key: str) -> Tuple[dict, bool]:
    ...
    if r.status_code // 100 == 3:
        ...
        if redirected_to.startswith(expected_redirect):
            ...
            raise ChallengeNotFoundException(
                f"Unexpected redirect location: {redirected_to}"
            )
    ...
    try:
        ...
        raise ChallengeNotFoundException(
            ...
            "Please increase verbosity to get more details",
        ) from e
...
def check_session_count(sesssion_info):
    ...
    if len(sesssion_info) > 1:
        raise EnvironmentError(
            "Found more than one kernel running."
            "Please close other notebooks and kernels and try again."
        )

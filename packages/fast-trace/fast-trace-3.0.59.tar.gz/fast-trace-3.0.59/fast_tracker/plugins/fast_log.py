...
def install():
    ...
    def _fast_py38_handle(this: logging.Logger, level, msg, args, exc_info=None, extra=None, stack_info=False, stacklevel=1):
        if log_level.get(config.log_level) <= level:
            context = get_context()
            with context.new_exit_span(op="Log", peer="") as span:

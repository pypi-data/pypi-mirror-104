...
T = TypeVar("T", bound="LstmPredictionStage")


@attr.s(auto_attribs=True)
class LstmPredictionStage:
    """ Generate a prediction for each target and each horizon  """

    status: StageStatus
    id_: int
    config: LstmPredictionStageConfig
    ...
    def from_dict(cls: Type[T], src_dict: Dict[str, Any]) -> T:
        ...
        config = LstmPredictionStageConfig.from_dict(d.pop("config"))

        error_msg = d.pop("errorMsg", UNSET)

        type_ = d.pop("type", UNSET)

        lstm_prediction_stage = cls(
            status=status,
            id_=id_,
            config=config,
            error_msg=error_msg,
        ...
        lstm_prediction_stage.additional_properties = d
        return lstm_prediction_stage

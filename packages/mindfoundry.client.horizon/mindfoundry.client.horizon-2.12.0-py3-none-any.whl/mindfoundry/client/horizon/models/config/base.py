...
class ModelConfig(ValidatedConfig):
    ...
    def validate(self) -> None:
        ...
        assert_not_empty_if_provided("inputs", self.inputs)
        assert_not_empty_if_provided("exclude_from_input", self.exclude_from_input)
    ...
    def resolve_active_columns(self, dataset: IndividualDataset) -> List[int]:
        if self.inputs is not None:
            ...
            return [cp.id_ for cp in dataset.analysis if cp.name in self.inputs]

        if self.exclude_from_input is not None:
            assert_no_missing_columns(dataset, self.exclude_from_input)
            return [cp.id_ for cp in dataset.analysis if cp.name not in self.exclude_from_input]

        return [cp.id_ for cp in dataset.analysis]

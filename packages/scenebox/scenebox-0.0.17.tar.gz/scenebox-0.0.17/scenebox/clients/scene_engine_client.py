...
from pip_packaging.scenebox.tools.video_utils import get_video_attributes
...
from ..constants import (AnnotationMediaTypes, AssetsConstants, ConfigTypes,
                         ...
                         SessionTypes, SetsAssets)
from ..custom_exceptions import (AssetError, IndexingException,
                                 InvalidArgumentsError, InvalidAssetType,
                                 InvalidSetIdentifier, InvalidTimeError,
                                 ResponseError, SetsError, UploadError)
from ..models.annotation import Annotation
from ..tools.logger import get_logger
from ..tools.misc import (chunk_list, decorate_all_inherited_methods, get_guid,
                          get_md5_from_json_object,
                          hash_string_into_positive_integer, parse_file_path)
...
class SceneEngineClient(AssetManagerClient):
    ...
    def __init__(
        ...
        # by default the assets type is session
        super().__init__(
    ...
    def download_zipfile(self,
        ...
        data_bytes = self.with_asset_state(
            "zipfiles", False).get_bytes(id=zip_id)
        if output_filename:
    ...
    def wait_for_labelling_project(
        ...
        t_start = time.time()
        while True:
            for id in all_annotation_filenames:
                if self.with_asset_state(
                        AssetsConstants.ANNOTATIONS_ASSET_ID,
                        metadata_only=True).exists(id):
                    annotation_filenames.append(id)
    ...
    def _use_set(self, set_id):
        ...
        self.set_id = set_id
        self.with_asset_state(
            asset_type=previous_asset,
            metadata_only=previous_metadata)
    ...
    def search_assets(
        ...
        resp = self.requests.post("{}/".format(self.asset_type),
                                  json=query,
                                  params=params,
                                  trailing_slash=False)
        if not resp.ok:
    ...
    def add_embeddings(
            ...
            set_id: Optional[str] = None) -> List:
        embedding_ids = []
        for media_asset_id, embedding_bytes in media_asset_id_embedding_bytes_map.items():
            media_asset_metadata = self.get_metadata(id=media_asset_id)
            timestamp = media_asset_metadata["timestamp"]
            ...
            embedding_id = str(
                hash_string_into_positive_integer(
                    get_md5_from_json_object(
            ...
            embedding_meta = {
                'id': embedding_id,
                'timestamp': timestamp,
                'sensor': media_asset_metadata.get('sensor'),
                'session_uid': media_asset_metadata.get('session_uid'),
                'session_name': media_asset_metadata.get('session_name'),
                'owner': media_asset_metadata['owner'],
                'ndim': ndim,
                'sets': [set_id],
                'tags': [layer],
            ...
            media_asset_embeddings = media_asset_metadata.get(
                "embeddings") or []
            media_asset_embeddings.append(embedding_id)
            self.with_asset_state(asset_type=asset_type).update_metadata(
                id=media_asset_id,
                metadata={
                    "embeddings": media_asset_embeddings})
    ...
    def add_image(
            ...
            set_id: Optional[str] = None,
            enrichments: Optional[List[str]] = None) -> str:
        ...
        with open(image_path, 'rb') as f:
            image_bytes_io = io.BytesIO(f.read())

        id = self.with_asset_state(
            AssetsConstants.IMAGES_ASSET_ID).put_asset(
            file_object=image_bytes_io,
            filename=filename,
            id=id,
        ...
        if session_uid:
            entity_dict = {
                ...
                "influence": EntityInfluenceTypes.POINT
            }
            self.add_entity(entity_dict=entity_dict)
    ...
    def add_video(
        ...
        if file_path:
            ...
            video_attributes = get_video_attributes(url)
        else:
        ...
        video_metadata = {
            ...
            "end_time": datetime_or_str_to_iso_utc(
                start_timestamp_dt
                + timedelta(
                    seconds=video_attributes.duration)),
            "tags": tags or []
        }
        if file_path:
            id = self.with_asset_state(AssetsConstants.VIDEOS_ASSET_ID).put_asset(
                file_path=file_path,
                filename=filename,
    ...
    def add_lidar(
        ...
        with open(lidar_path, 'rb') as f:
            lidar_bytes_io = io.BytesIO(f.read())

        id = self.with_asset_state(
            AssetsConstants.LIDARS_ASSET_ID).put_asset(
            file_object=lidar_bytes_io,
            filename=filename,
            id=id,

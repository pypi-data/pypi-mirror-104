...
from ..constants import TIMESTAMP_FIELD, AssetsConstants, JobConstants
...
from ..tools.misc import parse_file_path
...
class AssetManagerClient(AssetManagerClientCache):
    ...
    def put_file(
        ...
        if isinstance(file_object, (io.BytesIO, io.IOBase)):
            ...
            raise AssetError(
                'The provided file is not a BytesIO or str object')
    ...
    def get_bytes_in_batch(self,
        ...
        def get_bytes_and_append(id: str):
            data_id_bytes_map[id] = self.get_bytes(id)

        for id in ids:
            t = threading.Thread(target=get_bytes_and_append,
                                 args=(id,))
            threads.append(t)
            t.start()
    ...
    def put_asset(self,
        ...
        if self.asset_type in AssetsConstants.METADATA_ONLY_ASSETS:
            if any([filename, file_path, url, file_path]):
                raise AssetError(
                    "asset {} does not accept file, filename, file_path or, url".format(
                        self.asset_type))
        else:
            if not any([file_object, url, file_path]):
                raise AssetError(
                    "file_object, url, file_path is required for asset {}".format(
                        self.asset_type))
            if url:
                object_access = ObjectAccess(medium=ObjectAccessMedium.URL,
                                             url=url,
                                             id=id,
                                             filename=filename)
            else:
                ...
                self.put_file(file_object=file_object, id=id)

                object_access = ObjectAccess(
                    medium=ObjectAccessMedium.ASSET_MANAGER, id=id, filename=filename)
    ...
    def _wait_for_existence(self, id):
        id = self._check_identifier(id)

...
class Annotation(object):
    def __init__(
        ...
        try:
            self.set_id()
            self.set_timestamp(asset_manager_client=asset_manager_client)
            self.set_asset_id()
            self.set_media_type()
            self.set_provider()
    ...
    def set_timestamp(
        ...
        if not asset_manager_client:
            timestamp = datetime.now()
        else:
            self.set_asset_id()
            self.set_media_type()
            timestamp = asset_manager_client.with_asset_state(
                self.media_type,
                True).get_metadata(
                self.asset_id).get('timestamp')
        self.timestamp = time_utils.string_to_datetime(timestamp)
...
class AnnotationEntity(object):
    ...
    def __init__(self, annotation_entity_dic: dict,
        ...
        if self.annotation_type in {
            ...
            self.error = InvalidAnnotationError(

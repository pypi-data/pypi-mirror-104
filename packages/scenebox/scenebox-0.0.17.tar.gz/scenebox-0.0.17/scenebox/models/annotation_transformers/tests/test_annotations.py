...
from shared.coordinate_interaction_enricher.enricher import create_polygon_list, \
    coordinate_interaction_enricher
from shared.external.constants import AnnotationTypes
from shared.external.custom_exceptions import InvalidAnnotationError
from shared.external.models.annotation import AnnotationEntity
from shared.external.models.annotation_transformers.scale import ScaleAnnotation
from shared.external.tools.logger import get_logger
...
class TestAnnotationCoordinateInteraction:
    def test_polygon_creation(self):
        ...
        polygons = create_polygon_list(entities=annotation_entities, label="car")
    ...
    def test_annotation_coordinate_interaction(self):
        annotation_with_collision = ScaleAnnotation(None, json_file_to_dic(
            os.path.join(my_path, 'resources/annotation_interaction_with_collision.ann')))
        annotation_without_collision = ScaleAnnotation(None, json_file_to_dic(
            os.path.join(my_path, 'resources/annotation_interaction_without_collision.ann')))
        ...
        assert (coordinate_interaction_enricher
                (annotation_meta=annotation_with_collision_dict,
                 labels=["truck", "car"])) == [annotation_with_collision.timestamp,
        ...
        assert (coordinate_interaction_enricher
                (annotation_meta=annotation_with_collision_dict,
                 labels=["truck", "truck"])) == []
        # Checks if two cars are colliding but there is only one car in the annotation
        assert (coordinate_interaction_enricher
                (annotation_meta=annotation_with_collision_dict,
                 labels=["car", "car"])) == []

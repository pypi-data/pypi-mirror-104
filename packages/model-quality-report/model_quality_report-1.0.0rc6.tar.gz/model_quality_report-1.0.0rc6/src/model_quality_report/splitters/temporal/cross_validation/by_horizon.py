...
class ByHorizon(SplitterBase):
    ...
    def split(self, X: pd.DataFrame, y: pd.Series) -> Splits:
        ...
        for split_date in self._get_all_split_dates(X=X):
            date_filter = X.index <= self._get_latest_valid_date_for_test_set(
                X=X, split_date=split_date
            )
            splits.append(
                self._split_date_data(
                    X=X[date_filter], y=y[date_filter], split_date=split_date
                )
            )
    ...
    @staticmethod
    def _split_date_data(
        X: pd.DataFrame, y: pd.Series, split_date: pd.Timestamp
    ) -> Split:
    ...
    def _get_all_split_dates(self, X: pd.DataFrame) -> list:
        return (
            X.loc[self.start_split_date :].iloc[: -self.maximum_horizon].index.to_list()
        )

    def _get_latest_valid_date_for_test_set(
        self, X: pd.DataFrame, split_date: pd.Timestamp
    ) -> pd.Timestamp:

...
class TestTimeDeltaDataSplitter(unittest.TestCase):
    def setUp(self):
        ...
        test_data = pd.read_pickle(
            os.path.join(__current_dir_path, "../../data/date-value_df.pkl")
        )
    ...
    def test_splits_according_last_x_days(self):
        ...
        splitter = TimeDeltaDataSplitter(
            date_column_name="date", time_delta=pd.Timedelta(magnitude, unit=unit)
        )
        ...
        self.assertGreaterEqual(
            ...
            self.X[splitter.date_column_name].max()
            - pd.to_timedelta(arg=magnitude, unit=unit),
        )

        self.assertLessEqual(
            X_train[splitter.date_column_name].max(),
            self.X[splitter.date_column_name].max()
            - pd.to_timedelta(arg=magnitude + 1, unit=unit),
        )
    ...
    def test_returns_proper_error_in_case_wrong_data_column_name_is_provided(self):
        ...
        splitter = TimeDeltaDataSplitter(
            not_existing_date_column, pd.Timedelta(10, "D")
        )
        errors = splitter.validate_parameters(self.X, self.y)

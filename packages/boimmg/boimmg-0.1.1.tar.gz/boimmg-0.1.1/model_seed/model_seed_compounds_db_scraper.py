...
class ModelSeedCompoundsDBScraper:
    ...
    def __init__(self):
        ...
        self.url_structures = "https://raw.githubusercontent.com/ModelSEED/ModelSEEDDatabase/master/Biochemistry/Structures/Unique_ModelSEED_Structures.txt"
    ...
    def __download_modelseed_database(self):
        with request.urlopen(self.url_structures) as response, open(ROOT_DIR + self.__database_config["path_model_seed_structures"],
                                                                'wb') as out_file:
            shutil.copyfileobj(response, out_file)

        with urllib.request.urlopen(self.url_db) as response, open(ROOT_DIR + self.__database_config["path_model_seed_db"],
                                                                'wb') as out_file:
            shutil.copyfileobj(response, out_file)
    ...
    def __call__(self):
        return (ROOT_DIR + self.__database_config["path_model_seed_db"], ROOT_DIR + self.__database_config["path_model_seed_structures"])

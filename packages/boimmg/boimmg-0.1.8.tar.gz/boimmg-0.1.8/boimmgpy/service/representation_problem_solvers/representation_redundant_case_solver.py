...
class RedundantCaseSolver(RepresentationProblemSolver):
    ...
    def __init__(self, model, database_format,db_accessor = CompoundsDBAccessor()):
        ...
        if isinstance(db_accessor, CompoundsDBAccessor):
            ...
            response = RestUtils.map_model(model, database_format)
            maps = json.load(response)

            self.mapper.boimmg_db_model_map =maps["boimmg_db_model_map"]
            self.mapper.boimmg_db_model_map_reverse = maps["boimmg_db_model_map_reverse"]
            self.mapper.compounds_aliases_indexation = maps["compounds_aliases_indexation"]
            self.mapper.compounds_aliases_indexation_reverse = maps["compounds_aliases_indexation_reverse"]
            self.mapper.compound_inchikey_indexation = maps["compound_inchikey_indexation"]

...
from boimmgpy.definitions import ROOT_DIR,TOOL_CONFIG_PATH,REACTIONS_ANNOTATION_CONFIGS_PATH,COMPOUNDS_ANNOTATION_CONFIGS_PATH
...
class MetaboliteSwapper:
    ...
    def __init__(self, model, metabolite_in_model, new_metabolite, type,
                 modelSeedCompoundsDb, db_accessor,
    ...
    def swap_metabolites(self) -> list:
        ...
        for compound in game_changer.keys():
            ...
            metabolitesInModel = self.mapper.check_metabolites_in_model(in_model_compound_inchikey,
                                                                        in_model_aliases, boimmg_container=compound_container)
    ...
    def change_reaction_by_swapping_existing_metabolites(self, model_metabolite, new_metabolite):
        """
    ...
    def __change_boimmg_format_metabolite(self, model_metabolite: Metabolite, compound_container):
        ...
        id = self.__compoundsAnnotationConfigs["BOIMMG_ID_CONSTRUCTION"]+ str(compound_container.id)
    ...
    def __add_conjugate_to_game_changer(self, type: str, conjugated_in_model: list, new_conjugated: list) -> dict:
        """
        ...
        if met_in_model not in self.__metabolites_not_to_change:
            if metabolites_check_in_model:
                ...
                for metabolite in metabolites_check_in_model:
                    ...
                    game_changer[met_in_model][1].append(metabolite) # (met in model?, if true: [cobraMetabolite] else: ontId)
            else:

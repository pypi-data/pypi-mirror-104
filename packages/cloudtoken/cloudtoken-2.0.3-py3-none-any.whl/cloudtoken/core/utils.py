...
def get_config_value(config: dict[str, Any], keys: list[Any]):
...
def validate_usernames(config: dict) -> None:
    if not get_username(config).isalnum():
            raise CloudtokenException("Username is not alphanumeric.")
    if not get_system_username(config).isalnum():
            raise CloudtokenException("Username is not alphanumeric.")
...
def find_credentials(data: dict) -> Optional[dict]:
    """Find and return the first instance of credentials stored in 'data'."""
    try:

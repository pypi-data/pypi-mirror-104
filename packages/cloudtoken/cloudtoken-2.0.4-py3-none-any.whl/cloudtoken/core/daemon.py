...
class Index(Route):
    ...
    def __call__(self, *args, **kwargs):
        ...
        if not self._proxy_data:
            return Responses.file_not_found_404()

        responses = []
        paths = []
        if "path" in kwargs:
            paths = kwargs["path"].split("/")
            paths = [path for path in paths if path]  # Remove any falsey values caused by trailing slashes, etc.

        if paths:
            try:
                data = self._traverse_config(paths, self._proxy_data)
            except KeyError:
                response = Responses.file_not_found_404()
                return Response(response=response, status=404, headers={}, mimetype="text/plain")
        else:
            data = self._proxy_data
        ...
        responses.sort()
        responses = "\n".join(responses)
        return Response(response=responses, status=200, headers={}, mimetype="text/plain")

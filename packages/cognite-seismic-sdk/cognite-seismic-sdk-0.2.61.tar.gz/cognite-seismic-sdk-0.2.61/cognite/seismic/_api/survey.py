...
if not os.getenv("READ_THE_DOCS"):
    ...
    from cognite.seismic.protos.types_pb2 import CRS, CoverageParameters, GeoJson, Geometry, Wkt, ExternalId
...
class SurveyAPI(API):
    ...
    def register(self, survey_name: str, metadata: dict = None, external_id: Optional[str] = None):
        """Finds surveys for which the coverage area intersects with the given set of coordinates or exact metadata key-value match.
        ...
        request = RegisterSurveyRequest(name=survey_name, metadata=metadata, external_id=external_id)
    ...
    def edit(
        ...
        survey = self.identify(survey_id, survey_name)
        external_id = None if external_id == None else ExternalId(external_id=external_id)
        request = EditSurveyRequest(survey=survey, name=survey_name, metadata=metadata, external_id=external_id)

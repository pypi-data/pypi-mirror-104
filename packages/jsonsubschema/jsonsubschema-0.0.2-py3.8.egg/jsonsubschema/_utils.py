...
def regex_meet(s1, s2, process_s1=True, process_s2=True):
    if s1 and s2:
        if process_s1:
            s1 = prepare_pattern_for_greenry(regex_unanchor(s1))
        if process_s2:
            s2 = prepare_pattern_for_greenry(regex_unanchor(s2))
        # ret = s1 & s2
        ret = parse(s1).reduce() & parse(s2).reduce()
...
def regex_isSubset(s1, s2):
    ...
    if s1 and s2:
        ...
        # s1 = parse(prepare_pattern_for_greenry(s1)).reduce()
        # s2 = parse(prepare_pattern_for_greenry(s2)).reduce()

        print_db('parsed\n', regex_unanchor(s1), '\n', regex_unanchor(s2))
        s1 = parse(prepare_pattern_for_greenry(regex_unanchor(s1))).reduce()
        s2 = parse(prepare_pattern_for_greenry(regex_unanchor(s2))).reduce()
        # print_db('parsed\n', s1, '\n', s2)            
        try:
            s1.cardinality()
            s2.cardinality()

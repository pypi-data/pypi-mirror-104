...
class JSONTypeInteger(JSONTypeNumeric):
    ...
    def neg(s):
        ...
        non_ints = boolToConstructor.get("anyOf")(
            {"anyOf": get_default_types_except("number", "integer").append({"allOf": [{'not': {'multipleOf': 1}}, {'type': 'number'}]})})
        # numb = {"allOf": [{'not': {'multipleOf': 1}}, {'type': 'number'}]}
    ...
    def _meet(self, s):
        ...
        def _meetObject(s1, s2):
            if s2.type == "object":
                ...
                if ret['maxProperties'] == I.inf:
                    del ret['maxProperties']
                #

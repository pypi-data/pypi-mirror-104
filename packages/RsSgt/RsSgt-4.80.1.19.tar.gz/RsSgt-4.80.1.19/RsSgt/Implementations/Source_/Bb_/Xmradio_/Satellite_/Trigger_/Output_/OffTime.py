...
class OffTime:
	...
	def set(self, off_time: int, output=repcap.Output.Default) -> None:
		...
		Snippet: driver.source.bb.xmradio.satellite.trigger.output.offTime.set(off_time = 1, output = repcap.Output.Default) \n
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:XMRadio:SATellite:TRIGger:OUTPut{output_cmd_val}:OFFTime {param}')

	def get(self, output=repcap.Output.Default) -> int:
		"""SCPI: [SOURce<HW>]:BB:XMRadio:SATellite:TRIGger:OUTPut<CH>:OFFTime \n
		Snippet: value: int = driver.source.bb.xmradio.satellite.trigger.output.offTime.get(output = repcap.Output.Default) \n
		No command help available \n
			:param output: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Output')
			:return: off_time: No help available"""
		output_cmd_val = self._base.get_repcap_cmd_value(output, repcap.Output)
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:XMRadio:SATellite:TRIGger:OUTPut{output_cmd_val}:OFFTime?')

...
class WorkerSourcing(WorkerBase):
    sourcing_func: Callable[[
        Any], Generator[Union[graphio.NodeSet, graphio.RelationshipSet], None, None]] = None
    ...
    def run(self):
        ...
        if not isinstance(self.worker_parameter, dict):
            raise ValueError("worker_parameter not a dict:",
                             self.worker_parameter)
        for set in self.sourcing_func(**self.worker_parameter):
            if isinstance(set, graphio.NodeSet):
                log.debug(
                    f"Recieved graphio.NodeSet '{set.labels}'")

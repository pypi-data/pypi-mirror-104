...
class Manager:
    ...
    def __init__(self, worker_sourcing_class: Type[WorkerSourcing], worker_parameters: List[Dict], cache_backend: Type[CacheInterface] = RedisCache, cache_backend_params: Dict = None):
    ...
    def _start(self):
        ...
        self.manager_sourcing = ManagerWorkersSourcing(
            self, self.worker_sourcing_class)
        self.manager_loading = ManagerWorkersLoading(
            self, self.worker_loading_class)
        finished = False
        self.worker_logging_reciever.handle()
        while not finished:
            log.info(
                f"_blocked_nodesets: {self._blocked_nodesets}")
            self.manager_sourcing.manage()
            self.manager_loading.manage()
            finished = self.manager_sourcing.is_done() and self.manager_loading.is_done()
    ...
    def _get_blocked_nodeset_types(self) -> List[SetsMetaBase]:
        ...
        return list({val for val_list in self._blocked_nodesets.values()
                     for val in val_list})
...
class ManagerWorkersBase():
    ...
    def _get_worker_status(self, worker):
        if worker.is_alive():
            ...
            if worker.exitcode is None:
                ...
                return "finished"
        raise ValueError(
            "Could not determine worker status. Something went wrong", worker.exitcode, worker)

    def _get_cache_sets_meta_data(self, sort_attr="total_size_bytes", set_type=None) -> List[SetsMetaBase]:
        meta_data = self.parent.cache.list_SetsMeta(set_type=set_type)
        return meta_data

    def _init_workers(self, parameters: List[Dict], names: List[str] = None, tags: List = None):
        if not names:
    ...
    def _get_workers(self, status: str, tag=None):
        return [w for w in self.workers if (tag is None or tag in w.tags) and (status is None or self._get_worker_status(w) == status)]
...
class ManagerWorkersSourcing(ManagerWorkersBase):
    def __init__(self, manager: "Manager", worker_class: Type[WorkerBase]):
        ...
        log.debug(
            f"self.parent.worker_parameters: {self.parent.worker_parameters}")
    ...
    def manage(self):
        ...
        # Start next worker
        if len(self._get_workers("running")) < available_cores and len(waiting_workers) > 0:
            new_worker_started = True
            next_worker = waiting_workers.pop(0)
            log.debug(
                f"CALL START ON SOURCING {next_worker} - {next_worker.params}")
            next_worker.start()
            next_worker.timer.__enter__()

        workers_did_finished = self._finish_workers()
        if workers_did_finished or new_worker_started:
            log.debug(f"SOURCING: {len(self.finished_workers) + len(self.failed_workers)} Workers finished / {len(self._get_workers('running'))} Workers running / {len(self._get_workers('initial'))} Workers waiting / {len(self.failed_workers)} Workers failed / {available_cores} Max sourcing workers running simultaneously")
...
class ManagerWorkersLoading(ManagerWorkersBase):
    def __init__(self, manager: "Manager", worker_class: Type[WorkerBase]):
        ...
        self.parent.worker_loading_class.cache_backend_params = self.parent.cache_backend_params
    ...
    def is_done(self):
        ...
        if self.parent.manager_sourcing.is_done() and not self._get_cache_sets_meta_data():
            # ... and all workers are finished
            if len(self._get_workers("running")) + len(self._get_workers("initial")) == 0:
                return True
        else:
            return False
    ...
    def manage_nodeset_loading(self, assigned_no_of_cores):
        ...
        cached_sets_worked_on: List[SetsMetaBase] = [cs for cs in cached_sets if cs in [
            worker.set_meta for worker in self._get_workers("running", tag=self.worker_tag_nodesets) + self._get_workers("initial", tag=self.worker_tag_nodesets)]]
        ...
        if cached_sets_not_worked_on_and_not_blocked:
            ...
            workers_params: List[Dict] = [{"set_meta": cached_set_meta, "graph_params": self.parent.graph_params, "insert_action": self.parent.insert_action, "create_index": self.parent.create_indexes, "create_unique_constraints": self.parent.create_unique_constraints}
                                          for cached_set_meta in cached_sets_not_worked_on_and_not_blocked]
            # generate human readable names for workers
            worker_names: List[str] = [':'.join(self.parent.cache.get_NodeSetMeta(
                cached_set_meta).labels) for cached_set_meta in cached_sets_not_worked_on_and_not_blocked]
            # Initialize workers
            self._init_workers(parameters=workers_params,
                               names=worker_names, tags=[self.worker_tag_nodesets])
        no_of_free_cores = assigned_no_of_cores - \
            len(self._get_workers("running", tag=self.worker_tag_nodesets))

        new_worker_started = False
        if no_of_free_cores > 0:
            # start one new worker
            for next_worker in self._get_workers("initial", tag=self.worker_tag_nodesets)[:1]:
                new_worker_started = True
                next_worker.timer.__enter__()
                log.debug(
        ...
        workers_did_finished = self._finish_workers(
            self.worker_tag_nodesets)
        if workers_did_finished or new_worker_started:
            log.debug(f"LOADING-NODESETS: {len(self.finished_workers) + len(self.failed_workers)} Workers finished / {len(self._get_workers('running'))} Workers running / {len(self._get_workers('initial'))} Workers waiting / {len(self.failed_workers)} Workers failed / {assigned_no_of_cores} Max sourcing workers running simultaneously")

    def manage_relation_loading(self, assigned_no_of_cores):
        cached_sets = self._get_cache_sets_meta_data(
            set_type=graphio.RelationshipSet)

        # find RelSets that have workers assigned (allready running or queued)
        cached_sets_worked_on: List[SetsMetaBase] = [cs for cs in cached_sets if cs in [
            worker.set_meta for worker in self._get_workers("running", tag=self.worker_tag_relsets) + self._get_workers("initial", tag=self.worker_tag_relsets)]]
        log.info(f"cached_sets_worked_on: {cached_sets_worked_on}")
        cached_sets_not_worked_on: List[SetsMetaBase] = [
            cs for cs in cached_sets if cs not in cached_sets_worked_on]
        ...
        if cached_sets_not_worked_on:
            ...
            for cached_set_meta in cached_sets_not_worked_on:
                cache_set_details = self.parent.cache.get_RelSetMeta(
                    cached_set_meta)
            ...
            # Initialize workers
            self._init_workers(parameters=workers_params,
                               names=worker_names, tags=[self.worker_tag_relsets])
        no_of_free_cores = assigned_no_of_cores - \
            len(self._get_workers("running", tag=self.worker_tag_relsets) +
        ...
        if no_of_free_cores > 0:
            ...
            for next_worker in self.drain_ready_workers[:1]:
                ...
                next_worker.timer.__enter__()
                log.info(
                    f"Start {next_worker} - {next_worker.params}")
                next_worker.start()
        ...
        if workers_did_finished or new_worker_started:
            log.debug(f"LOADING-GRAPH: {len(self.finished_workers) + len(self.failed_workers)} Workers finished / {len(self._get_workers('running'))} Workers running / {len(self._get_workers('initial'))} Workers waiting / {len(self.failed_workers)} Workers failed / {assigned_no_of_cores} Max sourcing workers running simultaneously")

    def _order_drains(self):
        # Order drain for relationsSet attached NodeSets for next waiting worker
        for next_worker in [w for w in self._get_workers("initial", tag=self.worker_tag_relsets)[:1] if w not in self.drain_waiting_workers and w not in self.drain_ready_workers]:
        ...
        log.debug(
            f"CHECK DRAINING WAITING WORKERS: {self.drain_waiting_workers}")
        for drain_wait_worker in self.drain_waiting_workers:
            ...
            if not self.parent.cache.is_drain_done(drain_wait_worker.drain_order_ticket):
                log.info(
                    f"DRAIN IS YET RUNNING {drain_wait_worker.drain_order_ticket}")
                drains_done = False
            ...
            nodesets_currently_worked_on: List[SetsMetaBase] = [cs for cs in self._get_cache_sets_meta_data(set_type=graphio.NodeSet) if cs in [
                worker.set_meta for worker in self._get_workers("running", tag=self.worker_tag_nodesets) + self._get_workers("initial", tag=self.worker_tag_nodesets)]]
            log.info(
                f"nodesets_currently_worked_on: {nodesets_currently_worked_on}")

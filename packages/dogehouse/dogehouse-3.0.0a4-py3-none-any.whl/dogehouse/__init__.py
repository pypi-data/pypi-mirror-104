...
from .constants import (
    GET_TOP_ROOMS, JOIN_ROOM, READY,
    CREATE_ROOM, ROOMS_FETCHED, ROOM_CREATED, ROOM_JOINED,
    USER_INFO, USER_JOINED, USER_LEFT,
...
class DogeClient:
    def __init__(self, token: str, refresh_token: str, prefix: str = '.') -> None:
        ...
        self._listeners: Set[Callable[[ApiData], Awaitable[Any]]] = set()
    ...
    # TODO: split this into command handler
    async def _event_handler(self, data: ApiData) -> None:
    ...
    async def _fetch(self, opcode: str, **data: Any) -> None:
        ref = await self._send(opcode, **data)
        return_data: Any = None

        async def fetch_handler(data: ApiData) -> Any:
            nonlocal return_data

            print('fetch handler got data:', data)
            if data.get('ref') != ref:
                print('WOOOOOOOOOOOOOOO')
                # self._listeners.remove(fetch_handler)
                return data

        self._listeners.add(fetch_handler)
    ...
    async def _start(self) -> None:
        ...
        self._listeners.add(self._event_handler)

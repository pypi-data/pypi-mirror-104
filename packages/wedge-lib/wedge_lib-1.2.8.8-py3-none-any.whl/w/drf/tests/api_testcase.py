...
class ApiTestCase(DjangoTestCase):
    ...
    def post_multipart_content(self, url, form_data):
        """
        Post multipart content.

        Args:
            url (str): url for the post request
            form_data (dict): form data to encode as multipart content
        """
        data = encode_multipart(BOUNDARY, {**form_data})
        return self.client_post(url, data=data, content_type=MULTIPART_CONTENT)

    def post_real_file(self, url, dataset_file_path, form_data):
        """
        Post real file (allowing to make file upload).

        Args:
            url (str): url for the post request
            dataset_file_path (str): complete path of the file to post.
                                     (good practice is to put it in the datasets dir)
            form_data (dict): additional data (other than "file") to send in the
                              post request
        """
        with open(dataset_file_path, "rb") as f:
            form_data["file"] = f
            return self.post_multipart_content(url, form_data)

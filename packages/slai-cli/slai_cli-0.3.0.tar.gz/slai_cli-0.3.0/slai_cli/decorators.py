...
from slai_cli.profile.configure import store_credentials, get_credentials
...
def requires_slai_credentials(callback, *outer_args, **outer_kwargs):
    def wrapper(*args, **kwargs):
        try:
            ...
            detect_credentials(
                runtime=runtime, profile=outer_kwargs.get("profile", "default")
            )
...
def requires_slai_project(callback):
    def wrapper(*args, **kwargs):
        ...
        if not slai_directory_present:
            log.warn(
                "No project detected in this directory, create one with 'slai create <project_name>'."
            )
            return

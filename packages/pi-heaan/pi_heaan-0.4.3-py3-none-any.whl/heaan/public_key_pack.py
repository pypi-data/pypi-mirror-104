...
class PublicKeyPack:
    ...
    def __init__(self, context: Context, secret_key: SecretKey=None, public_keypack_path: str=""):
    ...
    def set_key_dir_path(self, public_keypack_path):
        if not os.path.isdir(public_keypack_path+"/PK/"):
            try:
                os.makedirs(name=public_keypack_path+"/PK/", mode=0o775, exist_ok=True)
            except Exception as e:
                print("Could not make public_keypack: ", e)
                return

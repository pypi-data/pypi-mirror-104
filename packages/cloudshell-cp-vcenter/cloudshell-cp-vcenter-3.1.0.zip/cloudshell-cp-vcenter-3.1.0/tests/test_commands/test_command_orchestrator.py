...
class TestCommandOrchestrator(TestCase):
    def setUp(self):
        ...
        self.deploy_action.actionParams.deployment.deploymentPath = (
    ...
    def test_disconnect_all(self):
        ...
        # assert
        self.assertTrue(
            self.command_orchestrator.command_wrapper.execute_command_with_connection.called
        )
    ...
    def test_deploy_from_image(self):
        # act
        cancellation_context = object()
        self.command_orchestrator.deploy_from_image(
            self.context, self.deploy_action, cancellation_context
        )
        # assert
        self.assertTrue(
    ...
    def test_refresh_ip(self):
        ...
        self.command_orchestrator.refresh_ip(
            self.context, cancellation_context=cancellation_context, ports=self.ports
        )
        # assert
        self.assertTrue(
            self.command_orchestrator.command_wrapper.execute_command_with_connection.called
    ...
    def test_orchestration_save_double_quotes_in_snapshot_name_should_be_stripped(self):
        ...
        with patch(SAVE_SNAPSHOT) as save_snapshot_mock:
            ...
            saved_result = CommandOrchestrator().orchestration_save(
                context=remote_command_context, mode="shallow", custom_params=None
            )
            ...
            self.assertEqual(
                saved_result_dict["saved_artifacts_info"]["resource_name"], "vcenter"
            )

...
from cloudshell.cp.vcenter.common.utilites.command_result import (
...
from cloudshell.cp.vcenter.models.DriverResponse import (
    DriverResponse,
    DriverResponseRoot,
)
...
from cloudshell.cp.vcenter.models.OrchestrationSavedArtifact import (
    OrchestrationSavedArtifact,
)
from cloudshell.cp.vcenter.models.OrchestrationSavedArtifactsInfo import (
    OrchestrationSavedArtifactsInfo,
)
...
class CommandOrchestrator(object):
    def __init__(self):
        ...
        self.vm_details = VmDetailsCommand(
            pyvmomi_service=pv_service, vm_details_provider=vm_details_provider
        )
    ...
    def power_off(self, context, ports):
        ...
        return self._power_command(
            context, ports, self.vm_power_management_command.power_off
        )
    ...
    def power_on(self, context, ports):
        ...
        """
        return self._power_command(
            context, ports, self.vm_power_management_command.power_on
        )
    ...
    def _power_command(self, context, ports, command):
        ...
        # execute command
        res = self.command_wrapper.execute_command_with_connection(
            context, command, resource_details.vm_uuid, resource_details.fullname
        )
        return set_command_result(result=res, unpicklable=False)
    ...
    def _parse_remote_model(self, context):
        ...
        if not context.remote_endpoints:
            raise Exception(
                "no remote resources found in context: {0}",
    ...
    def orchestration_save(self, context, mode="shallow", custom_params=None):
        ...
        created_snapshot_path = self.save_snapshot(
    ...
    def orchestration_restore(self, context, saved_details):
        ...
        snapshot_name = saved_artifacts_info["saved_artifacts_info"]["saved_artifact"][
            "identifier"
        ]

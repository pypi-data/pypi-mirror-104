...
class BottleClient:
    ...
    def observe_event(self, data: Dict = None, unique_id: str = None) -> List:
        ...
        for node, node_data in data.items():
            response, uid = self._query(requests.post, 'observe', node_data, nodes=[node], unique_id=unique_id)
            results.append(result)
            response['unique_id'] = uid
            results[node['name']] = response
        if len(results) == 1:
            results = results[0]
        return results, uid
    ...
    def get_vector(self, vector_name: str, nodes: List = None) -> List:
        ...
        for node in nodes:
            try:
                response = requests.post(self.url, verify=self.secure, json={"method": "getVector", "params": {"api_key": self.api_key, "primitive_id": node['id'], "data": vector_name},

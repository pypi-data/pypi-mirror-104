...
from xyw_macro.notify import Notification, TextEvent, VisibleEvent
...
class KbEvent:
    ...
    def __init__(self, n_code, w_param, l_param):
        # self.__original = (n_code, w_param, l_param)
...
class Condition:
    def __init__(self, vk=None, alias=None, repeat=False):
        self.vk = vk
        self.alias = alias
        self.repeat = repeat
...
class Configuration:
    ...
    def __init__(self, name, mode='part', parts=None):
        self.name = name
        self.__mode = mode
        if parts is None:
            self.__parts = self.KEYPAD
        else:
            self.__parts = parts
    ...
    def add_command(self, condition, func):
        ...
        self.__commands[condition.vk] = Command(condition, func)
...
class Core(metaclass=SingletonType):
    def __init__(self, window=None, switch_key=None, active_time=500):
    ...
    def __call__(self, event):
        ...
        if config.mode == 'part' and event.vk_code not in config.parts:
            return 0
        ...
        for command in config.commands.values():
            if command.condition.inspect(event):
                ...
                break
            elif HookConstants.is_keyup(event.w_param):
                self.__show_message('命令\n空')
        return 1

    def __show(self, text, duration):
        QCoreApplication.postEvent(self.__window, TextEvent(text))
        QCoreApplication.postEvent(self.__window, VisibleEvent(True))
        time.sleep(duration)
        QCoreApplication.postEvent(self.__window, VisibleEvent(False))

    def __show_message(self, text, duration=1):
        threading.Thread(target=self.__show, args=(text, duration)).start()


if __name__ == '__main__':
    from xyw_macro.notify import *
    ...
    def main():
        ...
        core = Core(window)
        core.add_config(config1)
        core.add_config(config2)

...
def Track(boxes, classes, confidence, frame):
    ...
    for track in tracker.tracks:
        if not track.is_confirmed() or track.time_since_update > 1:
            continue
        bbox = track.to_tlbr()
        # cv2.rectangle(frame, (int(bbox[i][0]), int(bbox[i][1])), (int(bbox[i][2]), int(bbox[i][3])), (255, 255, 255), 2)
    ...
    for i, label in enumerate(classes):
        ...
        cv2.rectangle(frame, (bbox[i][0],bbox[i][1]), (bbox[i][2],bbox[i][3]), color, 2)

        cv2.putText(frame, label, (bbox[i][0],bbox[i][1]-15), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 2)

...
class StripCutouts(DoFn):
    ...
    def process(self, alertDict):
        ...
        # in decat schema, cutouts are nested
        if schema_map['SURVEY'] == 'decat':
            source = alertStripped[schema_map['source']]
            tmp = {k:v for k, v in source.items() if k not in cutouts}
            alertStripped[schema_map['source']] = tmp

...
class Client:
    ...
    def init_root_dir(
        ...
        self.secret_dir = f"etl/{data_type}/{etl_name_no_version}/secret/token.json"
    ...
    def upload_file(self, src, dst, print_progress=True):
        progress_callback = None
        if print_progress:
            progress_callback = ProgressPercentage(src)
        self.s3_client.upload_file(Bucket=self.bucket, Filename=src, Key=dst, Callback=progress_callback)

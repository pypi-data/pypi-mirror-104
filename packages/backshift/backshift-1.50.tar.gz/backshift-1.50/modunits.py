...
class Item(object):
    ...
    def __init__(self, amount, type_, after, units, fractional_part_length):
        """Initialize."""
...
def modunits(unitsvector, number, detail='highest-and-fraction', units="abbreviated", comma=1, reverse=0,
             after=1, fractional_part_length=-1):
    # pylint: disable=R0913
    # R0913: We need a number of arguments; I'd make them named-only if this didn't need to run on older python's
    """
    ...
    if isinstance(internalvector, str):
        ...
        return _chopit(unitsvector, internalnumber, detail, units, comma, reverse, after, fractional_part_length)


def _chopit(list_, number, detail, units, comma, reverse, after, fractional_part_length):
...
#        print temp, quotient, remainder, item_list
    ...
    if detail == 'string-list':
        ...
        if item_list[1:]:
            ...
            item.amount += item_list[-2].amount / float(item_list[-2].type_.threshold)
        return str(item)
        # return early - don't fall through
    elif detail == 'all':

...
from .cassette_polling import CassettePolling
...
# RESULT_TEXT        3
    ...
    def run (self):
        ...
        for i in range(5):
            CassettePolling(i, self.cmrMap[i][I2C], self.cmrMap[i][CAMERA], self.qForResult, self.qCom, self.deviceQue[i]).start()
...
def threadTest():
    qForGui=queue.Queue()
    qForCom=queue.Queue()
    TestMonitor=TestChipHandlerThread(3,"TCH",qForCom, qForGui)
    TestMonitor.start()

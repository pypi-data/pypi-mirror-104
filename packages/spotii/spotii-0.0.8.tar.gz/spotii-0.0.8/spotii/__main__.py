...
# Form implementation generated from reading ui file 'main_skeleton.ui'
#
...
#from spotii.gui.gui import GuiThre
#import guifolder
...
##        dialog = QtWidgets.QDialog(self.backGround)
##        dialog.ui = Form()
...
#     MainWindow.setWindowFlags(flags)
    ...
    testQue =PeriodQueThread(qForGui,1)
    testQue.start()
...
IMG_FOLDER = '/home/pi/app/spotii/img'
LOCAL_LAUCHER_FOLDER = '/home/pi/app/spotii/launcher'
LOCAL_LAUCHER_CHK_FILE = LOCAL_LAUCHER_FOLDER+'/chk_sum.md5-2'
DESK_TOP = '/home/pi/Desktop'
def preStart():
    import shutil
#     from pathlib import Path
#     Path(LAUNCHER_FOLDER).mkdir(parents=True, exist_ok=True)
    ...
    if lib_path == '':
        ...
        try:
            ...
            if os.path.exists(LOCAL_LAUCHER_CHK_FILE):
                with open(lib_path+'/launcher/'+'chk_sum.md5-2',"rb") as lib_file:
            ...
            if upgrade_launcher:
                ...
                for item in os.listdir(src):
                    if item.endswith('.sh'):
                        shutil(os.path.join(src, item), os.path.join(DESK_TOP, item))
                    else:
                        shutil(os.path.join(src, item), os.path.join(LOCAL_LAUCHER_FOLDER, item))

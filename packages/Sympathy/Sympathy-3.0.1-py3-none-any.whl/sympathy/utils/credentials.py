...
from .. platform.exceptions import (
    SyLoginCredentialError, SySecretCredentialError)
...
def _validate_login(connection, secrets):
    if not secrets:
        secrets = {}
    name = _get_name(connection) or get_resource(connection)
    missing_login = any(k not in secrets or secrets[k] is None
                        for k in ['username', 'password'])

    if missing_login:
        raise SyLoginCredentialError.from_resource(name)
...
def _validate_secrets(connection, secrets):
    ...
    if missing_secrets:
        raise SySecretCredentialError.from_resource(
            get_resource(connection), missing_secrets)
...
def _request_credentials(node, connection):
    ...
    if need_credentials(connection):
        ...
        if mode == login_mode:
            resource = name or resource
        secrets = node.request_credentials(resource, mode.name)
...
def get_credentials(node, connection):
    ...
    res = {}
    if mode == login_mode:
        res = get_login_credentials(node, connection)
    elif mode == secrets_mode:
        res = get_secret_credentials(node, connection)
    return res
...
def get_credentials_async(node, connection, handler):
    ...
    if need_credentials(connection):
        node.request_credentials_async(resource, mode.name, handler)
    else:
        handler({})
...
def need_credentials(connection):
    resource = get_resource(connection)
    mode = _get_mode(connection)
    return mode == login_mode or mode == secrets_mode and has_secrets(resource)

...
def install_args(package, executable=sys.executable, constraints=None):
    res = [executable, '-m', 'pip', 'install', package]
    if constraints:
        res.extend(['-c', constraints])
    return res


def uninstall_args(package, executable=sys.executable):
    return [executable, '-m', 'pip', 'uninstall', '--yes', package]

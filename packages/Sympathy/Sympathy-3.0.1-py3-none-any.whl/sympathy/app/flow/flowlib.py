...
def infertype_flow(flow_, check=False, replace=None):
    ...
    ctx = flow_.get_infertype_ctx()
    remove = set()
    for src, dst, disconnect in ctx.edges:
        if disconnect:
            remove.add((src, dst))
        else:
    ...
    if remove:
        remove = list(remove)
    infertype(ctx, all_edges, check=check, replace=replace, remove=remove)
...
def infertype_connect(port0, port1, disconnect=False,
    ...
    infertype(flow_.get_infertype_ctx(), all_edges, check=check, remove=remove)
...
def infertype(ctx, all_edges, check=False, remove=None, replace=None):
    type_mapping = {}

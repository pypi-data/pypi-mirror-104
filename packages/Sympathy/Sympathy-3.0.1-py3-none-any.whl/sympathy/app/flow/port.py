...
class Port(ElementInterface):
    ...
    def is_connected(self):
        return self._flow.is_connected(self)

    def is_connected_to_node(self):
        """
        Will return True only when connected to the port of a node at the
        other end. If the connection ends with a FlowIO it will return False.
        """
...
class InputPort(Port):
    ...
    def __init__(self, node=None, *args, **kwargs):
        ...
        infertype_ctx = (self.flow or node.flow).get_infertype_ctx()
        if infertype_ctx.delayed:
            infertype_ctx.edges.append((self, node, True))
...
class OutputPort(Port):
    ...
    def __init__(self, node, *args, **kwargs):
        ...
        infertype_ctx = (self.flow or node.flow).get_infertype_ctx()
        if infertype_ctx.delayed:
            infertype_ctx.edges.append((node, self, True))

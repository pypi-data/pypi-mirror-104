...
# Types
Flow = 'sympathy.app.flow.flow.Flow'
...
class UndoCommand:
    ...
    def debug_str(self) -> str:
        ...
        affected = ", ".join(
            f.display_name for f in self.affected_flows())
        added_removed = ", ".join(
            f.display_name for f in self.added_removed_flows())
        if added_removed:
...
class MacroCommand(UndoCommand):
    ...
    def __init__(self, commands: List[UndoCommand], **kwargs):
        ...
        self.__affected_flows = None
        self.__added_removed_flows = None
...
class UndoStack:
    ...
    def get_dirty_flows(self) -> List[Flow]:
        """
        Return all dirty flows tracked by this undo stack.

        Note that the returned flows might be deleted from their parent flow.
        """
        known_flows = (set(self._flow_states.keys())
                       | set(self._clean_states.keys()))
        return [flow for flow in known_flows
                if not self.is_clean(flow)]
    ...
    def command_debug_str(self, index: int) -> dict:
        return "<pre>{}</pre>".format(self._cmds[index].debug_str())

    def flow_debug_str(self, flow: Flow) -> dict:
...
class UndoStackView(QtWidgets.QListView):
    ...
    def get_undo_action(self) -> QtWidgets.QAction:
        return self._undo_action

    def get_redo_action(self) -> QtWidgets.QAction:

...
from kinnaird_utils.system_processes import kill_process_id
logger = logging.getLogger(__name__)


def run_shell_command(command: str, shell: bool = True, wait_for_finish: bool = True, timeout: int = None) -> Union[subprocess.Popen, subprocess.CompletedProcess]:
    """
...
def run_click_command(click_function_to_run, args: str, env: dict = None) -> Result:
    """
    ...
    response = runner.invoke(cli=click_function_to_run, args=args, catch_exceptions=False, env=env)
    return response

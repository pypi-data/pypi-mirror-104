...
class ThreatData:
    ...
    def fetch_GREYNOISE_summary(ip: str) -> str:
        ...
        if gn_ip_data.get('seen', False):
            vowel = 'aeiou'
            org = gn_ip_data['metadata']['organization']
            gn_summary = f"This is a{'n' if org.lower()[0] in vowel else ''} {org} " \
                         f"IP that is marked as {gn_ip_data['classification'].upper()} with tags: {str(gn_ip_data['tags'])}"
        else:

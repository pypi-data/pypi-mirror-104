...
from platone.packages.eth_abi import (
    decode_abi,
)
from platone.packages.eth_abi.exceptions import (
    DecodingError,
)
...
from platone.utils.contracts import (
    encode_abi,
    wasmdecode_abi,
...
from platone.utils.datatypes import (
...
from platone.utils.events import (
    get_event_data,
)
from platone.utils.filters import (
    construct_event_filter_params,
)
from platone.utils.function_identifiers import (
    FallbackFn,
)
...
from platone.packages.platone_keys.utils import bech32
...
class ContractFunctions:
    ...
    def __init__(self, abi, web3, vmtype=None, address=None, ledger=None):
        if abi:
            self.abi = abi
            self.vmtype = vmtype
...
class Contract:
    ...
    :meth:`web3.platone.Eth.contract` that takes compiled Solidity contract
    ...
    * Deploy a new smart contract using :py:meth:`Contract.deploy`
    ...
    def __init__(self, address=None):
        ...
        self.functions = ContractFunctions(self.abi, self.web3, self.vmtype, self.address, self.ledger)
        self.events = ContractEvents(self.abi, self.web3, self.vmtype, self.address, self.ledger)
        self.fallback = Contract.get_fallback_function(self.abi, self.web3, self.address, self.ledger)
    ...
    def factory(cls, web3, class_name=None, **kwargs):
        ...
        setattr(contract, 'functions', ContractFunctions(contract.abi, contract.web3, contract.vmtype, ledger=contract.ledger))
        setattr(contract, 'events', ContractEvents(contract.abi, contract.web3, contract.vmtype, ledger=contract.ledger))
        setattr(contract, 'fallback', Contract.get_fallback_function(contract.abi, contract.web3, ledger=contract.ledger))
    ...
    def deploy(cls, transaction=None, args=None, kwargs=None):
        ...
        txn_hash = cls.web3.platone.sendTransaction(deploy_transaction)
        return txn_hash
    ...
    def eventFilter(self, event_name, filter_params={}):
        ...
        log_filter = self.web3.platone.filter(event_filter_params)
    ...
    def estimateGas(self, transaction=None):
        ...
        if self.web3.platone.defaultAccount is not empty:
            estimate_transaction.setdefault('from', self.web3.platone.defaultAccount)
    ...
    def call(self, transaction=None):
        ...
        .. code-block:: python
            ...
            ContractFactory = w3.platone.contract(
        ...
        if self.web3.platone.defaultAccount is not empty:
            call_transaction.setdefault('from', self.web3.platone.defaultAccount)
    ...
    def transact(self, transaction=None):
        ...
        .. code-block:: python
            ...
            # Deposit to the `web3.platone.coinbase` account.
            >>> wallet.functions.deposit().transact({'value': 12345})
            '0x5c504ed432cb51138bcf09aa5e8a410dd4a1e204ef84bfed1be16dfba1b22060'
            # Deposit to some other account using funds from `web3.platone.coinbase`.
            >>> wallet.functions.deposit(web3.platone.accounts[1]).transact({'value': 54321})
        ...
        if self.web3.platone.defaultAccount is not empty:
            transact_transaction.setdefault('from', self.web3.platone.defaultAccount)
    ...
    @staticmethod
    def get_fallback_function(abi, web3, address=None, ledger=None):
...
class ContractConstructor:
    ...
    def estimateGas(self, transaction=None):
        ...
        if self.web3.platone.defaultAccount is not empty:
            estimate_gas_transaction.setdefault('from', self.web3.platone.defaultAccount)

        estimate_gas_transaction['data'] = self.data_in_transaction

        return self.web3.platone.estimateGas(estimate_gas_transaction)
    ...
    def transact(self, transaction=None):
        ...
        return self.web3.platone.sendTransaction(transact_transaction)
    ...
    def buildTransaction(self, transaction=None):
        ...
        if self.web3.platone.defaultAccount is not empty:
            built_transaction.setdefault('from', self.web3.platone.defaultAccount)
...
def call_contract_function(
    ...
    if block_id is None:
        return_data = web3.platone.call(call_transaction, ledger=ledger)
    else:
        return_data = web3.platone.call(call_transaction, block_identifier=block_id, ledger=ledger)
    ...
    if vmtype == 1:
        ...
        try:
            ...
            is_missing_code_error = (
                    ...
                    web3.platone.getCode(address) in ACCEPTABLE_EMPTY_STRINGS
            )
...
def parse_block_identifier(web3, block_identifier):
    if isinstance(block_identifier, int):
        ...
        return web3.platone.getBlock(block_identifier)['number']
    else:
...
def parse_block_identifier_int(web3, block_identifier_int):
    if block_identifier_int >= 0:
        ...
        last_block = web3.platone.getBlock('latest')['number']
...
def transact_with_contract_function(
    ...
    txn_hash = web3.platone.sendTransaction(transact_transaction)
    return txn_hash
...
def estimate_gas_for_function(
    ...
    gas_estimate = web3.platone.estimateGas(estimate_transaction)
    return gas_estimate
...
def build_transaction_for_function(
    ...
    prepared_transaction = fill_transaction_defaults(web3, prepared_transaction, ledger)

...
def load_data(star):
    ...
    if glob.glob('%s/%d_*'%(star.params['inpdir'],star.name)) != []:
        ...
        if not os.path.exists('%s/%d_PS.txt' % (star.params['inpdir'], star.name)):
            ...
            star.ofactor = int(round((1./((max(star.time)-min(star.time))*0.0864))/(star.frequency[1]-star.frequency[0])))
            if star.oversample:
                if star.ofactor == 5:
                    star.freq_cs = np.array(star.frequency[star.ofactor-1::star.ofactor])
                    star.pow_cs = np.array(star.power[star.ofactor-1::star.ofactor])
                    star.freq_os, star.pow_os = np.copy(star.frequency), np.copy(star.power)
                else:
                    star.freq_cs = np.array(star.frequency[star.ofactor-1::star.ofactor])
                    star.pow_cs = np.array(star.power[star.ofactor-1::star.ofactor])
                    star.freq_os, star.pow_os = lomb(star.time, star.flux).autopower(method='fast', samples_per_peak=5, maximum_frequency=star.nyquist)
                    star.ofactor = 5
                star.resolution = (star.freq_cs[1]-star.freq_cs[0])/star.ofactor
            else:
                if star.ofactor != 1:
                    star.freq_cs = np.array(star.frequency[star.ofactor-1::star.ofactor])
                    star.pow_cs = np.array(star.power[star.ofactor-1::star.ofactor])
        ...
        if star.ps:
            ...
            if star.oversample:
                ...
                star.freq, star.pow = np.copy(star.freq_os), np.copy(star.pow_os)
            else:
                star.note += '\n# No oversampling used'
                star.freq, star.pow = np.copy(star.freq_cs), np.copy(star.pow_cs)
...
def get_initial_guesses(star):
    ...
    if star.params[star.name]['lower_b'] is not None:
        bounds[0] = star.params[star.name]['lower_b']
    else:
    ...
    if star.params[star.name]['upper_b'] is not None:
        bounds[1] = star.params[star.name]['upper_b']
    else:
        bounds[1] = star.nyquist
    star.fitbg['bounds'] = bounds
    ...
    if star.oversample:
        mask = np.ma.getmask(np.ma.masked_inside(star.freq_os, star.fitbg['bounds'][0], star.fitbg['bounds'][1]))
        star.frequency, star.random_pow = np.copy(star.freq_os[mask]), np.copy(star.pow_os[mask])
    else:
        mask = np.ma.getmask(np.ma.masked_inside(star.freq_cs, star.fitbg['bounds'][0], star.fitbg['bounds'][1]))
        star.frequency, star.random_pow = np.copy(star.freq_cs[mask]), np.copy(star.pow_cs[mask])
    ...
    mask = np.ma.getmask(np.ma.masked_inside(star.frequency, star.fitbg['bounds'][0], star.fitbg['bounds'][1]))
    bin_freq, bin_pow, bin_err = mean_smooth_ind(star.frequency[mask], star.random_pow[mask], star.fitbg['ind_width'])

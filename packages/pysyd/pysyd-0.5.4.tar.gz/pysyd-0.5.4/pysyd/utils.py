...
def convert_samples(df):

    drop=[]
    for column in df.column.values.tolist():
        if 'a_' in column:
            n = int(column.split('_')[-1])
            sigma = ((df[column].values*np.pi)/(2.*df['b_%d'%n].values))**(0.5)
            df['sigma_%d'%n] = sigma
            drop.append(column)
        elif 'b_' in column:
            n = int(column.split('_')[-1])
            tau = (df[column].values*10**6.)/(2.*np.pi)
            df['tau_%d'%n] = tau
            drop.append(column)
        else:
            pass
...
def verbose_output(star, sampling=False):
    ...
    if sampling:
        ...
        for idx in df.index.values.tolist():
            note+=line%(df.loc[idx,'parameter'],df.loc[idx,'value'],df.loc[idx,'uncertainty'],params[df.loc[idx,'parameter']]['units'])
    else:
        note+='-------------------------------------------------\nOutput parameters:'
        line='\n%s: %.2f %s'
        for idx in df.index.values.tolist():
            note+=line%(df.loc[idx,'parameter'],df.loc[idx,'value'],params[df.loc[idx,'parameter']]['units'])
    note+='\n-------------------------------------------------'
    print(note)
...
def get_params_dict():
    ...
    params={'numax_smooth':{'unit':'muHz','label':r'$\rm Smoothed \,\, \nu_{max} \,\, [\mu Hz]$'}, 

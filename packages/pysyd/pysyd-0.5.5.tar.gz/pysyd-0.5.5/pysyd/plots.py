...
def plot_samples(star):
    ...
    if len(star.df.columns.values.tolist()) <= 3:
        x, y = 3, 1
    elif len(star.df['parameter'].values) > 3 and len(star.df['parameter'].values) <= 6:
        x, y = 3, 2
    elif len(star.df['parameter'].values) > 6 and len(star.df['parameter'].values) <= 9:
        x, y = 3, 3
    elif len(star.df['parameter'].values) > 9 and len(star.df['parameter'].values) <= 12:
        x, y = 4, 3
    elif len(star.df['parameter'].values) > 12 and len(star.df['parameter'].values) <= 16:
        x, y = 4, 4
    else:
        x, y = 5, 4

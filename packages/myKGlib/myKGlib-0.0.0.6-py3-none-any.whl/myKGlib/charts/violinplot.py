        ...
        if integer_label is not None and label_column is not None:
            ...
            if group_label is not None:
                if orientation is not None:
                    fig = px.violin(self.dataframe, x=integer_label, y=axis_label, color=group_label)
                    fig.show()
                else:
                    fig = px.violin(self.dataframe, x=axis_label, y=integer_label, color=group_label)
                    fig.show()
            else:
                if orientation is not None:
                    fig = px.violin(self.dataframe, x=integer_label, y=axis_label)
                    fig.show()
                else:
                    fig = px.violin(self.dataframe, x=axis_label, y=integer_label)
                    fig.show()                     
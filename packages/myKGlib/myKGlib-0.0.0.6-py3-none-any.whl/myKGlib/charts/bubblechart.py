...
class BubbleChart(Chart):
    ...
    def _check_requirements(self):
        ...
        Returns:
            (string) integer_label: label column with float data type
            (string) label_name: label column with string data type
        """
        integer_label = None
        label_name = None
        if self._is_integer_column_exist:
            integer_label = self._integer_column[0]
            if self._is_object_column_exist:
    ...
    def plot(self):
        ...
        if integer_label is not None and label_name is not None:
            bubble_chart = DrawBubbleChart(area=self.dataframe[integer_label], bubble_spacing=2)

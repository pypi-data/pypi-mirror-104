...
- auto_mount
  ...
  *Deprecated in version 0.7*
  ...
  If you call app.mount (), this option
...
.. _`Atila app example`: https://gitlab.com/skitai/skitai/-/blob/master/tests/level4/serve.py
...
It is important that your resource contains 'services', it will be
remove by services from function. For example, `myapp.services.index`
is identified as `index`, `myapp.services.events.list` is identified
as `events.list`.

But it does not contain 'services', full module name + function
name is used.
...
**CAUTION**: DO NOT app.mount () in \_\_mount\_\_ hook, but in
\_\_setup\_\_ hook.
...
For automated test, Atila use skitai.test_client (). Test client
emulates client-server communication.
...
For override static files, you just place file to your app's static directory.
App find resource at your app static first. And if not exists, find atila_vue/static
directory next. This is same on templates.
...
For examples, you can add services to tfserver_.
...
All mytfserver's services overwrite thserver's including
`statics` and `templates` if exists.

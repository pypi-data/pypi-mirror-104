...
class Decorators:
    def __init__ (self):
        ...
        self._binds_server = [ {} for _ in range (7) ]
    ...
    # app life cycling -------------------------------------------
    def _add_hook (self, store, func):
        store [self._base_func_id (func) [0]] = func

    def before_mount (self, f):
        self._add_hook (self._binds_server [0], f)
        return f
    start_up = before_mount
    startup = before_mount

    def mounted (self, f):
        self._add_hook (self._binds_server [3], f)
        return f

    def mounted_or_reloaded (self, f):
        self._add_hook (self._binds_server [6], f)
        return f

    def before_reload (self, f):
        self._add_hook (self._binds_server [5], f)
        return f
    onreload = before_reload
    reload = before_reload

    def reloaded (self, f):
        self._add_hook (self._binds_server [1], f)
        return f

    def before_umount (self, f):
        self._add_hook (self._binds_server [4], f)
        return f

    def umounted (self, f):
        self._add_hook (self._binds_server [2], f)
        return f
    shutdown = umounted

...
class Router:
    ...
    def coroutine (self, f):
        self._coroutines.add (self.get_func_id (f))
        return f

    def input_stream (self, f):
        self._input_stream.add (self.get_func_id (f))
        return f
    ...
    def add_route (self, rule, func, **options):
        ...
        if "coroutine" not in options:
            options ['coroutine'] = func_id in self._coroutines
        if "input_stream" not in options:
            options ['input_stream'] = func_id in self._input_stream
        ...
        s = rule.find ("/<")
        if s == -1:
            # if rule in self.route_map and self.is_overridable (func):
            #    return
            s_rule = rule
            try:
            ...
            # if re_rule in self.route_map_fancy [prefix] and self.is_overridable (func):
            #    return
        ...
        for method in options.get ("methods", methods):
            if method in resource:
                if self.is_overridable (func):
                    continue
                elif not self._started and not self._reloading and method in {"GET", "POST", "DELETE", "PUT", "PATCH"}:
                    self.log ("- method {} of {} is redefined".format (method, rule), "warn")

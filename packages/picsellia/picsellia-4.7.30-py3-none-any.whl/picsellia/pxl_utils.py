...
import logging
import zipfile
from picsellia import pxl_exceptions as exceptions
...
def check_status_code(request):
    ...
    if int(request.status_code) in [200, 201]:
        return 
    
    elif int(request.status_code) == 403:
        raise exceptions.UnauthorizedError("Not allowed to read/write on this element")

    elif int(request.status_code) == 404:
        raise exceptions.ResourceNotFoundError(request.text)

    elif int(request.status_code) == 400:
        raise exceptions.InvalidQueryError(request.text)
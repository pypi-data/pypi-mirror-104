...
class UploadPlayblastToKitsu(flow.Action):
    ...
    def get_buttons(self):
        ...
        if kitsu_entity is None or task_type is None:
            self.message.set((
                "<h2><font color=#D5000D>The Kitsu entity %s belongs to "
                "couldn't be detected.</font></h2>Please contact the "
                "support on the chat." % self._file.display_name.get()
            ))
            return ['Cancel']
        ...
        if user is None:
            self.message.set((
                "<h2><font color=#D5000D>It seems you (%s) have no "
                "user profile on Kitsu.</font></h2>Please contact the "
                "support on the chat" % self.root().project().get_user()
            ))
            return ['Cancel']
        
        if task is None:
            self.message.set((
                "<h2><font color=#D5000D>This file is not linked to any "
                "task on Kitsu.</font></h2>"
            ))
            return ['Cancel']
        
        # Check if user is assigned to the task or have sufficient rights
        valid_user_role = (user['role'] in ['admin', 'manager'])
        is_assigned = kitsu_api.user_is_assigned(user, task)
        
        if not is_assigned and not valid_user_role:
            self.message.set((
                "<h2><font color=#D5000D>You (%s) are not assigned to "
                "the task this file has been created for.</font></h2>"
        ...
        if valid_user_role and not is_assigned:
            msg += (
                "As %s, you can upload a preview for this file."

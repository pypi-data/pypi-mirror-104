...
from dli.client.exceptions import DataframeStreamingException, ConnectionException
...
class TestInstanceDataframe:
    ...
    @pytest.fixture
    def parquet_dataset(self, client):
        return client._DatasetFactory._from_v2_response({
            'data': {
                'id': '1234',
                'attributes': {
    ...
    def test_dataset_dask_dataframe_csv(self, benchmark, arrow_ipc_stream, csv_dataset):

        test_dataframe, stream = arrow_ipc_stream
        csv_dataset._client = MagicMock()
        csv_dataset._client._environment.consumption = 'https://test/'
        csv_dataset._client._environment.s3_proxy = 's3proxy-url'
        csv_dataset._client.session.get().status_code = 200
        msg = json.dumps({
            'status': 200,
        }).encode('utf')
        ...
        stream.seek(0)
        # reset our fake socket
        csv_dataset._client.session.get().raw = stream
        csv_dataset._client.session.auth_key = 'testjwt1234'
        paths = [[
                's3://abc/as_of_date=2019/country=uk/alltypes_dictionary.csv',
                's3://abc/as_of_date=2019/country=fr/alltypes_dictionary.csv',
            ]]
        from unittest.mock import patch
        with patch('dli.models.structured_dataset_model.StructuredDatasetModel._DatasetModel__list') as _list:
            _list.side_effect = paths

            import dask.dataframe as dd
            dd.read_csv = MagicMock()
            dd.read_parquet = MagicMock()

            csv_dataset.dask_dataframe(dtype='csv', columns=0, filters=0, partitions='as_of_date=2019')

            dd.read_parquet.assert_not_called()
            dd.read_csv.assert_called_once_with(
                urlpath=paths[0],
                blocksize='default',
                lineterminator=None,
                compression=None,
                ...
                storage_options={
                    'client_kwargs': {'endpoint_url': 'https://s3proxy-url'},
                    'key': 'testjwt1234',
                    'secret': 'noop'},
    ...
    def test_dataset_dask_dataframe_parquet(self, benchmark, arrow_ipc_stream, parquet_dataset):

        test_dataframe, stream = arrow_ipc_stream
        parquet_dataset._client = MagicMock()
        parquet_dataset._client._environment.consumption = 'https://test/'
        parquet_dataset._client._environment.s3_proxy = 's3proxy-url'
        parquet_dataset._client.session.get().status_code = 200
        msg = json.dumps({
            'status': 200,
        }).encode('utf')
        ...
        stream.seek(0)
        # reset our fake socket
        parquet_dataset._client.session.get().raw = stream
        parquet_dataset._client.session.auth_key = 'testjwt1234'
        paths = [[
                's3://abc/as_of_date=2019/country=uk/alltypes_dictionary.parquet',
                's3://abc/as_of_date=2019/country=fr/alltypes_dictionary.parquet',
            ]]
        from unittest.mock import patch
        with patch('dli.models.structured_dataset_model.StructuredDatasetModel._DatasetModel__list') as _list:
            _list.side_effect = paths
            ...
            parquet_dataset.dask_dataframe(dtype='parquet', columns=0, filters=0, partitions='as_of_date=2019')

            dd.read_csv.assert_not_called()
            dd.read_parquet.assert_called_once_with(
                path=paths[0],
                columns=0,
                filters=0,
                categories=None,
                index=None,
                storage_options={'client_kwargs': {'endpoint_url': 'https://s3proxy-url'},
                                 'key': 'testjwt1234',
                                 'secret': 'noop'},
                engine='auto',

...
save_path = os.path.expanduser('~\\Desktop\\agtest.log')
...
def apply_voucher(voucher_id):
    logging.info(f'Applying voucher via command line \"AgcoGT.exe APPLYVoucher {voucher_id} NA0001 30096\"')
    execute_command(rf'"C:\Program Files (x86)\AGCO Corporation\EDT\AgcoGT.exe" APPLYVoucher {voucher_id} NA0001 30096')
    ...
    for key, value in updated_content_dict.items():
        if value['type'] == 1:
            if value['test_report_url']:
                ...
                # convert_downloaded_file(full_file_path, key)
            else:
...
def convert_downloaded_file(downloaded_reports_path):
    for root, dirs, files in os.walk(downloaded_reports_path):
        for file in files:
            ...
            if extension == '.pdf':
                convert_pdf_to_img(f'{root}//{file}', root.split('\\')[-1])
            elif extension == '.docx' or extension == '.doc':
                convert_word_to_pdf(f'{root}//{file}', root.split('\\')[-1])
            # elif extension == '.xls' or extension == '.xlsx':
            #     convert_excel_to_pdf(f'{root}//{file}', root.split('\\')[-1])
            else:
                pass


def convert_word_to_pdf(file_path, master_key):
    wdFormatPDF = 17
    ...
    try:
        ...
        doc.Close()
        word.Quit()
    convert_pdf_to_img(renamed_path, master_key)
    delete_file(file_path)


def convert_pdf_to_img(file_path, master_key):
...
@main.command()
@click.option('--installer', '-i', default='auc', type=click.Choice(['auc', 'aucclean', 'choco']))
...
@main.command()
@click.option('--env', '-e', required=True, type=click.Choice(['prod', 'test', 'dev']))
@click.option('--tool', '-t', default='both', type=click.Choice(['auc', 'edt', 'both']))
def change(env, tool):
...
def get_list(ctx):
    ...
    for key, value in release_name_to_release_number_dict.items():
        click.secho(f'{key} : {value}')
    return release_dict, release_name_to_release_number_dict

...
from .common import dateToWord, versionToWord
from .derror import DApiCommandError
...
class DApi2(object):
    ...
    :param DBaseCom comm: Communication channel  
    '''


    def __init__(self, comm, regsfile=None, dev_mode=False):
        '''Constructor'''
    ...
    def sendMessage(self, msg):
        ...
        '''
        assert self.comm.side == DApi2Side.MASTER
        self.comm.sendMessage(msg)
        res = self.comm.receiveMessage(msg.type())
        if res.isError():
...
class DApi2Commands(MetaDApi2Commands):
    ...
    def peripheralActivate(self, value):
        if value != 0 and self.owner.regs.par.value != 0:
            self.standby()
    ...
    def motorFreewheelStop(self):
        ...
        self.owner.regs.smr.freewheel.set()
        self.owner.regs.smr.start.clear()
    ...
    def motorStop(self):
        ...
        if self.owner.regs.smr.isUndefined():
            self.owner.readReg(self.owner.regs.smr)
        self.owner.regs.smr.freewheel.clear()
        self.owner.regs.smr.start.clear()
    ...
    def motorReverse(self):
        ...
        if self.owner.regs.smr.isUndefined():
            self.owner.readReg(self.owner.regs.smr)
        self.owner.regs.smr.reverse.toggle()
    ...
    def lightOff(self):
        ...
        if self.owner.regs.smr.isUndefined():
            self.owner.readReg(self.owner.regs.smr)
        self.owner.regs.smr.light.clear()

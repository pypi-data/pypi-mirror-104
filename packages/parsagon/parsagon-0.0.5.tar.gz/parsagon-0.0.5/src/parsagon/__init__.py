...
class Client:
    ...
    def get_result(self, result_id, format, output, file_path):
        ...
        if not r.ok:
            self._display_errors(r)
        while True:
            r = requests.get(f'https://{self.host}/api/scrapers/results/{result_id}/', headers=headers, timeout=TIMEOUT)
            if not r.ok:
                self._display_errors(r)
            result_data = r.json()
    ...
    def execute(self, scraper_name, urls, env, max_page_loads=1, format='json', output='data', file_path='', is_retry=False):
        ...
        data = {'scraper_name': scraper_name, 'urls': urls, 'max_page_loads': max_page_loads, 'environment': ENVIRONMENTS[env], 'is_retry': is_retry}

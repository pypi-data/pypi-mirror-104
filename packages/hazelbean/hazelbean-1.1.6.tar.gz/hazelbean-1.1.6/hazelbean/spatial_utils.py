...
def reclassify_flex(input_flex, rules, output_path=None, output_data_type=None, array_threshold=10000, match_path=None, verbose=False):
    ...
    if isinstance(rules, dict):
        ...
        if max_key < 0:
            max_key = 0
        if keys_range < array_threshold and values_range < array_threshold:
            old_rules = rules
            if max_key <= 255 and max_value <= 255:
                # NOTE BIG CHANGE and awkward code. it doesn't matter how long it is for the OUTPUT type.
                rules = np.zeros(int(math.ceil(values_range)) + 1, dtype=hb.gdal_number_to_numpy_type[output_data_type])
    ...
    for var_name, var in ds.variables.items():
        ...
        for time_index in time_indices_to_extract:
            ...
            if var_name not in dim_names:
                ...
                output_geotiff_uri = os.path.join(output_dir, output_files_prefix + '_' + var_name + '_' + str(int(time_labels[time_index])) + '.tif')
                if verbose:
                    L.info('Saving array to ', output_geotiff_uri)
...
def extract_global_netcdf_to_geotiff_interactive(input_nc_path, output_dir, vars_to_extract=None, time_indices_to_extract=None, ndv=None):
    ...
    print(2222, np.array(ds.variables['time']))

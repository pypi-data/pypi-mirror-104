...
class Client:
    ...
    def get_atlas(self):
        ...
        data_providers = self.query_and_fetch("""
        select
          svc.[Name], svc.[MqUserName], svc.[Description], svc.[Type], svc.[Category], svc.[Client], 
        ...
        inner join
          Sys.Field as fld
         on
          ((svc.[Name] = fld.[TableName]) and (svc.[Type] = 'DataProvider')) and (fld.[FieldName] is not null)
        """)

        provider_descriptions = self.build_provider_defs(data_providers)
    ...
    def build_provider_defs(self, data_providers):
        ...
        provider_descriptions = []
        for p, p_df in data_providers.groupby('Name'):

            fields = [FieldDefinition.from_row(row) for _, row in p_df.iterrows()]

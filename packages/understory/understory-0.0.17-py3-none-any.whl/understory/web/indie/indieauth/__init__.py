...
root = web.application("IndieAuth", mount_prefix="auth",
                       db=False, client_id=r"[\w/.]+")
server = web.application("IndieAuthServer", mount_prefix="auth/sign-ins",
                         db=False, client_id=r"[\w/.]+")
client = web.application("IndieAuthClient", mount_prefix="auth/visitors",
                         db=False)
...
def wrap_server(handler, app):
    ...
    if tx.request.uri.path == "" and tx.response.body:
        ...
        try:
            ...
            head.append(f"<link rel=authorization_endpoint href={base}>",
...
def wrap_client(handler, app):
    ...
    tx.db.define(visitors="""account_created DATETIME NOT NULL DEFAULT
                                 CURRENT_TIMESTAMP, url TEXT, name TEXT,
                             email TEXT, access_token TEXT""")
    if tx.request.uri.path.startswith("auth/visitors") \
       and not tx.user.is_owner:
        raise web.Unauthorized("get outta 'ere")
    yield
...
class OwnerSignIn:
    ...
    def _get(self):
        return templates.owner_sign_in()
    ...
    def _post(self):
        ...
        if web.verify_passphrase(credential["salt"], credential["scrypt_hash"],
                                 form.passphrase):
            tx.user.session["uid"] = tx.owner["uid"]
...
class Authorize:
    ...
    def _get(self):
        ...
        tx.db.insert("visitors", url=response["me"], name=profile.get("name"),
...
class SignOut:
    ...
    def _post(self):
        token = tx.db.select("visitors", where="url = ?",
                             vals=[tx.user.session["uid"]])[0]["access_token"]

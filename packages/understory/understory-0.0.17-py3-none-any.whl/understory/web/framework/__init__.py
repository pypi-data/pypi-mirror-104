...
__all__ = ["application", "serve", "anti_csrf", "form", "secure_form",
           ...
           "session_table_sql", "resume_session", "add_job_tables"]
...
def add_job_tables(sqldb):
    """"""
    sqldb.define(job_signatures="""module TEXT, object TEXT, args BLOB,
...
class Application:
    ...
    def __init__(self, name, *wrappers, db=True, host=None, static=None,
                 icon=None, sessions=False, serve=False, **path_args):
        self.name = name
        self.wrappers = []
        self.pre_wrappers = []
        self.post_wrappers = []
        ...
        self.route(r"admin/db")(Database)
        ...
        self.route(r"admin/db/{table}")(Table)
    ...
    def __call__(self, environ, start_response):
        ...
        try:
            tx.host._contextualize(environ, self, tx.request.headers.host.name,
        ...
        try:
            ...
            if method == "_head":
                try:
                    ...
                    pass
            exc = MethodNotAllowed(f"{method.lstrip('_').upper()} not allowed")

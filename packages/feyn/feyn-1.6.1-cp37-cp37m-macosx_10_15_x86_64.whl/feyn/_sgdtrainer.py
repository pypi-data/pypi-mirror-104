...
class SGDTrainer:
    ...
    def fit(self, graphs, data, n_samples:Optional[int]=10000, loss_function=_feyn.DEFAULT_LOSS, threads:int=4, sample_weights=None) -> "QGraph":
        """
        Fit the `QGraph` with given data set.

        Arguments:
            graphs -- The graphs to fit
        ...
        Returns:
            graphs -- The fitted and sorted graphs.
        """
        ...
        graphs = list(filter(lambda g: np.isfinite(g.loss_value), graphs))
    ...
    def sort(self, graphs, data, loss_function=_feyn.DEFAULT_LOSS, threads:int=4, sample_weights=None) -> "QGraph":
        """
        Sort the `Graphs` with given data set.

        Arguments:
            graphs -- The graphs to sort
            data -- Training data including both input and expected values. Can be either a dict mapping register names to value arrays, or a pandas.DataFrame.

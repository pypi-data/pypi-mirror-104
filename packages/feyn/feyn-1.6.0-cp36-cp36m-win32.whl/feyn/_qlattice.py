...
class QLattice:
    ...
    def get_qgraph(self, registers: List[str], output: str, stypes: Dict[str, str] = None,
                   max_depth: int = 5) -> "QGraph":
        """
        Deprecated function to extract a QGraph. Use one of the following instead:
        * qlattice.get_classifier
        * qlattice.get_regressor
        """
        warnings.warn("The function get_qgraph is deprecated. Use either get_regressor or get_classifier",
                      DeprecationWarning, stacklevel=2)

        return self._get_qgraph(registers, output, stypes or {}, max_depth)

    def get_classifier(self, registers: List[str], output: str, stypes: Dict[str, str] = None,
                       max_depth: int = 4) -> "QGraph":
        """
    ...
    def get_regressor(self, registers: List[str], output: str, stypes: Dict[str, str] = None,
                      max_depth: int = 5) -> "QGraph":
        """
    ...
    def _get_qgraph(self, registers: List[str], output: str, stypes: Dict[str, str] = None,
                    max_depth: int = 5) -> "QGraph":
        """
    ...
    def _generate(self, specs: List[str], registers: Dict[str, str], max_depth: int, query_string: str = "") -> Dict:
    ...
    def update(self, graphs: typing.Union[Graph, typing.Iterable[Graph]]) -> None:
        ...
        if resp.status_code == 422 and "'graphs': ['Unknown field.']" in resp.text:
            warnings.warn(
                "Your QLattice does not support the update_multi endpoint. Contact Abzu support for help. Falling back to iterative update",
                DeprecationWarning, stacklevel=2)
            for graph in graphs:
    ...
    def _update_query(self, query_string: str) -> None:
        resp = self._http_client.post("/update-query", json={
...
def create_transient_qlattice(server: str = DEFAULT_SERVER):
    resp = requests.get( f"{server}/api/v1/qlattice/transient")
    resp.raise_for_status()
    data = resp.json()
    return QLattice(qlattice=data['qlattice_id'], api_token=data['token'], server=server)

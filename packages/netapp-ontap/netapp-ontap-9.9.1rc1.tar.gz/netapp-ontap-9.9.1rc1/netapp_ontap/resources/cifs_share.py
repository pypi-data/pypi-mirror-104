...
the display of folders or other shared resources that the user does not have access to. """
    ...
    acls = fields.List(fields.Nested("netapp_ontap.models.share_acl.ShareAclSchema", unknown=EXCLUDE), data_key="acls")
...
Example: HR_SHARE """
    ...
    no_strict_security = fields.Boolean(
...
* disable """
    ...
    """links,access_based_enumeration,acls,change_notify,comment,encryption,home_directory,name,no_strict_security,oplocks,path,svm.links,svm.name,svm.uuid,unix_symlink,volume.links,volume.name,volume.uuid,"""
    ...
    """access_based_enumeration,change_notify,comment,encryption,no_strict_security,oplocks,path,svm.name,svm.uuid,unix_symlink,volume.name,volume.uuid,"""
    ...
    """access_based_enumeration,acls,change_notify,comment,encryption,home_directory,name,no_strict_security,oplocks,path,svm.name,svm.uuid,unix_symlink,volume.name,volume.uuid,"""
...
"""
    ...
    if CLICHE_INSTALLED:
        ...
        def cifs_share_show(
            fields: List[Choices.define(["access_based_enumeration", "change_notify", "comment", "encryption", "home_directory", "name", "no_strict_security", "oplocks", "path", "unix_symlink", "*"])]=None,
        ) -> ResourceTable:

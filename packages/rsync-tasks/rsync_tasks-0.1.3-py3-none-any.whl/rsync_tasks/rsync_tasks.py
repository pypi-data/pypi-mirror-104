...
class RsyncTasks:
    def __init__(self,
        ...
        self._flags = flags
        self._base_path_source = Path(base_path_source)
        self._base_path_destination = Path(base_path_destination)
    ...
    def start(self):
        for task in self._tasks:
            try:
                source = self._base_path_source / task["s"]
                destination = self._base_path_destination / task["d"]
    ...
    def _mk_command(self, source: Path, destination) -> str:
        command = ["rsync"]
        ...
        # adds the paths
        command += ["\"{}\"".format(str(source)), "\"{}\"".format(str(destination))]

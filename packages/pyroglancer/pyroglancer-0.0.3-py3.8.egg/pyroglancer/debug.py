...
import os
import glob
from navis import read_swc
from navis.core import NeuronList
...
# neuronlist = []
# neuronlist += [read_swc(f, units='8 nm', connector_labels={'presynapse': 7, 'postsynapse': 8},
#                               id=int(os.path.splitext(os.path.basename(f))[0])) for f in swc_files]

neuronlist = []
layer_kws = {'type': 'skeletons', 'source': neuronlist, 'layer_annottype': 'in-json'}
startdataserver()
add_flywirelayer(ngdict, layer_kws)

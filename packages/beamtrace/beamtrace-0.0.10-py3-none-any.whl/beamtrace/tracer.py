...
class BeamTrace():
    ...
    def calc_beam_waists(self):
        ...
        for key in self.comp_dict.keys():
            if self.comp_dict[key]['type'] == 'space':
                ...
                try:
                    temp_beam_waist_index = np.argmin(self.ww[z_start_index:z_end_index])
                except ValueError:
                ...
                # check if "waist" is at the same location as the start of end of space, then it is no waist
                if beam_waist_index == z_start_index or beam_waist_index == z_end_index - 1:
                    continue

                self.beam_waist_locations = np.append(self.beam_waist_locations, temp_beam_waist_location)
    ...
    def plot_cavity_scan(self, steps=500, round_trip=False, plot_beam_waists=True, color='C3', ls='-', label=None, fig=None):
        ...
        if self.plot_beam_waists:
            ...
            for z_waist, w_waist in zip(self.beam_waist_locations, self.beam_waists):
                ...
                s1.annotate(f"Beam waist {beam_waists}\nLoc = {z_waist:.2f} m\nWaist = {plot_w_waist:.2f} mm",
                    xy=(z_waist, plot_w_waist), xycoords='data',
                    xytext=(z_waist, text_height), textcoords='data',
                    fontsize=12,
                ...
                s1.annotate("",
                    xy=(z_waist, plot_w_waist), xycoords='data',
                    xytext=(z_waist, plot_arrow_height), textcoords='data',
                    fontsize=10,

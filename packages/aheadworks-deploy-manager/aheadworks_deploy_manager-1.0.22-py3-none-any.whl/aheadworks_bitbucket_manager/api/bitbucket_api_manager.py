...
class BitbucketApiManager:
    ...
    def get_build_by_commit(self, commit_hash: str, pipe_name='build'):
        ...
        """
        if self.build_by_commit_depth is None:
            self.build_by_commit_depth = 1
        else:
            self.build_by_commit_depth += 1

        if self.build_by_commit_depth == 7:
            self.build_by_commit_depth = None
        ...
        if build is None:
            ...
            for _ in commit['parents']:
                commit_hash = _['hash']
                list_of_parents_build_number.append(self.get_build_by_commit(commit_hash, pipe_name))
            build = max(list_of_parents_build_number)

        return build

    def get_build_number(self, commit_hash: str, pipe_name='build'):
        ...
        for pipeline in pipelines['values']:
            ...
            if pipeline_commit_hash.find(commit_hash) == 0:
                if pipeline['state']['result']['name'] == 'SUCCESSFUL' \
                        and pipeline['target']['selector']['pattern'] == pipe_name:

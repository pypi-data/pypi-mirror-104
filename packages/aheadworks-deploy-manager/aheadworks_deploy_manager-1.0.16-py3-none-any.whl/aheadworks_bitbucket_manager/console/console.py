...
class Console:
    ...
    def renew_images_by_tag(self, commit_hash, repo=None):
        ...
        try:
            exit_code = 0
            repo = repo or os.getenv('DOCKERHUB_REPO')
            self.docker_hub_api_manager.renew_images_by_tag(commit_hash, repo)
    ...
    def run_common_app(self, check_containers, up_cmd, host=None, user=None):
        check_containers_arr = check_containers.split('|')
        host, user = self._get_default_ssh_cred(host, user)
        print(self.version_control_manager.run_common_app(check_containers_arr, up_cmd, host, user))

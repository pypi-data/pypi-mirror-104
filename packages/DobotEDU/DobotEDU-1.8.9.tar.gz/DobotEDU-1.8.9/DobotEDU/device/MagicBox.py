...
class MagicBoxApi():
    ...
    def set_ptpwithl_cmd(self,
                         ...
                         iswait_forfinish=True,
                         timeout=100000000):
        # 应用场景：滑轨+box+magicianlite
        pose_data = self.__dobotlink.MagicianLite.GetPose(portName=port_name,
                                                          isQueued=is_queued)
        return self.__dobotlink.MagicianLite.SetPTPWithLCmd(
            portName=port_name,
            ptpMode=1,
            x=pose_data["x"],
            y=pose_data["y"],
    ...
    def set_converyor(self,
                      ...
                      speed,
                      is_queued=False):
        STEP_PER_CIRCLE = 32000
        MM_PER_CIRCLE = 3.1415926535898 * 36
    ...
    def get_pixy_camera_obj(self, port_name, port, color, coordinate):
        return self.__dobotlink.MagicBox.GetColorObjCoordinate(
            portName=port_name, port=port, color=color, coordinate=coordinate)
    ...
    @get_portname
    def get_bt_receive(self, port_name):
        res = self.__dobotlink.MagicBox.BleReadMeshData(portName=port_name)
        result = {"data": res["data"], "from_id": res["devID"]}
        return result

    @get_portname
    def get_bt_is_received(self, port_name, from_id):
        res = self.__dobotlink.MagicBox.BleReadMeshData(portName=port_name)
        r_id = res["devID"]
        if r_id == from_id:
            return True

...
from kdmt.dict import nested_dict_get_value
import time
...
"""
    ...
    def __init__(self, parameters=None):
        ...
        self.language=self.get_prameter("language")
        if isinstance(parameters, ModelConfig):
            self.override_default_parameters(parameters.as_dict())
        else:
    ...
    def get_prameter(self, parmeter_name, default=None):
    ...
    def load(cls, model_dir=None, model_metadata=None, cached_component=None, **kwargs):
        ...
        if cached_component:
            ...
            hyperparameters = model_metadata.for_component(cls.name)
    ...
    def optimize(self, X, y):
        try:
            optimizer = OptunaTuner(
                    self.get_prameter('output-folder'),
                    eval_metric=self.get_prameter('opt-metric-name'),
                    time_budget=self.get_prameter('max-time-for-optimization'),
                    init_params={},
                    verbose=True,
                    n_jobs=-1,
                    random_state=self.get_prameter('random-state'),
                )

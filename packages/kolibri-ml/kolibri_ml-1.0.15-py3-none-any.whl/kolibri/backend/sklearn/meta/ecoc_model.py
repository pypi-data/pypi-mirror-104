...
class EcocEstimator(BaseEstimator):
    ...
    Parameters:
        base_estimator: string
            classfier name. This classfier name comes from kolibri model database. These are mainly sklearn models
    ...
    Methods:
        ...
        predict_proba(X): Predict class labels for samples in X.
        predict_top_n(X): Predict class labels for samples in X.
    """
    ...
    def __init__(self, base_estimator="logistic-regression", code_matrix='centroid', distance="euclidean",random_state=42):


        self.params={"base_estimator":base_estimator, "code_matrix":code_matrix, "distance": distance}
        self.base_estimator = self._load_model_from_parameters(get_model(base_estimator))
    ...
    # def predict(self, X):
    #
    #     Y = np.array([self._predict(self.estimators[i], X) for i in range(len(self.estimators))]).T
    #
...
#    def __init__(self, base_estimator="logistic-regression", code_matrix='rand', distance="euclidean",random_state=42):

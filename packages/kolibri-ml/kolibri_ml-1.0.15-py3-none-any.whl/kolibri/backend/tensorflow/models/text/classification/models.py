...
from kolibri.backend.tensorflow.models.text.classification.base_model import TFBaseTextClassificationModel
from kolibri.backend.tensorflow.models.text.classification.cnn_attention_model import CNN_Attention_Model
from kolibri.backend.tensorflow.models.text.classification.dpcnn_model import DPCNN_Model
from kolibri.backend.tensorflow.layers import L, AttentionWeightedAverageLayer, KMaxPoolingLayer, MultiHeadSelfAttention

class FastText(TFBaseTextClassificationModel):
    ...
    def build_model_arc(self):
        ...
        layer_stack = [
            L.GlobalAveragePooling1D(**config['global-pooling']),
            L.Dense(output_dim, **config['layer-output']),
...
class BiLSTM_Model(TFBaseTextClassificationModel):
    ...
    def build_model_arc(self):
        ...
        # build model structure in sequent way
        layer_stack = [
            L.Bidirectional(L.LSTM(**config['layer-bilstm'])),
            L.Dense(output_dim, **config['layer-output']),
            self._activation_layer()
        ]
...
class BiGRU_Model(TFBaseTextClassificationModel):
    ...
    def build_model_arc(self):
        ...
        layer_bi_gru = L.Bidirectional(L.GRU(**config['layer-bigru']))
        layer_dense = L.Dense(output_dim, **config['layer-dense'])
...
class CNN_Model(TFBaseTextClassificationModel):
    ...
    def build_model_arc_(self):
        ...
        #        layers_seq.append(L.GlobalMaxPooling1D(**config['max-pool-layer']))
        if config['dropout'] > 0:
            layers_seq.append(L.Dropout(config['dropout']))
            layers_seq.append(L.GlobalMaxPooling1D())

        layers_seq.append(L.Dense(**config['dense-layer']))
    ...
    def build_model_arc(self) -> None:
        ...
        layer_stack = [
            ...
            L.GlobalMaxPooling1D(**config['max-pool-layer']),
            L.Dense(**config['dense-layer']),
            L.Dense(output_dim, **config['layer-output']),
            self._activation_layer()
        ]
...
class CNN_LSTM_Model(TFBaseTextClassificationModel):
    ...
    def build_model_arc(self):
        ...
        layers_seq.append(L.MaxPooling1D(**config['max-pool-layer']))
...
class CNN_GRU_Model(TFBaseTextClassificationModel):
...
class AVCNN_Model(TFBaseTextClassificationModel):
...
class KMax_CNN_Model(TFBaseTextClassificationModel):
...
class R_CNN_Model(TFBaseTextClassificationModel):
...
class AVRNN_Model(TFBaseTextClassificationModel):
...
class Dropout_BiGRU_Model(TFBaseTextClassificationModel):
...
class Dropout_AVRNN_Model(TFBaseTextClassificationModel):
...
if __name__ == "__main__":
    ...
    from kolibri.datasets.corpus import ConsumerComplaintsCorpus

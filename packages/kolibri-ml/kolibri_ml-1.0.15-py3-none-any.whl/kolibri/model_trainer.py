...
from kdmt.objects import isinstance_of_class
import os
import pandas as pd
import numpy as np
import uuid
...
#        self.config
    ...
    def _build_pipeline(params):
        ...
        for component_name in params['pipeline']:
            if isinstance_of_class(component_name):
                component=component_name
                component_name=component.name
            elif isinstance(component_name, list):
                sub_pipelines.append(ModelTrainer._build_sub_pipeline(component_name, params))
            else:
    ...
    @staticmethod
    def _build_sub_pipeline(sub_pipeline, params):
        """Transform the passed names of the pipeline components into classes"""

        steps = []
        # Transform the passed names of the pipeline components into classes
        for component_name in sub_pipeline:
            if isinstance_of_class(component_name):
                component=component_name
                component_name=component.name
            else:
    ...
    def fit(self, X, y, X_val=None, y_val=None):
        ...
        if self.config['evaluate-performance']==True and self.config['save-evaluation-output']==True and self.pipeline.estimator.validatation_data is not None:
            self.performance_data=pd.DataFrame(np.column_stack((X, y, self.pipeline.estimator.validatation_data)))
            if len(self.performance_data.columns)==4:
                columns=['content', 'class', 'prediction', 'probability']
            else:
                columns=['feat_'+str(i) for i in range(len(self.performance_data.columns)-3)]
                columns.extend(['class', 'prediction', 'probability'])
            self.performance_data.columns=columns

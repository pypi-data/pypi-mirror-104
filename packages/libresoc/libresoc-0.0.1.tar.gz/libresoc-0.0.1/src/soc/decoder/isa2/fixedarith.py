...
from openpower.decoder.helpers import (EXTS, EXTS64, EXTZ64, ROTL64, ROTL32, MASK,
...
class fixedarith:
    ...
    def op_divw(self, RA, RB, RT):
        ...
        if eq(dividend, SelectableInt(value=0x80000000, bits=32)) & eq(divisor,
            SelectableInt(value=0xffffffff, bits=32)) | eq(divisor,
            SelectableInt(value=0x0, bits=32)):
    ...
    def op_modsw(self, RA, RB, RT):
        ...
        if eq(dividend, SelectableInt(value=0x80000000, bits=32)) & eq(divisor,
            ...
            RT[32:64] = MODS(dividend, divisor)
            RT[0:32] = undefined[0:32]
            overflow = 0
        return (RT, overflow,)
    ...
    def op_moduw(self, RA, RB, RT):
        ...
        divisor = concat(0, repeat=32)
        divisor[0:32] = RB[32:64]
        if eq(divisor, SelectableInt(value=0x0, bits=32)):
            RT[0:64] = undefined[0:64]
            overflow = 1
        else:
            RT[32:64] = MODS(dividend, divisor)
    ...
    def op_divd(self, RA, RB, RT):
        ...
        if eq(dividend, SelectableInt(value=0x8000000000000000, bits=64)) & eq(divisor,
            SelectableInt(value=0xffffffffffffffff, bits=64)) | eq(divisor,
    ...
    def op_divde(self, RA, RB, RT):
        ...
        if eq(divisor, concat(0, repeat=128)):
            ...
            result = DIVS(dividend, divisor)
            if eq(result[0:65], SelectableInt(value=0x0, bits=64)):
                RT = result[0:64]
                overflow = 0
            else:
                overflow = 1

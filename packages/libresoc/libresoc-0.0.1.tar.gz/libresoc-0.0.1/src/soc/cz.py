...
class ZeroCounter(Elaboratable):
    ...
    def elaborate(self, platform):
        ...
        # reverse input stream based on left/right
        rs_i = Signal(64, reset_less=True)
        with m.If(self.count_right_i):
            m.d.comb += rs_i.eq(self.rs_i)
        with m.Else():
            m.d.comb += rs_i.eq(self.rs_i[::-1])
        ...
        m.d.comb += z.eq(or4(rs_i[0:16], rs_i[16:32],
                             rs_i[32:48], rs_i[48:64]))
        ...
        with m.Switch(v.sel_hi):
            with m.Case(0):
                m.d.comb += v.v16.eq(rs_i[0:16])
            with m.Case(1):
                m.d.comb += v.v16.eq(rs_i[16:32])
            with m.Case(2):
                m.d.comb += v.v16.eq(rs_i[32:48])
            with m.Case(3):
                m.d.comb += v.v16.eq(rs_i[48:64])

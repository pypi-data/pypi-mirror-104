...
export class ScrollableDiv extends React.Component<IProps, IState> {
    ...
    private getFormattedContents = (): JSX.Element => {
		...
		if (source.length == 0) {
			return (<span style={{padding: '20px', display: 'inline-block', color: 'var(--jp-ui-font-color0)'}}>No output to display.</span>);
		} else {
            const lines: string[] = [];
            ...
            return (
                <AnsiLink style={{padding: '20px', display: 'inline-block', color: 'var(--jp-ui-font-color0)'}} key={lines[0]} text={ansiText} />
            );
        }
        return (<></>);
		// return content;

...
class Polyline(object):
    ...
    def vertex_tangents(self, normalized=False):
        i = np.arange(self.V)
        v1 = np.roll(i, 1)
        v2 = np.roll(i, -1)
        if not self.closed:
        ...
        if normalized:
            ...
            E1 = self.vertices[v2, :] - self.vertices[i, :]
            E2 = self.vertices[i, :] - self.vertices[v1, :]
            N = utilities.normalize(E1) + utilities.normalize(E2)
        N = utilities.normalize(N)
        return N

    def vertex_normals(self):
        from geolab.optimization.combnormals import comb_normals
        N = comb_normals(self)
        return N
    ...
    def vertex_bisectors(self, return_cosines=False):
        ...
        if not self.closed:
            ...
            B[0, :] = N0 / (np.linalg.norm(N0) + 1e-10)
            N1 = np.cross(np.cross(V2[-1], B[-2, :]), V2[-1])
            B[-1, :] = N1 / (np.linalg.norm(N1) + 1e-10)
    ...
    def _pipe_vectors(self):
        return self.vertex_bisectors(return_cosines=False)
...
if __name__ == '__main__':
    ...
    P = Polyline(10 * np.random.random((10, 3)))
    P.corner_tolerance = 1
    P.refine(2)

    plot = []

    plot.append(plotter.Polyline(P, color='r'))
    V = P._pipe_vectors()
    plot.append(plotter.Vectors(V, anchor=P.vertices))
    plotter.view(plot)

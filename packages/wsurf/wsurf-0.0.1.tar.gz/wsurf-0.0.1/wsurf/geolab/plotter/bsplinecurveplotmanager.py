...
from geolab.plotter.plotmanager import PlotManager

from geolab.plotter.pointsource import Points

from geolab.plotter.vectorsource import Vectors

from geolab.plotter.polylinesource import Polyline
...
class BsplineCurvePlotManager(PlotManager):
    ...
    def __init__(self, scene_model=None):
        ...
        self.control_color = 'cornflower'

        self.curve_color = 'cornflower'
    ...
    def bspline(self, bspline):
        if bspline.type != 'Bspline_Curve':
            raise ValueError('*bspline* attribute must be a geometry.BsplineCurve!')
    ...
    def _set_r(self):
        if self.geometry is None or self.geometry.V == 0:
            ...
            p_max = np.max(self.geometry.control_points, axis=0)
            p_min = np.min(self.geometry.control_points, axis=0)
    ...
    def _set_g(self):
        if self.geometry is None or self.geometry.V == 0:
            ...
            p_max = np.max(self.geometry.control_points, axis=1)
            p_min = np.min(self.geometry.control_points, axis=1)
    ...
    def virtual_vertices_on(self):
        if 'virtual-points' not in self.sources or not self.updating:
            M = Points(self.geometry.control_points,
                       name='virtual-points',
                       glyph_type='cube',
                       radius=self.r * 1.1,
                       opacity=0.001)
            ...
            self.update(name='virtual-points',
                        radius=self.r * 1.1)
        ...
        if len(self.selected_vertices) > 0:
            if 'selected-vertices' not in self.sources or not self.updating:
                if self.view_mode == 'wireframe' or self.view_mode == '3d':
                    ...
                    radius = self.r * 1.1
                    glyph = 'sphere'
                P = Points(self.geometry.control_points[self.selected_vertices],
                           color=self.selection_color,
                           radius=radius,
                           shading=False,
                           glyph_type=glyph,
                ...
                self.update(name='selected-vertices',
                            points=self.geometry.control_points[self.selected_vertices],
                            radius=self.r * 1.1)
        else:
    ...
    def move_vertex(self, vertex_index, interaction_callback=None,
        ...
        self.remove_widgets()
        S = tvtk.SphereWidget()
        point = self.geometry.control_points[vertex_index]
        S.center = point
        ...
        def i_callback(obj, event):
            ...
            self.geometry.control_points[vertex_index, :] = center

...
from wsurf.geolab.plotter import colorutilities
...
def rgb_lut_table(expansion=2, opacity=1):
...
def xyz_lut_table(points, variation=0):
    if variation == 0:
        a = b = c = 1
    else:
        a = variation + 1
    ...
    x = 255 * ramp((points[:, 0] - C[0]) * 1.01, a)
    y = 255 * ramp((points[:, 1] - C[1]) * 1.01, -b)
    z = 255 * ramp((points[:, 2] - C[2]) * 1.01, c)
    a = 255 * np.ones(len(points))
    lut = np.column_stack((x, y, z, a))
    lut = np.array(np.round(lut), 'i')
    return lut


def uv_wheel_lut_table(points):
    P = points[:, [0, 1]]
    C = (np.max(P, axis=0) + np.min(P, axis=0)) / 2
    P[:, 0] -= C[0]
    P[:, 1] -= C[1]
    P[:, 0] /= np.max(np.abs(P[:, 0]))
    P[:, 1] /= np.max(np.abs(P[:, 1]))
    r = np.max(np.abs(P), axis=1)
    v = utilities.normalize(P, axis=1)
    a = np.degrees(np.arccos(v[:, 0]))
    a[v[:, 1] < 0] = 360 - a[v[:, 1] < 0]
    rgb = hsv_to_rgb(np.column_stack((a, r, np.ones(len(r)))))
    RGB = np.round(rgb * 255)
    RGB[RGB > 255] = 255
    RGB[RGB < 0] = 0
    lut = np.column_stack((RGB, np.repeat(255, len(r))))
    lut = np.array(lut, 'i')
    return lut


def uv_wheel_lut_table2(points):
    ...
    w = eig[1][:, np.flip(np.argsort(np.abs(eig[0])))]
    P = np.dot(P, w)
    C = (np.max(P, axis=0) + np.min(P, axis=0)) / 2
    P[:, 0] -= C[0]
    P[:, 1] -= C[1]
    ...
    P[:, 0] /= np.max(np.abs(P[:, 0]))
    P[:, 1] /= np.max(np.abs(P[:, 1]))
    P[:, 2] /= np.max(np.abs(P[:, 2]))
    r = np.max(np.abs(P[:, [0, 1]]), axis=1)
    v = utilities.normalize(P[:, [0, 1]], axis=1)
    a = np.degrees(np.arccos(v[:, 0]))
    a[v[:, 1] < 0] = 360 - a[v[:, 1] < 0]
    rgb = hsv_to_rgb(np.column_stack((a, r, 0.95 + 0.05 * P[:, 2])))
    RGB = np.round(rgb * 255)
    RGB[RGB > 255] = 255
    RGB[RGB < 0] = 0
    lut = np.column_stack((RGB, np.repeat(255, len(r))))
...
def make_module(color, opacity, lut_range, lut_expansion,
    ...
    if data_range is not None:
        ...
        if isinstance(color, np.ndarray):
            ...
            lut_e = rgbm_lut_table(lut_expansion)
            module.scalar_lut_manager.lut.table = lut_e

        elif color == 'rgb_e':
            lut_e = rgb_lut_table(lut_expansion, opacity)
            module.scalar_lut_manager.lut.table = lut_e
            ...
            points = kwargs.get('points')
            lut = uv_wheel_lut_table(points)
            module.scalar_lut_manager.number_of_colors = len(points)
            module.scalar_lut_manager.lut.table = lut

        elif color == 'turbo':
            lut = colorutilities.turbo_lut_table()
            module.scalar_lut_manager.lut.table = lut
...
def lut_library():
    lut_mode = ['bwr_e', 'rgb_e', 'xyz', 'rgbm', 'uv_wheel', 'turbo',

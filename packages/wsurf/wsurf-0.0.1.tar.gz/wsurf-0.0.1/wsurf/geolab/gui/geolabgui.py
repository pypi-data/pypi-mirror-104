...
from wsurf.geolab.gui.scenemanager import SceneManager

from wsurf.geolab.gui import openobj

from wsurf.geolab.gui.multiscenemanager import MultiSceneManager

from wsurf.geolab.plotter.geolabscene import GeolabScene, GeolabSecondaryScene, \
    GeolabBaseScene

from wsurf.geolab.gui.geolabcomponent import GeolabComponent

from wsurf.geolab.gui.tools import CornerTolerance, \
    Loads, SaveGeometry, BsplineRebuilder
...
class GlHandler(Controller):
    ...
    def open_bspline_rebuilder(self):
        self.info.object.open_bspline_rebuilder()
...
class GeolabGUI(MultiSceneManager):
    ...
    bspline_rebuilder = BsplineRebuilder()
    ...
    bspline_rebuilder_button = Action(action='open_bspline_rebuilder',
                                      image=ImageResource('../icons/bspline_rebuild.png'),
                                      style='push',
                                      enabled_when='_current_object == "Bspline_surface_plot_manager"',
                                      tooltip='B-spline rebuilder',
                                      show_label=False)
    ...
    plot_manager_button = Action(action='open_plot_manager',
                                 image=ImageResource('../icons/plotmanager.png'),
                                 style='push',
    ...
    fix_vertices_button = Action(action='fix_vertices',
                                 image=ImageResource('../icons/fixvertices.png'),
                                 style='push',
                                 enabled_when='_current_object == "Bspline_surface_plot_manager"',
    ...
    unfix_vertices_button = Action(action='unfix_vertices',
                                   image=ImageResource('../icons/unfixvertices.png'),
                                   style='push',
                                   enabled_when='_current_object == "Bspline_surface_plot_manager"',
    ...
    reset_geometry_button = Action(action='reset_geometry',
                                   ...
                                   enabled_when=('_current_object == \
                                          "Bspline_surface_plot_manager"'),
                                   tooltip='Reset geometry',
                                   show_label=False)

    set_reference_button = Action(action='set_reference',
                                  image=ImageResource('../icons/setreference.png'),
                                  style='push',
                                  enabled_when='_current_object == "Bspline_surface_plot_manager"',
                                  tooltip='Set as reference geometry',
                                  show_label=False)
    ...
    def make_scenes(self):
        ...
        for key in self.__scenes:
            ...
            index += 1
        if index <= 3:
            self.scene_model_3.mayavi_scene.remove()
        if index <= 2:
            self.scene_model_2.mayavi_scene.remove()
        if index <= 1:
            self.scene_model_1.mayavi_scene.remove()

    def _closing(self):
        for component in self._components:
            component.geolab_closing()
        self.scene_model_0.scene.close()
        self.scene_model_1.scene.close()
        self.scene_model_2.scene.close()
        self.scene_model_3.scene.close()

    @on_trait_change('scene_model_0.activated, scene_model_1.activated,\
                     scene_model_2.activated,scene_model_3.activated')
    ...
    def object_change(self):
        SceneManager.object_change(self)
    ...
    def set_reference(self):
        for component in self._components:
            component.geolab_set_reference()
    ...
    def open_bspline_rebuilder(self):
        self.set_state()
        self.bspline_rebuilder.scenemanager = self
        self.bspline_rebuilder.start()
    ...
    def fix_vertices(self):
        ...
        for component in self._components:
            component.geolab_fix_vertices(selected)
        self.current_object.update_plot()

    def unfix_vertices(self):
        selected = self.current_object.selected_vertices
        for component in self._components:
            component.geolab_unfix_vertices(selected)
        self.current_object.update_plot()
...
class ViewerHandler(Controller):
...
class Viewer(SceneManager):

    scene = Instance(MlabSceneModel, ())

    _closed = True

    _handler = ViewerHandler()
    ...
    def start(self):
        view = View(Item('scene',
                         editor=SceneEditor(scene_class=GeolabScene),
                         show_label=False,
                         resizable=self._kwargs.get('resizable', True),
                         height=self._size[1],
                         width=self._size[0],
                         ),
                    handler=self._handler,
                    resizable=self._kwargs.get('resizable', True),
                    title=self._kwargs.get('title', 'GeoLab'),
                    icon=ImageResource('../icons/geolab_logo.png')
                    )
        self._closed = False
        self.configure_traits(view=view)
    ...
    def _scene_settings(self):
        position = self._kwargs.get('position', None)
        if position is not None:
            self.set_view(position)
        background = self._kwargs.get('background', None)
        if background is not None:
            self.background = background
        interaction = self._kwargs.get('interaction', None)
        if interaction == '2d':
            self.interaction_2d()
        elif interaction == 'locked':

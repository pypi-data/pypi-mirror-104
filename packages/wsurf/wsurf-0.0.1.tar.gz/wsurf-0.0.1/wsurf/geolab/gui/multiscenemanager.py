...
class MultiSceneManager(SceneManager):
    ...
    def delete_object(self, name):
        self.remove(name)
        scene = self.get_object_scene(name)
        scene.remove(name)
        del self._object_scenes[name]
    ...
    def plot_mesh_faces(self, mesh, **kwargs):
        scene = self._format_scene(kwargs)
        self.get_scene(scene).plot_mesh_faces(mesh, **kwargs)

    def plot_mesh_edges(self, mesh, **kwargs):
        scene = self._format_scene(kwargs)
        self.get_scene(scene).plot_mesh_edges(mesh, **kwargs)
    ...
    def select_object(self):
        ...
        for key in self._objects:
            ...
            count += N
        if self._selected is None:    
            self._selected = list(self._objects.keys())[0]

...
from wsurf.geolab.optimization import spsolve

from wsurf.geolab.optimization import spdot, hdot
...
class GuidedProjectionBase(object):
    ...
    def _open_string(self):
        ...
        string = '.' + ' ' * 8 + 'Relative Error:'
        string += '     mean    |    max     |   weight   .\n'
        out += string
        return out

    def _optimization_info(self):
        m = 'cumulative time: {:.3f}'.format(self.__timer)
        out = self._format_message(m)
        m = 'step: {}'.format(self._step)
        out += self._format_message(m)
        m = 'number of variables: {}'.format(self.N)
        out += self._format_message(m)
        constraints = 0
        for c in self._constraints:
            constraints += c[1]
        m = 'number of constraints: {}'.format(constraints)
        out += self._format_message(m)
        return out
    ...
    def _make_report(self):
        ...
        out += self._split_string()
        out += self._optimization_info()
        for message in self._messages:
            out += self._format_message(message)
        out += self._separator_string()
        self._report = out
        if self.make_residual:
    ...
    def optimize(self):
        ...
        while iteration < self.iterations and not stop:
            ...
            R = self._R.tocsr()
            if self.plot_matrix:
                from geolab.plotter.matrix import plot_matrix
                plot_matrix(H)
            A = spdot(H.transpose(), H) + spdot(K.transpose(), K) + R
            # A = hdot(H) + spdot(K.transpose(), K) + R
            if self.plot_matrix:
                from geolab.plotter.matrix import plot_matrix
                plot_matrix(A)
            a = spdot(H.transpose(), r) + spdot(K.transpose(), s) \
                + self.epsilon ** 2 * X

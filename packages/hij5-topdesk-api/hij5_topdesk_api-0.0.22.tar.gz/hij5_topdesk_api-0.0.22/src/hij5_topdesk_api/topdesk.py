...
class TopdeskConnector:
	...
	def incidents_by_supplier(self, supplier_id, completed: bool or None = None):
		...
		if completed is None:
			params = f'query=supplier.id=={supplier_id}'
		if completed:
			params = f'query=supplier.id=={supplier_id};completed==True'
		if completed is False:
			params = f'query=supplier.id=={supplier_id};completed==False'
	...
	def operational_activity_with_status(self, activity_id: str) -> dict:
		"""
		Verkrijg informatie over 1 operational activity. Deze is specifiek gemaakt omdat er geen headers meegestuurd
		moeten worden om de status te verkrijgen...
		...
		params = {"fields": "all"}
		return self.make_request('get', f'operationalActivities/{activity_id}/', headers=headers, params=params)

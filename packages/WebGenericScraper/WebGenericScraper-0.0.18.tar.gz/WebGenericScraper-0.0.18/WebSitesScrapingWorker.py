...
class WebSiteScarperWorker(WorkerAbstract):
        ...
        if len(sub_actions) > 0:
            for action in sub_actions:
                if 'actionType' in action:
                    ...
                    elif action['actionType'] == 'rec' and single_action_result is not None:
    ...
    def run_single_action(self, target_value, action_name, action_params):
        ...
        if action_name == 'addToVar':
            ...
            var_value = "" if not var_value else var_value
            if '$' in var_value:
                ...
                if identifiers and len(identifiers) != 0:
                    for identifier, identifier_val in identifiers.items():
                        if not elem.has_attr(identifier):
                            all_match = False
                            break
                        if identifier_val not in elem.get(identifier) :
                            all_match = False
                            break
    ...
    def tree_branch_to_csv(self, branch, html_table_idx,  pre_defined_columns):
        ...
        for key, val in pre_defined_columns.items():
            #or logic between two potential values the first one that is not empty will be choose
            if 'or' in val:
                val_as_list = val.replace(" ","").split('or')
                for value in val_as_list:
                    regexp_res = re.search(r"\[([A-Za-z0-9_]+)\]", value)
                    index = int(regexp_res.group(1))
            ...
            if type(val) == str:
                pre_def_columns_completed[key] = self.fix_text(val)
            else:
                pre_def_columns_completed[key] = self.fix_text(val.text)

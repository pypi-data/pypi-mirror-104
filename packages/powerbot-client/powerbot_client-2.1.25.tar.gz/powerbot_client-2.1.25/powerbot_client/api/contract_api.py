...
class ContractApi(object):
    ...
    def get_order_books(self, **kwargs):  # noqa: E501
        ...
        Shows the public order book for the requested products (or all available products) for a given delivery_area. Default delivery area will be applied if left blank and default is set. This request will by default only deliver the order book statistics (best bid, best ask, last etc.) and does not include the full orderbook depth. If you are querying for only one contract, you can retreive the full order book by setting with_order_details to \"true\". Alternatively you can request the full order book with GET /contract/contract_id/delivery_area/orders.  The order book containts portfolio relevant data (like net_position, signals etc.) and can thus be filtered by a list of portfolio IDs. In addition, the request can be filtered by a lot of other parameters like contract_id (array), contract_name, delivery_time, product type etc. Setting the limit parameter allows to limit the number of entries returned per request (sorted by ascending delivery time).  The filter with_bid_or_ask_only returns only non empty order books. The filter \"with_signal\" (default true) allows to control if trading signals are delivered with the order book. If past_hours is set, the orderbook will include historical contracts that have been closed in the last X hours.  Using delivery_from and delivery_to, the order book can be further limited to a given timespan. If used in conjunction with past_hours, delivery_from has to be after the time defined by past_hours (now - past_hours < delivery_from < delivery_to).  Please note the SLA of 100 requests/h when requesting the full orderbook depth for more than one contract.  # noqa: E501
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please pass async_req=True
    ...
    def get_order_books_with_http_info(self, **kwargs):  # noqa: E501
        ...
        all_params = ['portfolio_id', 'product', 'with_bid_or_ask_only', 'with_signals', 'contract_id', 'contract_name', 'delivery_start', 'delivery_end', 'delivery_within', 'past_hours', 'delivery_area', 'delivery_from', 'delivery_to', 'with_order_details', 'contract_type', 'limit']  # noqa: E501
        all_params.append('async_req')
        all_params.append('_return_http_data_only')
        all_params.append('_preload_content')
        all_params.append('_request_timeout')
        ...
        if 'limit' in params and params['limit'] < 0:  # noqa: E501
            raise ValueError("Invalid value for parameter `limit` when calling `get_order_books`, must be a value greater than or equal to `0`")  # noqa: E501
        collection_formats = {}

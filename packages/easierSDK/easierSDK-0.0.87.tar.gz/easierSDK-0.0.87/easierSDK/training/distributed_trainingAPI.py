...
class DistributedTrainingAPI():
    ...
    def create_training(self, x, y, easier_model:EasierModel, training_metadata:TrainingMetadata=None, x_test=None, y_test=None, public=False, num_workers=3):
        ...
        Args:
            ...
            num_workers ([int], optional): Number of workers to launch to distribute the training. 
        ...
        with kubernetes.client.ApiClient() as api_client:
            ...
            for i in range(num_workers):
                ...
                config_map['metadata']['name'] += '-' + self._easier_user.replace('.', '-') + '-' + random_name + '-w-' + str(i)
                ...
                service['metadata']['name'] += '-' + self._easier_user.replace('.', '-') + '-' + random_name + '-w-' + str(i)
                service['metadata']['labels']['app'] += '-' + self._easier_user.replace('.', '-') + '-' + random_name + '-w-' + str(i)
                service['spec']['selector']['app'] += '-' + self._easier_user.replace('.', '-') + '-' + random_name + '-w-' + str(i)
                ...
                deployment['metadata']['name'] += '-' + self._easier_user.replace('.', '-') + '-' + random_name + '-w-' + str(i)
                deployment['metadata']['labels']['app'] += '-' + self._easier_user.replace('.', '-') + '-' + random_name + '-w-' + str(i)
                deployment['spec']['template']['metadata']['labels']['app'] += '-' + self._easier_user.replace('.', '-') + '-' +  random_name + '-w-' + str(i)
                deployment['spec']['template']['spec']['containers'][0]['name'] += '-' + self._easier_user.replace('.', '-') + '-' + random_name + '-w-' + str(i)
                deployment['spec']['template']['spec']['containers'][0]['envFrom'][0]['configMapRef']['name'] += '-' + self._easier_user.replace('.', '-') + '-' + random_name + '-w-' + str(i)
    ...
    def clear_training_deployment(self, deployment_identifier:str, namespace:str):
        """Deletes the deployment resources of a training.

        Args:
            deployment_identifier (str): random name given as identifier when a training was created
        ...
        with kubernetes.client.ApiClient() as api_client:
            ...
            try:
                ...
                config_map['metadata']['name'] += '-' + self._easier_user.replace('.', '-') + '-' + deployment_identifier
                api_response = api_instance.delete_namespaced_config_map(deployment, namespace)
                # print(api_response)
            except ApiException as e:
            ...
            try:
                ...
                deployment['metadata']['name'] += '-' + self._easier_user.replace('.', '-') + '-' + deployment_identifier
                api_response = api_instance.delete_namespaced_pod(deployment['metadata']['name'], namespace)
                # print(api_response)
            except ApiException as e:

...
def run(
    ...
    try:
        ...
        logging.info("Error when monitoring project - connection to server "
            "may have been lost.")

    try:
        simulation = load(task_id)
    except Exception as e:
        logging.warning(f"Error when loading simulation: \"{e}\". Returning "
            "task_id but monitors have not been loaded in simulation.")
    finally:
        return task_id


def load(task_id, target_folder="out"):
    ...
    try:
        ...
        sim = Simulation.import_json(f"{target_folder}/simulation.json")
        if len(sim.monitors) > 0:
            logging.info("Loading monitor data into simulation.")
            sim.load_results(target_folder + "/monitor_data.hdf5")
        else:
...
def new_project(
    ...
    resp = s3Client.put_object(
        ...
        Key="users/{0}/{1}/{2}".format(keys["UserId"], task["taskId"], "fdtd3d.json"),
    )
...
def download_json(task_id, target_folder=""):
    ...
    if not sim_dict:
        s3Client.download_file(
            ...
            Key="users/{0}/{1}/{2}".format(keys["UserId"], task_id, "fdtd3d.json"),
        )
    else:

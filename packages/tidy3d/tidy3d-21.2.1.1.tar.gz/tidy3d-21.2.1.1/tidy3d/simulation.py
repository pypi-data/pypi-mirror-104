...
from .utils.check import check_3D_lists, check_poles
...
from .source import Source, ModeSource, SourceData, PlaneWave, PlaneSource
    ...
    def _add_structure(self, structure):
        ...
        try:
            ...
            if len(self.materials) < 200:
                ...
                check_poles(mat, self._mat_names[-1], self.grid.dt)
            else:
                log_and_raise(
    ...
    def set_time(self, run_time=None, courant=None):
        ...
        if courant is not None:
            ...
            for (mat, mat_name) in zip(self.materials, self._mat_names):
                check_poles(mat, mat_name, self.grid.dt)
    ...
    def load_results(self, dfile, ind_src_norm=0):
        ...
        try:
            for (im, mnt_data) in enumerate(self._monitor_data):
                ...
                # Load field data
                mname = mnt_data.name
                mnt_data._load_fields(mfile[mname],
                                self.symmetries, self.grid.Nxyz)
                mnt_data.mesh_step = self.grid.mesh_step

                # Load mode amplitude data for a ModeMonitor
                mnt_data._load_modes(mfile[mname])

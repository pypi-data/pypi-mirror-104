...
from bergen.types.node.ports.arg import *
from bergen.types.node.ports.kwarg import *
from bergen.types.node.ports.returns import *
import logging
...
def parseFunctionToDict(function: Callable, widgets: dict = {}, allow_empty_doc=False, interface=None):
    ...
    for key, value in function_inputs.items():
        ...
        if value.default == Parameter.empty: # Non default parameters are args
            if issubclass(the_class, models.Model):
                args.append(ModelArgPort.fromParameter(value, the_class._meta, widget=widget))
            ...
            if issubclass(the_class, models.Model):
                kwargs.append(ModelKwargPort.fromParameter(value, the_class._meta, widget=widget))
            if issubclass(the_class, int):
    ...
    try:
        ...
        if function_output._name == "Tuple":
            for type in function_output.__args__:
                if issubclass(type, models.Model):
                    returns.append(ModelReturnPort(value._meta, key=type.__name__))
        ...
        if issubclass(function_output, models.Model):
                returns.append(ModelReturnPort(function_output._meta, key=function_output.__name__))
...
class NodesRegistry:
    ...
    def __init__(self):
        self.node_dicts: Dict[str, Dict] = {}


    def register(self):
        for dictio in self.node_dicts.values():
            print(Node.objects.update_or_create(**dictio))


    def func(self, widgets: dict = {}):

        def decorating(func):
            node_dict = parseFunctionToDict(func, widgets=widgets)
            self.node_dicts[f"{node_dict['interface']}"] = node_dict

...
class MD5(object):
	...
	def _step_1(cls):
		...
		bit_array.frombytes(cls._input)
	...
	def _step_2(cls, step_1_result):
		...
		length = (len(cls._input) * 8) % pow(2, 64)
	...
	def _step_5(cls):
		...
		return b''.join([int2bytes(i) for i in (A, B, C, D)])


def md5(data: bytes):
	"""
	Returns a hexadecimal string of the input's MD5 digest.
	Define a simple function for users so that they don't have to use MD5 object.
	"""
	return binascii.b2a_hex(MD5.hash(data))

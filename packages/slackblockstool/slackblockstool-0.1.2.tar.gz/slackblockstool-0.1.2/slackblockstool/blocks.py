    ...
    def create(cls, params: dict):
        if "type" in params:
            if params["type"] == "button":
                ...
                return ObjectText.createU(params)
        ...
        if "type" in params:
            if params["type"] == cls.TYPE:
                return BlockSection(
                    ...
                    accessory=None if not "accessory" in params else None,
                )
...
class BlockHeader(Block):
    ...
    def create(cls, params:dict) -> Block:
        return BlockHeader(
            text = ObjectPlainText.create(params['text']),
            block_id=None if not 'block_id' in params else params['blockid']
        )
...
class SlackBlocks:
        ...
        if block_ids == None:
            ...
            for id in block_ids:
                for index, originalBlock in enumerate(self.blocks):
                    if 'block_id' in originalBlock:
                        if originalBlock['block_id'] == id:
                            self.blocks.pop(index=index)
    ...
    def getPosition(self, block_id:int) -> int:
        for index, originalBlock in enumerate(self.blocks):
            if 'block_id' in originalBlock:
    ...
    def parseBlocks(self, paramsList:List[dict]):
        ...
        for param in paramsList:
            for class in creationClasses:
                if param['type'] == class.TYPE:
                    block = class.create(param)
                    if block != None:
                        self.blocks.append(block)

...
class FixSpreadModel(SpreadingModel):
    ...
    def init(self):
        self.status = {
            str(k): self.initial_status[k].copy() for k in self.initial_status.keys()
        }
        self.times = np.zeros(self.ids.size)
        self.received = np.array([len(self.initial_status[k]) for k in self.initial_status.keys()])
    ...
    def iteration_bunch_comm(self, num_iter, tick, prnt=False):
        ...
        for _ in np.arange(num_iter):
            ...
            ticks = np.append(ticks, rs_n)
            nodes.append(np.sum(self.received == self.check))
        return ticks, nodes

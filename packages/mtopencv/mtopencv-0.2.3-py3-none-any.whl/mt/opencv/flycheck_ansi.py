...
def to_ansi(img, imgres=None):
    ...
    if imgres is None:
        ...
        img_imgres[1] *= 2 # to counter the fact that each whitespace is half a blockh
        ...
        width = (imgres[1]*img_imgres[0])//img_imgres[1]
        if imgres[0] > width:
            imgres[0] = width

        height = (imgres[0]*img_imgres[1])//img_imgres[0]
        if imgres[1] > height:
            imgres[1] = height

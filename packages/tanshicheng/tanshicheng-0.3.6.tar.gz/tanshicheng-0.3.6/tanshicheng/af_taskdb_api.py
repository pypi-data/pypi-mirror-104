...
class TaskDB:
    ...
    def update_task(self, result, no):
        """
        ...
        try:
            ...
            result = self._numpy_to_base(result, True)
            print('result:')
            pprint(result)
    ...
    def update_tasks(self, info_L):
        """
        更新多个结果-加速, 如果已执行的话会生成完成标志的镜像文件夹, main_path没有提供会生成默认
        :param info_L: [{'result':dict,..},..]; 一个list, 每个元素是一个和 update_task 参数一样的 dict
        :return: list of result
        """
        results_L = []
        for info in info_L:
            try:
                result = self._numpy_to_base(info['result'])
                no = Query()['no'] == info['no']
            except:
                pprint('info:', info)
                raise
    ...
    def clean(self):
        ...
        if info_L:
            print('重置缺失镜像文件夹任务...')
            print(len(self.update_tasks(info_L)), '个任务重置')
        # 删除多余镜像主目录中的文件夹
        n = 1
    ...
    def test(db):
        ...
        obj.run_tasks()
        print('更新任务:')
        print(obj.update_task({'executed': False}, q_L[0]['no']))
        info_L = [
            {'result': {'executed': False}, 'no': q_L[1]['no']},
            {'result': {'test': 123}, 'no': q_L[-1]['no']},
        ]
        print(obj.update_tasks(info_L))
        print()
        print('统计结果:')
        ...
        def api():
            ...
            try:
                ...
                # 完成任务
                    ...
                    # 更新任务
                    db_obj_D[db].update_task(result, no)

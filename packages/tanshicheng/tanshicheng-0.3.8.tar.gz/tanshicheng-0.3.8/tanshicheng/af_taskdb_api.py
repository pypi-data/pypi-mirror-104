...
class TaskDB:
    ...
    def output_table(self, path=None, cols_limit=100, col_front=('main_path', 'executed', 'priority'),
                     col_back=('no',), min_unique_prior=2, max_null_rate_prior=0.1):
        """
    ...
    def test(db):
        ...
        info_L = [
            ...
            {'result': {'executed': False}, 'no': q_L[2]['no']},
...
class TaskDBapi:
    ...
    def app_run(db_dirs: list, port=19999, log_path=f'log/TaskDBapi.app_run.log'):
        ...
        # 获取任务方法
        def get_one_task(db, query: dict = None):
            """
            获取一个任务, 获取任务考虑优先级问题
            :param db: post_dict['db']
            :param query: dict; 用于que_tasks的查询语句, 用于过滤
            :return:
            """
            task = {}
            if len(db_no8num_D[db]) == 0:
                ...
                if ii is None:
                    ...
                    describe = f'获取任务成功, 该任务已被分配过{assigned_num}次'
                    status = 4
        ...
        def api():
            ...
            try:
                ...
                try:
                    ...
                    raise NameError('query 需要 str 形式的 dict !')
                ...
                out_request = f'请求信息:\n' + pformat(post_dict) + \
                              f"\n数据库({db})未完成/总任务: {len(sum(db_no8num_D[db], []))}/{len(db_obj_D[db].tasks)}; " \
    ...
    def request_api(request_data: dict, url='http://127.0.0.1:19999', sleep=0.5, try_times=5, db_dir=None,
        ...
        for i in range(try_times):
            ...
            try:
                ...
                times = 1
                while request_data['type'] == 'request' and response['status'] == 2:

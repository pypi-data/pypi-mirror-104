...
def set_gpu(showAllGpu=False, return_more=False):
    ...
    if return_more:
        ...
        return {
            ...
            'ip': ip,  # 主机局域网ip
            'gpu_power': w,  # 每张gpu的使用功率和上限功率
            'ext_gpu_mem': [i[1] for i in i_m],  # 每张gpu的剩余显存数量, 单位MB
            'ext_cpu': ext_cpu,  # 剩余的cpu使用率, 单位百分比
            'ext_mem': ext_mem,  # 剩余内存数量, 单位MB
            'cpu_info': f"{n}:{cpu_num}-{cpu_cores}-{core_pro}",  # 总线程数:cpu数-每个cpu的核心数-每个核心的超线程数
        }

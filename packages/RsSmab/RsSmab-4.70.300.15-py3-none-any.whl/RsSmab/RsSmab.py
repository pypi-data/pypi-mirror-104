...
class RsSmab:
	...
	driver_options = "SupportedInstrModels = SMA/SMAB/SMCV, SupportedIdnPatterns = SMA/SMCV, SimulationIdnString = 'Rohde&Schwarz,SMA100B,100001,4.70.300.0015'"
	...
	def __init__(self, resource_name: str, id_query: bool = True, reset: bool = False, options: str = None, direct_session: object = None):
		...
		self._core = Core(resource_name, id_query, reset, RsSmab.driver_options, options, direct_session)
		self._core.driver_version = '4.70.300.0015'
	...
	def assert_minimum_version(min_version: str) -> None:
		...
		curr_version_list = '4.70.300.0015'.split('.')
		...
		for i in range(count):
			...
			if curr > minimum:
				break
			if curr < minimum:
				raise RsInstrException(f"Assertion for minimum RsSmab version failed. Current version: '4.70.300.0015', minimum required version: '{min_version}'")

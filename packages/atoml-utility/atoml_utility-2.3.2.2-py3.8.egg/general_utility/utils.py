...
def initialize(path, prefix = 'output'):
    PIP_MIRRORS_URL = os.getenv("PIP_MIRRORS_URL", None)

    initialize_file = os.path.join(path, ('%s.initialize.log' % prefix))
    error_filename = os.path.join(path, ('%s.initialize.err' % prefix))
    ...
    if (path_exists(init_filename) == True):
        if (sysstr.lower() == 'linux') or (sysstr.lower() == 'darwin'):
            subprocess.call(['chmod', '+x', init_filename])
        subprocess.call(init_filename, stdout=open(initialize_file, 'a'), stderr=open(error_filename, 'a'))

    if (path_exists(os.path.join(path, 'requirements.txt')) == True):
        filename = os.path.join(path, 'requirements.txt')
        cmd_list = ['python', '-m', 'pip', 'install', '-r', filename]
        if (PIP_MIRRORS_URL is not None):
            cmd_list.append('-i')
            cmd_list.append(PIP_MIRRORS_URL)
        subprocess.call(cmd_list, stdout=open(initialize_file, 'a'), stderr=open(error_filename, 'a'))

...
class Query(QueryableReference):
    ...
    def compoundBody(self):
        if self.queryType == QueryType.IS_EQUAL_TO:
            self.compoundQuery["query"]["bool"]["must"].append(self.equalToBody())
        
        elif self.queryType == QueryType.IS_GREATER_THAN:
            self.compoundQuery["query"]["bool"]["must"].append(self.comparisonBody("gt"))
        
        elif self.queryType == QueryType.IS_GREATER_THAN_OR_EQUAL_TO:
            self.compoundQuery["query"]["bool"]["must"].append(self.comparisonBody("gte"))
        
        elif self.queryType == QueryType.IS_LESS_THAN:
            self.compoundQuery["query"]["bool"]["must"].append(self.comparisonBody("lt"))
        
        elif self.queryType == QueryType.IS_LESS_THAN_OR_EQUAL_TO:
            self.compoundQuery["query"]["bool"]["must"].append(self.comparisonBody("lte"))

...
class Signer:
    ...
    def sign(self, requestType, resource, bodyBytes = b"", queryParameters = {}, presigned = False, expires = "432000"):
        ...
        resource = [urllib.parse.quote(res, safe = '') for res in resource.split('/') ]
    ...
    def canonicalStringFromQuery(self, query):
        ...
        keys.sort()
        result = [f"{urllib.parse.quote(key, safe = '')}={urllib.parse.quote(query[key], safe = '')}" for key in keys]
        return "&".join(result)
...
class SignDetails:
    ...
    def flatQuery(self):
        ...
        data = [f"{k}={v if k == 'X-Amz-SignedHeaders' else urllib.parse.quote(v, safe = '')}" for k,v in query.items()]
        return "&".join(data)
...
ignore_codes = [
    1628 # Sorry, you cannot pick this member..
]
...
async def _req(
    ...
    *,
    ...
    return_:    str                   = 'json',
) -> Req_json | str | bytes | ClientResponse:
    ...
    async with request(
        ...
        if res.status >= 400 and (j := await res.json(loads=loads))['api:statuscode'] and j['api:statuscode'] not in ignore_codes:
            print(j, ignore_codes, j not in  ignore_codes)
...
class _Reply:
    ...
    def __init__(self, ext):
        self.icon = ext['replyMessage']['author']['icon']
        self.id = ext['replyMessageId']
        self.nickname = ext['replyMessage']['author']['nickname']
        self.uid = ext['replyMessage']['author']['uid']
...
class Message:
    ...
    def from_ws(self, j: Req_json) -> Message:
        ...
        self.reply:           _Reply           = _Reply(self.extensions) if 'replyMessage' in self.extensions else None
...
class User:
    ...
    def __init__(self, j: Req_json | None = None):
        if j:
            self.bio:             str | None = get_value(j, 'content')
            self.blogs_count:     int | None = get_value(j, 'blogsCount')
            self.com:             str | None = get_value(j, 'ndcId', convert=str)
            self.comments_count:  int | None = get_value(j, 'commentsCount')
            self.created_time:    int | None = int(datetime.fromisoformat(j['createdTime'][:-1]).timestamp()) if j['createdTime'] else None
            self.followers_count: str | None = get_value(j, 'membersCount')
            self.following_count: str | None = get_value(j, 'joinedCount')
            self.im_following:    bool       = get_value(j, 'followingStatus') == 1
            self.level:           str | None = get_value(j, 'level')
            self.nickname:        str | None = get_value(j, 'nickname')
            self.posts_count:     int | None = get_value(j, 'postsCount')
            self.id:              str | None = get_value(j, 'uid')
            self.reputation:      int | None = get_value(j, 'reputation')
            self.role:            str | None = {0: 'member', 101: 'curator', 100: 'leader', 102: 'leader-agent'}[j['role']]
            self.visitors_count:  int | None = get_value(j, 'visitoresCount')
...
class Chat:
    ...
    def __init__(self, j=None):
        if j:
            t:                       Dict[str, Any] | None = get_value(j, 'thread') or j
            self.extensions:         Dict[str, Any] | None = get_value(t, 'extensions')

            self.announcement:       str | None = get_value(self.extensions, 'announcement') or ''
            self.bg:                 str | None = get_value(self.extensions, 'bm')[1] if 'bm' in self.extensions else None
            self.can_send_coins:     bool       = get_value(t, 'tipInfo', 'tippable')
            self.co_hosts:           list[str]  = get_value(self.extensions, 'coHost') or []
            self.created_time:       int | None = int(datetime.fromisoformat(t['createdTime'][:-1]).timestamp()) if t['createdTime'] else None
            self.adm:                str | None = get_value(t, 'author', 'uid')
            self.icon:               str | None = get_value(t, 'icon')
            self.id:                 str | None = get_value(t, 'threadId')
            self.is_pinned:          bool       = get_value(t, 'isPinned')
            self.is_private:         bool       = get_value(t, 'membersQuota') == 2
            self.members_can_invite: bool       = get_value(t, 'membersCanInvite') or True
            self.name:               str | None = get_value(t, 'title')
            self.only_fans:          bool       = get_value(self.extensions, 'fansOnly')
            self.only_view:          bool       = get_value(self.extensions, 'viewOnly') or False
            self.text:               str | None = get_value(t, 'content') or ''

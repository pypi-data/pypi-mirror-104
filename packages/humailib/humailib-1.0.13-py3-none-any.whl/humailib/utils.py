...
def check_table(df, table_type):
    
    for c in humai_table_field_types.get(table_type,[]):
        if c in df:
            print(f"'{c}' -> Exists")
        else:
            warnings.warn(
...
def keep_most_recent_transaction_id(df, inplace=True):
    if not inplace:
        return df.rename_axis('_index').sort_values(by=['Transaction_ID','Datetime','_index']).drop_duplicates(subset='Transaction_ID', keep='last')

    df.rename_axis('_index').sort_values(by=['Transaction_ID','Datetime','_index'], inplace=True)
...
def mann_whitney_test(dfA, dfB, column, group_A_name='grp_A', group_B_name='grp_B', scale = 1.0, unit='', verbose=False):
    ...
    try:
        ...
        df_out = pd.DataFrame(data, columns=[
            'driver',
            #f'sum_{group_A_name}',
            'mean_{}'.format(group_A_name),
            'median_{}'.format(group_A_name),
            'size_{}'.format(group_A_name),
            
            #f'sum_{group_B_name}',
            'mean_{}'.format(group_B_name),
            'median_{}'.format(group_B_name),
            'size_{}'.format(group_B_name),

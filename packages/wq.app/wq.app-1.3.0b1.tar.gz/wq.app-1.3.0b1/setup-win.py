...
WQJS_CSS = [
    "packages/app/css/wq.css",
    "packages/map-gl/node_modules/mapbox-gl/dist/mapbox-gl.css",
    "packages/map-gl/node_modules/@mapbox/mapbox-gl-draw/dist/mapbox-gl-draw.css",
]
...
class BuildJS(build_py):
    def run(self):
        if shutil.which('make'):
            ...
            print("Warning: 'make' not found; building wq.js only")
            os.makedirs("static/app/js", exist_ok=True)
            os.makedirs("static/app/css", exist_ok=True)
            for cmd in []:# WQJS_CMD:
                subprocess.check_call(cmd, shell=True)
            for filename in WQJS_CSS:
                shutil.copy(filename, "static/app/css")
        super().run()

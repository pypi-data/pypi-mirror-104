    ...
    _createClass(Outbox, [{
        ...
        value: function init(opts) {
            ...
            if (opts && opts.parseBatchResult) {
                throw new Error('parseBatchResult() no longer supported.  Use an ajax() plugin instead.');
            }

            if (opts && opts.validate) {
                throw new Error('config.outbox.validate() no longer supported.  Use a validate() plugin instead');
            }
        ...
        key: "_enqueue",
        value: function _enqueue(array, action) {
        ...
        key: "validate",
        value: function validate(data, modelConf) {
            var errors = {};
            this.app.callPlugins('validate', [data, modelConf]).forEach(function (pluginErrors) {
        ...
        value: function _peek(array) {
            ...
            if (this.batchService && pending.length && pending.length >= this.batchSizeMin) {
                var action = this._createBatchAction(pending.slice(0, this.batchSizeMax));
        ...
        key: "_dequeue",
        value: function _dequeue(array, action) {
            var _this5 = this;
            ...
            if (action.type === REMOVE_ITEMS) {
                ...
                this._processBatchResponse(action).forEach(function (nextAction) {
                    array = _this5._dequeue(array, nextAction);
                });
...
function mergeErrors(errors, newErrors) {
    if (!newErrors || _typeof(newErrors) !== 'object' || !Object.keys(newErrors).length) {
        return errors;
    }

    Object.entries(newErrors).forEach(function (_ref6) {
        ...
        if (!error || _typeof(error) !== _typeof(newError) || Array.isArray(error) !== Array.isArray(newError)) {
            errors[key] = newError;
        } else if (typeof error === 'string') {
            errors[key] = error + ' • ' + newError;
        } else if (Array.isArray(error)) {
            errors[key] = error.map(function (err, i) {
                return mergeErrors(err, newError[i]);

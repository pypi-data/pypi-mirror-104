...
class ModelManager:
    ...
    def __new__(cls):
        if ModelManager.__instance is None:
            cls.model_manager_logger = logging.getLogger("PIModelManager")
            cls.model_manager_logger.setLevel(logging.INFO)
            ...
            cls.model_manager_logger.addHandler(ch)
            
            ModelManager.__instance = object.__new__(cls)
            cls.model_manager_logger.info("__new__: instance is set")
        else:
            cls.model_manager_logger.info("__new__")
        ...
        if model["model_type"] == "spacy":
            ...
            if name and version:
                if cls.__pip_package_is_available(name, version):
                    cls.model_manager_logger.info(f"Package {name}-{version} is already installed.")
                else:
                    #model_manager_logger.info(f"Downloading package {name}-{version}")
                    package = name + "-" + version
                    url = f"https://github.com/explosion/spacy-models/releases/download/{package}/{package}.tar.gz"
                    cls.model_manager_logger.info(f"Downloading package {url}")
                    ...
                    cls.model_manager_logger.info(f"Return code: {output.returncode}")
                    if(output.returncode != 0):
                        raise Exception(f"Error when installing package {package}")
                    else:
                        cls.model_manager_logger.info(f"Successfully installed package: {package}")
    ...
    def __download_model_dataservices(cls, model: Dict[str, str]) -> None:
        ...
        if len(matched_models) == 0:
            ...
            if temp_path.exists():
                ...
                if pandora_model not in cls._downloaded_models:
                    cls._downloaded_models.append(pandora_model)
                    cls.model_manager_logger.info(f"Model already downloaded in {temp_path}")
            else:
                # Download the model and add to _downloaded_models
                cls.model_manager_logger.info(f"Downloading model in {temp_path}")
                ...
                if response.ok:
                    ...
                    if extension == ".zip":
                        ...
                        cls.model_manager_logger.info(f"Extracted model to: {pandora_model.path}")
                    else:
                        ...
                        cls.model_manager_logger.info(f"Saved model to: {pandora_model.path}")
    ...
    def upload_model(
        ...
        if path.exists():
            ...
            if response.ok:
                cls.model_manager_logger.info(f"Successfully uploaded file:\n {response.text}")
            else:
                cls.model_manager_logger.info(f"Error with uploading: {response.status_code}, {response.text}")
        else:
    ...
    def delete_models(cls) -> None:
        """Delete all downloaded and/or installed models."""
        
        for m in cls._downloaded_models:
            if not isinstance(m.path, Path):
                cls.model_manager_logger.info(f"Uninstalling {m.path}")
                subprocess.check_call([sys.executable, "-m", "pip", "uninstall", m.path, "-y"])
        ...
        if("allennlp" in model_types):
            if not cls.__pip_package_is_available("allennlp"):
                cls.model_manager_logger.warning("You have allennlp model, but allennlp package is missing. Not possible to delete cached allennlp models.")
            else:
                try:
                    from allennlp.common.file_utils import remove_cache_entries
                    cleaned_bytes = remove_cache_entries(patterns=[])
                    cls.model_manager_logger.info(f"Removed {cleaned_bytes} bytes of cached allennlp models.")
                except ImportError:
                    cls.model_manager_logger.warning("Could not import allennlp. Not possible to delete cached allennlp models.")
        
        models_path = Path(Path.cwd(), "models")
        if(models_path.exists()):
            cls.model_manager_logger.info(f"Deleting downloaded models")

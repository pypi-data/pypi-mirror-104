...
from lazy_budget.money_provider import DEFAULT_CURRENCY, Money, get_zero, money_mean, Currency, parse_money
...
class FinancialOperation:
    ...
    @classmethod
    def from_file(cls, filename: os.PathLike, default_currency: Currency = DEFAULT_CURRENCY) -> None:
        with open(filename, "r") as fp:
            reader = csv.DictReader(fp, fieldnames=cls.CSV_FIELD_NAMES)
            yield from map(
                partial(cls.from_row, default_currency=default_currency),
                reader,
            )
...
class Budget:
    ...
    def available_per_day(self) -> Money:
        return (self.total - self.keep) / self.total_days

    def get_spent_by_day(self, operations: List[FinancialOperation]) -> Money:
    ...
    def get_days_until_can_spend(self, can_spend_today: Money, days_left: int) -> int:
        if not days_left or can_spend_today >= self.zero:
            return 0

        return -int(round((can_spend_today / days_left).amount))
    
    def get_spent_today(self, operations: List[FinancialOperation]) -> Money:
        spent_by_day = self.get_spent_by_day(operations)
        return spent_by_day[self.today]
    ...
    def filter(self, operations: List[FinancialOperation]) -> List[FinancialOperation]:
        return list(filter(
            lambda op: op.dttm in self and op.currency == self.currency,
            operations,
        ))
...
class BudgetStats:
    ...
    @classmethod
    def get_stats(cls, budget: Budget, operations: List[FinancialOperation]) -> "BudgetStats":
        ...
        return cls(
            ...
            currently_keeping=budget.keep + can_spend_today,

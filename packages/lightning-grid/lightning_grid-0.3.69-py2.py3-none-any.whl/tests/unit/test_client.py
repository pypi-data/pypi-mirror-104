...
def monkey_patch_observables():
    ...
    class MonkeyPatchedObservable:
        ...
        def get(self, *args, **kwarrgs):
            return {self.key: [{'columnn': 'value'}, {'column_fail_a': []}, {'column_fail_b': {}}]}
...
class TestGridClient:
    ...
    def setup_class(cls):
        ...
        def monkey_patch_client(self, *args, **kwargs):
    ...
    def remove_env(self):
        ...
        envs_to_remove = ['GRID_CREDENTIAL_PATH', 'GRID_USER_ID', 'GRID_API_KEY']
    ...
    def test_client_local_path(self):
        ...
        G = grid.Grid(credential_path=self.creds_path, load_local_credentials=False)
    ...
    def test_train_raises_exception_github_not_accessible(self, monkeypatch):
        ...
        assert "Please grant access on the settings page" in ex_info.value.args[0]
    ...
    def test_check_user_github_token_raises_exception(monkeypatch):
        ...
        with pytest.raises(click.exceptions.ClickException):
            monkeypatch.setattr(resolvers, 'check_user_github_token', monkey_patch_token)
            G._check_user_github_token()

        with pytest.raises(AuthenticationError):
            monkeypatch.setattr(GridAIBackenedTestServer, 'execute', monkey_patch_execute)
    ...
    @staticmethod
    def test_login_handles_invalid_credentials(monkeypatch):
        """Grid.login() handles HTTPError exception."""
        G = grid.Grid(load_local_credentials=False)
        G._init_client()
        monkeypatch.setattr(
    ...
    def test_validate_datastore_version_exception(monkeypatch):
        ...
        config = {"compute": {"train": {"datastore_name": "mnist", "datastore_version": None}}}
        with pytest.raises(click.ClickException) as ex_info:
            monkeypatch.setattr(GridAIBackenedTestServer, 'execute', monkey_path_get_datastores_exc)
    ...
    def test_validate_datastore_version(monkeypatch):
        ...
        monkeypatch.setattr(resolvers, 'get_datastores', monkey_path_get_datastores)
        config = {}
        G.validate_datastore_version(grid_config=config)
        assert config == {}
        ...
        config = {"compute": {"train": {"datastore_name": "mnist", "datastore_version": ""}}}
        G.validate_datastore_version(grid_config=config)
        assert config["compute"]["train"]["datastore_name"] == "mnist"
        assert config["compute"]["train"]["datastore_version"] == "2"

        config = {"compute": {"train": {"datastore_name": "mnist", "datastore_version": "1"}}}
        G.validate_datastore_version(grid_config=config)
        assert config["compute"]["train"]["datastore_name"] == "mnist"
        assert config["compute"]["train"]["datastore_version"] == "1"

        config = {"compute": {"train": {"datastore_name": "wrong-name", "datastore_version": None}}}
        with pytest.raises(click.ClickException) as ex_info:
            G.validate_datastore_version(grid_config=config)
        assert "unable to find a ready-to-use version" in ex_info.value.args[0]

        config = {"compute": {"train": {"datastore_name": "wrong-name", "datastore_version": "1"}}}
        G.validate_datastore_version(grid_config=config)
    ...
    def test_resuming_upload_datastore():
        ...
        session = DatastoreUploadSession(name="test_session", version=1, source_dir="data", credential_id="cc-abcdef")
        upload = MagicMock()
        session.upload = upload

        DatastoreUploadSession.recover_sessions = MagicMock(return_value=[session])
    ...
    def test_create_interactive_node(monkeypatch):
        ...
        result = G.create_interactive_node(name='test-interactive-id', config='', description="test-description")
        assert result

        def monkey_path_create(*args, **kwargs):
            return {'success': False, 'message': ''}

        monkeypatch.setattr(resolvers, 'create_interactive_node', monkey_path_create)
        env.DEBUG = True
        with pytest.raises(click.ClickException):
            G.create_interactive_node(name='test-interactive-id', config='', description='test-description')
    ...
    def test_create_interactive_node_handles_error():
        ...
        with pytest.raises(click.ClickException):
            G.create_interactive_node(name='test-interactive-id', config='', description="test-desc")
    ...
    def test_pause_interactive_node(monkeypatch):
        ...
        result = G.pause_interactive_node(interactive_node_name='test-interactive-id')
        assert result

        def monkey_path_pause(*args, **kwargs):
            return {'success': False, 'message': ''}

        monkeypatch.setattr(resolvers, 'pause_interactive_node', monkey_path_pause)
        env.DEBUG = True
        with pytest.raises(click.ClickException):
            G.pause_interactive_node(interactive_node_name='test-interactive-id')
    ...
    def test_resume_interactive_node(monkeypatch):
        ...
        result = G.resume_interactive_node(interactive_node_name='test-interactive-id')
        assert result

        def monkey_path_pause(*args, **kwargs):
            return {'success': False, 'message': ''}

        monkeypatch.setattr(resolvers, 'resume_interactive_node', monkey_path_pause)
        env.DEBUG = True
        with pytest.raises(click.ClickException):
            G.resume_interactive_node(interactive_node_name='test-interactive-id')
    ...
    def test_delete_interactive_node(monkeypatch):
        ...
        result = G.delete_interactive_node(interactive_node_name='test-interactive-id')
        assert result

        def monkey_path_delete(*args, **kwargs):
            return {'success': False, 'message': ''}

        monkeypatch.setattr(resolvers, 'delete_interactive_node', monkey_path_delete)
        env.DEBUG = True
        with pytest.raises(click.ClickException):
            G.delete_interactive_node(interactive_node_name='test-interactive-id')
    ...
    @staticmethod
    def test_get_experiment_id(monkeypatch):
        G = grid.Grid(load_local_credentials=False)
        G._init_client()

        result = G.get_experiment_id(experiment_name='some-name')
        ...
        with pytest.raises(click.ClickException) as ex_info:
            G.get_experiment_id(experiment_name='some-name')
    ...
    def test_delete():
        ...
        result = G.delete(experiment_name='test-experiment-name')
        assert result

        result = G.delete(run_name='test-run-name')
        assert result
    ...
    def test_delete_handles_errors(monkeypatch):
        ...
        with pytest.raises(click.ClickException):
            G.delete(experiment_name='test-experiment-name')
        ...
        with pytest.raises(click.ClickException):
            G.delete(experiment_name='test-run-name')

        G.client = MonkeyPatchClient()
        with pytest.raises(click.ClickException):
            G.delete(run_name='test-run-name')
    ...
    def test_cancel():
        ...
        result = G.cancel(run_name='test-run-name')
        assert result is True

        result = G.cancel(experiment_name='test-experiment-name')
        assert result is True
    ...
    def test_cancel_handles_errors(monkeypatch):
        ...
        monkeypatch.setattr(G, 'experiment_details', monkey_patch_experiment_details)
        with pytest.raises(click.ClickException):
    ...
    def test_history(self):
        ...
        observable_fact.assert_called_once_with(client=G.client, identifier="foo")
    ...
    def test_experiment_logs_queued(self):
        ...
        G.experiment_details.assert_called_once()
        G.get_experiment_id.assert_called_once()
    ...
    def test_experiment_logs_succeeded(self):
        ...
        G.get_experiment_id = MagicMock(return_value='some-id')
        G.experiment_details = MagicMock(return_value={'getExperimentDetails': {'status': 'succeeded'}})
    ...
    def test_experiment_metrics_succeeded(self):
        ...
        G.client.execute = MagicMock(
            return_value={'getExperimentScalarLogs': {
                'steps': "[0, 1]",
                'values': "[0.1, 0.2]",
                'nextIndex': 10
            }})
    ...
    def test_check_is_blocked(self, monkeypatch):
        G = grid.Grid(credential_path=self.creds_path, load_local_credentials=False)
        G.client = MagicMock(Client, autospec=True)

        # test is blocked does not raise an exception
        G.check_is_blocked()

        # check user isn't verified
        def get_user_not_verified(*args, **kwargs):
            return {"getUser": {"isVerified": False}}

        monkeypatch.setattr(G, "query", get_user_not_verified)
        with pytest.raises(click.UsageError):
            G.check_is_blocked()

        # check user hasn't completed signup
        def get_user_not_completed_signup(*args, **kwargs):
            return {"getUser": {"completedSignup": False}}

        monkeypatch.setattr(G, "query", get_user_not_completed_signup)
        with pytest.raises(click.UsageError):
            G.check_is_blocked()

        # check user is blocked
        def get_user_is_blocked(*args, **kwargs):
            return {"getUser": {"isBlocked": True}}

        monkeypatch.setattr(G, "query", get_user_is_blocked)
        with pytest.raises(click.UsageError):
            G.check_is_blocked()
    ...
    def test_check_version_compatibility(self, capsys, monkeypatch):
        ...
        httpretty.register_uri(httpretty.GET, urljoin(url, 'metadata'), body=json.dumps({'version': __version__}))
        G._check_version_compatibility()

        captured = capsys.readouterr()
        assert 'pip install' not in captured.out

        httpretty.register_uri(httpretty.GET, urljoin(url, 'metadata'), body=json.dumps({'version': 'v100.1.1'}))
    ...
    def test_gen_ssh_config():
        assert grid.Grid._gen_ssh_config([], "", start_marker="#START", end_marker="#END") == '\n'.join([
            "#START",
            "#END",
        ])

        assert grid.Grid._gen_ssh_config([], "happy little config", start_marker="#START",
                                         end_marker="#END") == '\n'.join([
                                             "happy little config",
                                             "#START",
                                             "#END",
        ...
        with pytest.raises(ValueError):
            grid.Grid._gen_ssh_config([], '\n'.join(["#START"]), start_marker="#START", end_marker="#END")

        with pytest.raises(ValueError):
            grid.Grid._gen_ssh_config([], '\n'.join(["#END"]), start_marker="#START", end_marker="#END")

...
"""
        ...
        if grid_version > client_version:
            if grid_version.major > client_version.major:
                click.echo(message)
                raise click.ClickException('Incompatible versions: {grid_version} (server) and '
    ...
    def _init_client(self, websocket: bool = False) -> None:
        ...
        if websocket:
            ...
            self.transport = RequestsHTTPTransport(url=self.url,
                                                   use_json=True,
    ...
    def train(self, config: str, kind: WorkflowType, run_name: str, run_description: str, entrypoint: str,
              script_args: List[str], invocation_command: str) -> None:
        """
    ...
    def create_interactive_node(self, config: str, name: str, description: str) -> bool:
        """
        ...
        #  Base64-encode the config object either passed
        #  or constructed.
        config_encoded = base64.b64encode(config.encode('ascii')).decode('ascii')
        ...
        if success:
            ...
            raise click.ClickException(f"Failed to create interactive node '{name}'")
    ...
    def resume_datastore_session(self, session_name: str):
        ...
        if not session:
            ...
            raise click.ClickException(f"No session named {session_name} found, please specify an " +
                                       "available session")

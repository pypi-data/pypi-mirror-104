...
def multiclass(parent):
    def __init__(self, parent):
        self.parent = parent

    def bar(self, key_list, target_line, score_list):
        multiclass_bar(self.parent, key_list, target_line, score_list)

    def scatter(self, key_list, target_line, explanatory_line_list):
        multiclass_scatter(self.parent, key_list, target_line, explanatory_line_list)

    def matrix(self, key_list):
        multiclass_matrix(self.parent, key_list)

    def rank(self, key_list, target_line ,score_type = "abs", comvert="default", show_range="top50"):
        multiclass_rank(self.parent, key_list, target_line, score_type, comvert, show_range)

    def radarchart(C_mod, key_list, target_line):
        multiclass_radarchart(C_mod, key_list, target_line)
...
def multiclass_radarchart(C_mod, key_list, target_line):
    ...
    if c_type == "ba":
        ...
        for cc in range(len(score_out[0])):
            score_out[3][cc] = math.log10(score_out[3][cc])
            score_out[4][cc] = math.log10(score_out[4][cc])

        max_list = []
        for so in score_out: max_list.append(max(so))

        for cc in range(len(score_out[0])):
            score_out[0][cc] /= max_list[0]
            score_out[1][cc] /= max_list[1]
            score_out[2][cc] /= max_list[2]
            score_out[3][cc] = (max_list[3] - score_out[3][cc]) / max_list[3]
            score_out[4][cc] = (max_list[4] - score_out[4][cc]) / max_list[4]
        ...
        for cc in range(len(score_out[0])):
            score_out[2][cc] = math.log10(score_out[2][cc])
            score_out[3][cc] = math.log10(score_out[3][cc])
        ...
        for cc in range(len(score_out[0])):
            score_out[0][cc] /= max_list[0]
            score_out[1][cc] /= max_list[1]
            score_out[2][cc] = (max_list[2] - score_out[2][cc]) / max_list[2]
            score_out[3][cc] = (max_list[3] - score_out[3][cc]) / max_list[3]
        ...
        for cc in range(len(score_out[0])):
            score_out[0][cc] = math.log10(score_out[0][cc])
            score_out[1][cc] = math.log10(score_out[1][cc])
            score_out[2][cc] = math.log10(score_out[2][cc])
            score_out[3][cc] = math.log10(score_out[3][cc])
        ...
        for cc in range(len(score_out[0])):
            score_out[0][cc] = (max_list[0] - score_out[0][cc]) / max_list[0]
            score_out[1][cc] = (max_list[1] - score_out[1][cc]) / max_list[1]
            score_out[2][cc] = (max_list[2] - score_out[2][cc]) / max_list[2]
            score_out[3][cc] = (max_list[3] - score_out[3][cc]) / max_list[3]

    val_list = list(zip(*score_out))

    ax = fig.add_subplot(1, len_key_l + 1, 1)
    for count in range(len_key_l):
        vals = val_list[count]
    ...
    for count in range(len_key_l):
        vals = val_list[count]
        ax = fig.add_subplot(1, len_key_l+1, count + 2)
        angles = np.linspace(0, 2 * np.pi, len(e_title) + 1, endpoint=True)
        values = np.concatenate((vals, [vals[0]]))  # 閉じた多角形にする
        ax.plot(angles, values, 'o-', color=color_list[count])  # 外枠

...
class Cannaibase:
    ...
    def __init__(self, classtype, dirct=None, save_model=True, binary_class = False):
        ...
        if classtype == "r" or classtype == "regression":
            ...
            cl2 = "b"
        elif classtype == "c" or classtype == "classification":
            cl2 = "c"
        else:
            raise IndexError("classtype does not match")
        self.class_type = cl2
        self.multiclass = multiclass(self)
    ...
    def output_to_df(self, output):
        if self.answer is not None:
            if self.out_is_S:
                if self.class_type == "b": output = self.prob_01_2l_to_1l(output)
...
def multi_Cmodel(Cannaibase):
    def __init__(self, type, dirct=None, save_model=True, binary_class=False):
        super().__init__(dirct, type,save_model, binary_class)
        self.multiinput = True

    def save_input(self):
        count = 0
        for si in self.input:
            self.save_df(si, "input" + str(count))
            count += 1

    def add_input(self, in2):
        if type(in2) == list:
            self.input = self.input + in2
        elif self.is_df_or_series(in2):
            self.input.append(in2)
        else:
            raise IndexError("input must be \'list of df(series)\' or df(series)")
        self.input_size = self.input.size

    def set_input(self, inp):
        if type(inp) == list:
            self.input = inp
        elif self.is_df_or_series(inp):
            self.input = [inp]
        else:
            raise IndexError("input must be \'list of df(series)\' or df(series)")
        self.input_size = self.input.size

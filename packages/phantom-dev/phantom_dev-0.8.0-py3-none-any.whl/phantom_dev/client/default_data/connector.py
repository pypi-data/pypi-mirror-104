...
class Connector:
    ...
    @ActionHandler(action_type='generic', data_contains={'slept': int})
    async def dummy_async(self, concurrency: int, sleep: int):
        ...
        results = await asyncio.gather(
            *(asyncio.sleep(sleep, result=x) for x in range(concurrency)))

        for data in results:
            yield {'slept': data}

    @dummy_async.summary_contains({'max_slept': int, 'count': int})
    def summarise_async(self, results):
        return {
            'max_slept': max(results, key=lambda r: r['slept']),
            'count': len(results)
        }

...
from .callbacks import handle_incorrect_unit
...
def run(dicom: t.Dict, out_dir: Path) -> t.List[t.Dict[str, t.List[str]]]:
    ...
    with setup_callbacks():
        if zipfile.is_zipfile(str(dicom_path)):
            dcms = DICOMCollection.from_zip(dicom_path, track=True, decode=True)
        else:
            dcms = DICOMCollection(dicom_path, track=True, decode=True)
        for dcm in dcms:
            dcm.tracker.trim()
    ...
    if all([len(ev) > 0 for ev in events]) or added_uid:

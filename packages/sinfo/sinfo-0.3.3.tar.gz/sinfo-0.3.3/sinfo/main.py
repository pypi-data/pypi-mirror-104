...
def _find_version(mod_version_attr):
    ...
    if (isinstance(mod_version_attr, str)
        ...
        # https://gitlab.com/joelostblom/sinfo/-/issues/8. In these cases it
...
def sinfo(na=True, os=True, cpu=True, jupyter=None, dependencies=None,
    ...
    write_req_file: bool
        ...
        `notebook` module is installed, sinfo can automatically find the
        notebook name use it as a prefix for the requirments file. Otherwise
        it fallse back to `sinfo-requirements.txt`.
    ...
    '''
    print('''The `sinfo` package has changed name and is now called `session_info` to become more discoverable and self-explanatory. The `sinfo` PyPI package will be kept around to avoid breaking old installs and you can downgrade to 0.3.2 if you want to use it without seeing this message. For the latest features and bug fixes, please install `session_info` instead. The usage and defaults also changed slightly, so please review the latest README at https://gitlab.com/joelostblom/sinfo.''')

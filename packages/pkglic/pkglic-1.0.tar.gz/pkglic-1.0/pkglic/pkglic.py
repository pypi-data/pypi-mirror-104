...
class PackageInfo:
    ...
    def __str__(self):
        ...
        return f"[{self.type}] {name:30} {self.license} {self.licenseurl}"
...
def scan_nuget_deps(filename: str):
    """
    Create information for all packages found in filename.
...
PACKAGETYPES = {
    ...
    "nu": {"namefragment": "packages.config", "func": scan_nuget_deps},
}
...
def init_argparse() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help="Which fields to use to sort output; 0 - type, name, 1: license, name, 2: type, license, 3: group by license.")
    parser.add_argument(
        "-t", "--type",
        type=str,
...
def print_package_info(packages: list, unwanted_licenses: list, sortkey) -> bool:
    ...
    if unwanted:
        ...
        for package in sorted(unwanted, key=SORTORDER[0]):

...
class AFMLayer(Layer):
    ...
    def build(self, input_shape): # creating layer weights: 레이어를 초기화
        ...
        self.projection_p = self.add_weight(shape=(embedding_size, 1), 
                                            initializer=glorot_normal(seed=self.seed), name="projection_p")
    ...
    def call(self, inputs, training=None, **kwargs): # 입력을 처리해서 결과를 반환
        ...
        bi_interaction = inner_product # 모양은 (# of batch, # of combination, embedding_size)
        attention_temp = tf.nn.relu(tf.nn.bias_add(tf.tensordot( # 최종 모양: (# of batch, # of combination, attention_factor)
            bi_interaction, self.attention_W, axes=(-1, 0)), self.attention_b)) # bi_interaction의 -1번째 축은 embedding_size, attention_W의 0번째 축은 embedding_size
        #  Dense(self.attention_factor,'relu',kernel_regularizer=l2(self.l2_reg_w))(bi_interaction)
        self.normalized_att_score = softmax(tf.tensordot(    # 최종 shape: (# of batch, # of combination, 1)
            attention_temp, self.projection_h, axes=(-1, 0)), dim=1)   # attention_temp의 -1번째 축은 attention_factor, projection_h의 0번째 축은 attention_factor
        attention_output = reduce_sum(    # 최종 shape: (# of batch, embedding_size)
            self.normalized_att_score * bi_interaction, axis=1)   # (# of batch, # of combination, 1) * (# of batch, # of combination, embedding_size) -> (# of batch, # of combination, embedding_size)을 열에 대해 reduce_sum -> (# of combination,1)

        attention_output = self.dropout(attention_output, training=training)  # training

        afm_out = self.tensordot([attention_output, self.projection_p]) # 최종 모양: (# of batch, 1): (# of batch, embedding_size)과 (embedding_size, 1) 의 텐서닷
        return afm_out

    def compute_output_shape(self, input_shape): # 입력 모양이 주어졌을때 출력 모양을 알려주는 역할
...
class FEGCNNLayer(Layer):
    """Feature Generation Layer used in FGCNN,including Convolution,MaxPooling and Recombination.
      ...
      Output shape
        - 3D tensor with shape: ``(batch_size,extracted_feture_num,embedding_size)``.

      References
        - working
    """

    #def __init__(self, filters=(14, 16,), kernel_width=(7, 7,), pooling_width=(2, 2), 
    def __init__(self, filters=(14, 16,), kernel_width=((7, 1), (7, 1)), pooling_width=(2, 2), new_maps=(3, 3,),
                 **kwargs):
        if not (len(filters) == len(kernel_width) == len(pooling_width) == len(new_maps)):
            raise ValueError("length of argument must be equal")
        self.filters = filters
        self.kernel_width = kernel_width
        self.pooling_width = pooling_width
        self.new_maps = new_maps
        #super(FECNNLayer, self).__init__(**kwargs) # for Python2
        super().__init__(**kwargs) # for Python3
    ...
    def build(self, input_shape):
        ...
        for i in range(1, len(self.filters) + 1): 
            filters = self.filters[i - 1]
            width = self.kernel_width[i - 1]
            p_width = self.pooling_width[i - 1]
            new_filters = self.new_maps[i - 1]
            conv_output_shape = self._conv_output_shape(pooling_shape, width)
            pooling_shape = self._pooling_output_shape(conv_output_shape, (p_width, 1)) 
            #self.conv_layers.append(tf.keras.layers.Conv2D(filters=filters, kernel_size=(width, 1), strides=(1, 1),
            self.conv_layers.append(tf.keras.layers.Conv2D(filters=filters, kernel_size=width, strides=(1, 1),
                                                           padding='same',
                                                           activation='relu', use_bias=True, ))
            self.pooling_layers.append(tf.keras.layers.MaxPooling2D(pool_size=(p_width, 1)))
            self.dense_layers.append(tf.keras.layers.Dense(pooling_shape[1] * embedding_size * new_filters,
                                                           activation='relu', use_bias=True))
        self.flatten = tf.keras.layers.Flatten()    
        #super(FECNNLayer, self).build(input_shape)  # Be sure to call this somewhere!
        super().build(input_shape)  # For Python3
    ...
    def call(self, inputs, **kwargs):
        ...
        for i in range(1, len(self.filters) + 1):
            ...
            conv_result = self.conv_layers[i - 1](pooling_result)
            pooling_result = self.pooling_layers[i - 1](conv_result)
            ext_feature_list.append(
                tf.reshape(pooling_result, (-1, pooling_result.shape[1]*pooling_result.shape[-1], embedding_size))) #pooling_result.shape[1]: 풀링 결과 행 갯수, pooling_result.shape[-1]: conv 필터 갯수 
            flatten_result = self.flatten(pooling_result)
            recomb_result = self.dense_layers[i - 1](flatten_result)
            ext_feature_list.append(
                tf.reshape(recomb_result, (-1, int(pooling_result.shape[1])*new_filters, embedding_size)))

        ext_features = concat_func(ext_feature_list, axis=1)
        return ext_features
    ...
    def compute_output_shape(self, input_shape):
        ...
        for i in range(0, len(self.kernel_width)):
            pooled_features_num = features_num // self.pooling_width[i]
            ext_features_num += self.filters[i] * pooled_features_num
            features_num = pooled_features_num

        return (None, ext_features_num, input_shape[-1])

    def get_config(self, ):
        config = {'kernel_width': self.kernel_width, 'filters': self.filters, 'pooling_width': self.pooling_width}
        base_config = super(FECNNLayer, self).get_config()
        return dict(list(base_config.items()) + list(config.items()))
...
class FECNNLayer(Layer):
    ...
    #def __init__(self, filters=(14, 16,), kernel_width=(7, 7,), pooling_width=(2, 2), 
    def __init__(self, filters=(14, 16,), kernel_width=((7, 1), (7, 1)), pooling_width=(2, 2),
                 **kwargs):
        if not (len(filters) == len(kernel_width) == len(pooling_width)):
            raise ValueError("length of argument must be equal")
        self.filters = filters
        self.kernel_width = kernel_width
        self.pooling_width = pooling_width
...
class FECNNwBNLayer(Layer):
    ...
    def build(self, input_shape):
        ...
        for i in range(1, len(self.filters) + 1): 
            ...
            self.bn_layer.append(tf.keras.layers.BatchNormalization())
            self.activation_layer.append(tf.keras.layers.Activation('relu'))
            self.pooling_layers.append(tf.keras.layers.MaxPooling2D(pool_size=(p_width, 1)))
    ...
    def call(self, inputs, **kwargs):
        ...
        for i in range(1, len(self.filters) + 1):
            conv_result = self.conv_layers[i - 1](pooling_result)
            bn_result = self.bn_layer[i - 1](conv_result)
            activation_result = self. activation_layer[i - 1](bn_result)
            pooling_result = self.pooling_layers[i - 1](activation_result)
            ext_feature_list.append(
                tf.reshape(pooling_result, (-1, pooling_result.shape[1]*pooling_result.shape[-1], embedding_size))) #pooling_result.shape[1]: 풀링 결과 행 갯수, pooling_result.shape[-1]: conv 필터 갯수 
...
class ANLayer(Layer):
    """Attentonal Network models without linear term and bias.

      Input shape
        - A 3D tensor with shape: ``(batch_size, num_feature_interaction, embedding_size)``.
    ...
    def __init__(self, attention_factor=4, l2_reg_w=0, dropout_rate=0, seed=1024, **kwargs):
        ...
        self.dropout_rate = dropout_rate
        self.seed = seed
        #super(ANLayer, self).__init__(**kwargs)
        super().__init__(**kwargs) #for Python3
    ...
    def build(self, input_shape): # creating layer weights: 레이어를 초기화
        ...
        #                                   regularizer=l2(self.l2_reg_w), name="attention_W")
        self.attention_W = self.add_weight(shape=(embedding_size,
                                                  self.attention_factor), initializer=he_normal(seed=self.seed),
                                           regularizer=l2(self.l2_reg_w), name="attention_W")
        self.attention_b = self.add_weight(
            shape=(self.attention_factor,), initializer=Zeros(), name="attention_b")
        #self.projection_h = self.add_weight(shape=(self.attention_factor, 1),
        #                                    initializer=glorot_normal(seed=self.seed), name="projection_h")
        self.projection_h = self.add_weight(shape=(self.attention_factor, 1),
                                            initializer=he_normal(seed=self.seed), name="projection_h")
        #self.projection_p = self.add_weight(shape=(embedding_size, 1), 
        #                                    initializer=glorot_normal(seed=self.seed), name="projection_p")
        self.projection_p = self.add_weight(shape=(embedding_size, 1), 
                                            initializer=he_normal(seed=self.seed), name="projection_p")
        self.dropout = tf.keras.layers.Dropout(
            self.dropout_rate, seed=self.seed)
        ...
        # Be sure to call this somewhere!
        #super(ANLayer, self).build(input_shape)
    ...
    def call(self, inputs, training=None, **kwargs): # 입력을 처리해서 결과를 반환
        ...
        feat_interaction = inputs # 모양은 (# of batch, # of FI, embedding_size)

        attention_temp = tf.nn.relu(tf.nn.bias_add(tf.tensordot( # 최종 모양: (# of batch, # of FI, attention_factor)
            feat_interaction, self.attention_W, axes=(-1, 0)), self.attention_b)) # feat_interaction의 -1번째 축은 embedding_size, attention_W의 0번째 축은 embedding_size
        #  Dense(self.attention_factor,'relu',kernel_regularizer=l2(self.l2_reg_w))(bi_interaction)
        self.normalized_att_score = softmax(tf.tensordot(    # 최종 shape: (# of batch, # of FI, 1)
            attention_temp, self.projection_h, axes=(-1, 0)), dim=1)   # attention_temp의 -1번째 축은 attention_factor, projection_h의 0번째 축은 attention_factor
        attention_output = reduce_sum(    # 최종 shape: (# of batch, embedding_size)   
            self.normalized_att_score * feat_interaction, axis=1)   # (# of batch, # of FI, 1) * (# of batch, # of FI, embedding_size) -> (# of batch, # of FI, embedding_size)을 열에 대해 reduce_sum -> (# of combination,1)
        attention_output = self.dropout(attention_output, training=training)  # training
        att_out = self.tensordot([attention_output, self.projection_p]) # 최종 모양: (# of batch, 1): (# of batch, embedding_size)과 (embedding_size, 1) 의 텐서닷
        return att_out

    def compute_output_shape(self, input_shape): # 입력 모양이 주어졌을때 출력 모양을 알려주는 역할

        if not isinstance(input_shape, list):
            raise ValueError('A `ANLayer` layer should be called '
                             'on a list of inputs.')
        return (None, 1)
    ...
    def get_config(self, ):
        config = {'attention_factor': self.attention_factor,
                  'l2_reg_w': self.l2_reg_w, 'dropout_rate': self.dropout_rate, 'seed': self.seed}
        base_config = super(ANLayer, self).get_config()
        return dict(list(base_config.items()) + list(config.items()))
    
class NPCNNLayer(Layer):
    """CNN with No Pooling.
    ...
    def __init__(self, filters=(14, 16,), kernel_width=(7, 7,), new_maps=(3, 3,), recomb_apply=(1,1,), pooling_width=(2, 2), 
                 **kwargs):
        if not (len(filters) == len(kernel_width) == len(new_maps) == len(pooling_width)):
            raise ValueError("length of argument must be equal")
        self.filters = filters
        self.kernel_width = kernel_width
        self.new_maps = new_maps
        self.recomb_apply = recomb_apply
        self.pooling_width = pooling_width

        super(NPCNNLayer, self).__init__(**kwargs)
    ...
    def build(self, input_shape):
        ...
        for i in range(1, len(self.filters) + 1):
            ...
            width = self.kernel_width[i - 1]
            new_filters = self.new_maps[i - 1]
            recomb = self.recomb_apply[i - 1]
            pooling_width = self.pooling_width[i - 1]
            conv_output_shape = self._conv_output_shape(
                pooling_shape, (width, width))
            pooling_shape = self._pooling_output_shape(
                conv_output_shape, (pooling_width, 1))
            self.conv_layers.append(tf.keras.layers.Conv2D(filters=filters, kernel_size=(width, width), strides=(1, 1),
                                                           padding='same',
                                                           activation='tanh', use_bias=True, ))
            self.pooling_layers.append(tf.keras.layers.MaxPooling2D(pool_size=(pooling_width, 1)))
            if new_filters > 0 and recomb > 0:
                self.dense_layers.append(tf.keras.layers.Dense(pooling_shape[1] * embedding_size * new_filters,
                                                               activation='tanh', use_bias=True))

        self.flatten = tf.keras.layers.Flatten()

        super(NPCNNLayer, self).build(
            input_shape)  # Be sure to call this somewhere!
        ...
        for i in range(1, len(self.filters) + 1):
            ...
            if new_filters > 0:
                if recomb > 0:
                    ...
                    new_result = self.dense_layers[i - 1](flatten_result) # output dim: pooling_shape[1] * new_filters * embedding_size 
                
                    new_feature_list.append(
                        tf.reshape(new_result, (-1, int(pooling_result.shape[1]) * new_filters, embedding_size)))
                else:
                    new_feature_list.append(
                        tf.reshape(pooling_result, (-1, int(pooling_result.shape[1]) * new_filters, embedding_size)))
    ...
    def get_config(self, ):
        config = {'kernel_width': self.kernel_width, 'filters': self.filters, 'new_maps': self.new_maps,
                  'pooling_width': self.pooling_width}
        base_config = super(NPCNNLayer, self).get_config()
        return dict(list(base_config.items()) + list(config.items()))

...
# mxos url is subset of hg. mxos doesn't support ssh transport
regex_mxos_url = r'^(https?)://([\w\-\.]*mxos\.(co\.uk|org|com))/(users|teams)/([\w\-]{1,32})/(repos|code)/([\w\-]+)/?$'
# mxos sdk builds url
...
mxos_sdk_tools_url = 'https://mxos.org/users/mxos_official/code/mxos-sdk-tools'
...
eclispe_project_dir = 'mxos/makefiles/eclipse_project'
...
def warning(msg):
    for line in msg.splitlines():
        sys.stderr.write("[mxos] WARNING: %s\n" % line)
    sys.stderr.write("---\n")

def error(msg, code=-1):
    for line in msg.splitlines():
        sys.stderr.write("[mxos] ERROR: %s\n" % line)
    sys.stderr.write("---\n")
    sys.exit(code)
    ...
    def fetch_rev(url, rev):
        ...
        try:
            if not os.path.exists(rev_file):
                ...
                inurl = urlopen(url)
                outfd.write(inurl.read().decode('utf-8'))
                outfd.close()
        except:
...
class Hg(object):
    ...
    def isurl(url):
        ...
        if m_url and not re.match(regex_build_url, m_url.group(1)):
            return re.match(regex_hg_url, m_url.group(1)) or re.match(regex_mxos_url, m_url.group(1))
        else:
            return False
        '''
        return None
...
class Git(object):
    ...
    def isurl(url):
        m_url = re.match(regex_url_ref, url.strip().replace('\\', '/'))
        if m_url and not re.match(regex_build_url, m_url.group(1)) and not re.match(regex_mxos_url, m_url.group(1)):
...
class Repo(object):
    ...
    def fromurl(cls, url, path=None):
        ...
        if cache_repositories and cache_cfg and cache_cfg != 'none' and cache_cfg != 'off' and cache_cfg != 'disabled':
            ...
            repo.cache = loc or os.path.join(tempfile.gettempdir(), 'mxos-repo-cache')
    ...
    def fromlib(cls, lib=None):
        ...
        if not (m_local or m_bld_url or m_repo_url):
            warning(
                "File \"%s\" in \"%s\" uses a non-standard .comp or .codes file extension, which is not compatible with the mxos build tools.\n" % (os.path.basename(lib), os.path.split(lib)[0]))
            return False
        else:
    ...
    def fromcode(cls, code=None):
        ...
        if not (m_local or m_bld_url or m_repo_url):
            warning(
                "File \"%s\" in \"%s\" uses a non-standard .lib file extension, which is not compatible with the mxos build tools.\n" % (os.path.basename(code), os.path.split(code)[0]))
            return False
        else:
    ...
    def fromrepo(cls, path=None):
        ...
        if path is None:
            ...
            if path is None:
                error(
                    "Could not find mxos program in current path \"%s\".\n"
                    "You can fix this by calling \"mxos new .\" or \"mxos config root .\" in the root of your program." % os.getcwd())
        ...
        if repo.scm is None:
            warning(
                ...
                "To fix this you should use \"mxos new .\" in the root of your program." % (repo.name, repo.path))
    ...
    def clone(self, url, path, rev=None, depth=None, protocol=None, **kwargs):
        ...
        sorted_scms = filter(lambda m: m[0], sorted_scms)
    ...
    def check_repo(self, show_warning=None):
        ...
        if not os.path.isdir(self.path):
            err = (
                ...
                "You can use \"mxos deploy\" to import the missing libraries.\n"
                "You can also use \"mxos sync\" to synchronize and remove all invalid library references." % (os.path.basename(self.lib), self.path))
        elif not self.isrepo(self.path):
            err = (
                "Library reference \"%s\" points to a folder \"%s\", which is not a valid repository.\n"
                "You can remove the conflicting folder manually and use \"mxos deploy\" to import the missing libraries\n"
                "You can also remove library reference \"%s\" and use \"mxos sync\" again." % (os.path.basename(self.lib), self.path, self.lib))
...
class Program(object):
    ...
    def __init__(self, path=None, print_warning=False):
        ...
        self.is_classic = os.path.isfile(os.path.join(self.path, 'mxos.bld'))

        # is_cwd flag indicates that current dir is assumed to be root, not root repo
        if self.is_cwd and print_warning:
            warning(
                "Could not find mxos program in current path \"%s\".\n"
                "You can fix this by calling \"mxos new .\" in the root of your program." % self.path)
    ...
    # Gets MXOS OS dir (unified)
    def get_os_dir(self):
        if os.path.isdir(os.path.join(self.path, 'mxos')):
            return os.path.join(self.path, 'mxos')
        elif self.name == 'mxos':
            return self.path
        else:
            return None

    def get_mxoslib_dir(self):
        if os.path.isdir(os.path.join(self.path, 'mxos')):
            return os.path.join(self.path, 'mxos')
        else:
            return None

    # Gets mxos tools dir (unified)
    def get_tools_dir(self):
        paths = []
        # mxos dir identified and tools is a sub dir
        mxos_os_path = self.get_os_dir()
        if mxos_os_path:
            paths.append([mxos_os_path, 'tools'])
            paths.append([mxos_os_path, 'core', 'tools'])
        # mxos not identified but tools found under cwd/tools
        paths.append([self.path, 'tools'])
        paths.append([self.path, 'core', 'tools'])
        # mxos Classic deployed tools
    ...
    def check_requirements(self, show_warning=False):
        ...
        if missing:
            err = (
                "-----------------------------------------------------------------\n"
                "The mxos OS tools in this program require the following Python modules: %s\n"
    ...
    # Routines after cloning mxos
    def post_action(self):
        mxos_tools_path = self.get_tools_dir()

        if not mxos_tools_path and (self.is_classic or os.path.exists(os.path.join(self.path, Cfg.file))):
            self.add_tools(os.path.join(self.path, '.temp'))
            mxos_tools_path = self.get_tools_dir()

        if not mxos_tools_path:
            warning("Cannot find the mxos tools directory in \"%s\"" % self.path)
            return False

        if (not os.path.isfile(os.path.join(self.path, 'mxos_settings.py')) and
                os.path.isfile(os.path.join(mxos_tools_path, 'default_settings.py'))):
            shutil.copy(os.path.join(mxos_tools_path, 'default_settings.py'), os.path.join(self.path, 'mxos_settings.py'))
    ...
    def add_tools(self, path):
        ...
        with cd(path):
            ...
            if not os.path.exists(tools_dir):
                try:
                    action("Couldn't find build tools in your program. Downloading the mxos 2.0 SDK tools...")
                    repo = Repo.fromurl(mxos_sdk_tools_url)
                    repo.clone(mxos_sdk_tools_url, tools_dir)
                except Exception:
                    if os.path.exists(tools_dir):
                        rmtree_readonly(tools_dir)
                    error("An error occurred while cloning the mxos SDK tools from \"%s\"" % mxos_sdk_tools_url)
    ...
    def update_tools(self, path):
        ...
        if os.path.exists(os.path.join(path, tools_dir)):
            with cd(os.path.join(path, tools_dir)):
                try:
                    action("Updating the mxos 2.0 SDK tools...")
                    repo = Repo.fromrepo()
                    repo.update()
                except Exception:
                    error("An error occurred while update the mxos SDK tools from \"%s\"" % mxos_sdk_tools_url)

    def get_tools(self):
        mxos_tools_path = self.get_tools_dir()
        if not mxos_tools_path:
            error('The mxos tools were not found in "%s". \nRun `mxos deploy` to install dependencies and tools. ' % self.path, -1)
        return mxos_tools_path
    ...
    def get_env(self):
        ...
        for c in compilers:
            if self.get_cfg(c+'_PATH'):
                env['MXOS_'+c+'_PATH'] = self.get_cfg(c+'_PATH')
    ...
    def get_target(self, target=None):
        ...
        if target and (target.lower() == 'detect' or target.lower() == 'auto'):
            ...
            if targets == False:
                error("The target detection requires that the 'mxos-ls' python module is installed.\nYou can install mxos-ls by running 'pip install mxos-ls'.")
            elif len(targets) > 1:
        ...
        if target is None:
            error("Please specify target using the -m switch or set default target using command 'mxos target'", 1)
        return target
    ...
    def get_toolchain(self, toolchain=None):
        ...
        if tchain is None:
            error("Please specify toolchain using the -t switch or set default toolchain using command 'mxos toolchain'", 1)
        return tchain
    ...
    def ignore_build_dir(self):
        ...
        if not os.path.exists(build_path):
            os.mkdir(build_path)
        if not os.path.exists(os.path.join(build_path, '.mxosignore')):
            try:
                with open(os.path.join(build_path, '.mxosignore'), 'w') as f:
                    f.write('*\n')
            except IOError:
                error("Unable to write build ignore file in \"%s\"" % os.path.join(build_path, '.mxosignore'), 1)
    ...
    def get_detected_targets(self):
        ...
        try:
            ...
            mxoss = mxos_lstools.create()
            detect_muts_list = mxoss.list_mxoss()
            if os.name == 'nt':
...
class Global(object):
    def __init__(self):
        self.path = os.path.join(os.path.expanduser("~"), '.mxos')
...
def get_mxosder_dir():
    ...
    builtin_mxosder = os.path.join(Program().path,'mxos/MiCoder')
    mxosder_dir = Program().get_cfg('MICODER') or Global().get_cfg('MICODER') or \
    (builtin_mxosder if os.path.isdir(builtin_mxosder) else None)
    if not mxosder_dir:
        error('Can not find MiCoder!')
    return mxosder_dir

def _run_make(arg_list):
    mxosder_dir = get_mxosder_dir()

    # Decide which make to use according to the platform system 
    win_make = os.path.join(mxosder_dir,'cmd/Win32/make.exe')
    mac_make = os.path.join(mxosder_dir,'cmd/OSX/make')
    linux_make = os.path.join(mxosder_dir,'cmd/Linux64/make')
    ...
    make_cmd_str = ' '.join([make_cmd, '-e', 'HOST_OS='+host_os, 'TOOLS_ROOT='+mxosder_dir] + list(arg_list))
...
@subcommand('new',
    ...
    dict(name='--program', action='store_true', help='Force creation of an mxos program. Default: auto.'),
    dict(name='--component', action='store_true', help='Force creation of an mxos component. Default: auto.'),
    dict(name='--mirror', nargs='?', help='Mirrored mxos repository url'),
    dict(name='--create-only', action='store_true', help='Only create a program, do not import mxos or mxos component.'),
    dict(name='--depth', nargs='?', help='Number of revisions to fetch the mxos OS repository when creating new program. Default: all revisions.'),
    dict(name='--protocol', nargs='?', help='Transport protocol when fetching the mxos OS repository when creating new program. Supported: https, http, ssh, git. Default: inferred from URL.'),
    help='Create new mxos program or component',
    description=(
        "Creates a new mxos program if executed within a non-program location.\n"
        "Alternatively creates an mxos component if executed within an existing program.\n"
        "When creating new program, the latest mxos release will be downloaded/added\n unless --create-only is specified.\n"
...
def new(name, scm='git', program=False, component=False, mirror=None, create_only=False, depth=None, protocol=None):
    ...
    if d_type == 'program':
        ...
        p.path = cwd_root
        p.set_root()
        if not create_only and not p.get_os_dir() and not p.get_mxoslib_dir():
            url = mirror if mirror else mxos_os_url
            d = 'mxos'
            try:
                with cd(d_path):
    ...
    if d_type == 'program':
        ...
        target_file_list = ('template/main.c', 'template/mxos_config.h', 'README.md', 'template/template.mk')
        temp_dir = os.path.join(d_path, 'mxos/template/program')
        os.mkdir(os.path.join(d_path, target_name))
    else:
        target_dir = d_path
        target_name = name
        target_file_list = ('template.c', 'template.h', 'README.md', 'template.mk', 'test/README.md', 'test/main.c', 'test/mxos_config.h', 'test/test.mk')
        temp_dir = os.path.join(p_path, 'mxos/template/component')
...
@subcommand('import',
    ...
    description=(
        "Imports mxos program and its dependencies from a source control based URL\n"
        "(GitHub, Bitbucket, mxos.org) into the current directory or specified\npath.\n"
        "Use 'mxos add <URL>' to add a component into an existing program."))
def import_(url, path=None, mirror=None, ignore=False, depth=None, protocol=None, top=True):
    ...
    if top:
        ...
        if p and not p.is_cwd:
            error("Cannot import program in the specified location \"%s\" because it's already part of a program \"%s\".\n"
                  "Please change your working directory to a different location or use \"mxos add\" to import the URL as a component." % (os.path.abspath(repo.path), p.name), 1)
    ...
    if repo.name == 'mxos' and mxos_os_mirror:
        repo.url = repo.url.replace(mxos_os_url, mxos_os_mirror)
    ...
    if os.path.isdir(repo.path):
        ...
        if repo.name == 'mxos':
            ...
            temp_dir = os.path.join(Program().path, 'mxos/makefiles/eclipse_project', eclipse_subdir)
            for file in target_file_list:
                with open(os.path.join(temp_dir, file), 'r') as f:
                    content = f.read()
...
@subcommand('add',
    ...
    description=(
        "Adds mxos component and its dependencies from a source control based URL\n"
...
@subcommand('remove',
    ...
    description=(
        ...
        "You can re-add the component from it's URL via 'mxos add <component URL>'."))
def remove(path):
    repo = Repo.fromrepo()
...
@subcommand('deploy',
    ...
    description=(
        ...
        "Use 'mxos import <URL>' and 'mxos add <URL>' instead of cloning manually and\n"
        "then running 'mxos deploy'"))
...
def publish(all_refs=None, msg=None, top=True):
    ...
    if repo.is_local:
        error(
            ...
            "Read more about publishing local repositories here:\nhttps://code.aliyun.com/mxos/mxos-cube/#publishing-local-program-or-component" % ("Program" if top else "Library", repo.name, repo.path, repo.scm.name), 1)
...
def update(rev=None, clean=False, clean_files=False, clean_deps=False, ignore=False, top=True, depth=None, protocol=None):
    ...
    if top and not rev and repo.isdetached():
        error(
            ...
            "You can checkout a branch using \"%s checkout <branch_name>\" command before running \"mxos update\"." % (cwd_type, repo.scm.name), 1)
...
# Synch command
@subcommand('sync',
    help='Synchronize mxos component references',
    description=(
...
'''
# Compile command which invokes the mxos OS native build system
@subcommand('compile',
    ...
    dict(name=['--profile'], action='append', help='Path of a build profile configuration file. Example: mxos/tools/profiles/debug.json'),
    ...
    dict(name='--app-config', dest="app_config", help="Path of an app configuration file (Default is to look for 'mxos_app.json')"),
    help='Compile code using the mxos build tools',
    description=("Compile this program using the mxos build tools."))
...
def test_(toolchain=None, target=None, compile_list=False, run_list=False, compile_only=False, run_only=False, tests_by_name=None, source=False, profile=False, build=False, clean=False, test_spec=None, app_config=None):
    ...
    with cd(program.path):
        ...
        if run_list:
            popen(['mxosgt', '--test-spec', test_spec, '--list']
                  + (['-n', tests_by_name] if tests_by_name else [])
                  + (['-V'] if verbose else [])
                  + args,
                  env=env)
        ...
        if run_only or build_and_run_tests:
            popen(['mxosgt', '--test-spec', test_spec]
                  + (['-n', tests_by_name] if tests_by_name else [])
                  + (['-V'] if verbose else [])
                  + args,
                  env=env)
...
def export(ide=None, target=None, source=False, clean=False, supported=False):
    ...
    # Remember the original path. this is needed for compiling only the libraries and tests for the current folder.
    orig_path = os.getcwd()
    # Change directories to the program root to use mxos OS tools
    with cd(program.path):
        tools_dir = program.get_tools()
...
# Test command
@subcommand('detect',
    help='Detect connected mxos targets/boards\n\n',
    description=(
        "Detects mxos targets/boards connected to this system and shows supported\n"
        "toolchain matrix."))
def detect():
    # Gather remaining arguments
    args = remainder
    ...
    if tools_dir:
        ...
        warning("The mxos tools were not found in \"%s\". \nLimited information will be shown about connected mxos targets/boards" % program.path)
...
# Make command
@subcommand('make',
    help='Make mxos program/component',
    description=(
        "Make mxos program/component."))
def make():
   # Get the make arguments
    make_args = ' '.join(sys.argv[2:])
    _run_make(['-f mxos/makefiles/Makefile',make_args])
...
def makelib(path, arch=None, new=False):
    ...
    if new:
        ...
        with open(os.path.join(root_dir,'mxos/template/makefiles/mxos_library_makefile/template.mk'), 'r') as f:
            content = f.read()
    ...
    if arch:
        _run_make(['LIB_DIR='+path, 'HOST_ARCH='+arch, '-f mxos/makefiles/mxos_library_makefile.mk', make_args])
        return
...
@subcommand('config',
    dict(name='var', nargs='?', help='Variable name. E.g. "mxosder", "protocol"'),
    ...
    dict(name=['-L', '--list'], dest='list_config', action='store_true', help='List mxos tool configuration.'),
    help='Tool configuration\n\n',
    description=(
        "Gets, sets or unsets mxos tool configuration options.\n"
        ...
        "Currently supported options: mxosder, protocol, depth, cache"))
def config_(var=None, value=None, global_cfg=False, unset=False, list_config=False):
    ...
    if list_config:
        ...
        if not p.is_cwd:
            ...
            log("Couldn't find valid mxos program in %s\n" % p.path)
        ...
        if global_cfg:
            ...
            if program.is_cwd and not var == 'ROOT':
                error(
                    "Could not find mxos program in current path \"%s\".\n"
                    "Change the current directory to a valid mxos program, set the current directory as an mxos program with 'mxos config root .', or use the '--global' option to set global configuration." % program.path)
            with cd(program.path):
                if unset:
...
@subcommand('target',
    ...
    description=(
        ...
        "This is an alias to 'mxos config [--global] target [name]'\n"))
...
@subcommand('toolchain',
    ...
    description=(
        "Set or get default toolchain\n"
        "This is an alias to 'mxos config [--global] toolchain [name]'\n"))

...
config = Config(app.pyproject_path, defaults=defaults)


def read_or_default(key: str, default: str) -> str:
    try:
        return read_toml.read_from_file(env.r(app.pyproject_file), key)
    except KeyError:
        return default


database_image = env.str_from_config('DB_IMAGE', config=config)
database_name = env.str_from_config('DB_NAME', config=config)
database_user = env.str_from_config('DB_USER', config=config)
database_password = env.str_from_config('DB_PASSWORD', config=config)
database_port = env.int_from_config('DB_PORT', config=config)
database_host = env.str_from_config('DB_HOST', config=config)


@env.add
def database_container(e: env.Env) -> str:
    info = app.get_app_info()
    return f'{info.name}-db'
...
def start(c: Context, container_name: str = env.r(database_container)):
    ...
    container_port = env.r(database_port)

...
class PyrightOutput(BaseModel):
    ...
    general_diagnostics: Sequence[PyrightDiagnostic] = Field(..., alias='generalDiagnostics')
...
def types(c: Context, show_information: bool = False):
    ...
    res = cast(
        Result,
        c.run(f'poetry run pyright {directories} --outputjson > {tmp_handler.name}', echo=True, hide=True, warn=True),
    )
    ...
    skipped_statuses: Set[str] = set()
    if not show_information:
        skipped_statuses.add('information')

    for d in (d for d in results.general_diagnostics if d.severity not in skipped_statuses):  # noqa: WPS335
        if d.severity == 'error':
            color = 'red'
...
def isort(c: Context):
    ...
    directories = env.r(check_targets)
    if is_ci():
        c.run(f'poetry run isort {directories} --check-only')
    else:
        c.run(f'poetry run isort {directories}')

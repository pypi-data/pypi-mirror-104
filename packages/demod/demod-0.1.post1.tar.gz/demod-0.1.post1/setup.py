...
def get_all_subfiles(path):
    """return all the files in the requested path"""
    out = []
    for f in os.listdir(path):
        if os.path.isdir(path_f := os.path.join(path, f)):
            out += get_all_subfiles(path_f)
        else:
            out.append(path_f)
    return out


def get_datasets_packages_dir():
    """Returns the data dir for each dataset package with parsed data."""
    # List all whta is in datasets
    all_files = os.listdir('demod' + os.sep + 'datasets')
    package_files = {}
    # Checks all the folders among that
    for f in all_files:
        if os.path.isdir(parsed_folder := os.path.join(
            'demod', 'datasets', f, 'parsed_data')
            ):
            package_name = 'demod.datasets.' + f
            package_files[package_name] = parsed_folder

    return package_files

def get_datasets_packages_files():
    """Returns the data files for each dataset package with parsed data."""
    # List all whta is in datasets
    all_files = os.listdir('demod' + os.sep + 'datasets')
    package_files = {}
    files = []
    ...
    for f in all_files:
        if os.path.isdir(parsed_folder := os.path.join(
            ...
            package_name = 'demod.datasets.' + f
            package_files[package_name] = get_all_subfiles(parsed_folder)
            # Removes demod
            [files.append(i[6:]) for i in get_all_subfiles(parsed_folder)]
...
setup(
    ...
    package_data={'': get_datasets_packages_files()},
    install_requires=[
        "numpy>=1.19",
        "scipy>=1.4",
        "pandas>=1.2",

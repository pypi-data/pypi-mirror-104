...
class PostForm(FlaskForm):
    ...
    adtitle = StringField('Ad Title', [InputRequired(), Length(min=8, max=64, message='Title must be between %(min)d and %(max)d characters')], default="test ad title")
    description = TextAreaField('Description', [InputRequired(), Length(min=10, message='Description must be %(min)d or more characters')], default="test ad description")
    ...
    price = DecimalField('Price', [RequiredIfFieldEqualTo('pricetype', pricetype_choices[0][0]), NumberRange(min=1), Optional()], default=9999)
    ...
    postalcode = StringField('Postal Code', [InputRequired(), PostalCode()], default="A1A1A1")

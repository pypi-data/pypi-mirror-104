...
def histogramdd(
    ...
    Args:
        ...
        indices: Whether an histogram is returned or only the bin indices of `x`.
    ...
    Returns:
        The histogram or the indices, (*bins,) or (*, D).
    """
    ...
    **kwargs,
    ...
    for x in seq:
        ...
        if hist is None:
            ...
            if kwargs.get('indices', False):
                hist = [hist]
        else:
            if type(hist) is list:
                hist.append(temp.to(hist[0]))
            else:

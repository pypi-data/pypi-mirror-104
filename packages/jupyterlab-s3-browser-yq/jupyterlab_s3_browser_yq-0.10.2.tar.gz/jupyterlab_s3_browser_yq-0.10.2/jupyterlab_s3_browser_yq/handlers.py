...
def prepend_path(path, str_to_prepend):
    if not str(path).startswith(f'{str_to_prepend}/') and str(str_to_prepend):
        path = f'{str_to_prepend}/{path}'
        return path
    return path


def do_yq_redirection(bucket, path):
    yq_minio_env = os.environ["YQ_STORAGE_VIEWER_CONFIG"]
    try:
        yq_minio_storage_config = json.loads(yq_minio_env)
    except:
        # If configuration is wrong, default to common path
        return path
    if bucket == yq_minio_storage_config["project"]["bucket"]:
        path = prepend_path(path, str_to_prepend=yq_minio_storage_config["project"]["redirect_to"])
        return path
    elif bucket == yq_minio_storage_config["private"]["bucket"]:
        path = prepend_path(path, str_to_prepend=yq_minio_storage_config["private"]["redirect_to"])
        return path
    elif bucket == yq_minio_storage_config["shared"]["bucket"]:
        path = prepend_path(path, str_to_prepend=yq_minio_storage_config["shared"]["redirect_to"])
        return path
    else:
...
def get_s3_objects_from_path(s3, path):
    ...
    if path in ["", "/"]:
        ...
        if path == "" or path.endswith("/"):
            path = do_yq_redirection(bucket_name, path)

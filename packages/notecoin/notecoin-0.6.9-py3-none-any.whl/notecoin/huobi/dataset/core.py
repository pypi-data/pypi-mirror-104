...
class AutoSaveDetail(SqliteTable):
    def __init__(self, db_path=None, save_freq='d', ms=False, save_key=None, save_with_clear=True, *args, **kwargs):
        if db_path is None:
        ...
        self.save_time_unix = datetime.datetime.now()
    ...
    def auto_save(self):
        ...
        if self.save_freq in ('d', 'D', 'Day', 'day'):
            if self.save_time_unix.day != time_unix.day and time_unix.hour >= 1:
                self._save(now=self.save_time_unix, pop=self.save_with_clear)
                self.save_time_unix = time_unix
        elif self.save_freq in ('m', 'M', 'mon', 'Mon', 'month', 'Month'):
            if self.save_time_unix.month != time_unix.month and time_unix.hour >= 1:
                self._save(now=self.save_time_unix, pop=self.save_with_clear)
                self.save_time_unix = time_unix
        elif self.save_freq in ('m', 'M', 'mon', 'Mon', 'month', 'Month'):
            if self.save_time_unix.year != time_unix.year and time_unix.hour >= 1:
                self._save(now=self.save_time_unix, pop=self.save_with_clear)
                self.save_time_unix = time_unix

    def _save(self, pop=None, now=None):
        now = now or datetime.datetime.now()
        path_root = os.path.join(os.path.dirname(self.db_path), self.table_name)
        ...
        path = '{}/{}-{}.csv'.format(path_root, name, self.save_time_unix.strftime(fmt))
...
class TradeDetail(AutoSaveDetail):
    def __init__(self, table_name='trade_detail', db_path=None, *args, **kwargs):
        ...
        super(TradeDetail, self).__init__(db_path=db_path, ms=True,
                                          save_key='ts', table_name=table_name, *args, **kwargs)
...
class Kline60MinDetail(KlineDetail):
    def __init__(self, table_name='kline_60min', db_path=None, *args, **kwargs):
        super(Kline60MinDetail, self).__init__(table_name=table_name, db_path=db_path,
                                               save_freq='m', *args, **kwargs)

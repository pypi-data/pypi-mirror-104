...
class TradeUpdate:
    ...
    def insert_symbol(self):
        self.symbolDB.insert_list(self.replace_key(self.generic.get_exchange_symbols().dict_data['data']))
    ...
    def insert_kline(self, symbol):
        self.kline_1min.insert_list(self.replace_key(self.market.get_candlestick(
            symbol=symbol, period='1min', size=2000).dict_data['data'], symbol))
        self.kline_5min.insert_list(self.replace_key(self.market.get_candlestick(
            symbol=symbol, period='5min', size=2000).dict_data['data'], symbol))
        self.kline_15min.insert_list(self.replace_key(self.market.get_candlestick(
            symbol=symbol, period='15min', size=2000).dict_data['data'], symbol))
        self.kline_30min.insert_list(self.replace_key(self.market.get_candlestick(
            symbol=symbol, period='30min', size=2000).dict_data['data'], symbol))
        self.kline_60min.insert_list(self.replace_key(self.market.get_candlestick(
            symbol=symbol, period='60min', size=2000).dict_data['data'], symbol))
        self.kline_4hour.insert_list(self.replace_key(self.market.get_candlestick(
            symbol=symbol, period='4hour', size=2000).dict_data['data'], symbol))
        self.kline_1day.insert_list(self.replace_key(self.market.get_candlestick(
            symbol=symbol, period='1day', size=2000).dict_data['data'], symbol))
        self.kline_1mon.insert_list(self.replace_key(self.market.get_candlestick(
            symbol=symbol, period='1mon', size=2000).dict_data['data'], symbol))
        self.kline_1week.insert_list(self.replace_key(self.market.get_candlestick(
            symbol=symbol, period='1week', size=2000).dict_data['data'], symbol))
        self.kline_1year.insert_list(self.replace_key(self.market.get_candlestick(
            symbol=symbol, period='1year', size=2000).dict_data['data'], symbol))
    ...
    def _save(self):
        self.tradeDB._save(pop=False)
        self.kline_1min._save(pop=False)
        self.kline_5min._save(pop=False)
        self.kline_15min._save(pop=False)
        self.kline_30min._save(pop=False)
        self.kline_4hour._save(pop=False)
        self.kline_1day._save(pop=False)
        self.kline_1mon._save(pop=False)
        self.kline_1week._save(pop=False)
        self.kline_1year._save(pop=False)
    ...
    def run(self):
        ...
        while True:
            ...
            if t % 100 == 0:
                self.insert_data(trade=True, kline=True)
                self.auto_save()
            else:
                self.insert_data(trade=True, kline=False)
                time.sleep(60)
            t += 1
...
# ps -elf|grep coin
# nohup /root/anaconda3/bin/python3.8 /root/workspace/notechats/notecoin/notecoin/huobi/dataset/update.py >>/root/workspace/tmp/notecoin-run-$(date +%Y-%m-%d).log 2>&1 &
# 2509

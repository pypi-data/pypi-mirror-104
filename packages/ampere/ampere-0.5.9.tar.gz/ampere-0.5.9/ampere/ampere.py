...
class PseudoTwoDimFD(BaseBattery):
    ...
    def __init__(self, initial_parameters: Dict[str, float] = None, estimate_parameters: List[str] = None, verbose=False):
        ...
        self.internal_structure['solid_lithium_concentration'].update({f'positive_{n}': [n + len(self.charge_ICs) + 1] for n in range(n_positive_region)})
        count = len(self.charge_ICs) + n_positive_region + 1
        ...
        self.charge_ICs += [0.9780166774057617 for i in range(n_positive_region)]  # set the surface to initially lower concentration to help with initialization

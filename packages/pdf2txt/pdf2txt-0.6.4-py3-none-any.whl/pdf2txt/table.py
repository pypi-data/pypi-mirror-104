...
#        if len(utils.get_partially_overlapping_objects(utils.get_BoundingBox_from_objects([t for line in self.textlines for t in line]), h)) >= max(1, len(self.textlines)-3):
    ...
    def extract_from_textlines(self, rows, token_lines):
        self.top=rows[0][0].top if len(rows[0]) else -1
        self.bottom=rows[-1][0].bottom if len(rows[0]) else -1
        self.left=min([r[0].left for r in rows if len(r)])
        self.right=max([r[-1].right for r in rows if len(r)])
        if len(rows)>2:
            self.interline=(self.bottom-self.top)/len(rows)
        ...
        if not hasattr(self, 'ncols'):
            self.ncols = Counter([len(line) for line in token_lines]).most_common(1)[0][0]
        ...
        self._data = []
        if len(sorted_cols)>self.ncols+1:
            for line in sorted_rows:
                table_row=sorted(line, key=lambda x: (x.top, x.left))
                table_row=[[t] for t in table_row]
                self._data.append([r for r in table_row])
            return self
    ...
    def extract_from_lines(self):
        ...
        self.ncols = Counter([len(line) for line in self.textlines]).most_common(1)[0][0]
        if self.ncols==1:
            return None
        ...
        for i, (b, t) in enumerate(z):
            if i == 0:
                if self._edges_h[0].top < self.textlines[0][0].top:
                    ...
                    rows.extend(self._fix_header_rows(header, self.ncols))
                    rows.extend(self.horizontally_within_lines(b, t, self.ncols))
            else:

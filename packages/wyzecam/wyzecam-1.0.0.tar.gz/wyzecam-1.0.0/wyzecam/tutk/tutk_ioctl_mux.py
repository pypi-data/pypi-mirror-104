...
class TutkIOCtrlFuture(object):
    ...
    def __repr__(self):
        ...
        data_str = (
            f" resp_data={repr(self.resp_data)}" if self.resp_data else ""
        )
        return f"<TutkIOCtlFuture req={self.req}{errcode_str}{data_str}>"


class TutkIOCtrlMux(object):
    """
    ...
    def start_listening(self) -> None:
        ...
        See: [wyzecam.tutk.tutk_ioctl_mux.TutkCtrlMux.stop_listening][]
        """
        self.listener.start()
...
class TutkIOCtrlMuxListener(threading.Thread):
    def __init__(
        ...
        super(TutkIOCtrlMuxListener, self).__init__()
        self.tutk_platform_lib = tutk_platform_lib
        self.av_chan_id = av_chan_id
        self.queues = queues

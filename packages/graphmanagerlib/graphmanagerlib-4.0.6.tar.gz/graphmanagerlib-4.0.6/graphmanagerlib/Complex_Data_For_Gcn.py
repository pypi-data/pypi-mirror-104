...
def makeIndividualData(file,tags):
    ...
    feature_cols = ['rd_b', 'rd_r', 'rd_t', 'rd_l', 'line_number', 'n_upper', 'n_alpha', 'n_spaces', 'n_numeric', 'n_special']
    ...
    for val in ['right_text', 'left_text', 'top_text', 'bottom_text']:
        text_features = np.array(df[val].map(make_sent_bert_features).tolist()).astype(np.float32)

...
class SMWinservice(win32serviceutil.ServiceFramework):
    ...
    def write_to_db(self,status = 'start', options=['FINISH','SUCCESS'],retry=0):
        ...
        if db_conn:
            try:
                ...
                print(temp_daily)
                if status == 'start':
                    cursor.execute(self.start_query.format(table=self._svc_name_,description=self._svc_description_,looptime=str(self._looptime_),daily=temp_daily))
                    print(self.start_query.format(table=self._svc_name_,description=self._svc_description_,looptime=str(self._looptime_),daily=temp_daily))
                if status == 'finish':
                    cursor.execute(self.finish_query.format(description=self._svc_description_,looptime=str(self._looptime_),daily=temp_daily,table=self._svc_name_,status=options[0],status_type='FINISH',status_description=options[1]))
                if status == 'error':
                    cursor.execute(self.finish_query.format(description=self._svc_description_,looptime=str(self._looptime_),daily=temp_daily,table=self._svc_name_,status=options[0],status_type='ERROR',status_description=options[1]))
    ...
    def run_csharp_by_exe_path(full_exe_path):
        """
        ...
        if not os.exists(full_exe_path):
        ...
        with subprocess.Popen([full_exe_path, "--environment=Production"], stdout=subprocess.PIPE, stderr=subprocess.PIPE, universal_newlines=True) as process:
            ...
            a = "Finished" if return_code == 0 else "Failed"
            return a + " with return code: " + str(return_code) + " finalstring: " + finalstring

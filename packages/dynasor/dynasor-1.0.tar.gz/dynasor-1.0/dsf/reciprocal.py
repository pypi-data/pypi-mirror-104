...
for t in 'ds':
    ...
    ndp_c_2d_rw[t] = np_ndp(dtype=np_c[t], ndim=2,
                            flags='f_contiguous, aligned, writeable')
    _lib[t] = cdll.LoadLibrary(
        join(dirname(__file__),
             '_rho_j_k_{}{}'.format(t, get_config_var('EXT_SUFFIX'))))
        ...
        def fun(frame):
            ...
            if 'vs' in frame:
                rho_ks, j_ks = zip(*[calc_rho_j_k(x, v, self.k_points,
                                                  ftype=self.ftype)
                                     for x, v in zip(frame['xs'],
                                                     frame['vs'])])
                jz_ks = [np.sum(j * self.k_direct, axis=0) for j in j_ks]
                frame['j_ks'] = j_ks
                frame['jz_ks'] = jz_ks
                frame['jper_ks'] = [j - (jz * self.k_direct)
                                    for j, jz in zip(j_ks, jz_ks)]
                frame['rho_ks'] = rho_ks
            else:
                frame['rho_ks'] = [calc_rho_k(x, self.k_points,
                                              ftype=self.ftype)
                                   for x in frame['xs']]
            return frame
        return fun
...
class reciprocal_isotropic(reciprocal_processor):
    ...
    def __init__(self, box, max_points=10000, max_k=10.0, ftype='d'):
        ...
        N_k1, N_k2, N_k3 = [int(np.ceil(max_q / dq)) for dq in q_mins]
        k_points = \
        ...
        k_points = k_points.reshape((3, int(k_points.size / 3)))

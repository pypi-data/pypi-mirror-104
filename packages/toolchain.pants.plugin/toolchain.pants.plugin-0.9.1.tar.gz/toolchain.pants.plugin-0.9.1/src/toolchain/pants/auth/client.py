    ...
    def acquire_access_token(self, complete_env: Mapping[str, str]) -> AuthToken:
        ...
        if refresh_token:
            ...
            if self._should_disable(complete_env):
                raise AuthError("Restricted token expression didn't match, disabling Toolchain auth.")
...
def _load_from_env(complete_env: Mapping[str, str], env_var_name: str) -> Optional[AuthToken]:
    ...
    _logger.info(f"Tried loading token from env var '{env_var_name}'. success={bool(token_str)}")
    if not token_str:
        return None

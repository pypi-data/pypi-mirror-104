...
def cowboe(**kwargs):
    ...
    def Kcalc(windows, A, B, V, kgiven):
        ...
        plt.savefig('cowboe_window_potential_%.2f_%.2f.%s' % (A, B, cowboe_settings['fig extension']), bbox_inches = 'tight')
        plt.show()
        plt.close()
    ...
    plt.ylabel(r'$\Delta$ PMF')
    plt.xlabel(cowboe_settings['reaction coordinate unit'])
    plt.savefig('cowboe_converged_%.2f_%.2f.%s' % (A,B,cowboe_settings['fig extension']), bbox_inches='tight', dpi=300)
    plt.show()
    plt.close()

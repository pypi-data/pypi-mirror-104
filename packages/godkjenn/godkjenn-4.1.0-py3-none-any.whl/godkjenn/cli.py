...
def review(vault, config):
    ...
    default_command_template = config.get("review_tool")
    command_templates = config.get("review_tools", {})

    if (not command_templates) and (default_command_template is None):
        log.error(
            "No review tools configured. Set the godkjenn.review_tool and/or godkjenn.review_tools keys in your config.toml."
        )
        return ExitCode.CONFIG

    godkjenn.commands.review(vault, command_templates, default_command_template)
    return ExitCode.OK

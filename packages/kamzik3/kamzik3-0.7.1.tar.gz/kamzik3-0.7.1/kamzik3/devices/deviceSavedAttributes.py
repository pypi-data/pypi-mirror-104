...
class DeviceSavedAttributes(DeviceFileSync):
    ...
    def _init_attributes(self):
        ...
        # We set min_stream_timeout to zero to make sure all messages are broadcast
        attribute.min_stream_timeout = 0
    ...
    def set_filepath(self, filepath):
        ...
        if not os.path.exists(filepath):
            try:
                ...
                hdf.to_hdf(filepath, "default", format="table", complevel=1)
    ...
    def insert_groups(self, groups_data):
        ...
        @param groups_data: dict {group_path_1: {Color, Time, Comment, Device [Attribute][Unit]}, group_path_n: {...}}
        ...
        for group, data in groups_data.items():
            ...
            self.set_value(ATTR_LAST_UPDATE, SEP.join((UPDATE_INSERT_GROUPS, json.dumps(groups_data))))
    ...
    @expose_method({"group": "Path to existing group", "data": "Row dict with attributes"})
    def insert_row(self, group, data, position=0):
        ...
        self.store_dataframe()
        self.set_value(ATTR_LAST_UPDATE, SEP.join((UPDATE_INSERT_ROW, group, json.dumps(data), str(position))))

    @expose_method({"group": "Path to existing group", "row": "Row index", "column": "Column index",
    ...
    def update_value(self, group, row, column, value):
        ...
        self.store_dataframe()
        self.set_value(ATTR_LAST_UPDATE, SEP.join((UPDATE_VALUE, group, str(row), str(column), str(value))))

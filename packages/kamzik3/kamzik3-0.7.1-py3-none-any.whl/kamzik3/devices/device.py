...
class Device(Subject, YamlSerializable):
    ...
    def _set(self, attribute_list, value, callback=None):
        ...
        if not attribute_object[ALLOW_STREAM]:
            propagate_change = False
        elif attribute_list[-1] == VALUE and attribute_object.with_set_function:
    ...
    def handle_readout(self, readout_buffer):
        ...
        try:
            ...
            if len(self.latency_buffer) == self.latency_buffer.maxlen:
                self.set_attribute((ATTR_LATENCY, VALUE), sum(self.latency_buffer) / self.latency_buffer.maxlen)
    ...
    def server_disconnect(self):
        """
        This function serves just as a placeholder for
        disconnect function, but visible to client.
        """
        self.disconnect()
    ...
    def server_reconnect(self):
        """
        This function serves just as a placeholder for
        reconnect function, but visible to client.
        """
        self.reconnect()

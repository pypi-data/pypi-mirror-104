from curvenote.models import ArticleVersionPost, BlockChild, BlockFormat, BlockKind, BlockPost, ContentVersionPost
import logging
import os
import sys
...
@app.command()
def get_me(token: str = typer.Argument(
...
@app.command()
def get_my_projects(token: str = typer.Argument(
        ..., envvar="CURVENOTE_TOKEN", help="API token generated from curvenote.com"
    )):
    session = Session(token)
...
def pull_as_latex(
    ...
    try:
        LatexProject.build_single_article(session, target, project, article, version)
    except ValueError as err:
        typer.echo(err)
...
def clone_content(
    ...
    for o in version.order:
        ...
        if child_block.kind != BlockKind.content:
            ...
            new_child_block = session.upload_block(
                BlockPost(kind=child_block.kind),
                project=project_obj)
    ...
    try:
        new_article_block = session.upload_block(
                    BlockPost(name=new_article_name,

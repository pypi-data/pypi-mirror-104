...
from alnlp.metrics import Metric
...
class MentionRecall(Metric):
    ...
    def __call__(
        ...
        for top_spans, clusters in zip(batched_top_spans.tolist(), batch["clusters"]):
            gold_mentions: Set[Tuple[int, int]] = set(mention for cluster in clusters for mention in cluster)

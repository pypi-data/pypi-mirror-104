...
class InitializerApplicator(object):
    ...
    def __init__(
            self, regexes: List[Tuple[str, Union[Initializer, Dict]]] = None, prevent_regexes: List[str] = None
    ) -> None:
        if regexes:
            regexes = [(x[0], Initializer.from_config(x[1])) if isinstance(x[1], dict) else x for x in regexes]

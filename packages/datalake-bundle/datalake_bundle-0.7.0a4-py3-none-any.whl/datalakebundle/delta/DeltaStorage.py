...
class DeltaStorage:
    ...
    def recreate_table(self, table_definition: TableDefinition, options: dict = None):
        self.__spark.sql(f"DROP TABLE {table_definition.full_table_name}")
        self.__filesystem.delete(table_definition.target_path, True)
        self.create_table(table_definition, options or {})

    def overwrite_data(self, df: DataFrame, full_table_name: str, partition_by: list, options: dict = None):
        options = options or {}

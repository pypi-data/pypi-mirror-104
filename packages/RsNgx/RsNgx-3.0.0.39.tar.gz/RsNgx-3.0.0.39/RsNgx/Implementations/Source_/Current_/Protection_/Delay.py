...
class Delay:
	...
	def set_initial(self, new_value_for_voltage: float) -> None:
		"""SCPI: [SOURce]:CURRent:PROTection:DELay:INITial \n
		Snippet: driver.source.current.protection.delay.set_initial(new_value_for_voltage = 1.0) \n
		Sets or queries the initial fuse delay time once output turns on. \n
	...
	def set_value(self, new_value_for_voltage: float) -> None:
		"""SCPI: [SOURce]:CURRent:PROTection:DELay \n
		Snippet: driver.source.current.protection.delay.set_value(new_value_for_voltage = 1.0) \n
		Sets or queries the fuse delay time. \n
			:param new_value_for_voltage: No help available
		"""
		param = Conversions.decimal_value_to_str(new_value_for_voltage)

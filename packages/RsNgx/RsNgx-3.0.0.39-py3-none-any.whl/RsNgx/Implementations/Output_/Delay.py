...
class Delay:
	...
	def get_duration(self) -> float:
		...
		Sets or queries the duration for output delay. \n
			:return: new_value_for_sequence_delay: No help available
		"""
		response = self._core.io.query_str_with_opc('OUTPut:DELay:DURation?')
		return Conversions.str_to_float(response)

	def set_duration(self, new_value_for_sequence_delay: float) -> None:
		"""SCPI: OUTPut:DELay:DURation \n
		Snippet: driver.output.delay.set_duration(new_value_for_sequence_delay = 1.0) \n
		Sets or queries the duration for output delay. \n
			:param new_value_for_sequence_delay: No help available
		"""
		param = Conversions.decimal_value_to_str(new_value_for_sequence_delay)

...
class TokenPrototype:
	...
	def __repr__(self):
		return f"<{type(self).__name__}: [{', '.join(self.namespace)}] {self.name} (r'{self.pattern}') " \
			   f"args: {self.arg_num}>"
...
class Adicity:
	...
	def __init__(self, name, default_namespace):
	...
	# def group(self, *parts, namespace=None, end=None, custom_name=False, add=True, args=None, re_flags=[re.M]):
	...
	# 			tk = self.get_token_pattern(part.__name__)
	# 			tks.append(tk)
	# 			pattern += tk.pattern
	# 		else:
	# 			pattern += part
	...
	def tokenize(self, code, start=0, lineno=0, linepos=0, already=[]):
		...
		while position < len(code):
			...
			for token_proto in filter(lambda proto: proto.name not in already, self):
				...
				namespacegood = (namespace in token_proto.namespace or '*' in token_proto.namespace)
		...
		while len(tokens) > 0:
			...
			try:
				...
				try:
					...
					raise errors.AdicityBalanceError(program)
			if token:
	...
	def add(self, language, *a):
		o = []
		for i in language:
			o.append(i)
		self.protos = self.protos + o

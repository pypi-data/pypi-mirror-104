import json
import setuptools

kwargs = json.loads(
    """
{
    "name": "cdk-quicksight-constructs",
    "version": "0.1.3",
    "description": "cdk-quicksight-constructs",
    "license": "Apache-2.0",
    "url": "https://github.com/mmuller88/cdk-quicksight-constructs",
    "long_description_content_type": "text/markdown",
    "author": "Martin Mueller<damadden88@googlemail.com>",
    "bdist_wheel": {
        "universal": true
    },
    "project_urls": {
        "Source": "https://github.com/mmuller88/cdk-quicksight-constructs"
    },
    "package_dir": {
        "": "src"
    },
    "packages": [
        "cdk_quicksight_constructs",
        "cdk_quicksight_constructs._jsii"
    ],
    "package_data": {
        "cdk_quicksight_constructs._jsii": [
            "cdk-quicksight-constructs@0.1.3.jsii.tgz"
        ],
        "cdk_quicksight_constructs": [
            "py.typed"
        ]
    },
    "python_requires": ">=3.6",
    "install_requires": [
        "aws-cdk.aws-iam==1.100.0",
        "aws-cdk.core==1.100.0",
        "aws-cdk.custom-resources==1.100.0",
        "constructs>=3.2.27, <4.0.0",
        "jsii>=1.29.0, <2.0.0",
        "publication>=0.0.3"
    ],
    "classifiers": [
        "Intended Audience :: Developers",
        "Operating System :: OS Independent",
        "Programming Language :: JavaScript",
        "Programming Language :: Python :: 3 :: Only",
        "Programming Language :: Python :: 3.6",
        "Programming Language :: Python :: 3.7",
        "Programming Language :: Python :: 3.8",
        "Programming Language :: Python :: 3.9",
        "Typing :: Typed",
        "Development Status :: 5 - Production/Stable",
        "License :: OSI Approved"
    ],
    "scripts": []
}
"""
)

with open("README.md", encoding="utf8") as fp:
    kwargs["long_description"] = fp.read()


setuptools.setup(**kwargs)

	...
	def tlvRead(self,disp,df = None):
		...
		while True:
			try:
				...
				return self.list2df(False,v6,v7,v8) if (df == 'DataFrame') else (False,v6,v7,v8)
			...
			if lstate == 'idle':
				...
				if (idx%dataBytes == 0):
					...
					try:
						...
						if (df == 'DataFrame'):
							v7df.append((self.hdr.frameNumber,'v6',posX,posY,posZ,velX,velY,velZ,accX,accY,accZ,tid))
						else: 

...
from tktl.core.utils import DelayedLoadingFrame
...
def test_delayed_loaded_frame(create_proj):
    ...
    assert isinstance(input_schema.value, DelayedLoadingFrame)
    assert isinstance(output_schema.value, DelayedLoadingFrame)
    assert output_schema.value.is_label is True
    assert input_schema.value.profile_columns == input_schema.value.columns

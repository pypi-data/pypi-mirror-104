...
class imagedata:
  ...
  def extract_face(self,file):
    '''
    this function use mtcnn to extract faces from image file or numpy array to face numpy array
    '''
    if type(file) == str:
      ...
      array = file
    face_detected = self.face_extractor.detect_faces(array)
    if face_detected ==[]:
      return []

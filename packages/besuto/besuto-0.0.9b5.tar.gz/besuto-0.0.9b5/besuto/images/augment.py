...
class imagedata:
  ...
  def load_faces_dataset(self,directory =''):
    ...
    for folder in sorted(os.listdir(directory)):
      ...
      for filename in os.listdir(path):
        face = self.extract_face(path + filename)
        if face != []:
          faces.append(face)
        else:
          pass
      ...
      y.extend(labels)
      X = np.asarray(X)
      if self.face_extract:
        X = self.faces_get_feature(X)
  ...
  def faces_get_feature(self,array):
    ...
    self.face_feature_extractor.get_layer(layer_name)
    feature = tf.keras.layers.Flatten()(self.face_feature_extractor.predict(array))
    return feature
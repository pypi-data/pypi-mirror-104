...
def remote_job_control(protocol_triad, batch_job_code, fs_locations, tasks_per_job,
    ...
    while not is_over:
        ...
        for job_id, task_list in tasks_per_job:
            ...
            if protocol == 'remote':
                manager_cmd = ["ssh", remote_machine, "date", "-r", macname, "+%H:%M"]
                machinename_cmd = ["ssh", remote_machine, "cat", macname]
                chk_time_cmd = "ssh " + remote_machine + " echo $(date +%H:%M)"
            elif protocol == 'remote-sharedfs':
                manager_cmd = ["date", "-r", macname_sf, "+%H:%M"]
                machinename_cmd = ["cat", macname_sf]
                chk_time_cmd = "echo $(date +%H:%M)"
            elif protocol == 'local':
                manager_cmd = ["date", "-r", macname, "+%H:%M"]
                machinename_cmd = ["cat", macname]
                chk_time_cmd = "echo $(date +%H:%M)"
            if DEBUG:
            ...
            if machine_names[job_id] == "NoHostname":
                if DEBUG:
                    print("COMMAND", machinename_cmd)
                    mname_txt = subprocess.Popen(
                        machinename_cmd,
                        stdout=subprocess.PIPE
                    ).stdout.read().decode('utf-8')
                else:
                ...
                if mname_txt.strip():
                    machine_names[job_id] = mname_txt.split()[1]
            if touch_time_txt.strip():
                ...
                touch_time = int(touch_time_txt.split(":")[0])*60 + int(touch_time_txt.split(":")[1])
                local_time = int(local_time_l[0])*60 + int(local_time_l[1])
                coeff = (local_time - touch_time)%(24*60)
                is_active = False if coeff > 1 else True
                if is_active:
                    is_over = False
                    chrono_on[job_id] = True
                    print("Job", job_id, "running", machine_names[job_id])
                else:
                    chrono_on[job_id] = False
    ...
    for job_id, task_list in tasks_per_job:
        if not time_end[job_id]: 
            time_end[job_id] = time_now

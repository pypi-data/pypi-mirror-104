...
class Dialogs:
    ...
    **Python**
    ...
    def wait_dialog(self, dialog: Dialog, timeout: int = 300) -> Result:
        """Wait for a dialog to be handled by the user, which has been
        created with the keyword ``Create dialog``.

        :param dialog:  An instance of a Dialog
        :param timeout: Time to wait (in seconds)

        Blocks until a user has closed the dialog or until ``timeout``
        amount of seconds has been reached. If the user submitted
        the dialog, returns a ``Result`` object.
    ...
    def wait_all_dialogs(self, timeout: int = 300) -> List[Result]:
        ...
        Returns a list of *all* results that
    ...
    def close_dialog(self, dialog: Dialog) -> None:
        ...
        .. code-block:: robotframework
            ...
            ${dialog}=    Create dialog    title=Please wait
    ...
    def close_all_dialogs(self) -> None:
        ...
        .. code-block:: robotframework
            ...
            ${dialog1}=    Create dialog
            A keyword which runs during the dialog

            ${dialog2}=    Create dialog

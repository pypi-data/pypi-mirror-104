  ...
  if 'количество_запусков' in kwargs:
    ...
    for start in range(kwargs['количество_запусков']):
      ...
      def on_epoch_end(epoch, log):
        ...
        if len(k)>2:
            p4 = p3 + ' '* (55 - len(p3)) + 'Точность на проверочной выборке: ' + str(round(log[k[3]]*100,2))+'%'
            result += p4 + '\n'
    ...
    for i in range(len(stage_result)):
      ...
      print(s)
    print('Средняя точность на обучающей выборке: ' , round(np.mean(best_result_on_train_arr), 2),  '%', 
          'средняя точность на проверочной выборке:',  round(np.mean(stage_best_result), 2), '%')
...
def тест_модели_вакансии(нейронка, x_val, y_val):
    ...
    for i in idx:
        ...
        print('---------------------------------------------------------------')
        print(x_val.shape[0])
        result = data['Этап сделки'][idx_val[i]]

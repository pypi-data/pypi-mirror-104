...
def _poisson_containment(lambda_i: float, lambda_f: float, lambda_j: float, lim=1000, quiet=False) -> float:
    s = 0
...
def _poisson_depth(df: pd.DataFrame, curr: int, to_compute=None):
    n, p = df.shape
    S_nj = 0
...
def probabilistic_poisson_depth(df: pd.DataFrame, to_compute=None):
    n, p = df.shape
    depths = []
    for f in tqdm(df.columns):
        depths.append(1/binom(n, 2) * _poisson_depth(df, f, to_compute=to_compute))

...
class Imac2MasterController:
    ...
    def process_safe_gas(self):
        ...
        Message format:
        {
            ...
            'detector-data-flags-safe-gas-array': [...]
        }
        ...
        1. There is no gas detector detected. Return a '-2000' on that position in the array and all flags will be
        used set to 0.
        2. There is a gas detector detected. Return the gas value in the correct address position in the first
        array and the flags in the second.
        """
        # Initialise both registers as errors.
        resp = {
            "detector-gas-analog-safe-gas-array": [-2000] * 40,
            "detector-data-flags-safe-gas-array": [0] * 40,
        }
        for serial, dev in self.modules.items():
            try:
                if dev.module_type == "imac-module-gg2":
                    # Try setting the gas value, if not present then skip
                    gas_value_index = dev.current_state["address-analog"] - 41
                    # Enforce that gas value range to be from 41-80
                    if 0 <= gas_value_index < 40:
                        # Set the gas values
                        ...
                        resp["detector-data-flags-safe-gas-array"][
                            gas_value_index
                        ] = data_flags
                elif dev.module_type == "imac-module-aim":
                    # We have found a Gasguard module connected to the AIM module.
                    # NOTE: Documentation should specify that an AIM module in this address range should only have a
                    # Gasguard module connected to it.
                    gas_value_index = dev.current_state["address-analog"] - 41
                    if 0 <= gas_value_index < 40:
                        # Takes both the raw analog and flags values and sets them.
                        resp["detector-gas-analog-safe-gas-array"][
                            gas_value_index
                        ] = dev.current_state["data-analog"]
                        resp["detector-data-flags-safe-gas-array"][
                            gas_value_index
                        ] = dev.current_state["data-flags"]
            except (KeyError, IndexError):
                # Current state value found for the module
                # OR Index out of range of the 40 values
                pass
            except BaseException as e:
                log.exception(e)
                pass

...
class SetParameters:
    """
    Writes modbus parameters by name and value and transformed by the parameter definitions
    """

    async def do(self, cmd: dict, modbus_device, **kwargs):
        try:
            pyaware.triggers.write.run_triggers(
                modbus_device.triggers["write"], cmd["data"]
            )
        except AssertionError as e:
            raise InvalidCommandData("".join(e.args)) from e
        await modbus_device.write_parameters(cmd["data"])
...
class ModbusDevice:
    ...
    def __init__(
        ...
        self.commands = pyaware.commands.Commands(
            ...
            meta_kwargs={"modbus_device": self},
            device_id=device_id,
        )
    ...
    async def write_parameters(self, data: dict):
        """
        Writes parameters to the appropriate registers
        :param data: Dictionary of form parameter: value
        :return:
        """
        # todo investigate combining with process_write with the horizon power implementation
        await self.process_write("", data)

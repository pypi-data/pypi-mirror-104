...
def render_plot(fig: matplotlib.figure, block: bool = False, pause: float = 0.0001) -> None:
    ...
    if block:
        ...
        try:
            ...
            mt.exception_error('The plot was closed ! can\'t continue', tabs=0)
    else:
        plt.pause(pause)
    return
...
def save_plot(path: str, ack: bool = True, tabs: int = 1) -> None:
    ...
    if ack:
        print('{}Saved: {}.png'.format(tabs * '\t', path))
    return
...
def plot_2d_many_figures_iterative_init(
    ...
    :param win_d: dict, optional(default see WINDOW_DEFAULT)
            if not None, has:
                ...
                'location': optional: str, tuple of ints or None: top left corner location
                    ...
                    if str: top_right, top_center, top_left, bottom_right, bottom_center, bottom_left
    ...
    loc = win_d['location'] if 'location' in win_d and win_d['location'] is not None else 'top_left'
    ...
    for i, ax in enumerate(axes_list):
        ...
        for data in data_plot_i['datum']:
            x, y = mt.de_augment_numpy(data['data'])
...
def plot_2d_many_figures_iterative_update(
    ...
    see plot_2d_many_figures_iterative_test()
    """
    if fig_d['new_title'] is not None:
    ...
    for i, scatters_plot_i in enumerate(fig_d['scatters']):  # for each sub plot
        ...
        for j, scatter in enumerate(scatters_plot_i):  # for each scatter in sub plot
            new_data_i = new_data_plot_i[j]
            if new_data_i['data'] is not None and len(new_data_i['data']) > 0:
                scatter._offsets = new_data_i['data']
                if 'c' in new_data_i and new_data_i['c'] is not None and len(new_data_i['c']) > 0:
                    scatter._facecolors = new_data_i['c']
                    scatter._edgecolors = new_data_i['c']
...
def plot_2d_iterative_dashboards_init(
    ...
    for i, ax in enumerate(axes_list):
        ...
        for data in data_plot_i['datum']:
            ...
            if label is not None:
                saw_label = True
            sc = ax.scatter([0], [0], color=c, marker=marker, s=marker_s, label=label)
            scatters_plot_i.append(sc)
        ...
        if center_d is not None:
            ...
            ax.scatter(
                [cx], [cy],
                c=center_d['c'], marker=center_d['marker'],
                s=center_d['marker_size'], label='{}({},{})'.format(center_d['label'], cx, cy)
            )
        if saw_label and 'legend' in data_plot_i and data_plot_i['legend'] is not None:
            legend = data_plot_i['legend']
            ax.legend(loc=legend['loc'], ncol=legend['ncol'], fancybox=legend['fancybox'],
...
def plot_3d_iterative_dashboard_init(
    ...
    if 'face_color' in data_plot_i and data_plot_i['face_color']:
        ax.set_facecolor(data_plot_i['face_color'])

    if 'background_color' in data_plot_i and data_plot_i['background_color']:
    ...
    for data in data_plot_i['datum']:
        ...
        if label is not None:
            saw_label = True
        sc = ax.scatter(0, 0, 0, color=c, marker=marker, s=marker_s, label=label)
        scatters_plot_i.append(sc)
    ...
    if center_d is not None:
        ...
        ax.scatter(
            cx, cy, cz,
            c=center_d['c'], marker=center_d['marker'],
            s=center_d['marker_size'], label='{}({},{},{})'.format(center_d['label'], cx, cy, cz)
        )
...
def plot_3d_one_figure_iterative_update(
    ...
    for j, scatter in enumerate(fig_d['scatters']):  # for each scatter in sub plot
        new_data_i = new_data_plot_i[j]
        if new_data_i['data'] is not None and len(new_data_i['data']) > 0:
            scatter._offsets3d = new_data_i['data'].T
            if 'c' in new_data_i and new_data_i['c'] is not None and len(new_data_i['c']) > 0:
                scatter._facecolor3d = new_data_i['c']
                scatter._edgecolor3d = new_data_i['c']

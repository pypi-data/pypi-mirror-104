...
def move_figure_by_str(fig: matplotlib.figure, where: str = 'top_left') -> None:
    ...
    try:
        ...
        if window_w != -1 and window_h != -1:
            ...
            move_figure_x_y(fig=fig, x=0, y=0)
    except (ValueError, Exception) as e:
        mt.exception_error(e)
        move_figure_x_y(fig=fig, x=0, y=0)
    return
...
def on_click(event: KeyEvent) -> None:
    """
...
def finalize_plot(fig: matplotlib.figure) -> None:
    ...
    old_title_obj = fig._suptitle
    if old_title_obj is not None:
        fig.suptitle('{}\n{}'.format(old_title_obj.get_text(), 'click X to close'))
    else:
        fig.suptitle('click X to close')
    fig.canvas.mpl_connect('key_press_event', on_click)  # close on ESC
    plt.show(block=True)
    return

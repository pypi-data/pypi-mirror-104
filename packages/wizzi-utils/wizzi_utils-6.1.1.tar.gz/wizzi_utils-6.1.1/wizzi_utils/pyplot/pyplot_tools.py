...
LEGEND_DEFAULT = {'loc': 'upper right', 'ncol': 1, 'fancybox': True, 'framealpha': 0.5, 'edgecolor': 'black'}
...
def finalize_plot(fig: matplotlib.figure) -> None:
    ...
    old_title_obj = fig._suptitle
    if old_title_obj is not None:
        fig.suptitle('{} - {}'.format(old_title_obj.get_text(), end_str))
    else:
        fig.suptitle(end_str)
    # fig.canvas.set_window_title(end_str)
    fig.canvas.mpl_connect('key_press_event', on_click_close_all)  # close on any key pressed
    plt.show(block=True)
    return
...
def plot_2d_iterative_figure(
    ...
    :param render: whether to render the plot
    :param block: whether to block the plot
    :return:
    figure
    list of axes
    ...
    # fig.canvas.mpl_connect('close_event', block_x_button)
    ...
    for i, ax in enumerate(axes_list):
        ...
        if labels is not None:
            sc = ax.scatter([0], [0], color=default_color, marker='.', label=labels[i])
            ...
            sc = ax.scatter([0], [0], color=default_color, marker='.')
        scatters.append(sc)
    ...
    if render:
        render_plot(fig=fig, block=block)
...
def update_2d_scatters(
    ...
    :param tabs:
    :param render: whether to render the plot
    :param block: if render=True: whether to block the plot
    ...
    if render:  # not mandatory now - you may have custom add ons
        render_plot(fig=fig, block=block)
    return
...
def plot_2d_scatter(
    ...
    """
    if x_y_lims is None:
        # if no limits, must give something to plot_2d_iterative_figure to init with
        # otherwise it will be very close to 0
        x_min, y_min = np.min(data, axis=0)
        x_max, y_max = np.max(data, axis=0)
        ten_p_x = 0.1 * (x_max - x_min)
        ten_p_y = 0.1 * (y_max - y_min)
        x_y_lims = [x_min - ten_p_x, x_max + ten_p_x, y_min - ten_p_y, y_max + ten_p_y]

    if def_color is None and colors is not None:
        def_color = colors[0]

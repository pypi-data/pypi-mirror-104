...
class Connector:
	def __init__(self):
	    self.ws = asyncio.get_event_loop().run_until_complete(self.get_ws())	      
        

class Command:
    def __init__(self, func):
        self.name = func.qualified_name or func.name
...
class Gateway:
	...
	def identify_json(self, token : str, intents : int):
		...
		t = Template('{"op": 2,"d": {"token": "$token","intents": $intents, "properties": {"$os": "linux","$browser": "discord.wrapper","$device": "discord.wrapper"}},"s": null,"t": null}')
		...
		try:
			...
			for data in my_dict_with_list:			   
			    if data['op'] == 10:
			        ...
			        my_r = await self.get_data()
			        print(my_r)
#			        if events["t"] == "MESSAGE_CREATE":
#			        for i in range(1):
#			            try:
#			                print(events["d"]["content"])
#			            except:
#			                continue	    

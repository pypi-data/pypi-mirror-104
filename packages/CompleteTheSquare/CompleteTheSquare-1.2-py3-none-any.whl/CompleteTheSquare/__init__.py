...
class CompleteTheSquare:
  ...
  def _split(self):
    ...
    return int(one) if int(one) == one else float(one), int(two) if int(two) == two else float(two), int(three) if int(three) == three else float(three)
  ...
  def _solve(self, eq):
    ...
    ii = i + 1
    r = True if eq[ii] == "-" else False
    i += 2
    o = eq.index(")")
    uh = float("".join(eq[i:o]))
    if r:
      ik = float("".join(eq[ii:o]))
      use = uh
      fo = (uh + os)
      ft = (uh - os)
      return fo, ft

...
if 'sdist' not in dist.commands and compiler.find_library_file([library_path],
    ...
    # the sources.
    if compiler.compiler_type == 'msvc':
        compile_flags = ['/D'+flag for flag in ['_USE_MATH_DEFINES', 'NOMINMAX', 'JSBSIM_VERSION="1.1.6 [GitHub build 320/commit b477f6312bee2fd3af4c4e5ba1a3e732ed2b99d4]"','HAVE_EXPAT_CONFIG_H',]]
        if sys.version_info < (3,9):
            compile_flags = [flag.replace('"', '\\"') for flag in compile_flags]
    else:
        compile_flags = ['-D'+flag for flag in ['JSBSIM_VERSION="1.1.6 [GitHub build 320/commit b477f6312bee2fd3af4c4e5ba1a3e732ed2b99d4]"','HAVE_EXPAT_CONFIG_H',]]

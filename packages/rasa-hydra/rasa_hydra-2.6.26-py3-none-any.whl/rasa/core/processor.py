...
class MessageProcessor:
    ...
    def convert_pci_message(self, message):
        pci_msg = message
        if re.search(r"(?<!\d)(\d{13}|\d{15}|\d{16}|\d{19})(?!\d)", message):
            logger.warning(f"Warning: Possible PCI value detected in public message")
            pci_msg = re.sub(r"(?<!\d)(\d{13}|\d{15}|\d{16}|\d{19})(?!\d)", '****************', message)
        return pci_msg

    def convert_pci_entities(self, message):
        pci_msg = message #re.sub(r"(?<!\d)(\d{13}|\d{15}|\d{16}|\d{19})(?!\d)", '****************', message)
        for e in pci_msg:
            e['value'] = re.sub(r"(?<!\d)(\d{13}|\d{15}|\d{16}|\d{19})(?!\d)", '****************', str(e['value'])) #UserUttered.redact_msg(str(e['value']))
        return pci_msg
    ...
    async def _handle_message_with_tracker(
        ...
        redacted_text = self.convert_pci_message(message.text)    
        redacted_entities = self.convert_pci_entities(copy.deepcopy(parse_data['entities']))

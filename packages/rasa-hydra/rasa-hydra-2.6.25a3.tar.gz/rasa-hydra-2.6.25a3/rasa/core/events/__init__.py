...
class BotUttered(Event):
    ...
    def as_dict(self):
        d = super().as_dict()
        privacy = self.get_privacy_mode()
        d.update({"text": self.text, "data": self.data, "metadata": self.metadata, "message_id": self.message_id, "privacy_mode": privacy})
        return d

    def get_privacy_mode(self):
        privacy_mode = False
        if self.data.get('custom') and self.data.get('custom').get('privacy_mode') == True:
            privacy_mode = True
        return privacy_mode

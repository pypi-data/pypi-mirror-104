                ...
                if name in self.param_nodes_dict:
                    ...
                    for node in self.param_nodes_dict[name]:
                        ...
                        if value is None and node.value is not None:
                            value = node.value  # so that earlier updated node updates later ones
                            # in case the current value is None

                        if value is not None and node.value is None:  
                            node.set_value(value)  # update earlier nodes with later valued nodes


                # repeat the procedure for the options parameter nodes
                if name in self.option_nodes_dict:

                    for node in self.option_nodes_dict[name]:

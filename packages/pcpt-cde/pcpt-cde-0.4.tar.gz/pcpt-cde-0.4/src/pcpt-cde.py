...
def main():
    ...
    try:
        ...
        if ssm_operation:
            ...
            lb_table = PrettyTable()

            # aws loadbalancer details
            lb_list = get_lb(session, aws_config)
            lb_table.field_names = ["Service", "Scheme", "State", "DNS Name"]
...
def get_lb(session, aws_config):
    """
    Retrieve information about an Amazon loadbalancer
    """
    ...
    try:
        ...
        for lb in response["LoadBalancers"]:
            ...
            tags = lb_client.describe_tags(ResourceArns=[lb_arn])

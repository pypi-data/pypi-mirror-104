...
from bgeditor.dao.FFmpeg import create_suource_can_loop_path, create_loop, create_loop_audio, create_video_audio
...
def create_video(list_comp_data, path_video, job_id):
    ...
    arr_composite = []
    max_duration = 0
    max_real_duration=0 # thoi gian cua 1 video
    ...
    if not is_compilation:
        ...
        final_clip.write_videofile(path_video, fps=24, codec='libx264', logger=logger)
        final_clip.close()
    else:
        if audio_compilation_path is None:
            raise Exception("Audio Compulation Error!!!")
        cnt_real_duration=0
        while max_real_duration < 60*2/3 and cnt_real_duration<2 and cnt_effect_vid>1:
            max_real_duration*=2
            cnt_real_duration+=1
        tmp_path = get_dir('coolbg_ffmpeg') + str(uuid.uuid4()) +".avi"
        tmp_path_maked_loop = get_dir('coolbg_ffmpeg') + str(uuid.uuid4()) + ".avi"
        final_clip = CompositeVideoClip(arr_composite).subclip(0, max_real_duration)
        final_clip.write_videofile(tmp_path, fps=24, codec='libx264', audio=False, logger=logger)
        final_clip.close()
        if cnt_effect_vid>1:
            ...
            clip.write_videofile(tmp_path_maked_loop, fps=24, codec='libx264', audio=False, logger=logger)
            clip.close()
            final_clip.close()
        else:
...
class CompilationComp(Component):
    ...
    def get_audio(self):
        return MusicHelper.create_compilation_songs(self.music_data)

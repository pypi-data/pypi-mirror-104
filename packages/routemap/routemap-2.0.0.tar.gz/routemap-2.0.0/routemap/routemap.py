    ...
    for i, position in enumerate(positions):
        ...
        if position.get('Type') in ['BR', 'ER']:
            ...
            calldate = datetime.datetime.strptime(
                ...
                '%Y-%m-%dT%H:%M:%S-00:00').strftime('%d %b')
...
def plot(
    ...
    if currpos:
        ...
        annotations.append((currlon, currlat, currposlabel))
    ...
    earth = plt.figure(figsize=(16, 10))

    res = '10m'
    land_data = cfeature.NaturalEarthFeature(
        'physical', 'land', res
    )

    # Decide where we want our Parallels and Meridians to go.
    xlocs = getcardinals(west, east, 10)
    ylocs = getcardinals(south, north, 10)

    # This is the bit that creates the map.
    ax = earth.add_subplot(1,1,1, projection=proj)
    ax.set_extent([west, east, south, north])
    ax.add_feature(land_data, edgecolor='grey', facecolor='lightgrey')
    gl = ax.gridlines(crs=proj, draw_labels=True, xlocs=xlocs, ylocs=ylocs)
    gl.top_labels = False
    ...
    for txt in annotations:
        annot_lons.append(txt[0])
        annot_lats.append(txt[1])
        ax.text(*txt)
    ...
    plt.plot(
        ...
        color='red',
        linewidth=0.75,
        label='Distance = {} NM'.format(int(totaldistance / KM_IN_NM))
    )

    # Plot the annotations
    plt.plot(annot_lons, annot_lats, color='black', marker='o', linewidth=0)

...
You can update an existing cluster configuration, for example to add a different software
environment, change the number of CPUs or memory, by calling ``coiled.create_cluster_configuration``
with the name of the cluster configuration you want to update and the new specification for the
cluster configuration.

For example, let's update the memory of the cluster configuration that we have previously created.
...
By default, cluster configurations can be viewed and used by any coiled user. If
you want a cluster configuration to be visible only to members of your account,

...
# 分页并输出内容字典
def page_nums():
    from tky.t_sql import Mysql_db
    tang = Mysql_db('10.xxx.xxx.xxx', 3306, 'xxx', 'xxxx', 'xxx')  # 对应的数据链接参数
    zd = {'ltype like': '%用户%', 'ctime>': '2021-04-28'}
    # zd = {'ltype like ': '%用户%'}
    # page = Mysql_db.page_mus(tang, table='tang_pc_logs', order='ctime', ordertp='desc', page=1, listnum=15, zd=zd, tp='and')
    page = Mysql_db.page_mus(tang, table='tang_pc_logs', order='ctime', ordertp='desc', page=1, listnum=15)
    print(page[0])
    print(page[1])
    # for v in page[1]:
    #     print(v['id'])

...
class CtaTemplate(CtaTemplateBase):
    ...
    def on_trade(self, trade: TradeData):
        ...
        if trade.offset == Offset.OPEN:
            ...
            self.avg_entry_price = ((abs(self.pos) - trade.volume) * self.avg_entry_price + trade.volume * trade.price
                                    ) / abs(self.pos)
        else:
...
class TargetPosAndPriceTemplate(CtaTemplate):
    ...
    def set_target_pos(self, target_pos, price=None):
        ...
        # 当仓位从 非0 -> 0 或 反向建仓 时，重置 fire_stop_order 状态
        if self.fire_stop_order and (self._target_pos < 0 <= target_pos or self._target_pos > 0 >= target_pos):
            self.fire_stop_order = False

...
def run_backtest(
    ...
    engine_kwargs.setdefault('capital', 1000000)
    ...
    if strategy_stats_original is not None:
        ...
        if strategy_stats_original.short_name is not None:
            ...
            file_name_header = f"{strategy_stats_original.id_name}_[{strategy_stats_original.short_name}]"
        else:
    ...
    if output_orders:
        order_file_name = get_output_file_path(
            vt_symbol_str, cross_limit_method.name, backtest_status_path, 'orders',
            f'{file_name_header}_orders.csv', root_folder_name=root_folder_name)
    else:
        order_file_name = None

    if output_trades:
        trade_file_name = get_output_file_path(
            vt_symbol_str, cross_limit_method.name, backtest_status_path, "trades",
            f'{file_name_header}_trades.csv', root_folder_name=root_folder_name)
    else:
    ...
    if output_trades:
        ...
        pd.DataFrame([{
            ...
            "price": _.price,
            "volume": _.volume,
        } for _ in trades]).to_csv(trade_file_name, index=False)
...
def run_backtest_from_queue(strategy_cls, task_queue: Queue, engine_key_results_dic: dict, lock: Lock,
    ...
    while True:
        try:
            ...
            for k in statistic_item_dic.keys():
                if k not in stats_dic:
                    continue
                statistics_with_param_dic[k] = stats_dic[k]

            # 將关键标签顺序排在最后几列
            for _ in ['short_name', 'shown_name']:
                if _ in statistics_with_param_dic:
                    statistics_with_param_dic.move_to_end(_)

            with lock:
                # 多进程 dict 对象只能使用 赋值 操作。不可以使用 [].append 的操作。
                # 因此不可以使用 setdefault(key, []).append()
                if key in engine_key_results_dic:
                    results = engine_key_results_dic[key]
                    results.append(statistics_with_param_dic)
                else:
                    results = [statistics_with_param_dic]

                engine_key_results_dic[key] = results
...
def bulk_backtest_with_backtest_params_iter(
    ...
    if is_multi_process:
        ...
        for _ in range(multi_process):
            ...
            pool.apply_async(
                ...
                args=(strategy_cls, task_queue, engine_key_results_dic, lock,
    ...
    for n, (engine_kwargs, param_values, strategy_stats_original) in backtest_params_iter:
        ...
        if multi_symbols:
            ...
            description_vt_symbol = vt_symbol_str
        ...
        description = f"{strategy_cls.__name__} [{description_vt_symbol}]{description_header}"
    ...
    if result_df is not None and result_df.shape[0] > 0:
        ...
        csv_file_path = get_output_file_path(
            f'result_{strategy_cls.__name__}_{vt_symbol_str}_{date_2_str(date.today())}.csv',
            root_folder_name=root_folder_name)
        result_df.to_csv(csv_file_path, index=False, encoding='GBK')
        logger.info("总计 %d 条测试记录被保存到 %s", result_df.shape[0], csv_file_path)
        csv_file_path = get_output_file_path(
            f'result_{strategy_cls.__name__}_{vt_symbol_str}_{date_2_str(date.today())}.xlsx',
            root_folder_name=root_folder_name)
    ...
    if result_available_df is not None and result_available_df.shape[0] > 0:
        ...
        csv_file_path = get_output_file_path(
            f'available_{strategy_cls.__name__}_{vt_symbol_str}_{date_2_str(date.today())}.csv',
            root_folder_name=root_folder_name)
        result_available_df.to_csv(csv_file_path, index=False, encoding='GBK')
        csv_file_path = get_output_file_path(
            f'available_{strategy_cls.__name__}_{vt_symbol_str}_{date_2_str(date.today())}.xlsx',
            root_folder_name=root_folder_name)
        # Writing to Excel with MultiIndex columns and no index ('index'=False) is not yet implemented.

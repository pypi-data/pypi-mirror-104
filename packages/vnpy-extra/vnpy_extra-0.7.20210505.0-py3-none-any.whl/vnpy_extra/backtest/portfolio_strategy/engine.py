...
class BacktestingEngine(BacktestingEngineBase):
    ...
    def calculate_statistics(self, df: DataFrame = None, output=True) -> dict:
        ...
        if df is None:
            ...
            return_loss_ratio = net_pnl_s[net_pnl_s > 0].sum() / abs(net_pnl_s[net_pnl_s < 0].sum())
            # 卡玛比
            ...
            return_risk_ratio = np.round(win_ratio - (1 - win_ratio) / return_loss_ratio, 4)
        ...
        if df is None:
            ...
            if hasattr(self.strategy, 'calc_score'):
                ...
                score = np.std(benchmark - rr_s) * (100 if rr_s.iloc[-1] > 0 else -100)

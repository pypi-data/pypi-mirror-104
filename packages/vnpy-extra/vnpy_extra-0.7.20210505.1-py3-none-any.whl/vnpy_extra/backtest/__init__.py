...
def mock_load_bar_data(
    ...
    if load_continue_data:
        ...
        write_log_func(f"加载{is_main_str}[{main_symbol}] {data_len:,d}条，"
                       f"当期合约[{symbol}] {data_sub_len:,d}条，"
                       f"累计 {data_len + data_sub_len:,d} 条 [{datetime_2_str(start)} ~ {datetime_2_str(end)}]。")

...
@click.group()
@add_params(['disc_nr', 'input', 'output', 'title', 'decrypt', 'minlength',
             'cache',
             'info_file', 'json', 'verbose', 'quiet', 'no_bar', 'no_info', ])
@click.pass_context
def cli(ctx, **params):
    pass
...
@add_params(['disc_nr', 'input', 'minlength', 'cache',
             'info_file', 'json', 'verbose', 'quiet', 'no_bar', 'no_info', ])
@click.pass_context
def info(ctx, **params):
    ...
    with ProgressParser() as bar:
        ...
        print('input:', params['input'], 'disc_nr:',
              params['disc_nr'], 'result:', input)
        if params['verbose']:
...
@add_params(['disc_nr', 'input', 'output', 'title', 'minlength', 'cache',
             'info_file', 'json', 'verbose', 'quiet', 'no_bar', 'no_info', ])
@click.pass_context
def mkv(ctx, **params):
...
@add_params(['disc_nr', 'input', 'output', 'decrypt', 'minlength', 'cache',
             'info_file', 'json', 'verbose', 'quiet', 'no_bar', 'no_info', ])
@click.pass_context
def backup(ctx, **params):
    """Backup whole disc."""
    params = dict(ctx.parent.params, **params)
    with ProgressParser() as bar:
...
@add_params(['info_file', 'json', 'verbose', 'quiet', 'no_bar', 'no_info', ])
@click.argument('args', nargs=-1, type=click.UNPROCESSED)
@click.pass_context
def f(ctx, args, **params):
...
class DictTree(Tree):
    ...
    def walk_dict(self, data, parent_tree):
        if self.sort_keys:
        ...
        for key, value in data:
            ...
            if isinstance(value, int):
                parent_tree.add(f"{key}: {int(value)}", **self.config)
                continue
    ...
    def walk_list(self, data, parent_tree, label=''):
        if self.sort_keys:
            try:
                data.sort()
            except TypeError:
        ...
        for i, value in enumerate(data):
            if isinstance(value, str):
                parent_tree.add(
                    f"{value.replace('_', ' ').capitalize()}",
                    **self.config
                )
                continue
            if isinstance(value, int):
                parent_tree.add(f"{int(value)}", **self.config)
                continue
            if isinstance(value, list):
                child_tree = parent_tree.add(f"{label} {str(i + 1)}",
                                             **self.config)

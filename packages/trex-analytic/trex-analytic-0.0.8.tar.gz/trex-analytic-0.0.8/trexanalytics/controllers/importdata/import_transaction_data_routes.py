...
def import_all_customer_transaction_function(credential_info=None, dataset_name=MERCHANT_DATASET, 
    ...
    with db_client.context():
        (customer_list, next_cursor) = CustomerTransaction.list_all(limit = limit, start_cursor=start_cursor, return_with_cursor=True)
        import_count  = len(customer_list)
        
        for c in customer_list:
            customer_transaction_dict_list.append({
                                            "Key"                   : uuid.uuid1().hex,
                                            "UserKey"               : c.transact_user_acct_key,
                                            "CustomerKey"           : c.transact_customer_key,
                                            "MerchantKey"           : c.transact_merchant_acct_key,
                                            "TransactOutlet"        : c.transact_outlet_key,
                                            "TransactionId"         : c.transaction_id,
                                            "InvoiceId"             : c.invoice_id,
                                            "TransactAmount"        : c.transact_amount,
                                            "TransactDateTime"      : c.transact_datetime,
                                            "UpdatedDateTime"       : import_datetime,
                                            })
    
    
    
    customer_transaction_data_dict_to_stream = {'':customer_transaction_dict_list}
    ...
    errors = stream_data_by_datetime_partition(bg_client, dataset_name, CUSTOMER_TRANSACTION_TEMPLATE, 
                                               ...
                                               column_name_used_to_partition='transact_datetime', 
                                               partition_date=True,
                                               )

    ...
    cdef bint _evaluate_reciprocity(self, np.intp_t i, np.ndarray[np.intp_t, ndim=2] knn_indices, np.ndarray[np.double_t, ndim=2] knn_dist, Relation *rel):
        ...
        for ranki in range(0, self.max_neighbors_search + 1):
            ...
            if order < best:
                ...
                rel.value = pow(rel.rec_dis, 2.) * pow(rel.rec_rank+rel.penalty, 1.) * pow(1.*rel.my_members/rel.rec_members, 0.5)
...
#### initialization of reciprocities
        ...
        while self.result_edges < self.num_points - 1:
            ...
            self.result_add_edge(best_rel.index, best_rel.target, best_rel.value)
...
#### initialization of reciprocities
        ...
        while best_value != INF:
            ...
            if rel.value > best_value:
                upper[best_i] = rel.value
                if has_data:
            ...
            self.result_add_edge(best_i, rel.target, rel.value)

...
cdef class Amalgamation (object):
    ...
    cdef Border _whole(self, np.double_t g, Amalgamation other):
        cdef Border brd

        brd.dis = pow(g, 1.)
        # всё что не качество, то количество!
        ...
        brd.jump = brd.dis*brd.quantity*brd.measure
    ...
    cdef np.intp_t limit_to_ought(self, np.double_t g, Amalgamation other):
        ...
        # can a new whole overcome it's parts?
        cdef Border brd

        brd = self._whole(g, other)
        # print (whole > self.energy + EPS, whole, self.energy + EPS)
        return brd.limit > self.energy + EPS
...
# ----------------------
        ...
        brd1 = self._whole(g, other)
        brd2 = other._whole(g, self)

# ----------------------
        if brd1.limit > self.energy + EPS:
            ret.energy = brd1.jump*ret.size
            ret.clusters = 1
# ----------------------
        if brd2.limit > oenergy + EPS:
            oenergy = brd2.jump*osize
            oclusters = 1
# ----------------------

...
if Var.BOT_TOKEN:
    ...
    try:
        ...
        ultroid_bot.loop.run_until_complete(bot_info(ultroid_bot.asst))
...
async def hehe():
    if Var.LOG_CHANNEL:
        try:
            try:
                await ultroid_bot(
                    AddChatUserRequest(chat_id=Var.LOG_CHANNEL, asst.me.id, fwd_limit=1)
                )
            except BaseException:
                try:
                    await ultroid_bot(
                        InviteToChannelRequest(
                            channel=Var.LOG_CHANNEL, users=[asst.me.id]
                        )
                    )
                except BaseException:

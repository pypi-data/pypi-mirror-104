...
from gihadocs.structure.files import get_files
from gihadocs.structure.nav import get_navigation
import gihadocs
...
def get_context(nav, files, config, page=None, base_url=''):
    ...
    return {
        ...
        'gihadocs_version': gihadocs.__version__,
...
def _build_template(name, template, files, config, nav):
    ...
    if utils.is_error_template(name):
        ...
        # See https://github.com/gihadocs/gihadocs/issues/77.
        # However, if site_url is not set, assume the docs root and server root
        # are the same. See https://github.com/gihadocs/gihadocs/issues/1598.

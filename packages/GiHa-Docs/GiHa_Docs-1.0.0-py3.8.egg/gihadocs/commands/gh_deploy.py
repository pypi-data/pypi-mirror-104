...
import gihadocs
from gihadocs.utils import ghp_import

log = logging.getLogger(__name__)

default_message = """Deployed {sha} with GiHaDocs version: {version}"""
...
def _check_version(branch):
    ...
    currentv = parse_version(gihadocs.__version__)
    if not previousv:
        log.warning('Version check skipped: No version specified in previous deployment.')
    elif currentv > previousv:
        log.info(
            'Previous deployment was done with GiHaDocs version {}; '
            'you are deploying with a newer version ({})'.format(previousv, currentv)
        )
    elif currentv < previousv:
        log.error(
            'Deployment terminated: Previous deployment was made with GiHaDocs version {}; '
...
def gh_deploy(config, message=None, force=False, ignore_version=False):
    ...
    message = message.format(version=gihadocs.__version__, sha=sha)

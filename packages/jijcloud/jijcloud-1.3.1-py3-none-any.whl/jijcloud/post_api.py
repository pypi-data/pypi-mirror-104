...
def post_instance_and_query(
        ...
        instance_type: str, instance: Dict[str, Any], queue_name: str, solver: str, parameters: dict,
        sync: bool = True):
    ...
    solver_res = client.submit_solve_query(queue_name, solver, parameters, instance_id)
    logger.info("submitted to the queue")
    ...
    if sync:
        ...
        while (status == APIStatus.PENDING.value) or (status == APIStatus.RUNNING.value):
            ...
            if response['status'] == APIStatus.RUNNING.value:
                logger.info("running...")

...
class LanZouCloud(object):
    ...
    def sync_files(self, path_root, folder_id, only_directory=False):
        """
        将本地的文件和云端进行同步
        :param path_root: 本地路径
        :param folder_id: 云端路径
        :param only_directory: 只同步文件夹
        :return: 文件到folder_id的映射关系
        """
        yun_list = self.get_dir_list(folder_id)
        yun_dict = dict([(yun['name'], yun['id']) for yun in yun_list])
        ...
        for file in (os.listdir(path_root)):
            ...
            if os.path.isfile(file_path):
                if only_directory:
                    ...
                    if file in yun_dict.keys():
                        yun_id = yun_dict[file]
                    else:

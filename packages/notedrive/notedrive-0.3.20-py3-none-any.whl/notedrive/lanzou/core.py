...
class LanZouCloud(object):
    ...
    def sync_files(self, path_root, folder_id, only_directory=False, overwrite=False):
        """
        将本地的文件同步到云端，单向同步
            ...
            if os.path.isfile(file_path):
                ...
                if file in yun_file_dict.keys():
                    if overwrite:
                        ...
                        yun_id = self.upload_file(file_path=file_path, folder_id=folder_id)
                    else:
                        yun_id = yun_file_dict[file]
                else:
                    yun_id = self.upload_file(file_path=file_path, folder_id=folder_id)
                file_dict[file_path] = yun_id
            else:
                ...
                file_dict[file_path] = yun_id
                file_dict.update(self.sync_directory(file_path, yun_id))

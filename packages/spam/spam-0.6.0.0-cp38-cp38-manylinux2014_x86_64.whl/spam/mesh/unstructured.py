...
def createCuboid(lengths, lc, origin=[0., 0., 0.], verbosity=0, gmshFile=None, vtkFile=None, binary=False, skipOutput=False):
    """
    ...
    gmsh.model.geo.addPoint(ox, oy, oz, lc, 1)
    gmsh.model.geo.addPoint(lx + ox, oy, oz, lc, 2)
    gmsh.model.geo.addPoint(lx + ox, ly + oy, oz, lc, 3)
    gmsh.model.geo.addPoint(ox, ly + oy, oz, lc, 4)
    ...
    gmsh.model.geo.extrude([(2, 1)], 0, 0, lz)
    gmsh.model.geo.synchronize()
...
def createCylinder(centre, radius, height, lc, verbosity=0, gmshFile=None, vtkFile=None, binary=False, skipOutput=False):
    """

...
from typing import (
    Iterable, FrozenSet, Optional, Mapping, Union as Union_,
    Type, Callable, Any, Iterator, TYPE_CHECKING, cast,
    Tuple, List, Dict
)
...
VarType = Union_['rql.nodes.VariableRef', 'rql.nodes.Variable']


def variable_ref(var: "rql.nodes.VariableRef") -> "rql.nodes.VariableRef":
    """get a VariableRef"""
...
def variable_refs(node: "rql.nodes.VariableRef") -> Iterator[Union_["rql.base.BaseNode",
...
class OperatorExpressionMixin:
    ...
    def initargs(self, stmt: Optional["rql.stmts.Statement"]) -> Tuple[str, Optional[str]]:
        ...
        return (self.operator, None)  # type:ignore[attr-defined]

    def is_equivalent(self, other: Any) -> bool:
        if not Node.is_equivalent(self, other):
            return False
        # "OperatorExpressionMixin" has no attribute "operator"  [attr-defined]
        return self.operator == other.operator  # type:ignore[attr-defined]

    def get_description(self,
                        mainindex: int,
                        tr: Callable[[str], str]) -> Optional[str]:
        ...
        try:
            ...
            description = tr(self.get_type())  # type:ignore[attr-defined]
...
class HSMixin:
    ...
    def relation(self) -> Optional["rql.nodes.Relation"]:
        """return the parent relation where self occurs or None"""
        try:
            # "HSMixin" has no attribute "parent"  [attr-defined]
            # Indeed parent attribute is defined inside some children of BaseNode
            return self.parent.relation()  # type: ignore[attr-defined]
        except AttributeError:
            return None

    # mainindex ???
    def get_description(self, mainindex: int, tr: Callable[[str], str]) -> Optional[str]:
        # "HSMixin" has no attribute "get_type"  [attr-defined]
        # Indeed get_type is defined inside:
        # MathExpression, Function, VariableRef, Referenceable, ColumnAlias, ...
        mytype = self.get_type()  # type: ignore[attr-defined]
        if mytype != 'Any':
            return tr(mytype)
        return 'Any'
...
def make_relation(
        ...
        rhsargs: Tuple[Any, ...],
        rhsclass: type,
        operator: str = '=') -> "rql.nodes.Relation":
...
class EditableMixIn:
    ...
    def add_restriction(self,
                        relation: "rql.nodes.Relation") -> "rql.nodes.Relation":
    ...
    def add_relation(self,
                     lhsvar: "rql.nodes.Variable",
                     rtype: str,
                     rhsvar: "rql.nodes.Variable") -> "rql.nodes.Relation":
        """builds a restriction node to express '<var> eid <eid>'"""
        return self.add_restriction(make_relation(lhsvar, rtype, (rhsvar,),
                                                  VariableRef))

    def add_eid_restriction(self,
                            var: "rql.nodes.Variable",
                            eid: str,
                            c_type: str = 'Int') -> "rql.nodes.Relation":
        """builds a restriction node to express '<var> eid <eid>'"""
    ...
    def add_type_restriction(self,
                             var: "rql.nodes.Variable",
                             etype: str) -> "rql.nodes.Relation":
        ...
        if typerel:
            if typerel.r_type == 'is':
                ...
                if isinstance(istarget, Constant):
                    etypes: List[Any] = [istarget.value, ]
                else:  # Function (IN)
...
class SubQuery(BaseNode):
    ...
    def __init__(self,
                 ...
                 query: Optional["rql.stmts.Union"] = None) -> None:
    ...
    def copy(self,
             stmt: Optional["rql.stmts.Statement"] = None) -> "rql.nodes.SubQuery":
        # error: List comprehension has incompatible type List[BaseNode];
        # expected List[VariableRef]  [misc]
        # type: ignore[misc]
        return SubQuery([cast(VariableRef, v.copy(stmt)) for v in self.aliases], self.query.copy())
...
class And(BinaryNode):
    ...
    def ored(self,
             ...
             _fromnode: Any = None) -> "rql.nodes.Or":
    ...
    def neged(self,
              traverse_scope: bool = False,
              _fromnode: Any = None) -> "rql.nodes.Not":
        # Item "BaseNode" of "Optional[BaseNode]"
...
class Or(BinaryNode):
    ...
    def ored(self, traverse_scope: bool = False, _fromnode: Any = None) -> "rql.nodes.Or":
        return self

    def neged(self, traverse_scope: bool = False, _fromnode: Any = None) -> "rql.nodes.Not":
...
class Not(Node):
    ...
    def neged(self,
              traverse_scope: bool = False,
              _fromnode: Any = None,
              strict: bool = False) -> "rql.nodes.Not":
        return self
...
class Exists(EditableMixIn, BaseNode):
    ...
    def copy(self,
             stmt: Optional["rql.stmts.Statement"] = None) -> "rql.nodes.Exists":
    ...
    def ored(self,
             traverse_scope: bool = False,
             _fromnode: Any = None) -> Union_["rql.nodes.Or", bool]:
    ...
    def neged(self,
              traverse_scope: bool = False,
              _fromnode: Any = None,
              strict: bool = None) -> Union_["rql.nodes.Not", bool]:
        if not traverse_scope:
            if _fromnode is not None:  # stop here
                return False
...
class Relation(Node):
    """a RQL relation"""
    __slots__: Iterable[str] = ('r_type', 'optional',
...
class Comparison(HSMixin, Node):
    ...
    def __init__(self,
                 operator: str,
                 value: Union_["rql.nodes.Function",
                               "rql.nodes.Constant",
                               "rql.nodes.Constant",
                               "rql.nodes.VariableRef",
...
class MathExpression(OperatorExpressionMixin, HSMixin, BinaryNode):
    ...
    def __init__(self,
                 ...
                 rhs: Optional["rql.nodes.Node"] = None) -> None:
...
class Constant(HSMixin, LeafNode):
    ...
    def __init__(self,
                 ...
                 c_type: str,
                 _uid: bool = False,
                 _uidtype: Optional[str] = None) -> None:
    ...
    def initargs(
        ...
        Union_[float, int, bool, str, None],
        str,
        bool,
        Optional[str]
    ]:
...
class VariableRef(HSMixin, LeafNode):
    ...
    def __init__(self, variable: "rql.nodes.Variable", noautoref: Optional[int] = None) -> None:
        LeafNode.__init__(self)  # don't care about Node attributes
...
class Referenceable(VisitableMixIn):
    ...
    def get_description(self,
                        mainindex: int,
                        tr: Callable[[str], str],
                        none_allowed: bool = False) -> Optional[str]:
        """return :
        ...
        for rel in self.stinfo['relations']:
            ...
            if mainindex is not None:
                ...
                if lhsvar is not None and mainindex in lhsvar.stinfo['selected']:
                    ...
                    # # Unexpected keyword argument "context"  [call-arg]
                    return tr(rtype, context=context)  # type:ignore[call-arg]
                if rhsvar is not None and mainindex in rhsvar.stinfo['selected']:
                    ...
                    if schema is not None and rschema.symmetric:
                        # # Unexpected keyword argument "context"  [call-arg]
                        return tr(rtype, context=context)  # type:ignore[call-arg]
                    return tr(rtype + '_object', context=context)  # type:ignore[call-arg]
            if rhsvar is self:
                rtype += '_object'
...
class ColumnAlias(Referenceable):
    __slots__: Iterable[str] = ('colnum', 'query',
...
class Variable(Referenceable):
    ...
    """
    __slots__: Iterable[str] = ('_q_invariant', '_q_sql', '_q_sqltable')  # XXX ginco specific

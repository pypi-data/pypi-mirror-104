...
from typing import (
    TYPE_CHECKING, Set, Optional, Mapping, Any, Callable, Generator, List, Union as Union_
)
...
def common_parent(
    node1: Optional["BaseNode"],
    node2: Optional["BaseNode"]
) -> "BaseNode":
...
class VisitableMixIn:
    def accept(self,
               visitor: Union_["rql.stcheck.RQLSTChecker",
                               "rql.stcheck.RQLSTAnnotator"],
               *args: Optional[Union_["rql.stcheck.STCheckState",
                                      "rql.stmts.Select"]],
               **kwargs: Optional[Any]) -> None:
        visit_id: str = self.__class__.__name__.lower()
        visit_method: Any = getattr(visitor, "visit_%s" % visit_id)
        return visit_method(self, *args, **kwargs)

    def leave(self,
              visitor: "rql.stcheck.RQLSTChecker",
              *args: Optional[Union_["rql.stmts.Select"]],
              **kwargs: Optional[Any]) -> None:
        visit_id: str = self.__class__.__name__.lower()
        visit_method: Any = getattr(visitor, "leave_%s" % visit_id)
        return visit_method(self, *args, **kwargs)

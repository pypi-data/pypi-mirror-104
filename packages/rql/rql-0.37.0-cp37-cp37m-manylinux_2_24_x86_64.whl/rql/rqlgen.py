...
nuppletsType = Sequence[Union[Tuple[str, str, str], Tuple[str, str, str, bool]]]
...
class RQLGenerator:
    ...
    def select(
         ...
         * `nupplets`: a list of 4-tuples (subject, relation, object, not).
    ...
    def where(self, nupplets: nuppletsType) -> str:
        ...
        'WHERE X work_for S , S name "Logilab" , X firstname F , X surname S'
        """
        result: List[str] = ['WHERE']
        total: int = len(nupplets)

        for i in range(total):
            nupplet = nupplets[i]

            # mypy error: Tuple index out of range  [misc]
            # we test the len before
            if len(nupplet) == 4 and nupplet[3] == NOT:  # type: ignore[misc]
                result.append('NOT')
    ...
    def groupby(self, groups: Sequence[str]) -> str:
    ...
    def sortby(self, sorts: Sequence[str]) -> str:
    ...
    def insert(self,
        ...
        """
        restrictions: List[str] = ['X %s "%s"' % (attr_name, attr_value)
    ...
    def update(
        self,
        etype: str,
        old_descr: Sequence[Tuple[str, str]],
        new_descr: Sequence[Tuple[str, str]],
    ) -> str:
        ...
        """
        old_restrictions: List[str] = ['X is "%s"' % etype]
        old_restrictions += ['X %s "%s"' % (attr_name, attr_value)
                             for attr_name, attr_value in old_descr]
        new_restrictions: List[str] = ['X %s "%s"' % (attr_name, attr_value)
...
RQLGENERATOR: RQLGenerator = RQLGenerator()

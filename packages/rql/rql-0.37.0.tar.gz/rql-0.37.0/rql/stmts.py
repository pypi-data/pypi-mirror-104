...
from typing import (
    TYPE_CHECKING, Dict, List, Union as Union_,
    Any, Iterable, Optional, cast, Callable,
    Set as Set_, Tuple, Iterator
)
...
def _check_references(
        defined: Dict[str, Union_["rql.nodes.Variable",
                                  "rql.nodes.ColumnAlias"]],
        varrefs: Iterable[Union_["rql.nodes.VariableRef", "rql.base.BaseNode"]]) -> bool:
    refs = {}
...
class ScopeNode(BaseNode):
    ...
    def set_where(self,
                  node: "rql.base.Node") -> None:
        self.where = node
        node.parent = self

    def set_having(self,
                   terms: Iterable["rql.base.Node"]) -> None:
    ...
    def copy(self,
             ...
             solutions: rt.SolutionsList = None) -> "rql.base.BaseNode":
        new = self.__class__()
    ...
    def get_variable(self,
                     name: str) -> Union_["rql.nodes.Variable",
                                          "rql.nodes.ColumnAlias"]:
    ...
    def set_possible_types(self,
                           ...
                           kwargs: Optional[Union_[object,
                                                   Dict[str, str]]] = _MARKER,
                           key: str = 'possibletypes') -> None:
    ...
    def check_references(self) -> bool:
        """test function"""
        try:
            defined = cast(Dict[str,
                                Union_["rql.nodes.ColumnAlias",
                                       "rql.nodes.Variable"]],
                           self.aliases.copy())
...
class Statement:
    ...
    def ored(
            ...
            traverse_scope:
            bool = False,
            _fromnode: Optional["rql.nodes.And"] = None
    ) -> Optional["rql.nodes.Or"]:
        return None

    def neged(
            self,
            traverse_scope: bool = False,
            _fromnode: Optional["rql.nodes.Or"] = None
...
class Union(Statement, Node):
    ...
    def get_description(
            ...
            tr: Optional[Callable[[str], str]] = None) -> List[List[str]]:
        """
        `mainindex`:
    ...
    def as_string(self, kwargs: Optional[Dict] = None) -> str:
        ...
        strings: List[str] = [select.as_string(kwargs=kwargs)
                              for select in self.children]
        if len(strings) == 1:
            return strings[0]
    ...
    def copy(self,
             stmt: Optional["rql.stmts.Statement"] = None,
             copy_children: bool = True) -> "rql.stmts.Union":
    ...
    def get_variable_variables(self) -> Set_[int]:
        ...
        for idx in self.get_variable_indices():
            ...
            first_child = cast("rql.stmts.Select", self.children[0])
            vrefs = first_child.selection[idx].iget_nodes(
                nodes.VariableRef
            )
    ...
    def _locate_subquery(self,
                         col: int,
                         etype: str,
                         kwargs: Optional[Dict[Any, Any]] = None) -> Tuple:
    ...
    def subquery_selection_index(self,
                                 subselect: Any,
                                 col: int) -> int:
    ...
    def check_references(self) -> bool:
        """test function"""
        for select in self.children:
            # BaseNone has no attribute check_references
            # classes ScopeNode, Union have check_references attribute
            select = cast("rql.stmts.ScopeNode", select)
    ...
    def append_select(self, select: "rql.stmts.Select") -> None:
        if self.should_register_op:
            ...
            self.undo_manager.add_operation(
                AppendSelectOperation(self, select)
            )
...
class Select(Statement, nodes.EditableMixIn, ScopeNode):
    ...
    def get_description(
            self,
            mainindex: int,
            tr: Callable[[str, str], str]) -> List[str]:
    ...
    def copy(self,
             stmt: Optional["rql.stmts.Statement"] = None,
             copy_solutions: bool = True,
             solutions: Optional[rt.SolutionsList] = None) -> "rql.stmts.Select":
    ...
    def set_possible_types(self,
        ...
        for ca in self.aliases.values():
            ...
            for stmt in ca.query.children[:]:
                ...
                sols: List = [sol for sol in stmt.solutions
    ...
    def set_statement_type(self, etype: str) -> None:
        ...
        if etype != 'Any':
            variables: List["rql.nodes.VariableRef"] = list(self.get_selected_variables())
            if not variables:
    ...
    def set_limit(self, limit: int) -> None:
        ...
        if self.should_register_op and limit != self.limit:
            ...
            self.undo_manager.add_operation(
                SetLimitOperation(self.limit, self)
            )
        self.limit = limit
    ...
    def set_groupby(self,
                    terms: List[Union_["rql.nodes.Function",
                                       "rql.nodes.VariableRef"]]) -> None:
        self.groupby = terms
        for node in terms:
            node.parent = self

    def set_with(self,
                 terms: List["rql.nodes.SubQuery"],
    ...
    def get_variable(self,
                     name: str,
                     colnum: Optional[int] = None) -> Union_["rql.nodes.Variable",
                                                             "rql.nodes.ColumnAlias"]:
        """get a variable instance from its name
    ...
    def clean_solutions(self,
                        solutions: Optional[rt.SolutionsList] = None) -> None:
    ...
    def replace(
            self,
            oldnode: "rql.base.BaseNode",
            newnode: "rql.base.BaseNode"
    ...
    remove_group_var = callable_deprecated(
        '[rql 0.29] use remove_group_term instead'
    )(remove_group_term)

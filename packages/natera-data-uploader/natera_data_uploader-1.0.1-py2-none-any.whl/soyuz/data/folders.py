...
from soyuz.data.files import WesDataFile, RawDataFile, QcDataFile, PersonalisDataFile
    ...
    def is_valid(self):
        ...
        for folder in os.listdir(self._path):
            ...
            if not os.path.isdir(self.__alignment_subfolder.format(folder)):
                return False
            if not os.path.isdir(self.__fastq_subfolder.format(folder)):
                return False
            if len(self.__list_from(self.__alignment_subfolder.format(folder))) == 0:
                ...
                return False
            if len(self.__list_from(self.__fastq_subfolder.format(folder))) == 0:
    ...
    def __list_qc_metrics(self, path):
        ...
        for f in os.listdir(path):
            if os.path.isfile(os.path.join(path, f)):
                result.append(QcDataFile(path, f, self))
        return result

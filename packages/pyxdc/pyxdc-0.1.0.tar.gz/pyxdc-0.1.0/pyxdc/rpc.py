...
def get_transaction(transaction_hash: str,
    ...
    new_transaction_dict: dict = {}
    web3: Web3 = Web3(provider=provider)
    transaction_dict: dict = web3.eth.get_transaction(transaction_hash).__dict__
    for key in transaction_dict.keys():
        if isinstance(transaction_dict[key], HexBytes):
            new_transaction_dict.setdefault(key, transaction_dict[key].hex())
        else:
            new_transaction_dict.setdefault(key, transaction_dict[key])

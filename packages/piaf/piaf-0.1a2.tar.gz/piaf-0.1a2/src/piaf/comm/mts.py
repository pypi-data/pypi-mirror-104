...
from piaf.comm import ACLMessage, Envelope, Message, Performative, ReceivedObject, AID
...
class MailBox:
    ...
    def _lookup_for_matching_message(
        ...
        doesn't take the lock so callers must lock the call themself.
        Template is applied on the :class:`ACLMessage`.
        """
...
class MailBoxDelegate:
    ...
    def __init__(self, delegate: "piaf.comm.mts.Mailbox") -> None:
        """
        Create a new :class:`MailBoxDelegate` object.

        :param delegate: agent's mailbox
        """
        self._delegate: "piaf.comm.mts.MailBox" = delegate
...
class AgentCommunicationChannel:
    """
    :class:`AgentCommunicationChannel` is an entity providing the mesaaging service to the AP agents.
    ...
    def __init__(self, ptf: "AgentPlateform") -> None:
        """Create a new ACC without any registered agent or service."""
        self._knows: Dict[str, MailBox] = {}
        self._tasks: List[Future] = []
        self._ptf: "AgentPlateform" = ptf
        self._mtps: Dict[str, MessageTransferProtocol] = {}
    ...
    def register_mtp(self, mtp: "MessageTransferProtocol") -> None:
        """
        Register a new MTP.
    ...
    def forward(self, msg: "Message", context: "MessageContext" = None):
        ...
        Rules here.
        """
        task = asyncio.ensure_future(self._handle_message(msg, context))
    ...
    async def stop(self) -> None:
        ...
        This will cancel all running tasks.
        """
    ...
    async def _fill_received_field(
        ...
        rcv_obj = ReceivedObject(
            by=by, date=datetime.now(), id=await AgentCommunicationChannel._next_id(),
        )
    ...
    async def _send_via_mts(self, aid: "AID", msg: "Message"):
        ...
        for address in aid.addresses:
            ...
            if scheme in self._mtps:
                success = await self._mtps[scheme].send(msg)

            if success:
                break

        # If still not successfull, try name resolution
        # Not implemented yet
        if not success:
            pass
    ...
    async def _send_failure_message(self, msg: "Message") -> None:
        ...
        try:
            reply_msg = (
                ...
                .in_reply_to(msg.acl_message.reply_with)
...
class MessageTransferProtocol(metaclass=abc.ABCMeta):
    """
    ...
    def __init__(self, acc: "AgentCommunicationChannel") -> None:
        """
        Default constructor asking for an ACC.

        It stores the ACC reference into an attribute for later use. It should also start the MTP. After construction,
        the MTP is expected to work, ie start listening on address [scheme]://[hostname]:[port]/acc.
        It is up to the concrete implementation to ask the user the net par of the URL.
        """
        self.acc = acc

    @abc.abstractmethod
    async def send(self, message: "piaf.comm.Message") -> "bool":
        """

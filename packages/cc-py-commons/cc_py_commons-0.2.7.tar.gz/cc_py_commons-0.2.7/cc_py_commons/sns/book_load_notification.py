...
class BookLoadNotification:
  ...
  def send(self, load_id, user_id):
    ...
    logger.debug(f"sending messsage {message} to {self._app_config.BOOK_LOAD_SNS_TOPIC_ARN}")
    ...
    try:
      ...
      logger.debug(f"failed to send message to {self._app_config.BOOK_LOAD_SNS_TOPIC_ARN}: {message}")

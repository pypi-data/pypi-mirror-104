...
import gihadocs
from gihadocs import config
from gihadocs.config import config_options
from gihadocs.exceptions import ConfigurationError
from gihadocs.tests.base import dedent
...
class ConfigTests(unittest.TestCase):
    ...
    def test_config_option(self):
        ...
        Allows users to specify a config other than the default `gihadocs.yml`.
        """
        expected_result = {
            'site_name': 'Example',
            'pages': [
        ...
        with TemporaryDirectory() as temp_path:
            ...
            config_path = os.path.join(temp_path, 'gihadocs.yml')
    ...
    def test_theme(self):
        with TemporaryDirectory() as mytheme, TemporaryDirectory() as custom:
            ...
            gihadocs_dir = os.path.abspath(os.path.dirname(gihadocs.__file__))
            gihadocs_templates_dir = os.path.join(gihadocs_dir, 'templates')
            theme_dir = os.path.abspath(os.path.join(gihadocs_dir, 'themes'))

            results = (
                {
                    'dirs': [os.path.join(theme_dir, 'gihadocs'), gihadocs_templates_dir],
                    ...
                    'dirs': [os.path.join(theme_dir, 'readthedocs'), gihadocs_templates_dir],
                    'static_templates': ['404.html', 'sitemap.xml'],
                    'vars': {
                    ...
                    'dirs': [mytheme, gihadocs_templates_dir],
                    'static_templates': ['sitemap.xml'],
                    'vars': {}
                }, {
                    'dirs': [custom, os.path.join(theme_dir, 'readthedocs'), gihadocs_templates_dir],
                    'static_templates': ['404.html', 'sitemap.xml'],
                    'vars': {
                        'include_search_page': True,
                        'search_index_only': False,
            ...
            for config_contents, result in zip(configs, results):
                ...
                c = config.Config(schema=(('theme', config_options.Theme(default='gihadocs')),))
    ...
    def test_empty_nav(self):
        ...
        conf.load_dict({
            'site_name': 'Example',
            'config_file_path': os.path.join(os.path.abspath('.'), 'gihadocs.yml')
        })
        conf.validate()
    ...
    def test_doc_dir_in_site_dir(self):
        ...
        conf = {
            'config_file_path': j(os.path.abspath('..'), 'gihadocs.yml')
        }

        ...
        if nav_df is not None:
            ...
            if not adj_nav.empty:
                ...
                for row in adj_nav.itertuples():
                    fof_nav_to_set = db_session.query(FOFNav).filter(FOFNav.manager_id == row.manager_id, FOFNav.fof_id == row.Index, FOFNav.datetime == row.datetime).one_or_none()

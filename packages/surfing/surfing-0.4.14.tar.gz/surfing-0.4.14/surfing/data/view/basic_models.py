...
class FOFInfo(FOFBase):
    ...
    subscription_fee = Column(DOUBLE(asdecimal=False))  # 认购费率
...
class EMailParserInfo(FOFBase):
    ...
    manager_id = Column(CHAR(32), primary_key=True)  # 管理端ID
    p_id = Column(CHAR(16), primary_key=True)  # parser ID
    p_desc = Column(TEXT)                      # parser 描述
    p_type = Column(Integer)                   # parser 类型
    is_valid = Column(BOOLEAN)                 # parser 是否生效

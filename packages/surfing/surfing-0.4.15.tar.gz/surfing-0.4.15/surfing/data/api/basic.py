...
class BasicDataApi(metaclass=Singleton):
    ...
    def get_email_parser_info(self, manager_id: str, p_id_list: Tuple[str] = ()):
        with BasicDatabaseConnector().managed_session() as db_session:
            try:
                query = db_session.query(
                    EMailParserInfo,
                ).filter(
                    EMailParserInfo.manager_id == manager_id,
                )
                if p_id_list:
                    query = query.filter(
                        EMailParserInfo.p_id.in_(p_id_list),
                    )
                return pd.read_sql(query.statement, query.session.bind)
            except Exception as e:
                print(f'failed to get_email_parser_info <err_msg> {e} from {EMailParserInfo.__tablename__}')

    def get_email_parser_code(self, manager_id: str, p_id_list: Tuple[str] = ()):
        with BasicDatabaseConnector().managed_session() as db_session:
            try:
                query = db_session.query(
                    EMailParserCode,
                ).filter(
                    EMailParserCode.manager_id == manager_id,
                )
                if p_id_list:
                    query = query.filter(
                        EMailParserCode.p_id.in_(p_id_list),
                    )
                return pd.read_sql(query.statement, query.session.bind)
            except Exception as e:
    ...
    def get_product_details(self):
        try:
            ...
            '''
            fund_info = RawDataApi().get_hf_fund_info()
            miss_df = RawDataApi().get_hf_fund_nav_miss()
            fund_info = fund_info[~fund_info.fund_id.isin(miss_df.fund_id)]
            asset_list = fund_info.stg_type.unique().tolist()
            result = {}
            for asset_id in asset_list:
                result[asset_id] = []
                _df = fund_info[fund_info.stg_type == asset_id]
                for r in _df.itertuples():
                    result[asset_id].append([r.fund_id, r.desc_name])
            df = self.get_asset_info()
            df2 = df[df['asset_name'].isin(['商品','大盘股','中证500','创业板指'])]
    ...
    def get_asset_price_dt(self, asset_list, begin_date, end_date):
        try:
            ...
            if len(_asset_list_3) > 0:
                real_ids = [asset_dic[i] for i in _asset_list_3]
                _df = raw.get_hf_fund_nav(fund_ids=real_ids,start_date=_begin_date,end_date=end_date).pivot_table(index='datetime',columns='fund_id',values='nav')
                result.append(_df)
            if len(_asset_list_4) > 0:
                real_ids = [asset_dic[i] for i in _asset_list_4]
            ...
            if len(_asset_list_5) > 0:
                real_ids = [asset_dic[i] for i in _asset_list_5]
                _df = self.get_index_price_dt(index_list=real_ids,start_date=_begin_date,end_date=end_date).pivot_table(index='datetime',columns='index_id',values='close')
                result.append(_df)
            ...
            print(f'failed to asset price <err_msg> {e} from basic.get_asset_price_dt')
            return pd.DataFrame()
    ...
    def get_product_price(self, begin_date, end_date, time_para, product_list: Tuple[str] = (),):
        try:
            begin_date, end_date = RawDataApi().get_date_range(time_para, begin_date, end_date)
            raw = RawDataApi()
            hf_info = raw.get_hf_fund_info(product_list)
            name_dic = hf_info.set_index('fund_id')['desc_name'].to_dict()
            ...
            if not asset_info.empty:
                ...
                for k, v in _name_dic.items():
                    name_dic[k] = v
            _df = hf_info.rename(columns={'fund_id':'real_id','stg_type':'asset_name','desc_name':'real_name'})[['real_id','asset_name','real_name']]
            _df.loc[:,'asset_type'] = '典型产品'
            asset_info = asset_info.append(_df).drop_duplicates('real_id')
            result = []
            real_ids = product_list
            _df = raw.get_hf_fund_nav(fund_ids=real_ids,start_date=begin_date,end_date=end_date)
            if not _df.empty > 0:
                _df = _df.pivot_table(index='datetime',columns='fund_id',values='nav')
                result.append(_df)
            _df = self.get_index_price_dt(index_list=real_ids,start_date=begin_date,end_date=end_date)
            if not _df.empty > 0:
    ...
    def product_recent_rate(self, product_list, year:int=None, weekly:bool=False):
        try:
            ...
            result = []
            real_ids = product_list
            _df = raw.get_hf_fund_nav(fund_ids=real_ids,start_date='20091231')
            if not _df.empty > 0:
                _df = _df.pivot_table(index='datetime',columns='fund_id',values='nav')
                result.append(_df)
            _df = self.get_index_price_dt(start_date='20091231',index_list=real_ids)
            if not _df.empty > 0:
                _df = _df.pivot_table(index='datetime',columns='index_id',values='close')
                result.append(_df)
    ...
    def get_rzrqye(self, begin_date, end_date, time_para):
        try:
            ...
            df = raw.get_em_macroeconomic_daily(codes=asset_info.em_codes.tolist(),start_date=begin_date,end_date=end_date)
            df = df.pivot_table(index='datetime',columns='codes',values='value')

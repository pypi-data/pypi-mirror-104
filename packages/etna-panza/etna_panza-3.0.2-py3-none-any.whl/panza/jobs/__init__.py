...
from .errors import *
from ._directory_layout import WorkspaceLayout, EXECUTOR_WORKSPACE_ROOT
...
class ExecutableJobHandle:
    ...
    def _configure_job_directory(self, context: Dict[str, Any]):
        os.mkdir(self.workspace.work_directory)

        with open(self.workspace.context_file, 'w') as context_file:
    ...
    def _launch_additional_docker_daemon(self):
        ...
        try:
            ...
            _LOGGER.error(f"Unable to execute inspection phase for job {os.path.basename(self.workspace.root)}")
            raise
        return dockerd
    ...
    def _get_launch_command(self, name: str, with_docker_daemon: DockerDaemon = None):
        volumes = [(self.workspace.root, EXECUTOR_WORKSPACE_ROOT)]
    ...
    def _retrieve_execution_result(self) -> Dict[str, Any]:
        try:
            with open(self.workspace.result_file, 'r') as result_file:
...
class DataAwaitingJobHandle:
    ...
    def __init__(self, config: PanzaConfiguration, workspace: WorkspaceLayout, env_name: str, blueprint: quixote.Blueprint):
        self.config = config
        self.workspace = workspace
        self.env_name = env_name
        self.blueprint = blueprint
    ...
    def fetch_data(self, context: Dict[str, Any], *, cache: Cache) -> ExecutableJobHandle:
        ...
        if not cache.has_entry(entry_name):
            with cache.add_entry(entry_name) as path:
                ...
                with augment_syspath([self.workspace.root]):
        ...
        return ExecutableJobHandle(self.config, self.workspace, self.env_name, self.blueprint)
...
class JobWorkspaceHandle:
    ...
    def __init__(self, config: PanzaConfiguration, workspace: WorkspaceLayout, blueprint: quixote.Blueprint):
        self.config = config
        self.workspace = workspace
        self.blueprint = blueprint
    ...
    def _generate_build_file(self):
        with augment_syspath([self.workspace.root]):
            ...
            generator.generate_to_file(
                self.blueprint,
                os.path.join(self.workspace.root, "Dockerfile"),
                extra_context={"resources_path": self.workspace.resources_directory}
            )

    def _cleanup_build_file(self):
        _LOGGER.debug("Cleaning up after building...")
        os.remove(os.path.join(self.workspace.root, "Dockerfile"))

    def _get_build_command(self, env_name: str):
        extra_build_args = []
        if self.config.always_pull is True:
            extra_build_args.append("--pull")
        return ["docker", "build", *extra_build_args, "-t", env_name, self.workspace.root]
    ...
    def build_job_environment(self, env_name: str) -> DataAwaitingJobHandle:
        ...
        return DataAwaitingJobHandle(self.config, self.workspace, env_name, self.blueprint)
...
def new_job_workspace(config: PanzaConfiguration, job_files_dir: str, job_name: str) -> JobWorkspaceHandle:
    ...
    workspace = WorkspaceLayout(root=os.path.normpath(os.path.join(config.root_directory, job_name)))
    _LOGGER.info(f"Creating workspace {workspace.root} for job {job_name}...")

    try:
        shutil.copytree(job_files_dir, workspace.moulinette_directory)
    except OSError as e:
    ...
    try:
        with augment_syspath([workspace.moulinette_directory]):
            with scoped_module_imports():
                blueprint = panza.BlueprintLoader.load_from_directory(workspace.moulinette_directory)
    except BlueprintLoadError as e:
        shutil.rmtree(workspace.root)
        raise WorkspaceCreationError(e)

    return JobWorkspaceHandle(config, workspace, blueprint)

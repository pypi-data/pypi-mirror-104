...
class Point():
        ...
        for i in range(3):
            if not isinstance(point[i], (int, float)):
                raise TypeError(" ".join((
                    "invalid component type for 'Point':",
                    "must be 'int' / 'float'"
                )))
    ...
    def __str__(self, /):
        ...
        point = list(self.tuple)
        sign, plus, minus = ["", "", ""], "", "- "

        for i in range(3):
            sign[i] = plus if point[i] >= 0 else minus
            point[i], plus = abs(point[i]), "+ "

        return f"{sign[0]}{point[0]}î {sign[1]}{point[1]}ĵ {sign[2]}{point[2]}k̂"
    ...
    @staticmethod
    def plot_points(*points, show_legend=True):
...
class Line():
    ...
    def __bool__(self, /):
        ...
        return self.b.unit() not in {Vector.i, Vector.j, Vector.k}
    ...
    def __getitem__(self, k):
        ...
        indices for a Line must be int / float
        returns the Point on the Line at index 𝜿 -> a⃗ + 𝜿b⃗"""
    ...
    @classmethod
    def FromAngles(cls, alpha, beta, gamma, /, *, point=Point(0, 0, 0)):
    ...
    @classmethod
    def From2Points(cls, point1, point2, /):
    ...
    @staticmethod
    def plot_lines(*lines, show_legend=True):
    ...
    def directionratios(self, /, *, k=1):
        """returns a tuple containing direction ratios of the Line
        k is an int / float by which the DRs are scaled"""
    ...
    def get_points(self, k1, k2, /, *, step=1):
        """returns a tuple of Points on the Line from index k1 to index k2
        (including k1 and excluding k2) separated by step (int / float)
        analogous to:
            index in range(k1, k2, step)
        see help(__getitem__) for help on valid indices"""
...
class Plane():
    ...
    def __init__(self, n, d, /):
        ...
        d is the constant for the Plane in the equation (r⃗ • n⃗ = d)
        attrs of Plane Object:
            • self.n = n
            • self.d = d (can be any Real number)
    ...
    def __bool__(self, /):
        ...
        return self.n.unit() not in {Vector.i, Vector.j, Vector.k}
    ...
    def __contains__(self, item, /):
        ...
        left operand item can be a Point / Line
        returns True if given item lies on / in the Plane, False otherwise"""
    ...
    @classmethod
    def From3Points(cls, point1, point2, point3, /):
    ...
    @classmethod
    def From2Lines(cls, line1, line2, /):
    ...
    @classmethod
    def From2Planes(cls, plane1, plane2, /, *, point=Point(0, 0, 0)):
    ...
    @classmethod
    def FromLinePoint(cls, *, line, point):
    ...
    @staticmethod
    def plot_planes(*planes, show_legend=True):
...
def plot_linesegment(point1, point2, /):
...
def plot_objects(*objects, show_legend=True, plot_intersections=False):
    """plots all given 3D - objects in the same plot and displays it
    objects can be of type Point, Line, Plane, Vector and / or Line Segment
...
def plot_random_objects(*,
    points=1, lines=0, planes=0, vectors=0, linesegs=0, show_legend=True
    ):
    """plots given number of random Points, Lines, Planes, Vectors and
    Line Segments on a 3D plot
    show_legend is a boolean which displays legend on the plot if True"""

    total = sum([points, lines, planes, vectors, linesegs])

    if not total:
        return
    else:
        from random import randint, choice
        numbers = [randint(-15, 15) for _ in range(total*30)]

    objects = [
        *(
            Point(choice(numbers), choice(numbers), choice(numbers))
        ...
        *(
            Line(
                Point(choice(numbers), choice(numbers), choice(numbers)),
                Vector.FromSequence(
                    [choice(numbers), choice(numbers), choice(numbers)]
                )
            )
        ...
        *(
            ...
            for _ in range(linesegs)
        ),
    ]

    plot_objects(*objects, show_legend=show_legend)
...
def isclose(point1, point2, /, *, rel_tol=1e-09, abs_tol=0.0):
...
def section(point1, point2, /, m, n, *, external=False):
...
def intersection(obj1: (Line, Plane), obj2: (Line, Plane), obj3=None, /):
    """returns the 3D object of intersection of the given 3D objects
    objects 1 an 2 can be of type Line and / or Plane
    object3, if given along with two Planes, must be a Plane
...
def image(obj1: (Point, Line, Plane), obj2: (Line, Plane), /):
    """returns the image of 3D - object1 taking object2 as a mirror
    object1 can be of type Point, Line or Plane
...
def projection(obj1: (Point, Line), obj2: (Line, Plane), /):

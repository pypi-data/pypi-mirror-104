...
class TermTrend():
    ...
    def __init__(self, df_tweets):
        ...
        TWEETS_COLUMNS = [
            'twitter_id', 'screen_name', 'created_at',
            'text', 'ac_followers_count', 
            'user_mentions', 'profile_image', 'engagement_rate_by_post'
            ]
        ...
        try:
            ...
            if not df_tweets.empty:
                df_tweets_in['created_at'] = pd.to_datetime(df_tweets_in['created_at']) #revisar con back si se tiene que hacer corrección horaria
    ...
    def time_series(self, level='hour'):
        ...
        try:
            ...
            df_tweets_out['created_at'] = df_tweets_out['created_at'].apply(lambda dt: datetime(year=dt.year, 
                                                                                                month=dt.month,
                                                                                                day=dt.day, 
                                                                                                hour=dt.hour))
            df_tweets_out = df_tweets_out.groupby('created_at').count().reset_index().sort_values('created_at')
    ...
    def mentions_network(self, n_most_posts=None, min_group=10):
        """
        ...
        try:
            df_tweets_out = deepcopy(self.df_tweets_in[['twitter_id', 'screen_name', 'user_mentions', 'text', 'ac_followers_count', 'profile_image']])
            ...
            data_nx = data_nx.sort_values('text', ascending=False).reset_index().head(n_most_posts)
            
            #Nodes with posts in the input DataFrame
            nodes_ini = data_nx[['twitter_id', 'screen_name', 'text', 'ac_followers_count', 'profile_image']]
            nodes_ini = nodes_ini.rename(columns={'twitter_id': 'id', 'screen_name': 'label', 'text': 'value', 'profile_image': 'image'})
            nodes_ini['title'] = nodes_ini['ac_followers_count'].apply(lambda fw: f'Followers: {fw}')
            ...
            nodes_fin = pd.DataFrame({'id': id_list, 'label': list(ment_list.keys()), 'value': list(ment_list.values())})
            ...
            to_number = edges.groupby(['from']).agg(
                {
                'to': lambda x: ','.join(x),
                'value': 'count'
                }
            ...
            one_to_edges = list(set(one_to_edges) - set(edges[edges['to'].isin(one_to_edges)]['to']))

            group_edges_from = edges[edges['from'].isin(one_to_edges)].groupby('to').agg(
                {
                    'value': 'count',
            ...
            if not new_nodes_from.empty:
                new_nodes_from['label'] = new_nodes_from.apply(lambda row: f'{row["value"]} usuarios', axis=1)
                new_nodes_from['title'] = new_nodes_from.apply(lambda row: f'Se agruparon: {row["value"]} usuarios', axis=1)
                new_nodes_from['image'] = 'group_from'
                new_nodes_from['id'] = new_nodes_from.apply(lambda row: f'{row["value"]}_a_{row["to"]}', axis=1)
                ...
                new_edges['to'] = new_edges['id'].apply(lambda index: index.split('_', 2)[2])
            ...
            from_number = edges.groupby(['to']).agg(
                {
                'from': lambda x: ','.join(x),
                'value': 'count'
                }
            ...
            one_from_edges = list(set(one_from_edges) - set(edges[edges['from'].isin(one_from_edges)]['from']))

            group_edges_to = edges[edges['to'].isin(one_from_edges)].groupby('from').agg(
                {
                    'value': 'count',
                    'to': lambda x: ','.join(x)
                }
            ...
            if not new_nodes_to.empty:
                new_nodes_to['label'] = new_nodes_to.apply(lambda row: f'{row["value"]} usuarios', axis=1)
                new_nodes_to['title'] = new_nodes_to.apply(lambda row: f'Se agruparon: {row["value"]} usuarios', axis=1)
                new_nodes_to['image'] = 'group_to'
                new_nodes_to['id'] = new_nodes_to.apply(lambda row: f'{row["from"]}_a_{row["value"]}', axis=1)
                ...
                new_edges['from'] = new_edges['id'].apply(lambda index: index.split('_')[0])
    ...
    def accounts(self, engagement_rate='engagement_rate_by_post'):
        ...
        try:
            df_tweets_out = deepcopy(self.df_tweets_in[['screen_name', engagement_rate]])
            df_tweets_out['posts_count'] = 1
            df_tweets_out = df_tweets_out.groupby('screen_name').agg({
                'posts_count': 'count',

...
class STTMgroupsIG():
    ...
    def __init__(self, df_comments, account_ids=[], account_names=[]):
        ...
        if account_ids:
            self.df_comments_full = self.df_comments_full[self.df_comments_full.post_owner_id.isin(
                account_ids)]
        elif account_names:
    ...
    def get_sttm_groups(self, list_remove=[], min_texts=100):
        ...
        try:
            ...
            if 'processed_text' not in self.df_comments_full.keys():
                self.df_comments_full['processed_text'] = self.df_comments_full['text'].apply(
    ...
    def message_to_group_columns(self):
        ...
        if '__date_cot' not in data_texts.keys():
            data_texts['__date_cot'] = pd.to_datetime(
                data_texts['created_at_utc']) - timedelta(hours=5)

        if 'post_owner_username' in data_texts.keys():
            data_texts = data_texts[[
                'sttm_group', 'tokenized_text', 'post_owner_username', '__date_cot', 'polarity'
            ]]
            data_texts = data_texts.rename(columns={'post_owner_username': '__mention'})
        else:
            data_texts = data_texts[['sttm_group', 'tokenized_text', '__date_cot', 'polarity']]

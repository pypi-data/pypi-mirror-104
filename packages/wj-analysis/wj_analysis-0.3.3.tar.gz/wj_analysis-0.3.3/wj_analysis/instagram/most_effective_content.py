...
class MostEffectiveContentIG():
    ...
    def __init__(self, df_posts, groups):
        ...
        POSTS_COLUMNS = [
            'owner_id', 'owner_username', 'shortcode', 'caption', 'typename', 'caption_hashtags',
    ...
    def posts(self, n_most_eff=5, engagement_rate='engagement_rate_by_post', **kwargs):
        ...
        **kwargs:
        ...
        if 'account_ids' in kwargs.keys() and kwargs['account_ids']:
            ...
            df_most_eff_posts_ig = self.df_posts_full[self.df_posts_full.owner_id.isin(
                account_ids)][[
                    'owner_username', 'shortcode', 'caption', engagement_rate,
                    'rel_' + engagement_rate
                ]]
        elif 'account_names' in kwargs.keys() and kwargs['account_names']:
            account_names = kwargs['account_names']
            df_most_eff_posts_ig = self.df_posts_full[self.df_posts_full.owner_username.isin(
                account_names)][[
                    'owner_username', 'shortcode', 'caption', engagement_rate,
                    'rel_' + engagement_rate
                ]]
            ...
            df_most_eff_posts_ig = deepcopy(self.df_posts_full)[[
                'owner_username', 'shortcode', 'caption', engagement_rate, 'rel_' + engagement_rate
            ]]
        try:
            ...
            return pd.DataFrame(
                columns=[
                    'owner_username', 'shortcode', 'caption', engagement_rate, 'rel_' +
                    engagement_rate, 'url'
                ])
    ...
    def posts_type(
        ...
        **kwargs:
        ...
        if 'account_ids' in kwargs.keys() and kwargs['account_ids']:
            account_ids = kwargs['account_ids']
            df_most_eff_posts_type_ig = self.df_posts_full[self.df_posts_full.owner_id.isin(
                account_ids)][PAGE_COLUMNS +
                              ['typename', 'caption', engagement_rate, 'rel_' + engagement_rate]]
        elif 'account_names' in kwargs.keys() and kwargs['account_names']:
            account_names = kwargs['account_names']
            df_most_eff_posts_type_ig = self.df_posts_full[self.df_posts_full.owner_username.isin(
        ...
        try:
            ...
            if PAGE_COLUMNS:
                df_most_eff_posts_type_ig['percentage'] = df_most_eff_posts_type_ig[
                    'counts'].groupby(level=0).apply(lambda c: 100. * c / float(c.sum())).round(2)
                df_most_eff_posts_type_ig = df_most_eff_posts_type_ig.reset_index().rename(
                    columns={
                ...
                df_most_eff_posts_type_ig['percentage'] = (
                    100. * df_most_eff_posts_type_ig['counts'] /
                    df_most_eff_posts_type_ig['counts'].sum()).round(2)
            ...
            return pd.DataFrame(
                columns=PAGE_COLUMNS + [
                    'typename', 'counts', 'avg_engagement_rate', 'avg_rel_engagement_rate',
                    'percentage'
                ])
    ...
    def construct_most_eff_objects(
        ...
        try:
            ...
            for _, row in df_most_eff_objects_ig.iterrows():
                ...
                objs_rel_eff_ig = objs_rel_eff_ig + [row['rel_' + engagement_rate]] * len(
                    row['caption_' + objs])
    ...
    def hashtags_mentions(
        ...
        **kwargs:
        ...
        if 'account_ids' in kwargs.keys() and kwargs['account_ids']:
            account_ids = kwargs['account_ids']
            df_most_eff_hashtags_mentions_ig = self.df_posts_full[self.df_posts_full.owner_id.isin(
                account_ids)][PAGE_COLUMNS + [
                    'caption_hashtags', 'caption_mentions', 'group', engagement_rate, 'rel_' +
                    engagement_rate
                ]]
        elif 'account_names' in kwargs.keys() and kwargs['account_names']:
            account_names = kwargs['account_names']
            df_most_eff_hashtags_mentions_ig = self.df_posts_full[
                self.df_posts_full.owner_username.isin(account_names)][PAGE_COLUMNS + [
                    'caption_hashtags', 'caption_mentions', 'group', engagement_rate, 'rel_' +
                    engagement_rate
                ]]
        else:
        ...
        df_compare_hashtags_mentions_ig = self.df_posts_full[
            self.df_posts_full.group == compare_group][PAGE_COLUMNS + [
                'caption_hashtags', 'caption_mentions', 'group', engagement_rate, 'rel_' +
                engagement_rate
            ]]

        try:
            df_most_eff_hashtags_mentions_ig['caption_hashtags'] = df_most_eff_hashtags_mentions_ig[
                'caption_hashtags'].apply(lambda hashtags: ast.literal_eval(hashtags))
            df_most_eff_hashtags_mentions_ig['caption_hashtags'] = df_most_eff_hashtags_mentions_ig[
                'caption_hashtags'].apply(lambda hashtags: hashtags if hashtags else None)

            df_most_eff_hashtags_mentions_ig['caption_mentions'] = df_most_eff_hashtags_mentions_ig[
                'caption_mentions'].apply(lambda mentions: ast.literal_eval(mentions))
            df_most_eff_hashtags_mentions_ig['caption_mentions'] = df_most_eff_hashtags_mentions_ig[
                'caption_mentions'].apply(lambda mentions: mentions if mentions else None)

            df_compare_hashtags_mentions_ig['caption_hashtags'] = df_compare_hashtags_mentions_ig[
                'caption_hashtags'].apply(lambda hashtags: ast.literal_eval(hashtags))
            df_compare_hashtags_mentions_ig['caption_hashtags'] = df_compare_hashtags_mentions_ig[
                'caption_hashtags'].apply(lambda hashtags: hashtags if hashtags else None)

            df_compare_hashtags_mentions_ig['caption_mentions'] = df_compare_hashtags_mentions_ig[
                'caption_mentions'].apply(lambda mentions: ast.literal_eval(mentions))
            df_compare_hashtags_mentions_ig['caption_mentions'] = df_compare_hashtags_mentions_ig[
                'caption_mentions'].apply(lambda mentions: mentions if mentions else None)

            df_most_eff_hashtags_mentions_ig = df_most_eff_hashtags_mentions_ig.sort_values(
                engagement_rate, ascending=False).head(from_n_most_eff)
            df_compare_hashtags_mentions_ig = df_compare_hashtags_mentions_ig.sort_values(
                engagement_rate, ascending=False).head(from_n_most_eff)
            #Hashtags
    ...
    def construct_most_eff_words(
        ...
        try:
            ...
            for _, row in df_most_eff_words_ig.iterrows():
                ...
                words_rel_eff_ig = words_rel_eff_ig + [row['rel_' + engagement_rate]] * len(
                    row.words)
    ...
    def words(
        ...
        **kwargs:
        ...
        if 'account_ids' in kwargs.keys() and kwargs['account_ids']:
            account_ids = kwargs['account_ids']
            df_most_eff_words_ig = self.df_posts_full[self.df_posts_full.owner_id.isin(
                account_ids)][PAGE_COLUMNS +
                              ['caption', 'group', engagement_rate, 'rel_' + engagement_rate]]
        elif 'account_names' in kwargs.keys() and kwargs['account_names']:
            account_names = kwargs['account_names']
            df_most_eff_words_ig = self.df_posts_full[self.df_posts_full.owner_username.isin(
                account_names)][PAGE_COLUMNS +
                                ['caption', 'group', engagement_rate, 'rel_' + engagement_rate]]
        else:
        ...
        try:
            ...
            if 'processed_text' not in df_most_eff_words_ig.keys():
                df_most_eff_words_ig['processed_text'] = df_most_eff_words_ig.caption.apply(
                    lambda msg: CleanText(msg).process_text(
                        mentions=True, hashtags=True, links=True, spec_chars=True))
            df_most_eff_words_ig['processed_text'] = df_most_eff_words_ig['processed_text'].apply(
                lambda txt: self.features.pos_tags(txt))

            df_most_eff_words_ig['words'] = df_most_eff_words_ig['processed_text'].apply(
                lambda pt: pt['words'])
            df_most_eff_words_ig['lemmas'] = df_most_eff_words_ig['processed_text'].apply(
                lambda pt: pt['lemmas'])
            df_most_eff_words_ig['pos_tags'] = df_most_eff_words_ig['processed_text'].apply(
                lambda pt: pt['pos_tags'])

            df_most_eff_words_ig = df_most_eff_words_ig[~df_most_eff_words_ig['processed_text'].
                                                        isna()]

            most_eff_words_ig, most_eff_nouns_ig, most_eff_verbs_ig, most_eff_adjs_ig = self.construct_most_eff_words(
                df_most_eff_words_ig,
                lemmatize=lemmatize,
            ...
            return most_eff_words_ig, most_eff_nouns_ig[show_columns], most_eff_verbs_ig[
                show_columns], most_eff_adjs_ig[show_columns]

...
class PolarityDistributionIG():
    ...
    def __init__(self, df_comments, groups):
        """
    ...
    def grouped_polarities(self, group_by='group'):
        ...
        try:
            if 'post_owner_id' in self.df_comments_full.keys():
                self.df_comments_full['group'] = self.df_comments_full['post_owner_id'].apply(
        ...
        try:
            if (group_by == 'account'):
                df_groupedpolarity = self.df_comments_full.groupby(['post_owner_id', 'post_owner_username', 'group'])\

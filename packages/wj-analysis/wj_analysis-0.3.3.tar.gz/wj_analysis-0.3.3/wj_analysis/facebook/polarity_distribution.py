...
class PolarityDistributionFB():
    ...
    def __init__(self, df_comments, df_pages, groups):
        """
    ...
    def grouped_polarities(self, group_by='group'):
        ...
        try:
            if (group_by == 'account'):
                df_groupedpolarity = self.df_comments_full.groupby(['page_id', 'page_name', 'group'])\
                                          ['polarity'].value_counts().unstack().fillna(0)
                df_groupedpolarity['sentiment'] = df_groupedpolarity.to_dict(orient='records')
                df_groupedpolarity = df_groupedpolarity.reset_index().rename(
                    columns={
                        'page_id': '_object_id',
                        'page_name': '_object_name'
                    })
                return df_groupedpolarity[['_object_id', '_object_name', 'group', 'sentiment']]
            elif (group_by == 'group'):
                df_groupedpolarity = self.df_comments_full.groupby(
                    'group')['polarity'].value_counts().unstack().fillna(0)
                df_groupedpolarity['sentiment'] = df_groupedpolarity.to_dict(orient='records')
                return df_groupedpolarity.reset_index()[['group', 'sentiment']]
            elif (group_by == 'all'):
                df_groupedpolarity = self.df_comments_full.groupby(
                    'all')['polarity'].value_counts().unstack().fillna(0)
                df_groupedpolarity['sentiment'] = df_groupedpolarity.to_dict(orient='records')
            ...
            if (group_by == 'account'):
                ...
                df_groupedpolarity = pd.DataFrame(columns=['all', 'sentiment'])
            return self.df_groupedpolarity

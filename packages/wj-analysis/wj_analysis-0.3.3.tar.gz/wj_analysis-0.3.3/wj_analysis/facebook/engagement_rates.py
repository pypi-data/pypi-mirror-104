...
class EngagementRateFB():
    ...
    def __init__(self, df_posts, df_pages, groups):
        ...
        POSTS_COLUMNS = [
            "page_id", "created_time", "post_id", "likes", "reactions_love", "reactions_wow",
            "reactions_haha", "reactions_sad", "reactions_angry", "reactions_thankful", "shares",
            "comments", "message", "permalink_url", "type", "message_tags"
        ]
        OUTPUT_COLUMNS = [
            "fan_count", "page_id", "page_name", "date", "group", "created_time", "post_id",
            "likes", "reactions_love", "reactions_wow", "reactions_haha", "reactions_sad",
            "reactions_angry", "reactions_thankful", "shares", "comments", "message",
            "permalink_url", "type", "message_tags"
        ]
        ...
        if len(df_posts) > 0 and len(df_pages) > 0:
            try:
                ...
                df_posts_full['created_time'] = pd.to_datetime(
                    df_posts_full['created_time']) - timedelta(hours=5)
                ...
                df_posts_full['fan_count'] = df_posts_full.sort_values(
                    ['page_id', 'date'])['fan_count'].fillna(method='bfill')
    ...
    def by_post(self):
        ...
        try:
            ...
            df_posts_full = metric_transformation.MetricCategorization(
                df_posts_full, METRIC, ITEM_COLUMN).categorize()

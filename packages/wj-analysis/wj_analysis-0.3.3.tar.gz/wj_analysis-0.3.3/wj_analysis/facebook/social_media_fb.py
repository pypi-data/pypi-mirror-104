...
class SocialpresFB():
        ...
        try:
            ...
            for idd, row in df_c.set_index('nombre').iterrows():
                if idd:
                    ...
                    df_m[f'{idd}'] = df_m['message'].str.contains(account_terms_regex, regex=True)
            ...
            df_med_c = df_m2.drop(
                columns=['created_time', 'message', 'description', 'post_id', "auto"]).groupby(
                    'name_id')
    ...
    def id_pres_fb(self):
        ...
        brand = self.brand
        auto_n = self.auto_n
        df_empty_id = pd.DataFrame(columns=['created_time', 'message', 'name_id', 'post_id'])

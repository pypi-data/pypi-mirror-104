...
def get_series(df_input, item_name, item_col, metric, agg_cols=None, agg_func='mean'):
    ...
    if agg_cols is not None:
        df_out = df_out[agg_cols + [item_col, metric]].groupby(agg_cols + [item_col]).agg(
            {metric: agg_func})
    return df_out
...
def get_boundaries(df_s, method='std', p_window=28, width_bound=1, metric='engagement_rate_by_day'):
    ...
    if method == 'qtl':
        ...
        df_out[['q1', 'q2', 'q3', 'min', 'max']] = df_out[['q1', 'q2', 'q3', 'min',
                                                           'max']].fillna(method='bfill')
    ...
    if method == 'std':
        if nrows > p_window:
            ...
            df_out['mstd'] = df_out[metric].rolling(
                p_window).std()    # Get DEVIATION rolling column
        else:
            df_out['mavg'] = df_out[metric].mean()    # Get MEAN column
            df_out['mstd'] = df_out[metric].std()    # Get DEVIATION column
        df_out[['mavg', 'mstd', 'min', 'max']] = df_out[['mavg', 'mstd', 'min',
                                                         'max']].fillna(method='bfill')
        # df_out = df_out.dropna()
        # Get BOUNDARIES
        ...
        try:
            ...
            for row in range(len(df_out)):
                if df_out['mavg'].iloc[row] == 0:
                    ...
                    temp_1 = 100 * (df_out[self.metric].iloc[row] -
                                    df_out['mavg'].iloc[row]) / df_out['mavg'].iloc[row]

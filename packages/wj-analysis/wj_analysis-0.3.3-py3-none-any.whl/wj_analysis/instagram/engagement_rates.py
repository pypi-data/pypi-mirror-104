...
class EngagementRateIG():
    ...
    def __init__(self, df_posts, df_pages, groups):
        ...
        POSTS_COLUMNS = [
            "owner_id", "owner_username", "date_utc", "shortcode", "likes_count", "comment_count",
            "caption_hashtags", "typename", "caption_mentions", "caption"
        ]
        OUTPUT_COLUMNS = [
            "followers", "owner_id", "owner_username", "group", "created_at", "shortcode",
            "likes_count", "comment_count", "caption_hashtags", "typename", "caption_mentions",
            "caption"
        ]
        ...
        if len(df_posts) > 0 and len(df_pages) > 0:
            try:
                ...
                df_posts_full['created_at'] = pd.to_datetime(
                    df_posts_full['date']) - timedelta(hours=5)
                ...
                df_posts_full['followers'] = df_posts_full.sort_values(
                    ['owner_id', 'date'])['followers'].fillna(method='bfill')

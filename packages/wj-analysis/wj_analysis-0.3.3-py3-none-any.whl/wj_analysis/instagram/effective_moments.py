...
class EffectMomentsIG():
    ...
    def moments(self):
        ...
        colum_ig = [
            'owner_id', 'created_at', 'owner_username', 'shortcode', 'group',
        ...
        if self.grouped is True:
            df_empty_day = pd.DataFrame(
                columns=[
                    'day', 'group', 'counts', 'engagement_rate_by_post',
                    'rel_engagement_rate_by_post'
                ])
            df_empty_hours = pd.DataFrame(
                columns=[
                    'hour', 'group', 'counts', 'engagement_rate_by_post',
                    'rel_engagement_rate_by_post'
                ])
            df_empty_days_week = pd.DataFrame(
                columns=[
                    'hour', 'day', 'group', 'counts', 'engagement_rate_by_post',
                    'rel_engagement_rate_by_post'
                ])
        else:
            df_empty_day = pd.DataFrame(
                columns=[
                    'day', 'group', '_object_name', 'counts', '_object_id',
                    'engagement_rate_by_post', 'rel_engagement_rate_by_post'
                ])
            df_empty_hours = pd.DataFrame(
                columns=[
                    'hour', 'group', '_object_name', 'counts', '_object_id',
                    'engagement_rate_by_post', 'rel_engagement_rate_by_post'
                ])
            df_empty_days_week = pd.DataFrame(
                columns=[
                    'hour', 'day', 'group', 'counts', '_object_name', '_object_id',
                    'engagement_rate_by_post', 'rel_engagement_rate_by_post'
                ])
        ...
        try:
            ...
            for sel_group in group:
                ...
                if grouped is True:
                    ...
                    temp_1 = temp_1.groupby(["day", "group", grouped_id], as_index=False).agg({
                        'engagement_rate_by_post': 'mean',
                        'owner_username': 'last',
                        'shortcode': 'count'
                    })
            ...
            df_eff_day = df_eff_day.sort_values(
                "engagement_rate_by_post", ascending=False).reset_index()
            ...
            for sel_group in group:
                ...
                if grouped is True:
                    ...
                    temp_1 = temp_1.groupby(["hour", "group", grouped_id], as_index=False).agg({
                        'engagement_rate_by_post': 'mean',
                        'owner_username': 'last',
                        'shortcode': 'count'
                    })
            ...
            for group in group:
                ...
                if grouped is True:
                    temp_1 = temp_1.groupby(["hour", "day", grouped_id], as_index=False).agg({
                        'engagement_rate_by_post': 'mean',
                        'shortcode': 'count'
                    })
                else:
                    temp_1 = temp_1.groupby(["hour", "day", "group", grouped_id],
                                            as_index=False).agg({
                                                'engagement_rate_by_post': 'mean',
                                                'owner_username': 'last',
                                                'shortcode': 'count'

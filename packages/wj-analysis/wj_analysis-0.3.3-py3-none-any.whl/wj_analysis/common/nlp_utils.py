    ...
    def process_text(
        ...
        if mentions:
            ...
            input_txt = input_txt.replace(
                '@', 'xxatsignxx')    #procect '@' signs of being removed in spec_chars
        if hashtags:
            hashtag_pattern = re.compile(r'\#[a-zA-Z0-9\-\_]+\b')
            input_txt = re.sub(hashtag_pattern, '', input_txt)
        else:
            input_txt = input_txt.replace(
                '#', 'xxhashtagsignxx')    #procect '#' signs to being removed in spec_chars
        if links:
...
class Features():
    ...
    def pos_tags(self, input_txt):
        ...
        try:
            ...
            out_dict = {'words': words, 'lemmas': lemmas, 'pos_tags': pos_tags}
...
class Polarity(object):
        ...
        if isinstance(df_text, pd.DataFrame):
            try:
                df_text['polarity'] = self.batch_forward(
                    list(df_text[text_column_name]), batch_size=batch_size, max_length=max_length)
            ...
            print(f'Class: {self.__str__()}\nMethod: {METHOD_NAME}')
            print(
                f'Must recieve a list of string of pandas DataFrame with strings. returning -100 ...'
            )
            return -100
    ...
    def polarity(self, df_text, text_column_name='processed_text'):
        ...
        if isinstance(df_text, list) and all(isinstance(text, str) for text in df_text):
            ...
            df_text['processed_text'] = df_text['message'].apply(
        ...
        if isinstance(df_text, pd.DataFrame):
            ...
            try:
                df_text['tokenized_data'] = df_text[text_column_name].apply(
                    lambda msg: self.tokenizer.encode(msg))
                df_text['len_tokenized_data'] = df_text.tokenized_data.apply(lambda l: len(l))
                df_text = df_text[df_text.tokenized_data.str.len() < 511]
                df_text['tokenized_data'] = df_text.tokenized_data.apply(
                    lambda l: torch.tensor([l]))
                df_text['polarity'] = df_text.tokenized_data.apply(
                    lambda tkd: self.polscore[self.softmax(self.model(tkd.to(self.device))[0]).
                                              argmax()])
            except Exception as e:
                print(e)
                error_1 = sys.exc_info()[0]
...
class MovieGroupProcess:
    ...
    @staticmethod
    def from_data(
            K, alpha, beta, D, vocab_size, cluster_doc_count, cluster_word_count,

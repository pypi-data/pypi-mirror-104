...
class MostEffectiveContentTW():
    ...
    def __init__(self, df_tweets, groups, mode='status'):
        ...
        TWEETS_COLUMNS = [
            'twitter_id', 'screen_name', 'tweet_id', 'text', 'media_entities', 'hashtags',
    ...
    def posts(self, n_most_eff=5, engagement_rate='engagement_rate_by_post', **kwargs):
        ...
        **kwargs:
        ...
        if 'account_ids' in kwargs.keys() and kwargs['account_ids']:
            ...
            df_most_eff_posts_tw = self.df_posts_full[self.df_posts_full.twitter_id.isin(
                account_ids)][[
                    'screen_name', 'tweet_id', 'text', engagement_rate, 'rel_' + engagement_rate
                ]]
        elif 'account_names' in kwargs.keys() and kwargs['account_names']:
            account_names = kwargs['account_names']
            df_most_eff_posts_tw = self.df_posts_full[self.df_posts_full.screen_name.isin(
                account_names)][[
                    'screen_name', 'tweet_id', 'text', engagement_rate, 'rel_' + engagement_rate
                ]]
        else:
        ...
        try:
            ...
            df_most_eff_posts_tw = df_most_eff_posts_tw.sort_values(
                engagement_rate, ascending=False).head(n_most_eff)
            ...
            return pd.DataFrame(
                columns=[
                    'screen_name', 'tweet_id', 'text', engagement_rate, 'rel_' +
                    engagement_rate, 'url'
                ])
    ...
    def posts_type(
        ...
        **kwargs:
        ...
        if 'account_ids' in kwargs.keys() and kwargs['account_ids']:
            account_ids = kwargs['account_ids']
            df_most_eff_posts_type_tw = self.df_posts_full[self.df_posts_full.twitter_id.isin(
                account_ids)][PAGE_COLUMNS +
                              ['media_entities', 'text', engagement_rate, 'rel_' + engagement_rate]]
        elif 'account_names' in kwargs.keys() and kwargs['account_names']:
            account_names = kwargs['account_names']
            df_most_eff_posts_type_tw = self.df_posts_full[self.df_posts_full.screen_name.isin(
                account_names)][
                    PAGE_COLUMNS +
                    ['media_entities', 'text', engagement_rate, 'rel_' + engagement_rate]]
        else:
        ...
        try:
            df_most_eff_posts_type_tw = df_most_eff_posts_type_tw.sort_values(
                engagement_rate, ascending=False).head(from_n_most_eff)
            df_most_eff_posts_type_tw['type'] = df_most_eff_posts_type_tw['media_entities'].apply(
                lambda med_ent: get_content_type(med_ent))
            ...
            if PAGE_COLUMNS:
                df_most_eff_posts_type_tw['percentage'] = df_most_eff_posts_type_tw[
                    'counts'].groupby(level=0).apply(lambda c: 100. * c / float(c.sum())).round(2)
                df_most_eff_posts_type_tw = df_most_eff_posts_type_tw.reset_index().rename(
                    columns={
                ...
                df_most_eff_posts_type_tw['percentage'] = (
                    100. * df_most_eff_posts_type_tw['counts'] /
                    df_most_eff_posts_type_tw['counts'].sum()).round(2)
            ...
            return pd.DataFrame(
                columns=[
                    'type', 'counts', 'avg_engagement_rate', 'avg_rel_engagement_rate', 'percentage'
                ])
    ...
    def construct_most_eff_objects(
        ...
        try:
            ...
            most_eff_objs_tw = pd.DataFrame(
                ...
                [objs, engagement_rate, 'rel_' + engagement_rate, 'group', f'{objs}_count'])
        ...
        **kwargs:
        ...
        if 'account_ids' in kwargs.keys() and kwargs['account_ids']:
            ...
            df_most_eff_hashtags_mentions_tw = self.df_posts_full[
                self.df_posts_full.twitter_id.isin(account_ids)][PAGE_COLUMNS + [
                    'hashtags', 'user_mentions', 'group', engagement_rate, 'rel_' + engagement_rate
                ]]
        elif 'account_names' in kwargs.keys() and kwargs['account_names']:
            account_names = kwargs['account_names']
            df_most_eff_hashtags_mentions_tw = self.df_posts_full[
                self.df_posts_full.screen_name.isin(account_names)][PAGE_COLUMNS + [
                    'hashtags', 'user_mentions', 'group', engagement_rate, 'rel_' + engagement_rate
                ]]
        else:
            df_most_eff_hashtags_mentions_tw = deepcopy(
                self.df_posts_full[PAGE_COLUMNS + [
                    'hashtags', 'user_mentions', 'group', engagement_rate, 'rel_' + engagement_rate
                ]])
        ...
        try:
            df_most_eff_hashtags_mentions_tw['hashtags'] = df_most_eff_hashtags_mentions_tw[
                'hashtags'].apply(lambda hashtags: hashtags.split(',') if hashtags else None)
            df_most_eff_hashtags_mentions_tw['user_mentions'] = df_most_eff_hashtags_mentions_tw[
                'user_mentions'].apply(lambda mentions: mentions.split(',') if mentions else None)
            df_compare_hashtags_mentions_tw['hashtags'] = df_compare_hashtags_mentions_tw[
                'hashtags'].apply(lambda hashtags: hashtags.split(',') if hashtags else None)
            df_compare_hashtags_mentions_tw['user_mentions'] = df_compare_hashtags_mentions_tw[
                'user_mentions'].apply(lambda mentions: mentions.split(',') if mentions else None)

            df_most_eff_hashtags_mentions_tw = df_most_eff_hashtags_mentions_tw.sort_values(
                engagement_rate, ascending=False).head(from_n_most_eff)
            df_compare_hashtags_mentions_tw = df_compare_hashtags_mentions_tw.sort_values(
                engagement_rate, ascending=False).head(from_n_most_eff)
            #Hashtags
            ...
            most_eff_hashtags_tw = pd.DataFrame(
                columns=[
                    'hashtag', engagement_rate, 'rel_' + engagement_rate, 'group', 'hashtag_count'
                ])
            ...
            most_eff_mentions_tw = pd.DataFrame(
                columns=[
                    'mention', engagement_rate, 'rel_' + engagement_rate, 'group', 'mention_count'
                ])
    ...
    def construct_most_eff_words(
        ...
        try:
            ...
            for _, row in df_most_eff_words_tw.iterrows():
                ...
                words_rel_eff_tw = words_rel_eff_tw + [row['rel_' + engagement_rate]] * len(
                    row.words)
            ...
            most_eff_words_tw = most_eff_words_tw.sort_values(
                PAGE_COLUMNS + ['pos_tag', 'group', engagement_rate],
            ...
            most_eff_words_tw = pd.DataFrame(
                columns=PAGE_COLUMNS + [
                    'pos_tag', 'group', word_column, 'word_count', engagement_rate, 'rel_' +
                    engagement_rate
                ])
            show_columns = PAGE_COLUMNS + [
                'group', 'word', engagement_rate, 'rel_' + engagement_rate, 'word_count'
            ]
    ...
    def words(
        ...
        **kwargs:
        ...
        if 'account_ids' in kwargs.keys() and kwargs['account_ids']:
            account_ids = kwargs['account_ids']
            df_most_eff_words_tw = self.df_posts_full[self.df_posts_full.twitter_id.isin(
                account_ids)][PAGE_COLUMNS +
                              ['text', 'group', engagement_rate, 'rel_' + engagement_rate]]
        elif 'account_names' in kwargs.keys() and kwargs['account_names']:
            account_names = kwargs['account_names']
            df_most_eff_words_tw = self.df_posts_full[self.df_posts_full.screen_name.isin(
                account_names)][PAGE_COLUMNS +
                                ['text', 'group', engagement_rate, 'rel_' + engagement_rate]]
        else:
        ...
        try:
            ...
            if 'processed_text' not in df_most_eff_words_tw.keys():
                df_most_eff_words_tw['processed_text'] = df_most_eff_words_tw.text.apply(
                    lambda msg: CleanText(msg).process_text(
                        mentions=True, hashtags=True, links=True, spec_chars=True, rts=True))
            df_most_eff_words_tw['processed_text'] = df_most_eff_words_tw['processed_text'].apply(
                lambda txt: self.features.pos_tags(txt))

            df_most_eff_words_tw['words'] = df_most_eff_words_tw['processed_text'].apply(
                lambda pt: pt['words'])
            df_most_eff_words_tw['lemmas'] = df_most_eff_words_tw['processed_text'].apply(
                lambda pt: pt['lemmas'])
            df_most_eff_words_tw['pos_tags'] = df_most_eff_words_tw['processed_text'].apply(
                lambda pt: pt['pos_tags'])

            df_most_eff_words_tw = df_most_eff_words_tw[~df_most_eff_words_tw['processed_text'].
                                                        isna()]

            most_eff_words_tw, most_eff_nouns_tw, most_eff_verbs_tw, most_eff_adjs_tw = self.construct_most_eff_words(
                df_most_eff_words_tw,
                lemmatize=lemmatize,
            ...
            return most_eff_words_tw, most_eff_nouns_tw[show_columns], most_eff_verbs_tw[
                show_columns], most_eff_adjs_tw[show_columns]
            ...
            most_eff_words_tw = pd.DataFrame(
                columns=PAGE_COLUMNS + [
                    'pos_tag', 'group', 'word', 'word_count', engagement_rate, 'rel_' +
                    engagement_rate
                ])
            show_columns = PAGE_COLUMNS + [
                'group', 'word', engagement_rate, 'rel_' + engagement_rate, 'word_count'

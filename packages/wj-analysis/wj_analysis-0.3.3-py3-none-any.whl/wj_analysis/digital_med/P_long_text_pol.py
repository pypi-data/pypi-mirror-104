...
for i in range(len(DF_FILTER_1)):
    plt.plot(range(len(DF_FILTER_1.Pos.iloc[i])), DF_FILTER_1.Pos.iloc[i], "o-", label=str(i))
    ...
    data=[
        go.Scatter(
            ...
            marker=dict(
                ...
                colorbar=dict(
                    title="Tamaño frase",
                    tickvals=[min(len_text), mean(len_text),
                              max(len_text)],
...
fig.update_layout(
    title=str(DF_FILTER_1.title.iloc[text] + \
              ", tamaño total texto " + \
                  str(round(sum(len_text) * scale, 0))),
    xaxis=dict(title="Frase"),
    yaxis=dict(title="Polaridad, " + "(promedio " + \
               str(round(mean(pol), 2)) + ")" + " (rango " +\

...
class STTMgroupsFB():
    ...
    def __init__(self, df_comments, df_pages, account_ids=[], account_names=[]):
        ...
        try:
            if 'post_id' in self.df_comments_full.keys():
                ...
                self.df_comments_full['page_name'] = self.df_comments_full.page_id.apply(
        ...
        if account_ids:
            self.df_comments_full = self.df_comments_full[self.df_comments_full.page_id.isin(
                account_ids)]
        elif account_names:
            self.df_comments_full = self.df_comments_full[self.df_comments_full.page_name.isin(
                account_names)]
    ...
    def get_sttm_groups(self, with_remove=False, list_remove=[], min_texts=100):
        ...
        try:
            ...
            if 'processed_text' not in self.df_comments_full.keys():
                self.df_comments_full['processed_text'] = self.df_comments_full['message'].apply(
                    lambda msg: CleanText(str(msg)).process_text(
            ...
            self.df_comments_full = self.df_comments_full.drop(
                self.df_comments_full[self.df_comments_full['processed_text'] == ""].index)
            ...
            if list_remove:
                ...
                self.df_comments_full['processed_text'] = self.df_comments_full[
                    'processed_text'].apply(lambda x: str(x).split(" "))
                self.df_comments_full['processed_text'] = self.df_comments_full[
                    'processed_text'].apply(lambda x: self.remove_from_text(x, list_remove))
                self.df_comments_full['processed_text'] = self.df_comments_full[
                    'processed_text'].apply(lambda msg: str(msg))

            # drop empty comments
            self.df_comments_full = self.df_comments_full.dropna(subset=['processed_text'])
            self.df_comments_full = self.df_comments_full.drop(
                self.df_comments_full[self.df_comments_full['processed_text'] == ""].index)
            self.df_comments_full['len_text'] = self.df_comments_full.processed_text.apply(
                lambda msg: len(msg))
            ...
            if 'tokenized_text' not in self.df_comments_full.keys():
                ...
                self.df_comments_full[
                    'tokenized_text'] = self.df_comments_full.processed_text.apply(
                        lambda msg: getfeatures.pos_tags(msg)['words'])

            # drop empty comments
            self.df_comments_full = self.df_comments_full.dropna(subset=['tokenized_text'])
            self.df_comments_full = self.df_comments_full.drop(
                self.df_comments_full[self.df_comments_full['tokenized_text'] == ""].index)
            self.df_comments_full['len_text'] = self.df_comments_full.tokenized_text.apply(
                lambda msg: len(msg))
            self.df_comments_full = self.df_comments_full[self.df_comments_full['len_text'] != 0]
            ...
            if self.df_comments_full.tokenized_text.shape[0] > min_texts:
                ...
                print(
                    "warning: in", f'Class: {self.__str__()}\nMethod: {METHOD_NAME}',
    ...
    def get_sttm_groups_withremove(self, df_preprocessed_sttm, list_toremove=[], min_texts=100):
        ...
        try:
            ...
            self.df_preprocessed_sttm['tokenized_text'] = self.df_preprocessed_sttm[
                'tokenized_text'].apply(lambda x: x[2:-2])
            self.df_preprocessed_sttm['tokenized_text'] = self.df_preprocessed_sttm[
                'tokenized_text'].apply(lambda x: str(x).split("', '"))
            self.df_preprocessed_sttm['tokenized_text'] = self.df_preprocessed_sttm[
                'tokenized_text'].apply(lambda x: self.remove_from_text(x, list_toremove))
            self.df_preprocessed_sttm['tokenized_text'] = self.df_preprocessed_sttm[
                'tokenized_text'].apply(lambda msg: str(msg))
            ...
            self.df_preprocessed_sttm = self.df_preprocessed_sttm.drop(
                self.df_preprocessed_sttm[self.df_preprocessed_sttm['tokenized_text'] == ""].index)
            self.df_preprocessed_sttm = self.df_preprocessed_sttm.drop(
                self.df_preprocessed_sttm[self.df_preprocessed_sttm['tokenized_text'] == " "].index)
            self.df_preprocessed_sttm['len_text'] = self.df_preprocessed_sttm.tokenized_text.apply(
                lambda msg: len(msg))
    ...
    def get_percentage(self, counter):
        ...
        percentage = dict([
            (i, perc / sum(percentage.values()) * 100.0) for i, perc in percentage.most_common()
        ])
        return percentage
    ...
    def message_to_group_columns(self):
        ...
        if '__date_cot' not in data_texts.keys():
            data_texts['__date_cot'] = pd.to_datetime(
                data_texts['created_time']) - timedelta(hours=5)
    ...
    def get_grouped_dataframe(self):
        ...
        data_grouped = deepcopy(
            self.data_texts[['sttm_group', 'tokenized_text', '__mention', '__date_cot',
                             'polarity']])
        ...
        data_grouped = data_grouped[[
            'sttm_group', '__n_docs', 'tokenized_text', '__mention', '__weekday', 'polarity'
        ]].groupby('sttm_group').agg({
            '__n_docs':
                'count',
            'tokenized_text':
                'sum',
            '__mention':
                'last',
            '__weekday':
                lambda x: ','.join(x) if len(unique(data_grouped['__weekday'])) > 1 else '',
            'polarity':
                lambda x: ','.join(x)
        })

        data_grouped['__percentage'] = data_grouped['__n_docs'] * 100 / sum(
            data_grouped['__n_docs'])
        ...
        data_grouped['__polarity_distribution'] = data_grouped['__polarity_distribution'].apply(
            lambda p: self.get_percentage(p))

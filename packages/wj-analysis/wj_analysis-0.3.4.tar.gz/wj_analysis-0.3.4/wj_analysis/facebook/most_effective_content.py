...
class MostEffectiveContentFB():
    ...
    def __init__(self, df_posts, df_pages, groups):
        ...
        POSTS_COLUMNS = [
            'page_id', 'page_name', 'post_id', 'message', 'permalink_url', 'type', 'message_tags',
    ...
    def posts(self, n_most_eff=5, engagement_rate='engagement_rate_by_post', **kwargs):
        ...
        **kwargs:
        ...
        if 'account_ids' in kwargs.keys() and kwargs['account_ids']:
            ...
            df_most_eff_posts_fb = self.df_posts_full[self.df_posts_full.page_id.isin(
                account_ids)][[
                    'page_name', 'post_id', 'message', engagement_rate, 'rel_' + engagement_rate,
                    'permalink_url'
                ]]
        elif 'account_names' in kwargs.keys() and kwargs['account_names']:
            account_names = kwargs['account_names']
            df_most_eff_posts_fb = self.df_posts_full[self.df_posts_full.page_name.isin(
                account_names)][[
                    'page_name', 'post_id', 'message', engagement_rate, 'rel_' + engagement_rate,
                    'permalink_url'
                ]]
        ...
        try:
            ...
            return pd.DataFrame(
                columns=[
                    'page_name', 'post_id', 'message', engagement_rate, 'rel_' +
                    engagement_rate, 'permalink_url'
                ])
    ...
    def posts_type(
        ...
        **kwargs:
        ...
        if 'account_ids' in kwargs.keys() and kwargs['account_ids']:
            account_ids = kwargs['account_ids']
            df_most_eff_posts_type_fb = self.df_posts_full[self.df_posts_full.page_id.isin(
                account_ids)][PAGE_COLUMNS +
                              ['type', 'message', engagement_rate, 'rel_' + engagement_rate]]
        elif 'account_names' in kwargs.keys() and kwargs['account_names']:
            account_names = kwargs['account_names']
            df_most_eff_posts_type_fb = self.df_posts_full[self.df_posts_full.page_name.isin(
        ...
        try:
            ...
            if PAGE_COLUMNS:
                df_most_eff_posts_type_fb['percentage'] = df_most_eff_posts_type_fb[
                    'counts'].groupby(level=0).apply(lambda c: 100. * c / float(c.sum())).round(2)
                df_most_eff_posts_type_fb = df_most_eff_posts_type_fb.reset_index().rename(
                    columns={
                ...
                df_most_eff_posts_type_fb['percentage'] = (
                    100. * df_most_eff_posts_type_fb['counts'] /
                    df_most_eff_posts_type_fb['counts'].sum()).round(2)
        ...
        try:
            ...
            most_eff_objs_fb = pd.DataFrame(
                columns=PAGE_COLUMNS + [
                    objs[:-1], engagement_rate, 'rel_' +
                    engagement_rate, 'group', f'{objs[:-1]}_count'
                ])
    ...
    def hashtags_mentions(
        ...
        **kwargs:
        ...
        if 'account_ids' in kwargs.keys() and kwargs['account_ids']:
            account_ids = kwargs['account_ids']
            df_most_eff_hashtags_mentions_fb = self.df_posts_full[self.df_posts_full.page_id.isin(
                account_ids)][PAGE_COLUMNS +
                              ['message_tags', 'group', engagement_rate, 'rel_' + engagement_rate]]
        elif 'account_names' in kwargs.keys() and kwargs['account_names']:
            account_names = kwargs['account_names']
            df_most_eff_hashtags_mentions_fb = self.df_posts_full[self.df_posts_full.page_name.isin(
                account_names)][
                    PAGE_COLUMNS +
                    ['message_tags', 'group', engagement_rate, 'rel_' + engagement_rate]]
        else:
        ...
        df_compare_hashtags_mentions_fb = self.df_posts_full[
            self.df_posts_full.group == compare_group][
                PAGE_COLUMNS + ['message_tags', 'group', engagement_rate, 'rel_' + engagement_rate]]
        ...
        try:
            ...
            df_most_eff_hashtags_mentions_fb.message_tags = df_most_eff_hashtags_mentions_fb.message_tags.apply(
                lambda _json: ast.literal_eval(_json))

            df_most_eff_hashtags_mentions_fb[
                'mentions'] = df_most_eff_hashtags_mentions_fb.message_tags.apply(
                    lambda tags:
                    [tag['name'] for tag in tags if not re.search(r'^#.*', tag['name'])])
            df_most_eff_hashtags_mentions_fb['mentions'] = df_most_eff_hashtags_mentions_fb[
                'mentions'].apply(lambda ments: ments if ments else None)

            df_most_eff_hashtags_mentions_fb[
                'hashtags'] = df_most_eff_hashtags_mentions_fb.message_tags.apply(
                    lambda tags: [tag['name'] for tag in tags if re.search(r'^#.*', tag['name'])])
            df_most_eff_hashtags_mentions_fb['hashtags'] = df_most_eff_hashtags_mentions_fb[
                'hashtags'].apply(lambda hashs: hashs if hashs else None)
            ...
            df_compare_hashtags_mentions_fb.message_tags = df_compare_hashtags_mentions_fb.message_tags.apply(
                lambda _json: ast.literal_eval(_json))

            df_compare_hashtags_mentions_fb[
                'mentions'] = df_compare_hashtags_mentions_fb.message_tags.apply(
                    lambda tags:
                    [tag['name'] for tag in tags if not re.search(r'^#.*', tag['name'])])
            df_compare_hashtags_mentions_fb['mentions'] = df_compare_hashtags_mentions_fb[
                'mentions'].apply(lambda ments: ments if ments else None)

            df_compare_hashtags_mentions_fb[
                'hashtags'] = df_compare_hashtags_mentions_fb.message_tags.apply(
                    lambda tags: [tag['name'] for tag in tags if re.search(r'^#.*', tag['name'])])
            df_compare_hashtags_mentions_fb['hashtags'] = df_compare_hashtags_mentions_fb[
                'hashtags'].apply(lambda hashs: hashs if hashs else None)

            df_most_eff_hashtags_mentions_fb = df_most_eff_hashtags_mentions_fb.sort_values(
                engagement_rate, ascending=False).head(from_n_most_eff)
            df_compare_hashtags_mentions_fb = df_compare_hashtags_mentions_fb.sort_values(
                engagement_rate, ascending=False).head(from_n_most_eff)
            #Hashtags
    ...
    def construct_most_eff_words(
        ...
        try:
            ...
            for _, row in df_most_eff_words_fb.iterrows():
                ...
                words_rel_eff_fb = words_rel_eff_fb + [row['rel_' + engagement_rate]] * len(
                    row.words)
    ...
    def words(
        ...
        **kwargs:
        ...
        if 'account_ids' in kwargs.keys() and kwargs['account_ids']:
            account_ids = kwargs['account_ids']
            df_most_eff_words_fb = self.df_posts_full[self.df_posts_full.page_id.isin(account_ids)][
                PAGE_COLUMNS + ['message', 'group', engagement_rate, 'rel_' + engagement_rate]]
        elif 'account_names' in kwargs.keys() and kwargs['account_names']:
            account_names = kwargs['account_names']
            df_most_eff_words_fb = self.df_posts_full[self.df_posts_full.page_name.isin(
                account_names)][PAGE_COLUMNS +
        ...
        try:
            ...
            if 'processed_text' not in df_most_eff_words_fb.keys():
                df_most_eff_words_fb['processed_text'] = df_most_eff_words_fb.message.apply(
                    lambda msg: CleanText(msg).process_text(
                        mentions=True, hashtags=True, links=True, spec_chars=True))
            df_most_eff_words_fb['processed_text'] = df_most_eff_words_fb['processed_text'].apply(
                lambda txt: self.features.pos_tags(txt))

            df_most_eff_words_fb['words'] = df_most_eff_words_fb['processed_text'].apply(
                lambda pt: pt['words'])
            df_most_eff_words_fb['lemmas'] = df_most_eff_words_fb['processed_text'].apply(
                lambda pt: pt['lemmas'])
            df_most_eff_words_fb['pos_tags'] = df_most_eff_words_fb['processed_text'].apply(
                lambda pt: pt['pos_tags'])

            df_most_eff_words_fb = df_most_eff_words_fb[~df_most_eff_words_fb['processed_text'].
                                                        isna()]

            most_eff_words_fb, most_eff_nouns_fb, most_eff_verbs_fb, most_eff_adjs_fb = self.construct_most_eff_words(
                df_most_eff_words_fb,
                lemmatize=lemmatize,
            ...
            return most_eff_words_fb, most_eff_nouns_fb[show_columns], most_eff_verbs_fb[
                show_columns], most_eff_adjs_fb[show_columns]

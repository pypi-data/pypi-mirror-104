...
class Synchronization():
    ...
    def __init__(self, df_posts, platform, engagement_rate='engagement_rate_by_post'):
        ...
        try:
            ...
            df_weekday_hour['weekday'] = df_weekday_hour[DATE_COLUMN].apply(lambda d: d.weekday())
            df_weekday_hour['hour'] = df_weekday_hour[DATE_COLUMN].apply(lambda d: d.hour)
    ...
    def weekday(self, n_groups=10, threshold_group=3):
        ...
        try:
            df_wd = df_wd.groupby('weekday').agg({
                'n_posts': 'count',
                engagement_rate: 'mean'
            }).reset_index()

            gu.discretize(df_wd, engagement_rate, to_column='eng_group', n_groups=n_groups)
            ...
            df_wd_out['synchronization'] = df_wd_out.apply(
                ...
                if row.activity >= threshold_group else 'N/A',
                axis=1)
            ...
            print(e)
            print(f'Class: {self.__str__()}\nMethod: {METHOD_NAME}\n')
            return pd.DataFrame(columns=['weekday', 'activity', 'effectivity', 'synchronization'])
    ...
    def hour(self, n_groups=10, threshold_group=3):
        ...
        try:
            df_h = df_h.groupby('hour').agg({
                'n_posts': 'count',
                engagement_rate: 'mean'
            }).reset_index()

            gu.discretize(df_h, engagement_rate, to_column='eng_group', n_groups=n_groups)
            ...
            print(e)
            print(f'Class: {self.__str__()}\nMethod: {METHOD_NAME}\n')
            return pd.DataFrame(columns=['hour', 'activity', 'effectivity', 'synchronization'])
    ...
    def weekday_hour(self, n_groups=10, threshold_group=3):
        ...
        engagement_rate = self.engagement_rate
        df_wdh = deepcopy(self.df_weekday_hour[['weekday', 'hour', 'n_posts', engagement_rate]])

        try:
            df_wdh = df_wdh.groupby(['weekday', 'hour']).agg({
                'n_posts': 'count',
                engagement_rate: 'mean'
            }).reset_index()

            gu.discretize(df_wdh, engagement_rate, to_column='eng_group', n_groups=n_groups)
            ...
            sync_df = pd.DataFrame(
                np.where(
                    ...
                    (n_groups - 1)**2 - (eff_grouped - act_grouped)**2, 'N/A'))

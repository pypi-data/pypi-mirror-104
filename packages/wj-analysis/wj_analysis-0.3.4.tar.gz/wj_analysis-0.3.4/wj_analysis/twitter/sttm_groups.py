...
class STTMgroupsTW():
    ...
    def __init__(self, df_replies, account_ids=[], account_names=[]):
        ...
        if account_ids:
            self.df_replies_full = self.df_replies_full[self.df_replies_full.replying_to_id.isin(
                account_ids)]
        elif account_names:
            self.df_replies_full = self.df_replies_full[self.df_replies_full.name.isin(
                account_names)]
    ...
    def get_sttm_groups(self, with_remove=False, list_remove=[], min_texts=100):
        ...
        try:
            ...
            if 'tokenized_text' not in self.df_replies_full.keys():
                ...
                self.df_replies_full['tokenized_text'] = self.df_replies_full.processed_text.apply(
                    lambda msg: getfeatures.pos_tags(msg)['words'])

            # drop empty comments
            self.df_replies_full = self.df_replies_full.dropna(subset=['tokenized_text'])
            self.df_replies_full = self.df_replies_full.drop(
                self.df_replies_full[self.df_replies_full['tokenized_text'] == ""].index)
            self.df_replies_full['len_text'] = self.df_replies_full.tokenized_text.apply(
                lambda msg: len(msg))
            self.df_replies_full = self.df_replies_full[self.df_replies_full['len_text'] != 0]
    ...
    def message_to_group_columns(self):
        ...
        if '__date_cot' not in data_texts.keys():
            data_texts['__date_cot'] = pd.to_datetime(data_texts['created_at']) - timedelta(hours=5)

        if 'replying_to' in data_texts.keys():
            data_texts = data_texts[[
                'sttm_group', 'tokenized_text', 'replying_to', '__date_cot', 'polarity'
            ]]

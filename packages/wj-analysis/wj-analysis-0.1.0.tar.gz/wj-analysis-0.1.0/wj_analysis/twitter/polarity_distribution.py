    ...
    def get_polarity(self):
        ...
        try:
            ...
            if 'processed_text' not in self.df_replies_full.keys():
                self.df_replies_full['processed_text'] = self.df_replies_full['text'].apply(lambda msg: CleanText(msg).process_text(mentions=True, hashtags=True, links=True, spec_chars=True))
            
            # drop empty comments
            self.df_replies_full = self.df_replies_full.dropna(subset=['processed_text'])
            self.df_replies_full = self.df_replies_full.drop(self.df_replies_full[self.df_replies_full['processed_text']==""].index)
    ...
    def grouped_polarities(self, group_by='group'):
        ...
        try:
            if 'replying_to' in self.df_replies_full.keys():
                ...
                self.df_replies_full['group'] = self.df_replies_full.replying_to_id.apply(lambda pid: general_utils.get_group(pid, self.groups)) 
                ...
                self.df_replies_full['group'] = self.df_replies_full.twitter_id.apply(lambda pid: general_utils.get_group(pid, self.groups))
            ...
            print(ERR_SYS + str(error_1))
            print(f'Class: {self.__str__()}\nMethod: {METHOD_NAME}')
            print("Warning: in Polarity_distibution_TW.grouped_polarities no 'replying_to' or 'screen_name' column in DataFrame")
        ...
        try:
            if (group_by == 'account'):
                ...
                df_groupedpolarity = df_groupedpolarity.reset_index().rename(columns={'page_id': '_object_id', 'page_name': '_object_name'})
                return df_groupedpolarity[['_object_id', '_object_name', 'group', 'sentiment']]
            elif(group_by == 'group'):    
                df_groupedpolarity = self.df_replies_full.groupby('group')['polarity'].value_counts().unstack().fillna(0)
                df_groupedpolarity['sentiment'] = df_groupedpolarity.to_dict(orient='records')
                return df_groupedpolarity.reset_index()[['group', 'sentiment']]
            elif(group_by == 'all'):    
                df_groupedpolarity = self.df_replies_full.groupby('all')['polarity'].value_counts().unstack().fillna(0) 
                df_groupedpolarity['sentiment'] = df_groupedpolarity.to_dict(orient='records')
                return df_groupedpolarity.reset_index()[['all', 'sentiment']]
            else:
                print(f'Warning: {group_by} Invalid parameter value for parameter group_by, grouping by all')
                df_groupedpolarity = self.df_replies_full.groupby('all')['polarity'].value_counts().unstack().fillna(0)
                df_groupedpolarity['sentiment'] = df_groupedpolarity.to_dict(orient='records')
                return df_groupedpolarity.reset_index()[['all', 'sentiment']]
            ...
            print(f'Class: {self.__str__()}\nMethod: {METHOD_NAME}')
            if (group_by == 'account'):
                df_groupedpolarity = pd.DataFrame(columns=['_object_id', '_object_name', 'group', 'sentiment']) 
            elif(group_by == 'group'):

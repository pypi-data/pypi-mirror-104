    ...
    if len(df_media_tw) > 2:
        ...
        while i_t < len(df_media_tw):
            if df_media_tw.in_reply_to_status_id.iloc[i_t] == None:
                ...
                if df_media_tw.in_reply_to_status_id.iloc[i_t] / df_media_tw.tweet_id.iloc[f_t] == 1:
                    thread.append(1)
                else:
                    thread.append(0)
            i_t += 1
...
class SocialpresTW():
    ...
    def c_pres_tw(self):
        ...
        try:
            ...
            for idd, row in df_c.set_index('nombre').iterrows():
                if idd:
                    account_terms_regex = create_regex([t.strip() for t in row.terminos.split(',')])
                    df_m[f'{idd}'] = df_m['text'].str.contains(account_terms_regex, regex=True)
            ...
            df_med_c = df_m2.drop(columns=['created_at',
                                           'description', 'text',
                                           'tweet_id', "auto"]).groupby('screen_name')
    ...
    def id_pres_tw(self):
        ...
        """
        col_med = ['created_at', 'description', 'text', 'tweet_id',
        ...
        auto_n = self.auto_n
        thread = self.thread
        df_empty_id = pd.DataFrame(columns=['created_at', 'text',
        ...
        try:
            ...
            df_med_id = df_m2[["created_at", "text", "screen_name",
                               "tweet_id", 'in_reply_to_status_id', brand]]

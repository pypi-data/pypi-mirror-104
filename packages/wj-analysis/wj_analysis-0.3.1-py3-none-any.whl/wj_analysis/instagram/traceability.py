...
class Traceability():
    ...
    def __init__(self, df_pages, groups):
        ...
        if len(df_pages)>0:
            try:
                df_fan_count = deepcopy(df_pages[['followers', 'userid', 'username', 'date']])
                ...
                df_fan_count = df_fan_count.groupby(['userid', 'username', 'date']).last().reset_index()
                
                df_fan_count['group'] = df_fan_count['userid'].apply(lambda pid: general_utils.get_group(pid, groups))
    ...
    def absolute(self):
        ...
        try:
            return df_fan_count[['group', 'userid', 'username', 'date', 'value']].dropna(subset=['value'])
            ...
            print(e)
            print(f'Class: {self.__str__()}\nMethod: {METHOD_NAME}\n')
            return pd.DataFrame(columns=['group', 'userid', 'username', 'date', 'value'])
    ...
    def differences(self):
        ...
        try:
            ...
            df_fan_count_diff = pd.concat(dfs).sort_values(['group', 'userid', 'username', 'date'])

            return df_fan_count_diff[['group', 'userid', 'username', 'date', 'value']].dropna(subset=['value'])
    ...
    def percentage(self):
        ...
        try:
            ...
            for pid in set(df_fan_count.userid):
                df_tmp = df_fan_count[df_fan_count.userid.eq(pid)].sort_values('date')
                df_tmp['value'] = 100.*df_tmp['followers'].pct_change()
                dfs.append(df_tmp)
    
            df_fan_count_perc = pd.concat(dfs).sort_values(['group', 'userid', 'username', 'date'])

            return df_fan_count_perc[['group', 'userid', 'username', 'date', 'value']].dropna(subset=['value'])

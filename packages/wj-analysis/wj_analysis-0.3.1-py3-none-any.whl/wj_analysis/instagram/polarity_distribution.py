    ...
    def get_polarity(self):
        ...
        try:
            ...
            if 'processed_text' not in self.df_comments_full.keys():
                self.df_comments_full['processed_text'] = self.df_comments_full['text'].apply(lambda msg: CleanText(msg).process_text(mentions=True, hashtags=True, links=True, spec_chars=True))
    ...
    def grouped_polarities(self, group_by='group'):
        ...
        try:
            if 'post_owner_id' in self.df_comments_full.keys():   
                self.df_comments_full['group'] = self.df_comments_full['post_owner_id'].apply(lambda pid: general_utils.get_group(pid, self.groups))
            else:
                self.df_comments_full['group'] = "no group"
                print("Warning: in Polarity_distibution_IG.grouped polarities no 'post_owner_id' column in DataFrame")

            if 'post_owner_username' not in self.df_comments_full.keys():
                print("Warning: in Polarity_distibution_IG.grouped polarities no 'post_owner_username' column in DataFrame")
        ...
        try:
            if (group_by == 'account'):
                ...
                df_groupedpolarity = df_groupedpolarity.reset_index().rename(columns={'post_owner_id': '_object_id', 'post_owner_username': '_object_name'})
                return df_groupedpolarity[['_object_id', '_object_name', 'group', 'sentiment']]
            elif(group_by == 'group'):    
                df_groupedpolarity = self.df_comments_full.groupby('group')['polarity'].value_counts().unstack().fillna(0)
                df_groupedpolarity['sentiment'] = df_groupedpolarity.to_dict(orient='records')
                return df_groupedpolarity.reset_index()[['group', 'sentiment']]
            elif(group_by == 'all'):    
                df_groupedpolarity = self.df_comments_full.groupby('all')['polarity'].value_counts().unstack().fillna(0) 
                df_groupedpolarity['sentiment'] = df_groupedpolarity.to_dict(orient='records')

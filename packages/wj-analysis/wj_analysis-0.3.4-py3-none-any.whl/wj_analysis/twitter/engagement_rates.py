...
class EngagementRateTW():
    ...
    def __init__(self, df_tweets, df_replies, groups, mode='status', remove_rt=True):
        ...
        TWEETS_COLUMNS = [
            "twitter_id", "screen_name", "created_at", "tweet_id", "favorite_count",
            "retweet_count", "ac_followers_count", "in_reply_to_status_id", "media_entities",
            "user_mentions", "hashtags", "text", "profile_image"
        ]
        OUTPUT_COLUMNS = [
            "reply_count_no_api", "twitter_id", "screen_name", "group", "created_at", "tweet_id",
            "ac_followers_count", "favorite_count", "retweet_count", "in_reply_to_status_id",
            "media_entities", "user_mentions", "hashtags", "text", "profile_image"
        ]
        ...
        if mode == 'status':
            if len(df_tweets) > 0 and len(df_replies) > 0:
                try:
                    df_tweets_full = deepcopy(
                        df_tweets[df_tweets['in_reply_to_status_id'].isna()][TWEETS_COLUMNS])
                    df_tweets_full = df_tweets_full.drop_duplicates(
                        subset=['screen_name', 'created_at', 'user_mentions', 'hashtags', 'text'],
                        keep=False)
                    df_replies_count = df_replies[REPLIES_COLUMNS]\
                                            .groupby('in_reply_to_status_id').count().reset_index()
                    ...
                    df_tweets_full['created_at'] = pd.to_datetime(
                        df_tweets_full['created_at']) - timedelta(hours=5)
                    ...
                    self.df_tweets_full = pd.DataFrame(columns=OUTPUT_COLUMNS)
            ...
            try:
                ...
                df_tweets_full = df_tweets_full.drop_duplicates(
                    subset=[
                        'screen_name', 'created_at', 'user_mentions', 'hashtags', 'text',
                        'media_entities'
                    ],
                    keep=False)
                df_tweets_full[
                    'reply_count_no_api'] = 0    #set replies to zero, temporary until official API
                df_tweets_full['group'] = 'terms'    #set group to 'terms'
                df_tweets_full = df_tweets_full[
                    df_tweets_full['ac_followers_count'] !=
                    0]    #Avoiding division by zero for users with zero followes.
                df_tweets_full['created_at'] = pd.to_datetime(
                    df_tweets_full['created_at']) - timedelta(hours=5)
                if remove_rt:
                    ...
                    for index, row in df_tweets_full.iterrows():
                        ...
                        if len(findings) > 0:
                            ...
                            author_name = re.sub(re.compile(r'^(?:RT|rt) \@'), '',
                                                 findings[0]).replace(': ', '')
    ...
    def by_reach(self):
        ...
        try:
            df_tweets_full['engagement_rate_by_reach'] = df_tweets_full.apply(lambda row: 100*(row.favorite_count\
                                                                              + row.retweet_count + row.reply_count_no_api)\
            ...
            print(e)
            print(f'Class: {self.__str__()}\nMethod: {METHOD_NAME}\n')
            return pd.DataFrame(columns=list(df_tweets_full.columns) + ['engagement_rate_by_reach'])
    ...
    def by_post(self):
        ...
        try:
            if self.mode == 'terms':
                df_tweets_full['engagement_rate_by_post'] = df_tweets_full.apply(lambda row: row.favorite_count + row.retweet_count\
                                                                                              +row.reply_count_no_api, axis=1)

                if self.remove_rt:
                    df_tweets_max_er = df_tweets_full[[
                        'screen_name', 'unique_text', 'engagement_rate_by_post'
                    ]]
                    df_tweets_max_er = df_tweets_max_er.groupby(['screen_name', 'unique_text'
                                                                ]).max().reset_index()
                    df_tweets_full = df_tweets_full.merge(
                        df_tweets_max_er,
                        left_on=['author_name', 'unique_text', 'engagement_rate_by_post'],
                        right_on=['screen_name', 'unique_text', 'engagement_rate_by_post'],
                        how='inner')
                    df_tweets_full = df_tweets_full.drop(
                        ['rt_flag', 'unique_text', 'author_name', 'screen_name_y'], axis=1)
                    df_tweets_full = df_tweets_full.rename(columns={"screen_name_x": "screen_name"})

            else:
                df_tweets_full['engagement_rate_by_post'] = df_tweets_full.apply(lambda row: 100*(row.favorite_count\
                                                                                                  +row.retweet_count + row.reply_count_no_api)\
                                                                                                  /row.ac_followers_count, axis=1)
            ...
            df_tweets_full = metric_transformation.MetricCategorization(
                df_tweets_full, METRIC, ITEM_COLUMN).categorize()
    ...
    def by_impressions(self):
        ...
        df_tweets_full = self.df_tweets_full
        try:
            df_tweets_full['engagement_rate_by_impressions'] = df_tweets_full.apply(lambda row: 100*(row.favorite_count\
                                                                                    +row.retweet_count + row.reply_count_no_api)\
            ...
            print(f'Class: {self.__str__()}\nMethod: {METHOD_NAME}\n')
            return pd.DataFrame(
                columns=list(df_tweets_full.columns) + ['engagement_rate_by_impressions'])
    ...
    def by_day(self, grouped=True):
        ...
        try:
            ...
            df_eng_by_day = df_eng_by_day[['twitter_id', 'screen_name', 'group', 'date', 'retweet_count',
                                           'reply_count_no_api', 'favorite_count', 'ac_followers_count']]\
            ...
            df_eng_by_day['engagement_rate_by_day'] = df_eng_by_day.apply(lambda row: 100*(row.favorite_count\
                                                                          +row.retweet_count + row.reply_count_no_api)\
                                                                          /row.ac_followers_count, axis=1)
            df_eng_by_day = df_eng_by_day.reset_index()

            if grouped:
                df_eng_by_day = df_eng_by_day[['group', 'date',
                                               'engagement_rate_by_day']].groupby(['group',
                                                                                   'date']).mean()
                df_eng_by_day = df_eng_by_day.reset_index()
                PAGE_COLUMNS = []
                ITEM_COLUMN = 'group'
                ...
                df_eng_by_day = df_eng_by_day.rename(
                    columns={
                        'twitter_id': '_object_id',
                        'screen_name': '_object_name'
                    })
            ...
            df_eng_by_day = metric_transformation.MetricCategorization(
                df_eng_by_day, METRIC, ITEM_COLUMN).categorize()

            return df_eng_by_day[PAGE_COLUMNS + [
                'group', 'date', 'engagement_rate_by_day', 'boundary', 'rel_engagement_rate_by_day'
            ...
            return pd.DataFrame(
                columns=[
                    '_object_id', '_object_name', 'group', 'date', 'engagement_rate_by_day',
                    'boundary', 'rel_engagement_rate_by_day'
                ])
        ...
        try:
            ...
            if len(df_eng_by_views) > 0:
                ...
                df_eng_by_views['engagement_rate_by_views'] = \
                                df_eng_by_views.apply(lambda row: 100*(row.favorite_count\
                                                      +row.retweet_count + row.reply_count_no_api)/row.view_count\
                                                      if row.media_entities[0]['type'] == 'video' else None, axis=1)
            ...
            print(e)
            print(f'Class: {self.__str__()}\nMethod: {METHOD_NAME}\n')
            return pd.DataFrame(columns=list(df_tweets_full.columns) + ['engagement_rate_by_views'])

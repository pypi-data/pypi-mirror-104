...
class Traceability():
    ...
    def __init__(self, df_pages, groups):
        ...
        OUTPUT_COLUMNS = ["ac_followers_count", "twitter_id", "screen_name", "group", "date"]

        if len(df_pages) > 0:
            try:
                df_fan_count = deepcopy(
                    df_pages[['ac_followers_count', 'twitter_id', 'screen_name', 'date']])
                ...
                df_fan_count = df_fan_count.groupby(['twitter_id', 'screen_name',
    ...
    def absolute(self):
        ...
        df_fan_count = df_fan_count.sort_values(['group', 'twitter_id', 'screen_name', 'date'])
        df_fan_count = df_fan_count.rename(columns={'ac_followers_count': 'value'})

        try:
            return df_fan_count[['group', 'twitter_id', 'screen_name', 'date',
                                 'value']].dropna(subset=['value'])
            ...
            print(e)
            print(f'Class: {self.__str__()}\nMethod: {METHOD_NAME}\n')
            return pd.DataFrame(columns=['group', 'twitter_id', 'screen_name', 'date', 'value'])
    ...
    def differences(self):
        ...
        try:
            ...
            for tid in set(df_fan_count.twitter_id):
                df_tmp = df_fan_count[df_fan_count.twitter_id.eq(tid)].sort_values('date')
            ...
            return df_fan_count_diff[['group', 'twitter_id', 'screen_name', 'date',
                                      'value']].dropna(subset=['value'])
    ...
    def percentage(self):
        ...
        try:
            ...
            for tid in set(df_fan_count.twitter_id):
                df_tmp = df_fan_count[df_fan_count.twitter_id.eq(tid)].sort_values('date')
                df_tmp['value'] = 100. * df_tmp['ac_followers_count'].pct_change()
                dfs.append(df_tmp)

            df_fan_count_perc = pd.concat(dfs).sort_values(
                ['group', 'twitter_id', 'screen_name', 'date'])

            return df_fan_count_perc[['group', 'twitter_id', 'screen_name', 'date',
                                      'value']].dropna(subset=['value'])

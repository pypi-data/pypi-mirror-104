...
class PostEngagementRateSensor():
    ...
    def __init__(self, df_posts):
        ...
        EXPECTED_COLUMNS = ['post_id', 'created_time', 'snapshot_time', \
            'post_clicks', 'post_impressions', 'post_impressions_organic', \
            'post_impressions_viral', 'post_impressions_paid']
    ...
    def load_sensor(self):
        ...
        try:
            ...
            df.loc[:, "created_time"] = deepcopy(pd.to_datetime(df.loc[:, "created_time"]).copy())
            df.loc[:, "snapshot_time"] = deepcopy(pd.to_datetime(df.loc[:, "snapshot_time"]))
            ...
            for i in range(len(df.index)):
                ...
                # Compute Delta if it is a regular post with today and yesterday's data.
                    ...
                    delta_clicks.append(df.loc[ idx, "post_clicks"] - \
                                        df.loc[ idx_prev, "post_clicks"] )
                    delta_effectivity.append(df.loc[ idx, "effectivity_fb"] -\
    ...
    def performance_indicator(
        ...
        return df.loc[:, [
            "created_time", "post_age_days", feature, "perf_upp_lim", "perf_mean", "perf_low_lim",
            "performance"
        ]]

...
def get_series(df_input, item_name, item_col, metric, agg_cols=None, agg_func='mean'):
    ...
    if agg_cols is not None:
        df_out = df_out[agg_cols + [item_col, metric]].groupby(agg_cols + [item_col]).agg({metric: agg_func})
    return df_out
...
def get_boundaries(df_s, method='std', p_window=28, width_bound=1, metric='engagement_rate_by_day'):
    """
    get_boundaries
    ...
    if method == 'qtl':
        ...
        if nrows > p_window:
            df_out['q1'] = df_out[metric].rolling(p_window).quantile(.25, interpolation='midpoint')
            df_out['q2'] = df_out[metric].rolling(p_window).quantile(.50, interpolation='midpoint')
            df_out['q3'] = df_out[metric].rolling(p_window).quantile(.75, interpolation='midpoint')
        else:
            df_out['q1'] = df_out[metric].quantile(.25, interpolation='midpoint')
            df_out['q2'] = df_out[metric].quantile(.50, interpolation='midpoint')
            df_out['q3'] = df_out[metric].quantile(.75, interpolation='midpoint')
        df_out[['q1', 'q2', 'q3', 'min', 'max']] = df_out[['q1', 'q2', 'q3', 'min', 'max']].fillna(method='bfill')
    ...
    if method == 'std':
        if nrows > p_window:
            df_out['mavg'] = df_out[metric].rolling(p_window).mean()  # Get MEAN rolling column
            df_out['mstd'] = df_out[metric].rolling(p_window).std()  # Get DEVIATION rolling column
        else:
            df_out['mavg'] = df_out[metric].mean()  # Get MEAN column
            df_out['mstd'] = df_out[metric].std()  # Get DEVIATION column
        df_out[['mavg', 'mstd', 'min', 'max']] = df_out[['mavg', 'mstd', 'min', 'max']].fillna(method='bfill')
        # df_out = df_out.dropna()
        # Get BOUNDARIES
...
class MetricCategorization:
    ...
    def categorize(self, agg_cols=None, method='std', p_window=28, wb=1, dr=0, dm='highest'):
        """
        ...
        try:
            ...
            df_ts = get_series(df_woo, item_names[0], self.item_col, self.metric, agg_cols)
            df_out = get_boundaries(df_ts, method, p_window, wb, self.metric)
            for item_name in item_names[1:]:
                df_ts = get_series(df_woo, item_name, self.item_col, self.metric, agg_cols)
            ...
            for row in range(len(df_out)):
                if df_out['mavg'].iloc[row] == 0:
                    ...
                    temp_1 = 100*(df_out[self.metric].iloc[row] - df_out['mavg'].iloc[row]) / df_out['mavg'].iloc[row]

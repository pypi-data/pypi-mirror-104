...
class EffectMomentsFB():
    ...
    def moments(self):
        ...
        column_post = ['page_id', 'created_time', 'group', 'post_id',
                       'engagement_rate_by_post', 'page_name',
        ...
        if self.grouped is True:
            df_empty_day = pd.DataFrame(columns=['day', 'group', 'counts',
                                                 'engagement_rate_by_post',
                                                 'rel_engagement_rate_by_post'])
            df_empty_hours = pd.DataFrame(columns=['hour', 'group', 'counts',
                                                   'engagement_rate_by_post',
                                                   'rel_engagement_rate_by_post'])
            df_empty_days_week = pd.DataFrame(columns=['hour', 'day', 'group', 'counts',
                                                       'engagement_rate_by_post',
                                                       'rel_engagement_rate_by_post'])
        else:
            df_empty_day = pd.DataFrame(columns=['day', 'group', '_object_name', 'counts',
                                                 '_object_id', 'engagement_rate_by_post',
                                                 'rel_engagement_rate_by_post'])
            df_empty_hours = pd.DataFrame(columns=['hour', 'group', '_object_name', 'counts',
                                                   '_object_id', 'engagement_rate_by_post',
                                                   'rel_engagement_rate_by_post'])
            df_empty_days_week = pd.DataFrame(columns=['hour', 'day', 'group', 'counts',
        ...
        try:
            ...
            for sel_group in group:
                ...
                if grouped is True:
                    temp_1 = temp_1.groupby(["day",
                                             grouped_id], as_index=False).agg(
            ...
            df_eff_day = df_eff_day.sort_values("engagement_rate_by_post",
                                                ascending=False).reset_index()
            df_eff_day = df_eff_day.drop(columns="index")
            df_eff_day = df_eff_day.rename(columns={'page_id': '_object_id',
            ...
            df_eff_hours = df_eff_hours.sort_values("engagement_rate_by_post",
                                                    ascending=False).reset_index()
            df_eff_hours = df_eff_hours.drop(columns="index")
            df_eff_hours = df_eff_hours.rename(columns={'page_id': '_object_id',
                                                        'page_name': '_object_name',
                                                        'post_id': 'counts'})
            ...
            df_eff_days_hours = df_eff_days_hours.sort_values("engagement_rate_by_post",
                                                              ascending=False).reset_index()
            df_eff_days_hours = df_eff_days_hours.drop(columns="index")
            df_eff_days_hours = df_eff_days_hours.rename(columns={'page_id': '_object_id',
                                                                  'page_name': '_object_name',
                                                                  'post_id': 'counts'})

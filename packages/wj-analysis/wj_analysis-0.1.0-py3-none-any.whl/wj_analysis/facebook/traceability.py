...
class Traceability():
    ...
    def __init__(self, df_pages, groups):
        ...
        if len(df_pages)>0:
            try:
                df_fan_count = deepcopy(df_pages[['fan_count', 'page_id', 'name', 'created_at']])

                df_fan_count['created_at'] = pd.to_datetime(df_fan_count['created_at']) - timedelta(hours=5)
                df_fan_count['date'] = df_fan_count['created_at'].apply(lambda d: d.date())
                df_fan_count = df_fan_count.drop(columns=['created_at'])

                df_fan_count = df_fan_count.groupby(['page_id', 'name', 'date']).last().reset_index()
                
                df_fan_count['group'] = df_fan_count['page_id'].apply(lambda pid: general_utils.get_group(pid, groups))
    ...
    def absolute(self):
        ...
        try:
            ...
            print(e)
            print(f'Class: {self.__str__()}\nMethod: {METHOD_NAME}\n')
            return pd.DataFrame(columns=['group', 'page_id', 'name', 'date', 'value']).dropna(subset=['value'])
    ...
    def differences(self):
        ...
        try:
            ...
            df_fan_count_diff = pd.concat(dfs).sort_values(['group', 'page_id', 'name', 'date'])

            return df_fan_count_diff[['group', 'page_id', 'name', 'date', 'value']].dropna(subset=['value'])
    ...
    def percentage(self):
        ...
        try:
            ...
            for pid in set(df_fan_count.page_id):
                df_tmp = df_fan_count[df_fan_count.page_id.eq(pid)].sort_values('date')
                df_tmp['value'] = 100.*df_tmp['fan_count'].pct_change()
                dfs.append(df_tmp)
    
            df_fan_count_perc = pd.concat(dfs).sort_values(['group', 'page_id', 'name', 'date'])

            return df_fan_count_perc[['group', 'page_id', 'name', 'date', 'value']].dropna(subset=['value'])

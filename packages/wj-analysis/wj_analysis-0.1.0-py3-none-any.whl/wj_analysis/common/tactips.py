...
class TacTips():
    ...
    def __init__(self, df_s, dim, item_col, item_name, met='engagement_rate_by_post', terms_size=5):
        """
    ...
    def get_tactip_values(self):
        ...
        try:
            ...
            if self.dim == 'most_effective_pages':
                df_pages = df_item[['page_name', self.met]].groupby(['page_name']).mean()
                df_pages = df_pages.reset_index()
                terms = df_pages.nlargest(self.terms_size, self.met)['page_name'].tolist()
                dim_v1 = ", ".join(terms[:-1])
                dim_v2 = terms[-1]
                return dim_v1, dim_v2
            ...
            if self.dim in ['day', 'hour']:
                ...
                df_item['perc_diff_vs_avg'] = (df_item[self.met] - metric_avg) / metric_avg
                df_item['perc_diff_vs_avg'] = df_item['perc_diff_vs_avg'] * 100
                best_m, best_p, temp_1 = \
                    df_item.nlargest(1, self.met)[[self.dim, self.met, 'perc_diff_vs_avg']].values[0].tolist()
                max_i = int(round(temp_1, 0))
                worst_m, worst_p, temp_2 = \
                    df_item.nsmallest(1, self.met)[[self.dim, self.met, 'perc_diff_vs_avg']].values[0].tolist()
            ...
            if self.dim in ['moment_of_day']:
                ...
                best_d, best_h, best_p, temp_1 = \
                    df_item.nlargest(1, self.met)[['day', 'hour', self.met, 'perc_diff_vs_avg']].values[0].tolist()
                max_i = int(round(temp_1, 0))
                worst_d, worst_h, worst_p, temp_2 = \
                    df_item.nsmallest(1, self.met)[['day', 'hour', self.met, 'perc_diff_vs_avg']].values[0].tolist()
                min_i = int(round(temp_2, 0))

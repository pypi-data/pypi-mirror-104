...
class SocialpresIG():
        ...
        try:
            ...
            for idd, row in df_c.set_index('nombre').iterrows():
                if idd:
                    ...
                    df_m[f'{idd}'] = df_m['caption'].str.contains(account_terms_regex, regex=True)
            ...
            if auto_n:
                ...
                for col in col_names:
                    for index in range(len(df_m)):
                        temp_1 = df_m[col]
                        if (temp_1[index] == True) & (df_m.owner_username[index] == col):
            ...
            df_med_c = df_m2.drop(columns=['date_local',
                                           'typename', 'caption',
                                           'shortcode', "auto"]).groupby('owner_username')
    ...
    def id_pres_ig(self):
        ...
        brand = self.brand
        auto_n = self.auto_n
        df_empty_id = pd.DataFrame(columns=['date_local', 'caption',
                                            'owner_username', 'shortcode'])
        ...
        try:
            ...
            df_med_id = df_m2[["date_local", "caption", "owner_username", "shortcode", brand]]

...
def prepare_test(production_name, job_name, dependent_input=None):
    ...
    if "bk_query" in job_data["input"] and dependent_input is not None:
        data["lfns"] = [dependent_input]
...
def enter_debugging(out_dir, env_cmd, gaudi_cmd, dependent_job=None):
    if (
        dependent_job is not None
    ):  # This means the requested job depends on another job and has not been told the location of that jobs output.
        raise NotImplementedError(

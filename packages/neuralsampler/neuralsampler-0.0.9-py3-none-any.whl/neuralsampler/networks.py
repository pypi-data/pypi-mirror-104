...
def net_fn(n_dims, n_out=1, n_hid=200, n_depth=2, layer=nn.Linear, UseSpectral=False, use_resnet=False):
    if use_resnet: 
        return resnet(n_dims, n_out=n_out, n_hid=n_hid, n_depth=n_depth, layer=layer, UseSpectral=UseSpectral)
    else: 
        return SmallMLP(n_dims, n_out=n_out, n_hid=n_hid, n_depth=n_depth, layer=layer, UseSpectral=UseSpectral)

class SmallMLP(nn.Module):
    def __init__(self, n_dims, n_out=1, n_hid=200, n_depth=2, layer=nn.Linear, UseSpectral=False):
...
class resnet(nn.Module):
    def __init__(self, n_dims, n_out=1, n_hid=200, n_depth=2, layer=nn.Linear, UseSpectral=False):
        ...
        self.layers = nn.ModuleList([
            layer(n_dims, n_hid) if not UseSpectral else SpectralNorm(layer(n_dims, n_hid)),
            *[layer(n_hid, n_hid) if not UseSpectral else SpectralNorm(layer(n_hid, n_hid)) for i in range(n_depth)] ,
            # layer(n_hid, n_hid) if not UseSpectral else SpectralNorm(layer(n_hid, n_hid)),
            # layer(n_hid, n_hid) if not UseSpectral else SpectralNorm(layer(n_hid, n_hid)),
            layer(n_hid, n_out) if not UseSpectral else SpectralNorm(layer(n_hid, n_out)), 
        ])

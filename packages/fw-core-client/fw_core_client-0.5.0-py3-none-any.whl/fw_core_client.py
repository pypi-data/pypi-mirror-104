...
import orjson
from fw_http_client import HttpClient, HttpConfig, errors
from memoization import cached
from packaging import version
from pydantic import root_validator

__all__ = ["CoreConfig", "CoreClient", "errors"]
...
class CoreClient(HttpClient):
    ...
    def request(self, method: str, url: str, **kwargs):  # type: ignore
        ...
        # route relative urls to core (/api) unless prefixed for known apis
        full_url = url.startswith("http")
        prefixed = any(url.startswith(api) for api in self.apis)
        if not full_url and not prefixed:
            url = f"/api{url}"
